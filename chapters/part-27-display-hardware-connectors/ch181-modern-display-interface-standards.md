...
- [1. DisplayPort Evolution: DP 1.1 through DP 2.1](#1-displayport-evolution-dp-11-through-dp-21)
  ...
  - [1.6 What is DisplayPort?](#16-what-is-displayport)
  - [1.7 What is HDMI?](#17-what-is-hdmi)
  - [1.8 What is USB4 Display Tunnelling?](#18-what-is-usb4-display-tunnelling)
  - [1.9 What is Thunderbolt?](#19-what-is-thunderbolt)
...
### 1.6 What is DisplayPort?

DisplayPort is an open digital display interface standard developed and maintained by the Video Electronics Standards Association (VESA). First released in 2007, it replaced VGA and DVI for internal PC-to-monitor connections by introducing a packetised data transport model rather than the time-multiplexed pixel clocks used by its predecessors. Each DisplayPort connection carries video, audio, and auxiliary data over a single connector using one to four differential lane pairs plus an AUX channel for out-of-band control signalling. DisplayPort was the first major display interface to incorporate link training — an active negotiation sequence in which source and sink establish the highest mutually supported lane count and bit rate before transmitting pixel data. In the Linux kernel, DisplayPort is handled within the DRM/KMS subsystem under `drivers/gpu/drm/display/`, where `drm_dp_helper.c` provides shared helpers for DPCD register access, link training, MST topology management, and DSC negotiation. Individual GPU drivers (i915, amdgpu, nouveau, msm) wrap these helpers with hardware-specific PHY bringup code. The connector appears to userspace through DRM connector objects and is interrogated via `sysfs` at `/sys/class/drm/`. This chapter traces how DisplayPort bandwidth has grown from the initial 8.64 Gbps effective data rate through the 128b/132b encoding leap of DP 2.0/2.1, pushing the aggregate ceiling to 77.37 Gbps across four lanes.

### 1.7 What is HDMI?

HDMI (High-Definition Multimedia Interface) is a royalty-bearing consumer electronics display and audio interface originally released in 2002 by a consortium of consumer electronics manufacturers. Unlike DisplayPort, HDMI bundles audio return channels, consumer electronics control (CEC), and optional Ethernet alongside the primary video stream, making it the dominant interface for televisions, home-theatre receivers, and gaming consoles. The original HDMI architecture used Transition-Minimised Differential Signalling (TMDS), the same line code as DVI, across three data pairs and a dedicated clock pair. With HDMI 2.1, the HDMI Forum replaced TMDS with Fixed Rate Link (FRL) — a four-lane 16b/18b encoded interface capable of carrying up to 48 Gbps of raw bandwidth. In the Linux kernel, HDMI output is driven through the DRM/KMS connector framework, with EDID parsing and CEC handling provided by shared helpers in `drivers/gpu/drm/` and hardware-specific implementations in GPU drivers such as amdgpu and i915. The HDMI CEC bus has its own subsystem at `drivers/media/cec/`. Unlike DisplayPort, HDMI embeds metadata and control signals within the active video stream as Info Frames rather than on a separate auxiliary channel. This chapter covers the TMDS-to-FRL transition in §2, including the FRL link-training negotiation that HDMI 2.1 introduced and how the amdgpu driver implements the FRL rate-stepping handshake.

### 1.8 What is USB4 Display Tunnelling?

USB4 is the fourth major revision of the USB specification, ratified by the USB Implementers Forum (USB-IF) in 2019 and based on the Thunderbolt 3 protocol. Where earlier USB versions were bus protocols for peripheral data transfer, USB4 introduced a tunnelling architecture in which protocol-specific traffic — including DisplayPort streams, PCIe transactions, and USB 3.x bulk transfers — is encapsulated in USB4 transport packets and arbitrated over shared Gen 3 (40 Gbps) or Gen 4 (80 Gbps) physical lanes. DisplayPort Tunnelling over USB4 allows a DP 2.1 display stream to be carried over a USB-C cable without requiring a separate DisplayPort Alt Mode negotiation at the Type-C layer. Instead, the USB4 host router allocates a fixed number of tunnel bandwidth units (TBUs) from the shared pool for the display stream; the remaining capacity is available for USB data or PCIe. In the Linux kernel, USB4 is managed by the `drivers/thunderbolt/` subsystem, which exposes tunnel configuration through debugfs and handles bandwidth arbitration between display and data tunnels. The DRM layer communicates its bandwidth requirements to the Thunderbolt driver through an internal coupling API. This chapter explains the bandwidth-sharing arithmetic — how display and USB data traffic compete for the same physical pipe — in §3 and §8.

### 1.9 What is Thunderbolt?

Thunderbolt is a high-speed interconnect technology that combines DisplayPort and PCIe signalling over a single cable, designed for docking stations, external storage, and external GPU enclosures. The first two generations used the Mini DisplayPort physical connector and reached 20 Gbps; Thunderbolt 3 adopted the USB-C connector and doubled bandwidth to 40 Gbps while also providing the protocol foundation for USB4; Thunderbolt 4 mandated capability improvements including required support for two 4K displays and PCIe tunnelling with higher minimum bandwidth guarantees; Thunderbolt 5 reaches 120 Gbps through a bandwidth-boost mode using asymmetric lane allocation. The protocol encapsulates both DisplayPort and PCIe into a Thunderbolt packet stream, allowing docking stations, NVMe storage, and displays to share a single cable with no user configuration. In the Linux kernel, Thunderbolt and USB4 share the `drivers/thunderbolt/` driver, reflecting their common protocol heritage. The driver registers with both the DRM display subsystem (to expose display tunnels) and the PCI subsystem (to manage PCIe tunnels through the router fabric). Device discovery and optional secure pairing are handled through a challenge-response mechanism with keys stored in `sysfs` under `/sys/bus/thunderbolt/`. This chapter covers the display capabilities of Thunderbolt 3 through 5 in §4, including the bandwidth implications of dual-display and daisy-chain configurations.
