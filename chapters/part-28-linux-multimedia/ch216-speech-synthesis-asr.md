...
- **NVIDIA Parakeet as a second on-device ASR architecture family.** whisper.cpp/ggml merged support for NVIDIA's Parakeet model alongside Whisper, with a dedicated example CLI and Ruby bindings; sherpa-onnx has separately begun exporting Parakeet CTC models for Qualcomm QNN NPU acceleration. Together these signal Parakeet spreading as an alternative to the Whisper encoder-decoder line covered in §5, rather than a one-off addition to a single project. [Source](https://github.com/ggml-org/whisper.cpp/releases/tag/v1.9.0) [Source](https://github.com/k2-fsa/sherpa-onnx/releases/tag/v1.13.4)
- **Piper and speech-dispatcher converging on a C++ entry point.** Piper's Python rewrite dropped its C++ code with a `libpiper` C API "planned"; that plan landed with a `libpiper` C++ CLI executable and test suite ported from the legacy codebase. Independently, speech-dispatcher already carries a `cxxpiper.cpp` output module (predating `libpiper` itself) that continues to see active work, including a CUDA acceleration option — two separate signals of the same de-Pythonization direction rather than one causing the other. [Source](https://github.com/OHF-Voice/piper1-gpl/releases/tag/v1.5.0) [Source](https://github.com/brailcom/speechd/commit/037a4910995785bd9667a65a54a39ddf6c7a6fc1)
- **Wyoming protocol extending its ASR and intent surface.** OHF-Voice's Wyoming protocol — the peer-to-peer protocol unifying TTS, ASR, wake-word, VAD, and intent handling for local voice assistants such as Home Assistant Assist — added VAD and audio-preference negotiation to its ASR program type and support for recognizing multiple intents per utterance, extending the protocol surface that on-device pipelines built from this chapter's components can target. [Source](https://github.com/OHF-Voice/wyoming/releases/tag/v1.10.0)

### Medium-term (1-3 years)

- **Piper trimming its dependency footprint.** Piper has begun replacing PyTorch-based components with onnxruntime-only equivalents: the Chinese g2pW phonemizer's tensor batching now runs through `piper.g2pw_onnx` instead of `g2pw.api`, dropping roughly 750 MB of installed dependencies (`torch`, `requests`) and running 1.5-2x faster. This continues the leaner, more embeddable direction already visible in Piper's move to bundled espeak-ng and the `libpiper` C++ path above. [Source](https://github.com/OHF-Voice/piper1-gpl/releases/tag/v1.6.1)
- **Vosk's release cadence diverging from the next-gen Kaldi stack.** Vosk's last tagged release predates this chapter's writing by roughly two years, with only small, infrequent commits since (typo fixes, minor API additions to the Java/Android bindings). The k2-fsa/sherpa-onnx project, built on the newer k2/icefall/lhotse toolchain, ships tagged releases far more frequently and has started exporting ASR models — including Parakeet CTC — for NPU backends such as Qualcomm QNN. No project has announced deprecating Vosk, but the two ecosystems' development velocity has diverged substantially, which matters for anyone choosing between them for a new embedded deployment. [Source](https://github.com/alphacep/vosk-api/releases) [Source](https://github.com/k2-fsa/sherpa-onnx/releases/tag/asr-models-qnn-binary-3)
- **whisper.cpp's VAD API maturing.** Streaming voice-activity detection with explicit state reset landed in whisper.cpp, and subsequent releases have continued refining VAD-related example flags and build packaging alongside the Parakeet work above, suggesting VAD is moving from an experimental addition toward a stable part of the library's public surface rather than a one-off example. [Source](https://github.com/ggml-org/whisper.cpp/releases/tag/v1.8.5)

### Long-term

- **espeak-ng persisting as infrastructure rather than an end-user voice.** espeak-ng's formant-synthesis voice quality has long trailed neural TTS, but Piper embeds espeak-ng directly as its grapheme-to-phoneme front-end (§2-3) rather than shelling out to a separate `piper-phonemize` library, and periodically bumps its embedded copy. espeak-ng's role looks likely to keep shifting from "the TTS engine users hear" toward "the phonemizer neural TTS engines depend on," rather than disappearing. [Source](https://github.com/OHF-Voice/piper1-gpl/releases/tag/v1.3.0)
- **openWakeWord's model-release velocity trailing its usage as a Wyoming wake-word backend.** openWakeWord's last tagged release goes back to early 2024, and recent commits have been dormant for long stretches, with sparse, packaging-focused changes (Python version bumps, an ONNX-to-TFLite conversion flag) rather than new wake-word models. If that trajectory continues, Wyoming-based assistants that rely on openWakeWord for the wake-word stage may need to look elsewhere for actively trained models even as the library itself keeps functioning. [Source](https://github.com/dscripka/openWakeWord/releases/tag/v0.6.0)
