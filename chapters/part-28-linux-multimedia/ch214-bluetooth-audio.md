...
- **BlueZ's LE Audio implementation continues to harden rather than expand.** Recent BlueZ point releases concentrate on BAP and BASS fixes — ASE Control Point error handling, QoS state transitions when a CIS drops, endpoint configuration edge cases, and broadcast source/receive-state crash fixes — signaling that the stack is moving from initial feature landing toward production-grade reliability rather than adding new profile surface. [Source](https://github.com/bluez/bluez/releases/tag/5.87)
- **The kernel ISO socket path is under sustained consolidation.** Commit history for `net/bluetooth` shows a concentrated run of reference-counting, use-after-free, and locking fixes in the ISO connection and socket-timeout code, alongside similar hardening across L2CAP, RFCOMM, and HIDP — consistent with a subsystem still settling rather than one with a stable, closed API surface. This chapter's existing note that `bt_iso_qos` layout compatibility needs verification (§6.3) reflects that same ongoing churn. [Source](https://github.com/torvalds/linux/commits/master/net/bluetooth)
- **PipeWire's `bluez5` SPA plugin is advancing on two fronts at once.** Commit activity shows continued BAP/broadcast feature completion (ASCS announcement properties, BIS transport configuration) running in parallel with a security-hardening pass across the Bluetooth backend (NULL-check audits after allocation failures, integer-overflow checks in codec size calculations, file-descriptor cleanup on error paths). [Source](https://github.com/PipeWire/pipewire/commits/master/spa/plugins/bluez5)

### Medium-term (1-3 years)

- **Dynamic Stream Adaptation for ASCS would move adaptive-bitrate control into the standard.** A Bluetooth SIG draft change request extends the Audio Stream Control Service so unicast Audio Stream Endpoints can be reconfigured in response to changing link conditions as a standardized procedure, rather than relying purely on host-side heuristics like PipeWire's `abr_process()` bitpool reduction described in §5. [Source](https://www.bluetooth.com/specifications/specs/dynamic-stream-adaptation-audio-stream-control-service-2/)
- **A higher-throughput LE PHY option is in draft to support richer LE Audio streams.** The "HDT PHY for LE Audio" change request extends ASCS with a selectable LE Higher Data Throughput PHY, which would raise the bandwidth ceiling available to CIS/BIS streams beyond what current LE Audio deployments use. [Source](https://www.bluetooth.com/specifications/specs/hdt-phy-for-le-audio-crr00_pr/)
- **Auracast broadcast audio continues expanding beyond early hearing-aid and demo deployments.** The Bluetooth SIG's Auracast program has moved into venue location profiles, retrofit hardware for existing public-address systems, and product/location cataloging intended to make broadcast discovery practical at scale — a distribution layer that BlueZ's `iso-broadcaster`/`sync-receiver` controller features (§7.1) and BASS implementation (§7.3) already provide the host-side building blocks for. [Source](https://www.bluetooth.com/auracast/)

### Long-term

- **A draft high-resolution/lossless codec exists alongside LC3, but Linux support is not yet a live question.** The Bluetooth SIG's "High-Resolution/Lossless Codec" specification remains in early draft (v0.9r05) status, aimed at high-resolution and lossless audio streaming use cases beyond what LC3 targets; a codec at this draft stage would need to clear SIG ratification, then controller/firmware support, before it could reach BlueZ's `media_codec` plugin interface. [Source](https://www.bluetooth.com/specifications/specs/high-resolution-lossless-codec/)
- **Controller and firmware ISO support, not the host stack, is likely to remain the binding constraint on LE Audio availability.** As this chapter notes in §7.1, controllers such as the Realtek RTL8852BE return `ENOSYS` on ISO connection attempts while the Intel AX210 and MediaTek MT7921 work reliably; recent `drivers/bluetooth` kernel activity is dominated by hardening and validation fixes and incremental per-device ID additions rather than a blanket rollout of ISO capability across existing controller families, so which headsets and dongles get LE Audio is likely to keep depending on individual vendor firmware decisions rather than a kernel-side fix. [Source](https://github.com/torvalds/linux/commits/master/drivers/bluetooth)
- **A voice-assistant service within the Common Audio Profile is speculative and early-stage.** A Bluetooth SIG draft change request associates a Voice Assistant Profile and Service with CAP's existing unicast/broadcast stream-management framework, but the specification is still in change-request/public-review status with no ratified profile or known BlueZ implementation to point to. [Source](https://www.bluetooth.com/specifications/specs/common-audio-profile-v1-1_pr/)
