...
- **MatrixRTC nears core-spec status.** MSC4143, the MatrixRTC group-call architecture referenced in §5, is in the proposed-final-comment-period with a disposition to merge, meaning its room-state-event session model and pluggable-transport design are close to becoming a formally adopted part of the Matrix specification rather than remaining an implementer-only proposal. [Source: MSC4143](https://github.com/matrix-org/matrix-spec-proposals/pull/4143)
- **VP9 RTP packetisation is now a full RFC.** The long-stalled `draft-ietf-payload-vp9` referenced in §11 was published as [RFC 9628](https://www.rfc-editor.org/rfc/rfc9628.html) (March 2025), putting VP9 on the same standards-track footing as H.264 (RFC 6184) and removing the "implementations reference the draft" caveat for new integrations.
- **TURN servers are tightening their default security posture.** coturn's 4.17 release line changed several long-standing defaults — DTLS listeners now require an explicit opt-in flag, challenge nonces default to stateless authenticated cookies instead of per-session state, and a mobility-related allocation-quota bypass was closed — continuing a trend of hardening existing behaviour rather than adding new relay features. [Source](https://github.com/coturn/coturn/releases/tag/4.17.0)

### Medium-term (1-3 years)

- **SFU-compatible end-to-end media encryption.** [RFC 9605 (SFrame)](https://www.rfc-editor.org/rfc/rfc9605.html), published August 2024, defines a lightweight authenticated-encryption layer for real-time media frames that lets an SFU — such as the LiveKit backend MatrixRTC already uses (§5) — route and forward streams by reading unencrypted metadata while the payload stays encrypted end-to-end between participants, complementing the "Per-Participant Keys" model Element already implements.
- **Post-quantum key exchange gains a standardised target.** NIST finalised ML-KEM as [FIPS 203](https://csrc.nist.gov/pubs/fips/203/final) in August 2024, giving the experimental Crystals-Kyber key encapsulation already used in LIME (§3) a standardised, production-grade parameter set to converge on, and providing the same target for any future post-quantum extension to the DTLS-SRTP handshake used by WebRTC and Jami.
- **Scalable asynchronous group-key management.** [RFC 9420 (MLS)](https://www.rfc-editor.org/rfc/rfc9420.html) provides logarithmic-cost, forward-secure group key agreement for members who are not simultaneously online — a property pairwise DTLS-SRTP or ZRTP key exchange does not offer — making it a candidate substrate for large encrypted group calls as an alternative to per-call pairwise key fan-out.

### Long-term

- **Network-signalled low-latency queuing as a QoS complement.** [RFC 9330 (L4S)](https://www.rfc-editor.org/rfc/rfc9330.html) targets sub-millisecond queuing delay through cooperative host/network congestion signalling rather than the DSCP EF per-hop marking described in §9. No access-network operator roadmap currently commits to ubiquitous L4S deployment, so DSCP marking and adaptive jitter buffers are likely to remain the primary latency-hiding mechanisms for VoIP for the foreseeable future, with L4S adopted incrementally where operators already support it.
- **Convergence around SFU-compatible E2E encryption.** The overlapping goals of MatrixRTC's per-participant keys, SFrame, and MLS suggest that Linux VoIP stacks built on SFU architectures will gradually converge on a shared frame-encryption approach. No cross-project standardisation body currently coordinates that convergence, however, and most of the SIP/PJSIP/liblinphone deployments described in this chapter remain pairwise (ZRTP or DTLS-SRTP) rather than SFU-routed, so this shift is likely to be gradual and uneven across the ecosystem.
