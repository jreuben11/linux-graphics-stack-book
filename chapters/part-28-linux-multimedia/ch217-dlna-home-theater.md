...
- **Jellyfin Desktop client rewrite continues stabilising.** The renamed Jellyfin Desktop client (formerly Jellyfin Media Player) has moved from Qt 5 to Qt 6 and adopted `mpvqt` as its playback foundation, bringing it onto the same mpv-based pipeline used by standalone mpv deployments; the migration is still shedding memory-leak regressions and is currently available only via Flathub and the Arch AUR, with Windows/macOS stable builds pending. [Source](https://jellyfin.org/posts/state-of-the-fin-2026-01-06/)
- **jellyfin-ffmpeg's 8.1 branch expands hardware-acceleration coverage ahead of Jellyfin 12.** Builds targeting the unreleased Jellyfin 12.0 add Vulkan support on Windows clang builds, D3D11 overlay/tonemap/deinterlace/scale filters for Qualcomm hardware, and extend Mesa-driven AMD acceleration to arm64. [Source](https://github.com/jellyfin/jellyfin-ffmpeg/releases)
- **Kodi's Piers (22.0) release cycle is finishing while Q* (23.0) work has already started.** Piers is in its RC/beta polish phase, and an early-development milestone for the next major version, codenamed "Q*", is already open in the upstream issue tracker. [Source](https://github.com/xbmc/xbmc/milestones)
- **UxPlay is de-risking its Avahi dependency.** The 1.74 experimental line adds a minimal built-in mDNS responder for Linux (and bundled Bonjour-equivalent behaviour elsewhere) so that AirPlay 2 mirroring no longer requires a running `avahi-daemon` by default; the traditional Avahi/Bonjour DNS-SD backend remains available via a `-DUSE_DNS_SD=1` build flag for environments that prefer it. [Source](https://github.com/FDH2/UxPlay)

### Medium-term (1-3 years)
- **Rygel's next feature milestone is in active, incremental development.** A "Rygel 46" milestone tracks a small set of open work items against the GStreamer-based server/renderer stack described in §3, consistent with the project's historical pattern of small, infrequent point releases rather than a large rewrite. [Source](https://gitlab.gnome.org/GNOME/rygel/-/milestones)
- **Jellyfin's media scanner is targeted for a ground-up rewrite.** A published proposal reworks the `BaseItem`/`Folder`-based scanning and metadata-provider architecture that Jellyfin's ContentDirectory-facing DLNA plugin (§4) ultimately depends on for library data; the project has framed this as multi-release work rather than a near-term patch. [Source](https://github.com/jellyfin/jellyfin-meta/discussions/125)
- **go2rtc continues rapid iteration on source-protocol breadth rather than the Cast bridge itself.** Recent releases have concentrated on adding native support for consumer camera ecosystems (Xiaomi, Wyze, Tuya) and on WebRTC/UDP transport hardening, expanding the pool of local streams that can be remuxed to the HLS/MP4 endpoints Chromecast and Android TV devices fetch via `LOAD` (§7.4), without changing the underlying Cast bridging model. [Source](https://github.com/AlexxIT/go2rtc/releases)

### Long-term
- **Avahi's release cadence has stalled while systemd-resolved's built-in mDNS grows more capable.** Avahi's last tagged release (0.8) dates to February 2020, whereas `systemd-resolved` now ships an in-tree "LLMNR and MulticastDNS resolver and responder" controllable via its `MulticastDNS=` setting. No formal deprecation of `avahi-daemon` has been announced by any distribution, and GUPnP/Rygel/UxPlay all still support Avahi as a discovery backend, but new tooling such as UxPlay 1.74's bundled mDNS responder suggests Linux media software is trending toward carrying a minimal built-in mDNS implementation rather than assuming a system Avahi daemon is present. [Source](https://raw.githubusercontent.com/systemd/systemd/main/man/systemd-resolved.service.xml)
- **DLNA has no active certifying body, so the ecosystem's future rests on de facto interoperability rather than new certification.** The DLNA Alliance dissolved as a non-profit trade association in 2017, with certification testing continuing only through a third-party contractor; no successor standards body has taken over active DLNA/UPnP-AV specification maintenance, so further evolution of the protocol stack described in §1 is likely to come from implementation-level convergence (as with Jellyfin's and Kodi's practical-interoperability approach in §12) rather than a new formal revision. [Source](https://en.wikipedia.org/wiki/Digital_Living_Network_Alliance)
- **AirPlay 2 Legacy Protocol support on Linux remains dependent on Apple not withdrawing it.** UxPlay's mirroring functionality is built entirely on the reverse-engineered Legacy Protocol subset that iOS has continued to support alongside full AirPlay 2; the project itself notes this could change in a future iOS release, at which point Linux AirPlay mirroring would need a fresh reverse-engineering effort with no guarantee of success. [Source](https://github.com/FDH2/UxPlay)
