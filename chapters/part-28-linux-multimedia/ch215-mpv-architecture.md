...
- **subrandr native subtitle renderer.** mpv has merged `subrandr`, a purpose-built subtitle rendering library that renders YouTube's SRV3 format and WebVTT directly instead of lossily converting them to ASS for libass. It targets the "user agents that do not support CSS" conformance class of the WebVTT specification while implementing CSS-style inline layout for positioning, ruby text, and ordering that libass-based conversion pipelines historically got wrong. [Source](https://github.com/mpv-player/mpv/pull/16271)
- **Dolby Vision Profile 7 FEL support.** Support for Dolby Vision Profile 7 (Full Enhancement Layer, used on UHD Blu-ray discs) has landed on mpv master, extending the existing Profile 5/8.1 RPU handling described in §8.3. [Source](https://github.com/mpv-player/mpv/pull/17932)
- **libplacebo v7.x Vulkan and colour-management refinement.** libplacebo's v7 release series continues shipping incremental fixes to the Vulkan swapchain path (adopting `VK_KHR_swapchain_maintenance1`, deferred swapchain memory allocation) and to HDR/SDR colour handling (extended HLG gamma, overlay colour-space consistency), the same code paths `vo_gpu_next` depends on for presentation and tone mapping. [Source](https://github.com/haasn/libplacebo/releases/tag/v7.360.0)

### Medium-term (1-3 years)

- **libplacebo (gpu-next) backend for the embeddable render API.** The `mpv_render_context` API described in §2.4 currently offers OpenGL feature parity only with the legacy `vo_gpu` driver; libplacebo-only features (RAVU upscaling, the full HDR tone-mapping chain, dynamic-metadata handling) are not yet exposed to embedding applications through `include/mpv/render.h`. Closing this gap is an open, tracked goal so that libmpv embedders get the same rendering quality as the standalone player. [Source](https://github.com/mpv-player/mpv/issues/10810)
- **Optical-disc menu and audio format coverage.** Work in progress adds interactive DVD and Blu-ray menu navigation and DVD-Audio support, extending mpv's demuxer and input layers (§3, §12) beyond straightforward file and stream playback toward full optical-disc semantics. [Source](https://github.com/mpv-player/mpv/pull/18080)
- **Vulkan Video decode hardening.** The Vulkan-based hardware decode path (`hwdec=vulkan`, §4.1) continues to see driver- and codec-specific corruption, hang, and stability reports across AMD, NVIDIA, and Intel Vulkan implementations; stabilizing this path toward parity with the long-established VAAPI and nvdec zero-copy routes remains an active area of triage. [Source](https://github.com/mpv-player/mpv/issues/17370)

### Long-term

- **Continued convergence on libplacebo as the shared GPU video-rendering layer.** libplacebo grew out of mpv's own shader pipeline and is now reused by VLC and as an FFmpeg filter; no competing shared library has emerged, so cross-project consolidation of colour management, tone mapping, and upscaling logic around libplacebo is likely to deepen rather than fragment. [Source](https://github.com/haasn/libplacebo/blob/master/README.md)
- **`vo_gpu` legacy driver fading toward compatibility-only status.** No formal deprecation timeline has been announced for the legacy OpenGL-only `vo_gpu` driver (§5), but since `gpu-next` became the default VO in mpv 0.41.0 and new colour-management and HDR work (Dolby Vision Profile 7, reference-white handling) lands in `gpu-next` first or exclusively, feature parity between the two is likely to keep diverging in `gpu-next`'s favour.
- **Broader native subtitle format coverage.** subrandr's own documentation describes itself as under heavy development with formats limited to SRV3 and WebVTT and known gaps (vertical text, justification); given mpv's history of gradually deprecating conversion-based pipelines once a native renderer matures (as happened with several legacy demuxers), further native format support — most plausibly ASS/SSA alongside libass — is a plausible but not yet committed direction. [Source](https://github.com/afishhh/subrandr)
