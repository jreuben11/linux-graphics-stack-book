...
- **`VK_EXT_descriptor_heap` moves toward specification.** A Khronos proposal to replace `VK_EXT_descriptor_buffer` with a heap-based bindless descriptor model — closer in spirit to D3D12's `ResourceDescriptorHeap[i]` dynamic indexing (SM6.6, cited in §3.9) — is in active drafting; VKD3D-Proton's 3.0.1 release notes flag it as imminent, noting "a ton of work on descriptor heaps have been happening in the background." [VK_EXT_descriptor_heap proposal](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/proposals/VK_EXT_descriptor_heap.adoc) · [VKD3D-Proton 3.0.1 release notes](https://github.com/HansKristian-Work/vkd3d-proton/releases/tag/v3.0.1)
- **A SPIR-V long-vector proposal targets the SM6.9 gap directly.** `VK_EXT_shader_long_vector`, a Khronos proposal generalizing `VK_NV_cooperative_vector`'s wide-vector support into a general `OpTypeVectorIdEXT` mechanism, is drafted specifically to let vectors with more than four components — and vectors sized by specialization constant — appear anywhere a vector is allowed, closing the SPIR-V equivalence gap the §3.9 table marks "None / DXIL-exclusive" for HLSL SM6.9 Long Vectors. [VK_EXT_shader_long_vector proposal](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/proposals/VK_EXT_shader_long_vector.adoc)
- **Nova's kernel-side Rust driver work continues at pace.** Both `drivers/gpu/nova-core` (GSP-RM firmware/falcon bring-up) and `drivers/gpu/drm/nova` (the DRM-facing driver) show continuous commit activity in mainline Linux through mid-2026, extending the memory-safety argument made in §2.4 well beyond the initial merge rather than stalling as a one-off demonstration. [Linux `nova-core` source tree](https://github.com/torvalds/linux/tree/master/drivers/gpu/nova-core)
- **DXVK and VKD3D-Proton keep closing hardware-specific gaps.** The 3.0.x release series (mid-2026) added Independent Devices support, `VK_EXT_present_timing`-based frame pacing, and vendor shader-intrinsic passthrough (AMD AGS, NVIDIA NVAPI) needed for titles using FSR4 Ray Reconstruction, sustaining the near-native-performance translation-layer story from §2.6. [VKD3D-Proton releases](https://github.com/HansKristian-Work/vkd3d-proton/releases)

### Medium-term (1-3 years)
- **Cooperative matrix becomes a mandatory Vulkan Roadmap baseline.** The Vulkan Roadmap 2026 milestone — requiring Vulkan 1.4 plus a set of required extensions including `VK_KHR_cooperative_matrix` — makes cross-vendor GEMM-in-shader a guaranteed baseline capability rather than an optional extension, strengthening the "transparent shader-level primitives" trajectory described in §3.10 relative to D3D12's opaque MetaCommands model. [Vulkan Roadmap milestones](https://docs.vulkan.org/spec/latest/appendices/roadmap.html)
- **Rust DRM abstractions are being generalized beyond a single driver.** A tracking effort for shared Rust DRM-subsystem abstractions, originating from Asahi's driver needs, remains active in the Rust-for-Linux project as of mid-2026, suggesting the memory-safety pattern described in §2.4 is moving toward reusable cross-driver infrastructure rather than staying one-off per-driver Rust code. [Rust-for-Linux DRM abstractions tracking issue](https://github.com/Rust-for-Linux/linux/issues/969)
- **Cross-vendor cooperative vectors remain the most plausible next ML convergence step, but are not yet formalized.** No `VK_KHR_cooperative_vector` proposal document exists in the Vulkan-Docs repository as of mid-2026; only the vendor-only `VK_NV_cooperative_vector` proposal is drafted. The precedent of `VK_NV_cooperative_matrix` → `VK_KHR_cooperative_matrix` (cited in §3.9) makes eventual promotion plausible but currently uncommitted. [VK_NV_cooperative_vector proposal](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/proposals/VK_NV_cooperative_vector.adoc)

### Long-term
- **Sampler feedback parity looks structurally stalled.** The Khronos tracking issue for a cross-vendor `VK_NV_shader_image_footprint` successor has remained open without any proposal document since it was filed in 2022 — in contrast to the active proposal-stage drafts for descriptor heaps and long vectors described above — suggesting §3.8's mip-access-telemetry gap is one of the least likely D3D12 Ultimate features to reach Vulkan parity this decade. [Khronos Vulkan-Docs issue tracker](https://github.com/KhronosGroup/Vulkan-Docs/issues/1773)
- **MetaCommands' opaque-dispatch model is unlikely to be replicated in Vulkan.** As §3.10 argues, the model depends on WDDM's IHV-NDA driver relationship; Vulkan's ML roadmap continues to favor transparent, compiler-visible primitives (cooperative matrix, and eventually cooperative vectors) over opaque per-vendor GUID dispatch, so this looks like a permanent philosophical divergence between the platforms rather than a temporary lag that further extension work will close.
- **ProRes and DirectStorage-equivalent gaps depend on decisions Linux does not control.** Hardware ProRes acceleration on Linux requires either Apple exposing the AGX media engine to Asahi or a non-Apple vendor shipping equivalent silicon — neither has any public roadmap commitment — and a native Linux DirectStorage-equivalent path requires a commercially motivated game to target it, which had not materialized as of mid-2026 despite the underlying infrastructure (io_uring, p2pdma, `VK_EXT_external_memory_fd`) already being in place per §3.3. [Asahi GPU driver](https://github.com/AsahiLinux/linux/tree/asahi/drivers/gpu/drm/asahi)
