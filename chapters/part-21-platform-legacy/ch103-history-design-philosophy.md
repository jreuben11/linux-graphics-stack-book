...
- [1. Introduction: Architecture Is History](#1-introduction-architecture-is-history)
  - [1.1 What is the Linux Graphics Stack?](#11-what-is-the-linux-graphics-stack)
  - [1.2 What is DRM?](#12-what-is-drm)
  - [1.3 What is Mesa?](#13-what-is-mesa)
  - [1.4 What is Wayland?](#14-what-is-wayland)
...
Wayland is a display server protocol for Linux and other Unix-like systems that supersedes the X Window System as the standard compositor and input-dispatch mechanism. Under Wayland, the compositor is also the display server: it receives rendering output from clients as shared memory buffers or DMA-BUF file descriptors, composites them into a scene, and presents the composed frame to the physical display via the DRM KMS interface. There is no intermediate X server layer between an application and the display.

The Wayland protocol is defined in XML wire-format descriptions and specifies objects (surfaces, buffers, seats, outputs) and events exchanged over a Unix domain socket. The core protocol is intentionally minimal; application-specific functionality — desktop window management, screen locking, layer surfaces for panels and notification overlays — is added through extension protocols such as `xdg-shell`, `xdg-output`, and the wlr-layer-shell family. XWayland provides backward compatibility for legacy X11 applications by embedding an X server that translates X11 drawing commands into Wayland surface buffers, letting unported applications run under a Wayland compositor without modification. Chapter 20 covers the Wayland protocol architecture; Chapters 21–23 cover the major compositors built on it.
...
### OpenVG: A Parallel Standard That Faded

**OpenVG** was Khronos's answer to a question adjacent to OpenGL's: not "how do you accelerate 3D scenes?" but "how do you accelerate 2D vector graphics — the SVG/Flash/PDF rendering model — on the same embedded GPUs that were beginning to appear in mobile phones?" A working group formed in July 2004; the 1.0 specification shipped August 1, 2005, and 1.1 followed in 2008 with text rendering and Flash-oriented additions. Zack Rusin added an OpenVG state tracker to Mesa's Gallium3D infrastructure on May 1, 2009, letting SVG-style vector graphics run hardware-accelerated on any Gallium pipe driver; it was removed again in Mesa 10.6, released June 15, 2015, once it had gone unmaintained. Embedded silicon vendors shipped dedicated hardware support during OpenVG's active window: Qualcomm's Adreno line via an Android SDK, Freescale's i.MX35/i.MX51/i.MX53 SoCs (via an AMD/ATI Z160/Z180 OpenVG core), and Vivante's GC350 2D core and successors. [Source: OpenVG — Wikipedia](https://en.wikipedia.org/wiki/OpenVG)

OpenVG never achieved OpenGL's ecosystem gravity, and the working group itself acknowledged the standstill: in September 2011 it decided to stop holding regular standardization meetings. Two forces made the API redundant rather than displaced by a direct competitor: general-purpose GPU compositors got fast enough that 2D vector rendering could simply run as a fragment shader over a triangulated path (the approach Skia, Cairo's GL backend, and modern browser compositors converged on), and mobile GPU vendors found it cheaper to ship one good 3D driver than two drivers for two APIs with overlapping hardware requirements. By the time Android standardized on `hwui`/Skia for 2D UI rendering in the early 2010s, OpenVG had no remaining first-party consumer. The specification is still listed among Khronos's standards and the `EGL_OPENVG_BIT`/`EGL_OPENVG_API` tokens remain in the EGL specification for backward compatibility (see Chapter 150), but no actively maintained Linux driver or application targets it today — the 2D-acceleration problem it addressed was absorbed into general-purpose GPU compositing instead of solved by a dedicated API. [Source: OpenVG 1.0 Specification — Khronos Registry](https://registry.khronos.org/OpenVG/specs/1.0/openvg-1.0.pdf)
...
OpenGL was not the first attempt at a standard graphics API. **GKS** (Graphical Kernel System), formalized by IFIPS committees starting in 1973 and later an ANSI/ISO standard, covered only 2D and could not be cleanly extended to 3D. Its successor **PHIGS** (Programmer's Hierarchical Interactive Graphics System) became the ANSI/ISO 3D standard of the 1980s, offering hierarchical scene structuring, but its committee-driven design lagged badly behind the pace of workstation hardware and never gained a performance-competitive implementation. Both were effectively dead by the time OpenGL shipped and are cited today only as the standards-committee approach that vendor-driven APIs (IrisGL, then OpenGL itself) displaced. [Source](https://en.wikipedia.org/wiki/PHIGS)

OpenGL's lineage instead begins at Silicon Graphics in 1982, where Jim Clark and colleagues built the first hardware geometry pipeline in a workstation product — the IRIS. The software interface to that hardware, IrisGL, became the internal standard for SGI's graphics programming. By the early 1990s SGI was licensing IrisGL to other hardware vendors, but the API carried SGI-specific extensions and dependencies that made portability difficult.
...
### Microsoft's Other 3D Flops: Talisman, XNA, WPF 3D, and Silverlight — Plus Sun's Java3D

Fahrenheit was not Microsoft's only abandoned 3D initiative — the pattern recurred across three decades, and none of the casualties ever touched Linux. **Talisman** (announced at SIGGRAPH 1996) was a proposed hardware architecture, not an API: it composited independently rendered, affine-transformed image layers at video-refresh rate to exploit spatial and temporal coherence rather than re-rendering full scenes. Cirrus Logic and Samsung committed to fabricate Talisman chips, but conventional immediate-mode 3D accelerators (3Dfx, then NVIDIA) improved in performance and price faster than Talisman's layered-compositing approach could deliver, and Microsoft's partners abandoned the project by 1997–1998 without shipping silicon. [Source](https://en.wikipedia.org/wiki/Microsoft_Talisman)

**XNA** (announced GDC 2004, first CTP 2006) was a managed-code (.NET/C#) game framework wrapping Direct3D, aimed at unifying Windows and Xbox 360 game development and lowering the barrier for hobbyist and student developers. It never got a Direct3D 10/11 rewrite and was left behind by Windows 8's Metro/WinRT app model, which XNA could not target; Microsoft ended XNA development in 2013 and support in April 2014. Its user base migrated largely to **MonoGame**, an open-source, API-compatible reimplementation that — unlike XNA itself — runs on Linux. [Source](https://en.wikipedia.org/wiki/Microsoft_XNA_Game_Studio)

**WPF 3D** (`Viewport3D`, part of .NET 3.0's Windows Presentation Foundation, 2006) exposed a retained-mode 3D scene graph — declarative XAML-defined meshes, cameras, lights, and materials — as a lightweight alternative to hand-rolled Direct3D for line-of-business applications that needed occasional 3D content. It rendered through Direct3D 9 internally but exposed none of Direct3D's shader pipeline or performance controls, and Microsoft never extended it past the fixed-function-era feature set (no pixel shaders, no post-processing) it launched with; it was effectively frozen once WPF's own investment wound down in the following decade. [Source](https://learn.microsoft.com/en-us/dotnet/desktop/wpf/graphics-multimedia/3-d-graphics-overview)

**Silverlight 3D**, added to Silverlight 5 at MIX11 (April 2011), was a cut-down XNA API surface (`GraphicsDevice`, `VertexBuffer`, `Texture2D`) exposed to Silverlight's browser-plugin runtime — deliberately incomplete at launch (no `BasicEffect`, no `SpriteBatch`, no math library) with those pieces filled in only later via the community Silverlight Toolkit. It arrived after WebGL had already been announced and after HTML5 had already won the "should the web need a plugin" argument; Microsoft ended Silverlight development within the same year, and official support lapsed on October 12, 2021 — the plugin never had a realistic adoption window. [Source](https://www.gamedev.net/blogs/entry/2250831-silverlight-5-brings-xna-3d-to-the-web/)

Microsoft was not alone in this shape of failure: **Java3D**, a retained-mode scene-graph API co-developed by Sun, Silicon Graphics, Intel, and Apple starting in 1996, followed an almost identical arc on a different platform — released 1998, development paused 2003–2004, revived as a community-source project, then formally deprecated by Sun in January 2008 in favor of JavaFX's own 3D graphics, before being open-sourced under GPLv2 later that same year and kept alive since only by volunteers (it moved to a JOGL 2.0-based OpenGL backend in 2012). [Source](https://en.wikipedia.org/wiki/Java_3D)

The common thread through Talisman, Fahrenheit, XNA, WPF 3D, Silverlight 3D, and Java3D is a vendor repeatedly building a retained-mode or managed-language 3D layer *above* or *beside* the platform's real, actively-developed 3D API (Direct3D for Microsoft's four; OpenGL for Java3D), trading performance or generality for a specific ease-of-adoption bet, and then discontinuing each one once the platform priority that justified it (Windows 95 multimedia PCs, the OpenGL/Direct3D truce, Xbox/Windows convergence, line-of-business XAML apps, browser plugins, cross-platform Java applets) moved on. None of these had a Linux port or a Linux-relevant open replacement to inherit — the through-line to this book's subject matter is only that the platforms' real 3D APIs, unburdened by any of these side-bets, kept evolving on their own tracks and never created the pressure that Glide, OpenGL, and later Vulkan did on Linux's own API history.
...
### Intel Larrabee: The GPU That Never Shipped (2008–2010)

Not every flop in this history is a software API — some are hardware projects cancelled before an API ever needed to be written against them. **Larrabee**, unveiled in 2008, was Intel's attempt to build a discrete GPU out of many simple, in-order x86 cores extended with a wide vector unit, rather than the fixed-function-plus-shader-core design every GPU vendor had converged on. The pitch was that a fully programmable, x86-compatible many-core chip could match dedicated GPUs on rasterization (via a software rendering pipeline) while also being a general-purpose HPC/vector processor — competing simultaneously with NVIDIA's GeForce and, on the compute side, with what became CUDA and Tesla.

Intel cancelled the consumer graphics product in May 2010 without ever shipping a Larrabee-based GPU; contemporary reporting attributed the decision to the software rendering pipeline underperforming dedicated GPU hardware on real workloads and schedule slippage. The silicon and architecture did not disappear, though: Intel repositioned the design, stripped of any graphics-output ambition, as the **Xeon Phi** many-integrated-core (MIC) HPC coprocessor line, first announced in 2010. For Linux, Larrabee's cancellation meant one less GPU architecture ever requiring a DRM/Mesa driver — but its instruction-set and wide-SIMD lineage lived on in Intel's compute stack rather than its graphics one. [Source](https://en.wikipedia.org/wiki/Xeon_Phi)
...
**Shockwave** (a distinct Macromedia plugin for content authored in **Director**, not a Flash variant) got a real 3D engine — **Shockwave 3D**, announced at SIGGRAPH 2000 and shipped in Director 8.5 (April 2001) — jointly developed with **Intel**, built on Intel's Internet 3D Graphics ("IFX Toolkit") software renderer/rasterizer, with OpenGL or Direct3D hardware acceleration where available. It was a genuine scene-graph 3D API exposed to Director's Lingo scripting language, and 3D content became Shockwave's main differentiator from Flash once Adobe acquired Macromedia in 2005 — *Myst Online* was among the games built on it. But it inherited Shockwave's own adoption ceiling: a second, separate plugin install (distinct from the far more ubiquitous Flash Player) that most sites never bothered to require. Adobe stopped selling the Director authoring tool in February 2017 and discontinued Shockwave Player entirely on April 9, 2019 — two years after Flash's own end-of-life announcement, but on the same trajectory. [Source](https://en.wikipedia.org/wiki/Adobe_Shockwave)

Flash itself finally got GPU-accelerated 3D late in its life: **Stage3D** (codenamed Molehill), shipped in Flash Player 11 (2011), exposed a low-level, shader-based 3D rendering API to ActionScript, with hardware acceleration extended to Android and iOS by Flash Player 11.2 (March 2012). It arrived the same year WebGL 1.0 shipped and after Apple had already excluded Flash from iOS — engines built on Stage3D (Starling, Away3D) had a brief window of relevance before Flash's 2020 end-of-life took Stage3D down with it, with no successor migration path the way Shockwave content had none either. [Source](https://en.wikipedia.org/wiki/Stage3D)
...
### Google O3D: The Plugin WebGL Made Redundant (2009–2010)

Before WebGL shipped, Google built its own answer to browser 3D: **O3D**, a JavaScript API backed by a browser plugin, aimed at establishing a new standard for hardware-accelerated 3D on the web. Google was, at the same time, one of the vendors helping develop WebGL at Khronos — and once WebGL matured into a cross-vendor standard also backed by Mozilla, Apple, and Opera, Google's own plugin-based approach became redundant by its own logic of wanting an open, plugin-free web. On May 7, 2010, Google announced it was abandoning the O3D plugin, retargeting the O3D name onto a JavaScript library built on top of WebGL instead. O3D is a rare case of a company killing its own project because a competing open standard it was co-developing succeeded — a much faster and less acrimonious death than Flash's decade-long decline. [Source](https://en.wikipedia.org/wiki/O3D)
