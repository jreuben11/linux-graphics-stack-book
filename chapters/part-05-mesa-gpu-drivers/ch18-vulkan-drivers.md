...
Section 1 establishes the anatomy common to every **Mesa** **Vulkan** driver:
- **ICD manifest discovery** — how **radeon_icd.x86_64.json** is found by the loader and **vk_icdGetInstanceProcAddr** resolved
- **Physical device embedding** — how **radv_physical_device**, **anv_physical_device**, and **tu_physical_device** embed the shared **vk_physical_device** base type from `src/vulkan/runtime/vk_physical_device.h`
- **DRM node initialisation** — the consistent pattern of opening `/dev/dri/renderD128`, querying hardware capabilities through **DRM** ioctls and vendor-specific interfaces, and mapping hardware ring types to **Vulkan** queue families
- **Winsys abstraction** — **amdgpu** winsys for **RADV**, **anv_kmd_backend** for **ANV**, and the **MSM** submit path for **Turnip**

**RADV** is the reference point for state-of-the-art **AMD** support, covering hardware from **GCN1** (**GFX6**) through **RDNA4** (**GFX12**):
- **Memory management** — three-heap layout (device-local **VRAM**, host-visible **GTT** via **IOMMU**, device-local+host-visible **BAR** window); **radv_alloc_memory()** calls **amdgpu_bo_alloc()** in the winsys; sparse binding via **amdgpu_va_op** batches
- **Command buffer recording** — **radv_cmd_buffer** dirty-flag lazy state emission; **PM4** packet encoding (**PKT3_DRAW_INDEX_AUTO**, **PKT3_INDIRECT_BUFFER** for **IB** chaining); **radv_QueueSubmit()** passes timeline semaphores as **AMDGPU_CHUNK_ID_SYNCOBJ_TIMELINE_WAIT** and **AMDGPU_CHUNK_ID_SYNCOBJ_TIMELINE_SIGNAL** chunks to the **amdgpu** kernel
- **Descriptor set architecture** — **AMD** **SRD** (Shader Resource Descriptor) format (16-byte buffer **SRDs**, 32-byte texture **SRDs**); **VK_EXT_descriptor_indexing** and **VK_EXT_descriptor_buffer** expose the global descriptor heap to applications
- **Pipeline compilation and caching** — shader key hash capturing fixed-function state; disk-backed **radv_pipeline_cache** serialising compiled **ELF** binaries; async compilation via background thread pool; **VK_EXT_graphics_pipeline_library** fast-link vs. monolithic paths
- **Debug and performance variables** — **RADV_DEBUG** and **RADV_PERFTEST** flags including **RADV_DEBUG=hang** for **CS** dumps, **RADV_DEBUG=noasyncshaders** for synchronous compilation, and **RADV_PERFTEST=emulate_rt** for software ray tracing; the **ACO** backend delivers sub-millisecond pipeline compile times
- **NGG pipeline** (**RDNA**) — collapses the legacy **GCN** **VGT** geometry stages into a single "primitive shader" running on **SIMD** compute units with **LDS** (Local Data Share); **s_sendmsg(gs_alloc_req)** and ballot-and-prefix-sum software culling path covering backface, frustum, and small-primitive culling
- **Mesh and task shaders** (**VK_EXT_mesh_shader**) — **ACE** (Asynchronous Compute Engine) / **GFX** queue synchronisation via **PKT3_REWIND** barriers and a **taskPayload** ring buffer
- **Hardware ray tracing** (**RDNA2**+) — **rtit** instructions for **BVH** traversal; compute-shader **BVH** builds in `src/amd/vulkan/bvh/`

**ANV** reveals **Intel**'s distinctive **bindless heap** design, where a permanently-mapped 64-byte surface state region (the surface state pool, allocated with **util_vma_heap_alloc()**) allows shaders to access resources by a 32-bit heap index without binding-table round-trips:
- **Bindless heap** — **VK_EXT_descriptor_buffer** exposes this layout directly to applications
- **Command buffer encoding** — **anv_batch** abstraction with **anv_batch_emit** macros generated from **genxml** Intel GPU register **XML** definitions; generation-specific **COMPUTE_WALKER** and other packets via the **GENX()** macro; chain batches linked via **MI_BATCH_BUFFER_START**
- **Shader compilation** — **Intel EU** compiler in `src/intel/compiler/`; **elk** backend (legacy **Gfx7**–**Gfx8**) and **brw** backend (**Gfx9**+); **SIMD8**/**SIMD16**/**SIMD32** thread dispatch width selection driven by register pressure analysis; push constants encoded into **EU** **GRFs** at kernel dispatch time
- **KMD backend abstraction** — **anv_kmd_backend** vtable cleanly separating the legacy **i915** kernel driver path from the modern **xe** driver path (**Linux 6.8+**)
- **Xe2 architecture** (**Battlemage**, **GFX20**) — 512-bit **SIMD** registers; **GuC** firmware version requirement; **XE_VM_BIND** operations; unified **L2** cache coherency changes
- **Hardware workarounds** — tracked in **intel_wa.c** / **intel_wa.h** with unique identifiers (e.g., **WA_14016820455**); applied at compile time via **GFX_VER** guards or at runtime via **anv_device::info::verx10** checks

**Turnip** shows the design pressures unique to **TBDR** (tile-based deferred rendering) hardware:
- **Adreno architecture** — the binning pass writes a **VSC** visibility stream to determine per-tile primitive coverage; per-tile rendering happens in on-chip **GMEM** (512 KB–1.5 MB of on-die **SRAM**); the **CP** (Command Processor) reads **PM4** command streams from **IB1** primary indirect buffers chaining to **IB2** secondary indirect buffers
- **Sysmem vs. GMEM rendering** — decision made in **tu_render_pass_decide_layout()**; autotune system using **XXH64** pass hashes to refine the static heuristic using real **GPU** performance feedback; **STORE_OP_DONT_CARE** and **LOAD_OP_DONT_CARE** flags directly control tile load and store bandwidth
- **Command buffer recording** — **tu_cs** abstraction (chained **BO**-backed blocks via **IB2_BASE_LO/HI**); main CS, draw CS, tile-load **IB**, and tile-store **IB**; **CP_SET_DRAW_STATE** packets for efficient draw state grouping
- **Descriptor sets** — **TEX_CONST** and **SAMP_CONST** hardware-format descriptors (16 **DWORDs**, 64 bytes each) in a bindless **BO**; **CP_LOAD_STATE6** packets encoding push constants and inline uniform blocks directly into the command stream
- **Shader compilation** — **ir3** compiler backend (`src/freedreno/ir3/`); **SSA**-based **IR** with half-precision flags, repeat counts, and a latency-hiding instruction scheduler; **ir3_const.c** manages the Adreno constant register file
- **Synchronisation** — **PC_CCU_FLUSH_COLOR_TS**, **CACHE_FLUSH_TS**, and **RB_DONE_TS** events; **VkSemaphore** timeline operations via **DRM_IOCTL_SYNCOBJ_TIMELINE_WAIT** / **DRM_IOCTL_SYNCOBJ_TIMELINE_SIGNAL** on the **MSM** kernel driver; explicit sync passed to the **Wayland** compositor via **wp_linux_drm_syncobj_v1**; **CCU** (Cache Coherency Unit) flush sequences require careful ordering

The chapter closes with:
- **Common bringup patterns** — five-phase progression from skeleton **ICD** through first triangle to extension coverage, demonstrated by **NVK** (**NVIDIA** **Vulkan**) and **Honeykrisp** (**Apple AGX**)
- **dEQP-VK conformance workflow** — using **deqp-runner** and per-driver expected lists in `src/amd/ci/`, `src/intel/ci/`, `src/freedreno/ci/`
- **Cross-driver topics** — **VK_KHR_external_semaphore_fd**, **DRM** syncobj export, **VK_KHR_present_wait** / **VK_KHR_present_id** via **KMS** vblank counters, **VK_EXT_debug_utils** object naming, **VK_EXT_device_fault** structured hang information, and the **MESA_VK_WSI_PRESENT_MODE** environment variable
- **Mesa WSI layer** (`src/vulkan/wsi/`) — **wsi_common_create_swapchain()** dispatching to the **X11** backend (**wsi_x11.c**, using **DRI3** and the **Present** extension with **xcb_present_pixmap**), the **Wayland** backend (**wsi_wayland.c**, using **zwp_linux_dmabuf_v1** format/modifier negotiation with **gbm_bo_create_with_modifiers2** and **wp_linux_drm_syncobj_v1** GPU-timeline explicit sync), and the direct-to-display backend (**wsi_display.c**, implementing **VK_KHR_display** via **drmModeAtomicCommit** and **drmModeAddFB2WithModifiers**)
- **VK_KHR_swapchain_maintenance1** (**wsi_common.c**) — per-present fences (**VkSwapchainPresentFenceInfoEXT**) and runtime swapchain mode switching
