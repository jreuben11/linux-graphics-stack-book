...
### 1.1 What is MMIO (Memory-Mapped I/O)?

Memory-Mapped I/O (MMIO) is the mechanism by which the CPU communicates with peripheral hardware by reading and writing specific physical memory addresses that are wired to hardware registers rather than system RAM. For a GPU, the primary MMIO region is exposed through the PCI configuration as BAR0 (Base Address Register 0), a physical address range that maps directly to the GPU's internal control registers. When the driver issues a 32-bit store to an address within BAR0, the write travels over PCIe and arrives at a specific register inside the GPU, selecting functional units, configuring DMA engines, changing clock states, or triggering command-processor operations. The GPU signals status and errors back to the CPU through MMIO reads. On Linux, the kernel maps the BAR into the driver's virtual address space with `ioremap()` and wraps accesses with `ioread32_native()` and `iowrite32_native()`. Because no hardware manual documents which address controls which behaviour on most consumer GPUs, discovering the semantics of each register address is the central problem that GPU reverse engineering addresses. MMIO probing tools such as `mmiotrace` (the kernel's built-in tracing facility under `kernel/trace/mmiotrace.c`) and the `nva` utilities in envytools exist specifically to observe and characterise this address space by comparing register states before and after known operations.

### 1.2 What is a GPU Command Stream?

A GPU command stream is a structured binary protocol through which a CPU-side driver programs the GPU's 3D and compute engines without issuing direct MMIO writes for every state change. Instead, the driver allocates a region of memory accessible to both CPU and GPU (via the GPU IOMMU and the system's DMA mapping infrastructure), fills it with encoded register-value pairs and DMA addresses, then writes the buffer's GPU-visible address to a command-processor trigger register. The GPU's command processor is a dedicated hardware engine that reads the stream sequentially, writes the encoded register values to the 3D engine's local registers, resolves DMA addresses for vertex buffers and textures, and processes synchronisation primitives such as fences and semaphores. This buffer is called a pushbuffer in NVIDIA terminology, an indirect buffer (IB) or PM4 stream in AMD/Adreno terminology, and a command list in Apple AGX. The encoding format — how opcodes are sized, how address fields are embedded, what packet types exist for different operations — is hardware-specific, proprietary, and undocumented for most consumer GPUs. Capturing and decoding this format from a running proprietary driver is one of the primary goals of command-stream reverse engineering, enabling an open driver to submit syntactically valid work to the hardware without needing a vendor hardware manual.

### 1.3 What is a Shader ISA?

A shader ISA (Instruction Set Architecture) is the native machine-code format executed by the programmable shader cores of a GPU. Unlike a CPU ISA, a GPU ISA is designed for SIMD-width execution across tens or hundreds of parallel threads simultaneously, with instruction encodings for floating-point arithmetic, texture sampling, memory load/store from multiple address spaces (register files, shared memory, constant buffers, global VRAM), and control flow through predication or divergence-handling mechanisms. Each GPU vendor defines a proprietary ISA per GPU family: NVIDIA's architectures progress from NV50 through Fermi, Kepler, Maxwell, Pascal, Volta, Turing, Ampere, and Ada Lovelace; ARM Mali covers Midgard, Bifrost, and Valhall; Apple AGX defines its own vector ISA per SoC generation. Shader programs written in GLSL, SPIR-V, or WGSL are compiled by the driver into the GPU's native ISA binary at runtime. For an open-source driver to compile shaders, it must implement a backend targeting the hardware ISA, which requires knowing instruction encodings, register file organisation, scheduling constraints, and the binary header format for shader programs. These details are never documented for consumer GPUs and must be recovered by disassembling shaders extracted from proprietary drivers and probing hardware behaviour with handcrafted binary test shaders uploaded directly to shader cores.

### 1.4 What is GPU Firmware?

GPU firmware refers to software running on embedded microprocessors inside the GPU itself, separate from both the host CPU and the main shader cores. On NVIDIA GPUs, these are Falcon microcontrollers — a proprietary RISC-like embedded processor architecture — instantiated in units including PGRAPH CTX (context save and restore), PDAEMON (power management and thermal monitoring), PMU, and the video decode engine. On ARM Mali CSF (Command Stream Frontend) GPUs, an ARM Cortex-M core runs the command scheduler firmware that mediates job submission from the host. On Apple AGX, an ARM Cortex-M processor runs ASC (Apple Silicon Controller) firmware that intermediates every GPU command submission and power transition. The host driver communicates with these embedded processors through shared-memory mailboxes: the host writes a structured message to a region of system RAM mapped into the firmware's IOMMU address space, then signals the firmware's doorbell register via MMIO. The firmware reads the message, acts on it (scheduling shader work, managing power states, reporting GPU faults), and writes a response to a reply ring buffer the host polls. Reverse engineering the firmware protocol requires either disassembling the firmware binary itself using tools such as Ghidra with architecture-specific loaders, or capturing the message traffic at the hypervisor level before the firmware has a chance to act on it. Without understanding this protocol, a driver cannot submit any GPU work, change power states, or handle GPU hardware faults.
