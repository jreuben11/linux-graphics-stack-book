...
> **Scope:** Ch30 covers **development-time debugging tools**:
>
> - **RenderDoc**
> - **Vulkan validation layers**
> - **Mesa debug environment variables**
> - **gdb integration**
> - **kernel GPU debugging** via `dyndbg`
>
> **Ch93** covers GPU performance analysis *methodology* (how to structure a bottleneck investigation, interpret counter data, move from symptom to root cause). **Ch137** surveys the Linux GPU profiling tool ecosystem:
>
> - **MangoHud**
...
RenderDoc is an open-source frame capture and replay debugger for graphics APIs, hosted at [renderdoc.org](https://renderdoc.org/). It operates at the API boundary: an in-process capture library (`librenderdoc.so`) intercepts Vulkan, OpenGL, or OpenGL ES calls and serialises all GPU resources — textures, buffers, pipeline objects, the complete command stream — into a portable `.rdc` capture file. A separate replay tool (`qrenderdoc` for interactive analysis, `renderdoccmd` for headless operation) re-issues those API calls against the local driver, making every resource inspectable at any draw call boundary.

On Linux, RenderDoc requires a render node (`/dev/dri/renderDN`) and no elevated privileges for most Vulkan and OpenGL use cases. Wayland surface capture depends on `VK_KHR_wayland_surface`; EGL-based applications use the `RENDERDOC_HOOK_EGL=1` path. The tool does not measure GPU timing or hardware counters — it is a correctness and inspection instrument. When a pixel is wrong, a buffer is corrupt, or a draw call is absent, RenderDoc lets you pause execution at the API level and examine every input and output. It is the first tool to reach for in any rendering correctness investigation on Linux. Section 2 covers the complete capture workflow, the `.rdc` format, the event browser, the shader debugger, and the Python replay API for CI integration.

### 1.2 What is the Vulkan Validation Layer?

The Vulkan Validation Layer (`VK_LAYER_KHRONOS_validation`) is the primary mechanism for catching API misuse before it reaches the driver. The Vulkan loader's layer system intercepts API calls via JSON manifests placed in `/usr/share/vulkan/explicit_layer.d/` (system-wide) or `~/.local/share/vulkan/explicit_layer.d/` (per-user). When the validation layer is active, every Vulkan call passes through multiple subsystems before reaching the driver's `VkLayerDispatchTable`.

Core Validation checks object lifetimes, descriptor set validity, render pass compatibility, and command buffer recording rules — the class of bugs that produce undefined behaviour according to the Vulkan specification. Thread Safety validation detects unsynchronised concurrent access to Vulkan objects. Best Practices validation emits non-fatal warnings for usage that is technically correct but suboptimal. Synchronisation Validation (syncval) maintains a hazard graph tracking write-after-read, read-after-write, and write-after-write relationships; it pinpoints missing pipeline barriers and image layout transitions with a precision that is otherwise impossible to achieve by inspection alone. GPU-Assisted Validation instruments shaders on the GPU itself to catch out-of-bounds descriptor indexing and buffer access at the instruction level. Enabling `VK_LAYER_KHRONOS_validation` in every development build eliminates the entire class of API-misuse bugs before they produce driver-specific behaviour. Section 3 details the configuration interface, the `vkconfig` settings UI, sub-layer enable flags, and CI integration patterns.

### 1.3 What is Mesa's Debug Infrastructure?

Mesa is the open-source GPU driver collection that implements Vulkan (RADV for AMD, ANV for Intel, NVK for NVIDIA) and OpenGL (iris, crocus, Zink, Nouveau) on Linux. Mesa exposes an extensive set of environment variables that control and expose driver internals at runtime without requiring source changes or driver recompilation. These variables are the primary interface for diagnosing incorrect rendering, shader compilation bugs, and synchronisation failures at the driver level.

Universal variables such as `MESA_DEBUG` and `LIBGL_DEBUG=verbose` enable general error logging across all Mesa drivers. Compiler-pipeline variables such as `NIR_DEBUG=print,validate` dump the driver's intermediate representation after each optimisation pass, making it possible to inspect how SPIR-V or GLSL is transformed through NIR into hardware-specific IR. Driver-specific variable families — `RADV_DEBUG` for the AMD Vulkan driver, `INTEL_DEBUG` for Intel's ANV and iris stacks, `ACO_DEBUG` for the ACO shader compiler backend — expose command ring dumps, compiled shader assembly, per-draw serialisation forcing, and pipeline cache bypass. The `MESA_SHADER_DUMP_PATH` variable writes compiled shader binaries to disk for offline analysis with tools such as `umr` or `iga64`. Full functionality requires a Mesa debug or debugoptimized build (`-Dbuildtype=debug` or `-Dbuildtype=debugoptimized`), which preserves the NIR validator and ACO IR validator that release builds compile out. Section 4 documents all variable families in depth.
