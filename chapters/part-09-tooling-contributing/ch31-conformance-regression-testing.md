...
### 1.1 What is dEQP / VK-GL-CTS?

dEQP (drawElements Quality Program) is the Khronos-maintained conformance test suite for the OpenGL, OpenGL ES, Vulkan, and EGL APIs. Originally developed at drawElements Oy and contributed to Khronos by Google in 2015, it resides today in the VK-GL-CTS repository at `https://github.com/KhronosGroup/VK-GL-CTS`. The suite is organized around separately compiled test binaries — `deqp-vk` for Vulkan, `glcts` for desktop OpenGL, `deqp-gles2`, `deqp-gles3`, and `deqp-gles31` for OpenGL ES, and `deqp-egl` for EGL — each exercising the corresponding API surface against the precise normative requirements of the Khronos specification.

Tests are identified by hierarchical dot-separated names that encode the API, the module, and the specific extension or feature under test — for example, `dEQP-VK.ray_tracing_pipeline.misc.build.device_address`. The Vulkan CTS alone contains more than 700,000 test cases. Khronos maintains a subset called the mustpass list, stored in `external/vulkancts/mustpass/main/vk-default.txt`, that defines the minimum set of tests a driver must pass to obtain Vulkan conformance certification under the Adopter Program. On Linux, dEQP integrates with Mesa through the Vulkan ICD JSON mechanism: setting `VK_ICD_FILENAMES` to a Mesa-built ICD file directs the test runner at a specific driver build without modifying the system driver. Results are written as `.qpa` XML logs that capture per-test status, reference images, and timing. Section 2 covers building, configuring, and interpreting dEQP output in depth.

### 1.2 What is IGT GPU Tools?

IGT GPU Tools is the primary test suite for Linux DRM kernel drivers. Hosted at `https://gitlab.freedesktop.org/drm/igt-gpu-tools`, it covers KMS (Kernel Mode Setting), GEM (Graphics Execution Manager) buffer management, DMA-BUF cross-device buffer sharing, and DRM synchronisation objects. In contrast to dEQP, which operates entirely in userspace through a high-level graphics API, IGT tests issue DRM ioctls directly — `DRM_IOCTL_MODE_SETCRTC`, `DRM_IOCTL_GEM_MMAP`, `DRM_IOCTL_SYNCOBJ_WAIT` — and verify kernel-level behavior: that display planes composite correctly at the hardware scanout level, that buffer objects are mapped into GPU-visible address space without corruption, and that timeline syncobjs signal at the right moment relative to GPU completion.

Each IGT test is a standalone executable linked against the `libigt` helper library. The `igt_runner` binary executes test suites described by JSON files, records pass/fail/skip/crash per subtest, and emits results in the GitLab CI-compatible JSON format consumed by the Linux kernel's CI infrastructure. KMS tests such as `kms_atomic` and `kms_cursor_crc` compare display engine pixel CRCs against reference values, catching rendering errors at the hardware level rather than through framebuffer readback. Because IGT drives real hardware through real kernel interfaces, it catches regressions that userspace-only suites cannot detect: mode-setting races, VRAM eviction ordering bugs, and DMA fence completion errors. Section 3 covers the repository layout, key test programs, and the process of writing a new IGT test from scratch.

### 1.3 What is piglit?

piglit is Mesa's long-standing GPU regression suite for OpenGL, OpenGL ES, and GLSL conformance. Hosted at `https://gitlab.freedesktop.org/mesa/piglit`, it contains thousands of test programs that each target a specific OpenGL behavior, GLSL language feature, or extension interaction. Unlike dEQP, which is specification-driven and curated by Khronos, piglit is community-maintained and grew historically by adding a test each time a specific Mesa bug was fixed — making it a direct record of regressions that have occurred across Mesa's development history.

Tests are organized into run groups: `quick.py` for a fast sanity pass, `gpu.py` for GPU-intensive tests, and `all.py` for the complete suite. Tests can run against Mesa's software rasterizers `llvmpipe` or `softpipe` without requiring GPU hardware, making them practical for developer workstations and CI environments that lack GPU runners. The `piglit summary` tool compares result directories from two runs and highlights regressions and improvements. A key format is `.shader_test`, a declarative text format consumed by the `shader_runner` binary that pairs GLSL source with expected rendering output, making it straightforward to add a focused test case for a specific compiler bug without writing C code. Section 4 describes running piglit, comparing results between commits, bisecting regressions with NIR IR dumps, and authoring new shader tests.
