...
- **Delegate ABI versioning lands in the LiteRT runtime.** Recent LiteRT releases have begun embedding ABI version metadata in the structs shared between the core runtime and accelerator plugins (GPU, NPU dispatch, compiler backends), a concrete step toward the "delegate `.so` as accelerator contract" convergence already noted in §17.5 — the goal is for different point releases of the runtime and a vendor delegate binary to interoperate rather than requiring lockstep upgrades. [Source: LiteRT release notes](https://github.com/google-ai-edge/LiteRT/releases/tag/v2.2.0)
- **MediaPipe consolidates its GenAI surface around LiteRT-LM.** MediaPipe's Tasks framework has reached its first 1.0 release and, in the same cycle, dropped its own embedded LLM Inference Engine for Android and iOS, leaving on-device generative-AI execution to the dedicated LiteRT-LM runtime described in §17.1 rather than duplicating it inside the calculator graph. [Source: MediaPipe release notes](https://github.com/google-ai-edge/mediapipe/releases/tag/v1.0.0)
- **Vendor delegate coverage expands past the Android SoC set.** LiteRT has added JIT/AOT compilation support for Intel NPU silicon on Windows and Linux, extending the vendor-delegate model of §3.3 — until now dominated on Android by Qualcomm QNN, MediaTek NeuroPilot, and Google's Tensor NPU — onto non-Android desktop and edge targets. [Source: LiteRT release notes](https://github.com/google-ai-edge/LiteRT/releases/tag/v2.2.0)

### Medium-term (1-3 years)
- **Vulkan gains a native tensor resource type.** Khronos's `VK_ARM_tensors` extension proposal introduces `VkTensorARM`/`VkTensorViewARM` as first-class Vulkan resources — with matching SPIR-V (`OpTypeTensorARM`) and GLSL (`GL_ARM_tensors`) support — to give N-dimensional ML data a proper representation instead of the buffer/image workarounds the proposal itself calls out as inadequate. If broadly ratified, this would give the GPU delegate's experimental Vulkan compute backend (§17.2) a standard tensor abstraction in place of the ad hoc SSBO/EGLImage bindings this chapter documents in §4. [Source: VK_ARM_tensors proposal](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/proposals/VK_ARM_tensors.adoc)
- **WebNN advances toward a W3C Recommendation alongside WebGPU compute.** The Web Neural Network API has reached Candidate Recommendation Draft status, standardising a browser-native `MLGraphBuilder`/`MLContext` inference path distinct from — and complementary to — the WebGPU compute shaders that back LiteRT-LM's browser target (§17.4). Browser engineers integrating on-device inference will increasingly have to choose between, or combine, a dedicated ML API and general-purpose GPU compute. [Source: WebNN specification](https://www.w3.org/TR/webnn/)
- **Runtime language bindings continue to broaden.** LiteRT has added an official Rust crate built on its C++ Binary SDK, joining the existing C, Python, Swift, and Kotlin/Java surfaces — widening which application languages can call directly into the delegate stack of §3 without a JNI hop, a trend also visible in ExecuTorch's own migration of Android-facing APIs toward Kotlin. [Source: LiteRT release notes](https://github.com/google-ai-edge/LiteRT/releases/tag/v2.2.0) [Source: ExecuTorch release notes](https://github.com/pytorch/executorch/releases/tag/v1.4.0)

### Long-term
- **No single on-device model format is likely to consolidate.** `.tflite`/LiteRT, `.onnx`/ORT Mobile, `.pte`/ExecuTorch, and GGUF/llama.cpp each remain anchored to a different framework vendor's training-framework gravity — TensorFlow/JAX, generic ONNX exporters, PyTorch, and community-quantised open weights respectively. The pattern already visible in §§5.6–5.10 and §17.9 — bidirectional converters bridging formats rather than one runtime displacing the others — is a more probable outcome than consolidation, echoing how Vulkan, Metal, and Direct3D coexist in graphics rather than merging into one API. No vendor roadmap currently commits to a single unifying on-device format.
- **NPU delegate discovery could re-converge with the graphics driver model.** Should tensor-resource extensions like `VK_ARM_tensors` see multi-vendor adoption, the vendor-specific `.so` delegate-discovery problem left by NNAPI's deprecation (§3.3) has a plausible long-run resolution in something closer to the Vulkan ICD loader model already used for GPU drivers — the same analogy this chapter draws in §17.5 — rather than persisting as a separate, ML-only plugin ecosystem. This is speculative: as of this writing only ARM has proposed the extension, and no cross-vendor commitment to that convergence exists yet.
