...
Android was one of the earliest mainstream platforms to mandate **Vulkan** as a first-class graphics API, turning the Linux graphics stack's investment in **Vulkan** into a mobile-scale deployment overnight. This chapter surveys the full Android Vulkan ecosystem:
...
- GPU architecture
- performance tooling
- Chrome's integration with the Android GPU stack

- **Section 1** — version mandate history from optional **Vulkan 1.0** in **Android 7.0 Nougat** to the required **Vulkan 1.1** floor on 64-bit devices in **Android 10**, and the **Android Vulkan Profiles** (**AVP**) that define guaranteed feature floors per year (AVP 2021, AVP 2022, AVP 2025).
- **Section 2 — The Android Vulkan Loader** (`/system/lib64/libvulkan.so`) — ICD enumeration via **Android HAL** system properties (`ro.hardware.vulkan`, `ro.board.platform`), the **`hwvulkan_device_t`** HAL interface (exporting **`EnumerateInstanceExtensionProperties()`**, **`CreateInstance()`**, **`GetInstanceProcAddr()`**), Vulkan layer loading rules for debuggable versus non-debuggable apps, **APEX** package delivery of ICDs (since **Android 15** via `ro.vulkan.apex`), and loader thread safety.
- **Section 3 — GPU Vendor Drivers** — three dominant proprietary **Vulkan** ICDs: **Qualcomm Adreno** (Snapdragon, **Adreno 740**, **Adreno 830**), **ARM Mali** (G-series, **Mali-G715**), and **Imagination PowerVR**; plus structural properties common to all proprietary Android drivers: closed-source binaries in `/vendor`, vendor-proprietary shader IR (not **NIR** or **LLVM IR**), and tight kernel-driver coupling (e.g. the `msm`, `mali`, and `pvr` kernel modules).
- **Section 4 — Mesa on Android (Turnip and freedreno)** — **freedreno** (OpenGL ES) and **Turnip** (Vulkan) over the **`msm`** DRM kernel driver; building with **`Android.bp`** and **meson**; the **`src/freedreno/`** source tree including the **`ir3/`** shader compiler and **`src/freedreno/vulkan/`** Turnip front-end; runtime detection via **`VK_DRIVER_ID_MESA_TURNIP`** and **`vkGetPhysicalDeviceProperties2()`**; and Turnip's widespread adoption in the emulator ecosystem.
- **Section 5 — ANGLE on Android** — **ANGLE**'s evolution into the system GLES driver on Pixel devices, **APEX** delivery via `com.android.angle`, enabling per-app via **ADB** or the Android 17+ manifest `<meta-data>` opt-in, the internal architecture built around **`vk::Renderer`** and **`ContextVk`**, and the **`EGL_ANDROID_GLES_layers`** debug layer mechanism.
- **Section 6 — AHardwareBuffer and Vulkan Interop** via **`VK_ANDROID_external_memory_android_hardware_buffer`** — import and export paths (**`vkGetAndroidHardwareBufferPropertiesANDROID()`**, **`vkGetMemoryAndroidHardwareBufferANDROID()`**), key structs (**`VkAndroidHardwareBufferPropertiesANDROID`**, **`VkImportAndroidHardwareBufferInfoANDROID`**, **`VkExternalFormatANDROID`**, **`VkAndroidHardwareBufferFormatPropertiesANDROID`**), a complete camera-frame-to-**`VkImage`** import workflow using **`VkMemoryDedicatedAllocateInfo`**, **YUV** camera formats and **`VkSamplerYcbcrConversion`**, and the reverse export path from **`VkImage`** to **`AHardwareBuffer`**.
- **Section 7 — Android-Specific Vulkan Extensions** — **`VK_KHR_android_surface`** and **`ANativeWindow`** for swapchain creation; **`VK_ANDROID_external_memory_android_hardware_buffer`** (guaranteed in AVP 2022); **`VK_ANDROID_external_format_resolve`** for in-tile **YCbCr** resolve; **`VK_EXT_swapchain_colorspace`** for wide-gamut and **HDR** display (guaranteed in AVP 2021); **`VK_GOOGLE_display_timing`** (**`vkGetRefreshCycleDurationGOOGLE()`**, **`vkGetPastPresentationTimingGOOGLE()`**) used by the **Android Frame Pacing** library (**Swappy**); **`VK_KHR_present_wait`** / **`VK_KHR_present_id`** for back-pressure control; and **`VK_QCOM_render_pass_transform`** for pre-rotation to avoid **DPU** bandwidth penalties.
- **Section 8 — Shader Compilation on Android** — **SPIR-V** mandatory for **`vkCreateShaderModule()`** (no runtime GLSL path); ahead-of-time compilation with **`glslc`** (bundled in the **NDK** under `<ndk>/shader-tools/`); runtime GLSL-to-SPIR-V compilation with the **`shaderc`** C++ library; deploying **`VK_LAYER_KHRONOS_validation`** on Android (APK bundle, `/data/local/debug/vulkan/`, or Play Store delivery); and the **SPIR-V** capability constraints imposed by the **AVP** profiles.
- **Section 9 — Memory Management on Mobile** — Android's unified-memory topology (no discrete VRAM; **CPU** and **GPU** share **LPDDR5** or **LPDDR5X**); the simplified **`VkPhysicalDeviceMemoryProperties`** heap structure; **`VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT`** for **MSAA** transient attachments that never leave on-chip tile **SRAM**; `VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT`; optimal **MSAA** render pass patterns (`VK_ATTACHMENT_STORE_OP_DONT_CARE`); and using **VMA** (`VMA_MEMORY_USAGE_GPU_LAZILY_ALLOCATED`) for portable lazy allocation.
- **Section 10 — Mobile GPU Architecture (TBDR)** — **Tile-Based Deferred Rendering** (**TBDR**) as the universal architecture of **Qualcomm Adreno**, **ARM Mali**, and **Imagination PowerVR** GPUs, contrasted with **IMR** of desktop GPUs (**RDNA**, **Arc**, **NVIDIA**); the binning pass / rendering pass split and on-chip tile **SRAM**; **`loadOp`**/**`storeOp`** declarations in **`VkRenderPass`** mapped to DRAM bandwidth; why explicit **`VkRenderPass`** objects remain important on mobile despite **`VK_KHR_dynamic_rendering`**; subpass merging for deferred renderers; and **`VK_EXT_multisampled_render_to_single_sampled`** as a simplified MSAA path on **TBDR** hardware.
- **Section 11 — Android GPU Performance Tools** — **Snapdragon Profiler** for **Adreno** hardware-counter and timeline analysis; **ARM Performance Studio** (including **Streamline**, **Graphics Analyzer**, and **Frame Advisor**) for **Mali**; **Android GPU Inspector** (**AGI**) (and its successor **Android Performance Analyzer** (**APA**)) for cross-vendor frame capture via Vulkan layer APK and **Perfetto**-based system profiling; the **Perfetto** GPU counter track (Android 12+); and **GFXReconstruct** for portable Vulkan API stream capture and replay.
- **Section 12 — Chrome and Chromium on Android** — **ANGLE**-on-**Vulkan** as Chrome's **OpenGL ES** backend; **WebGPU** shipping in Chrome 121 (Android 12+) via **Dawn**'s **Vulkan** backend with frames shared via **`AHardwareBuffer`** through **Mojo** IPC to the **Viz** compositor and on to **SurfaceFlinger** via **`ASurfaceControl`**; **WebGPU** compatibility mode on OpenGL ES 3.1 for older devices; and a comparison table of the Android GPU process versus the desktop (**GBM**, **DMA-BUF**, **DRM/KMS**) equivalent.
...
## 13. Android Runtime and Native Interop: JNI, NDK, and ART

The full treatment of ART's divergences from the JVM, the `@FastNative`/`@CriticalNative` JNI trampoline internals, the NativeActivity/GameActivity/ANativeWindow hosting model, and the NDK's Rust trajectory has been extracted into its own chapter to keep this chapter focused on the Vulkan stack.

**→ See [Chapter 164: Android Runtime and Native Interop: ART, JNI, and the NDK](ch164-android-runtime-native-interop.md)** for:

- ART vs HotSpot JVM: register-based DEX bytecode, dex2oat compilation tier ladder, Profile-Guided Compilation + Cloud Profiles, Concurrent Copying GC with Baker read barriers, userfaultfd GC (Android 13+), Zygote fork model and `.art` boot images, nterp interpreter (Android 12+), non-SDK interface restrictions, ART intrinsics, deoptimisation model.
- The ART JNI trampoline: standard JNI vs `@FastNative` vs `@CriticalNative` internals; GC interaction; `kAccFastNative`/`kAccCriticalNative` access flags; ART source files.
- NativeActivity, `android_native_app_glue`, and GameActivity (AGDK): lifecycle callbacks, `android_app` struct, `AInputQueue` vs double-buffered input arrays, `ANativeWindow` depth, Swappy frame pacing.
- Why Project Panama FFM does not apply to Android.
- Rust NDK ecosystem: `ndk`/`ndk-sys`, `ash`, `cargo-ndk`, `android-activity`, APEX ABI stability, NDK trajectory.
...
**Ch164 — Android Runtime and Native Interop (ART, JNI, NDK)**: Section 13 of this chapter redirects to Ch164, which covers the full ART/JNI/NDK material — `@FastNative`/`@CriticalNative` internals, the ART JNI trampoline, NativeActivity/GameActivity/ANativeWindow, Rust NDK ecosystem (`ndk`/`ndk-sys`, `ash`, `cargo-ndk`, `android-activity`), and NDK trajectory.

**Part I — The Kernel Layer (Nova driver)**: Ch164's NDK trajectory section notes that Rust is now entering the Android GPU *kernel* path via the Nova DRM driver (the Rust-language successor to `nouveau` for NVIDIA hardware on Linux). The same language shift visible in the NDK ecosystem at the application layer is simultaneous with the kernel-driver layer covered in the kernel chapters.
