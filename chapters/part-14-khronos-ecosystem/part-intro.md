...
The immediate horizon is dominated by **SPIR-V 1.7 and Vulkan 1.4 toolchain alignment**: `spirv-tools`, `glslang`, and `naga` are all being updated in parallel to emit and validate SPIR-V 1.7 modules, with promoted cooperative-matrix, ray-tracing, and `SPV_KHR_maximal_reconvergence` extensions arriving in sequential releases. Alongside this, **glslang's deprecated HLSL front end** is being removed, completing the transition to DXC for all HLSL-to-SPIR-V workloads in ANGLE and Dawn. On the asset-pipeline side, **KTX-Software v5.0** will ship its API-breaking `ktx_basis_codec_e` changes as stable, while the `XUASTC_LDR` supercompression scheme and improved UASTC HDR 6×6 transcoding will add new production codec tiers. The **glTF ecosystem** is seeing `EXT_meshopt_compression` promoted to a full `KHR` extension and `KHR_audio_emitter` ratified, with `fastgltf` v1.0 freezing its API. On the compute side, **Intel oneAPI DPC++ 2026.x** will complete the Unified Runtime Level Zero v2 adapter for Xe2/Arc B-series, removing the legacy PI codepath, while **AdaptiveCpp 26.x** stabilises its compile-once, run-anywhere distribution. **rusticl** is targeting fine-grain SVM completion on radeonsi toward a Khronos OpenCL 3.0 conformance submission, and **AMD ROCm** is pursuing CTS certification for CDNA3/RDNA3 hardware. The `cl_khr_unified_svm` extension is expected to be ratified, standardising what Intel's proprietary USM extension currently provides.

### Medium-term (1–3 years)

The medium-term converges on three overarching themes. First, **SPIR-V as a universal IR layer**: MLIR's `spirv` dialect is gaining `SPV_KHR_tensor_addressing` and cooperative-matrix layout parity, enabling ML compilers (IREE, XLA) to emit standard SPIR-V for all tile sizes; Mesa's `spirv_to_nir` is expected to absorb cooperative-matrix lowering and physical-storage-buffer arithmetic directly, reducing pre-processing dependencies on SPIRV-Tools. Second, **portable heterogeneous compute convergence**: SYCL 3.0 will add cooperative-groups semantics to close the last major feature gap with CUDA; a standardised Khronos AI/ML extension library is being scoped for portable matrix, convolution, and attention primitives; and the `cl_khr_external_semaphore`/`cl_khr_external_memory_dma_buf` extension set is expected to become first-class across Intel NEO, AMD CLR, and rusticl, enabling zero-copy V4L2-to-compute-to-display pipelines. Third, **format ecosystem maturation**: glTF 3.0 will formalise MaterialX 1.39 procedural materials and GPU-instancing extensions, while a GPU compute-shader transcoder in libktx will unlock streaming texture decode on the GPU; ASTC 3D block footprints will gain libktx encoder support for volumetric data pipelines; and Vulkan SC 2.0 (based on Vulkan 1.3) is under active scoping for safety-critical deployments. NNEF 2.0 will bring dynamic shapes and transformer operators to OpenVX embedded inference, and Clover's removal from Mesa will follow rusticl achieving full CTS coverage.

### Long-term

- **SPIR-V as a stable cross-vendor ML IR**: Khronos's `SPV_KHR_tensor_layout` and `SPV_KHR_subgroup_rotate` extensions are intended to form a standardised tensor-compute dialect within SPIR-V, potentially displacing vendor-specific PTX/GCN IR for portable inference on Linux; a versioned, signed binary-stable SPIR-V container (analogous to DXIL) remains an open discussion that would allow IHVs to cache and redistribute pre-compiled ISA.
- **Unified heterogeneous compute abstraction**: The long-term Khronos trajectory is for OpenCL C, SYCL kernel bundles, and Vulkan compute pipelines to converge at the `clCreateProgramWithIL`/SPIR-V ingestion boundary; simultaneously, SYCL's Unified Shared Memory model is expected to align with Linux kernel HMM advances, enabling coherent page-fault-driven GPU migration without explicit `mem_advise` hints — blurring the API distinction between an OpenCL NDRange and a Vulkan dispatch above a shared DRM GEM/syncobj substrate.
- **OpenUSD and neural-geometry convergence in asset formats**: glTF 3.0 and a planned bidirectional glTF ↔ USD interchange specification will align the Khronos and AOUSD material ecosystems; further out, NeRF/3D Gaussian splatting representations and neural texture compression schemes (sub-1 bpp MLP decoders) may be formalised as glTF and KTX2 extensions once hardware fixed-function support materialises.
