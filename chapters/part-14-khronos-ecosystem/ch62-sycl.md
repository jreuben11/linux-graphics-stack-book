...
- The **SYCL 2020** core abstractions — **`sycl::queue`**, **`sycl::buffer`**, **`sycl::accessor`**, **`sycl::event`** — and the **`sycl::handler`** command group handler, and how the task-**DAG** execution model differs from explicit synchronisation
- **Host** and **local** accessors (**`sycl::host_accessor`**, **`sycl::local_accessor`**) for synchronised host access and per-work-group shared memory, and how **`sycl::event`** profiling timestamps expose scheduling and driver overhead
- The three kernel dispatch forms — **`single_task`**, basic **`parallel_for`**, and **ND-range** **`parallel_for`** using **`sycl::nd_range`** and **`sycl::nd_item`** — and how to share data across work-items with **`sycl::local_accessor`** and **`sycl::group_barrier`**
- The group algorithms library (**`sycl::reduce_over_group`**, **`sycl::inclusive_scan_over_group`**, **`sycl::exclusive_scan_over_group`**, **`sycl::group_broadcast`**) and sub-groups (**`sycl::sub_group`**, **`sycl::select_from_group`**, **`sycl::permute_group_by_xor`**, **`sycl::shift_group_left`**) as the **SYCL** equivalent of **CUDA** warps, and built-in parallel reductions via **`sycl::reduction()`**
- When to use **Unified Shared Memory** (**USM**) versus the buffer/accessor model: the three **USM** allocation types (**`sycl::malloc_device`**, **`sycl::malloc_host`**, **`sycl::malloc_shared`**), explicit transfer with **`queue::memcpy`**, implicit page migration, and **`queue::prefetch`** / **`queue::mem_advise`** hints for discrete GPUs
- Specialisation constants (**`sycl::specialization_id`**, **`sycl::kernel_handler`**) for **JIT**-time constant folding and **`sycl::kernel_bundle`** for explicit control over compile/link state, including ahead-of-time (**AOT**) scenarios
- Device selection and the platform/device model: **`sycl::platform`**, **`sycl::device`**, built-in selectors (**`gpu_selector_v`**, **`cpu_selector_v`**, **`aspect_selector`**), custom selectors, and the **`SYCL_DEVICE_FILTER`** environment variable for restricting backend discovery
- How **DPC++** (**`icpx`**) implements the **single-source, multiple-compiler-passes** (**SMCP**) model to transform a single `.cpp` source into **SPIR-V** for Intel, **PTX** for NVIDIA, and **AMDGCN** for AMD; how the **Unified Runtime** (**UR**) loader dispatches through **`libur_adapter_level_zero.so`**, **`libur_adapter_opencl.so`**, **`libur_adapter_cuda.so`**, and **`libur_adapter_hip.so`**; and how the **Intel Graphics Compiler** (**IGC**) finishes the translation to native **Xe** ISA
- How **DPC++** targets **CUDA** (via the **NVPTX** backend, **`libdevice.bc`**, and **`ptxas`**) and **ROCm** (via the **AMDGCN** backend and Codeplay's **`libur_adapter_hip.so`**) for cross-device portability
