...
- **Section 2** explains why **GPU** texture compression exists at all — how fixed-ratio block formats enable hardware **TMU** decode, random-access sampling, and dramatically reduced **VRAM** and bandwidth pressure compared to CPU codecs like **JPEG** or **PNG**
- **Section 3** surveys the three block-compression format families: **BC1**–**BC7** (**S3TC**/**RGTC**/**BPTC**, desktop **NVIDIA**/**AMD**/**Intel**), **ETC1**/**ETC2**/**EAC** (the mandatory mobile baseline on all **Vulkan** **OpenGL ES 3.0** devices), and **ASTC** (**Adaptive Scalable Texture Compression**, variable block footprints from 4×4 to 12×12); it includes a hardware support matrix covering **Mesa** drivers (**radv**, **anv**, **nvk**, **panfrost**, **freedreno**, **etnaviv**) and shows how to query native hardware decode support at runtime via **vkGetPhysicalDeviceFormatProperties** and **VkPhysicalDeviceFeatures** rather than relying on feature bits alone; the offline **ETC2** encoder **etc2comp** and the reference **ASTC** encoder **astcenc** are also covered
- **Section 4** covers **Basis Universal** supercompression in depth:
  - The **ETC1S** global-codebook encoding pipeline applies **Vector Quantization** (**VQ**) across all mip levels, array layers, and cubemap faces to build shared endpoint and selector codebooks, achieving effective bit rates of 0.3–3 bpp with fast **BC1** transcoding via 1D lookup tables and near-lossless **BC7** transcoding via mode 6
  - **UASTC LDR 4×4** (a 19-mode subset of **ASTC LDR 4×4** at 8 bpp) uses per-block hint bits for rapid transcoding to **BC7** and **ASTC 4×4** at near-**BC7** quality; combining it with **Zstandard** post-compression (**KTX2_SS_ZSTANDARD**) reduces file sizes by 10–40% with fully lossless transcode
  - The **UASTC HDR** variants (**UASTC HDR 4×4**, **ASTC HDR 6×6**, **UASTC HDR 6×6 Intermediate**) extend supercompression to **HDR** content transcoding to **BC6H** and **VK_FORMAT_ASTC_4x4_HDR**
  - **XUASTC** LDR applies a **Weight Grid DCT** for latent-space supercompression achieving 0.3–5.7 bpp, though its **KTX2** scheme ID (**KTX2_SS_XUASTC_LDR**) is not yet ratified
  - The section also covers the **basisu** command-line encoder and the runtime transcoder **C++** API — **basisu_transcoder** for **.basis** files, **ktx2_transcoder** for **.ktx2** files — including the full **transcoder_texture_format** enum and low-level transcoders such as **basisu_lowlevel_etc1s_transcoder** and **basisu_lowlevel_uastc_ldr_4x4_transcoder**
- **Section 5** dissects the **KTX2** on-disk format: the 12-byte file magic identifier, the 80-byte little-endian header (carrying **vkFormat**, **supercompressionScheme**, **layerCount**, **faceCount**, **levelCount**, and offsets to the **DFD**, key-value metadata, and supercompression global data), the **KTX2LevelIndexEntry** level index table, the **supercompressionScheme** enum (**KTX2_SS_NONE**, **KTX2_SS_BASISLZ**, **KTX2_SS_ZSTANDARD**, **KTX2_SS_DEFLATE**, **KTX2_SS_UASTC_HDR_6x6I**), and the **Data Format Descriptor** (**DFD**) block with its **colorModel**, **colorPrimaries**, **transferFunction**, **KHR_DF_FLAG_ALPHA_PREMULTIPLIED**, and **texelBlockDimension** fields
- **Section 6** covers the **libktx** **C** API from **KTX-Software** (v4.4.2 / v5.0.0-rc1):
  - The **ktxTexture2** struct fields, the loading functions (**ktxTexture2_CreateFromNamedFile**, **ktxTexture2_CreateFromMemory**, **ktxTexture2_CreateFromStdioStream**), **ktxTexture2_NeedsTranscoding**, **ktxTexture2_TranscodeBasis** with the **ktx_transcode_fmt_e** enum (**KTX_TTF_BC7_RGBA**, **KTX_TTF_ASTC_4x4_RGBA**, etc.), creating and writing **KTX2** files via **ktxTexture2_Create**, **ktxTexture2_CompressBasis**, **ktxTexture2_CompressBasisEx**, **ktxTexture2_CompressAstcEx**, **ktxTexture2_DeflateZstd**, and **ktxTexture2_WriteToNamedFile**/**ktxTexture2_WriteToMemory**
  - The **ktxBasisParams** struct (with v5.0 breaking changes including the **ktx_basis_codec_e** enum replacing the **uastc** bool) gives full encoder control over **ETC1S** codebook sizing, **UASTC LDR** and **HDR** quality, **RDO** parameters, and normal-map mode
  - The **ktx** unified command-line tool (replacing the deprecated **toktx**) is covered with its **create**, **encode**, **validate**, and **info** subcommands
- **Section 7** covers the **Vulkan** upload path: the **ktxVulkanDeviceInfo** and **ktxVulkanTexture** structs, and the functions **ktxVulkanDeviceInfo_Create**, **ktxVulkanDeviceInfo_CreateEx**, **ktxTexture2_VkUploadEx**, and **ktxTexture2_VkUploadEx_WithSuballocator**; the full workflow — from feature query to **TranscodeBasis** to staging **VkBuffer** copy to **vkCmdCopyBufferToImage** to pipeline barrier to **VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL** — is shown with a format-selection decision tree covering **KTX_TTF_BC7_RGBA**, **KTX_TTF_ASTC_4x4_RGBA**, **KTX_TTF_ETC2_RGBA**, and the **KTX_TTF_RGBA32** fallback
- **Section 8** covers the **KHR_texture_basisu** **glTF 2.0** extension and two integration paths: a custom image hook for **tinygltf** (syoyo/tinygltf) via **TinyGLTF::SetImageLoader**, and the first-class **KHR_texture_basisu** support in **cgltf** (jkuhlmann/cgltf) via the **cgltf_texture.basisu_image** field; a full pipeline diagram traces the path from **.glb**/.gltf parse through **ktxTexture2_CreateFromMemory**, **ktxTexture2_TranscodeBasis**, **ktxVulkanDeviceInfo_Create**, **ktxTexture2_VkUploadEx**, and **vkCreateImageView** to binding a **VkDescriptorSet** for a **PBR** shader
- **Section 9** analyses compression quality trade-offs: a format selection matrix recommending **ETC1S**/**BasisLZ** for diffuse/albedo atlases and **UASTC LDR 4×4** for normal maps, metallic-roughness, and data textures; **PSNR** tier tables comparing **BC7**, **ASTC 4×4**, **UASTC 4×4**, **ETC2**, and **ASTC 8×8**; the effect of **ETC1S** global codebook size (**maxEndpoints**, **maxSelectors**, **qualityLevel**, **RDO**) on compression ratio; CPU versus GPU transcoder throughput trade-offs including **basisu_transcoder_state** reuse; and real-world compression numbers from the **glTF FlightHelmet** model and **Bevy**'s **KTX2** asset pipeline
...
- The **KHR_texture_basisu** **glTF** extension and how to wire a custom image loader in **tinygltf** or **cgltf** for **KTX2**-backed textures
