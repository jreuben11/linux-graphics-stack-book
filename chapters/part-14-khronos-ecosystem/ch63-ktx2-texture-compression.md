...
Section 4 covers **Basis Universal** supercompression in depth. The **ETC1S** global-codebook encoding pipeline applies **Vector Quantization** (**VQ**) across all mip levels, array layers, and cubemap faces to build shared endpoint and selector codebooks, achieving effective bit rates of 0.3–3 bpp with fast **BC1** transcoding via 1D lookup tables and near-lossless **BC7** transcoding via mode 6. **UASTC LDR 4×4** (a 19-mode subset of **ASTC LDR 4×4** at 8 bpp) uses per-block hint bits for rapid transcoding to **BC7** and **ASTC 4×4** at near-**BC7** quality; combining it with **Zstandard** post-compression (**KTX2_SS_ZSTANDARD**) reduces file sizes by 10–40% with fully lossless transcode. The **UASTC HDR** variants (**UASTC HDR 4×4**, **ASTC HDR 6×6**, **UASTC HDR 6×6 Intermediate**) extend supercompression to **HDR** content transcoding to **BC6H** and **VK_FORMAT_ASTC_4x4_HDR**. **XUASTC** LDR applies a **Weight Grid DCT** for latent-space supercompression achieving 0.3–5.7 bpp, though its **KTX2** scheme ID (**KTX2_SS_XUASTC_LDR**) is not yet ratified. The section also covers the **basisu** command-line encoder and the runtime transcoder **C++** API — **basisu_transcoder** for **.basis** files, **ktx2_transcoder** for **.ktx2** files — including the full **transcoder_texture_format** enum and low-level transcoders such as **basisu_lowlevel_etc1s_transcoder** and **basisu_lowlevel_uastc_ldr_4x4_transcoder**.
...
Section 6 covers the **libktx** **C** API from **KTX-Software** (v4.4.2 / v5.0.0-rc1). It details the **ktxTexture2** struct fields, the loading functions (**ktxTexture2_CreateFromNamedFile**, **ktxTexture2_CreateFromMemory**, **ktxTexture2_CreateFromStdioStream**), **ktxTexture2_NeedsTranscoding**, **ktxTexture2_TranscodeBasis** with the **ktx_transcode_fmt_e** enum (**KTX_TTF_BC7_RGBA**, **KTX_TTF_ASTC_4x4_RGBA**, etc.), creating and writing **KTX2** files via **ktxTexture2_Create**, **ktxTexture2_CompressBasis**, **ktxTexture2_CompressBasisEx**, **ktxTexture2_CompressAstcEx**, **ktxTexture2_DeflateZstd**, and **ktxTexture2_WriteToNamedFile**/**ktxTexture2_WriteToMemory**. The **ktxBasisParams** struct (with v5.0 breaking changes including the **ktx_basis_codec_e** enum replacing the **uastc** bool) gives full encoder control over **ETC1S** codebook sizing, **UASTC LDR** and **HDR** quality, **RDO** parameters, and normal-map mode. The **ktx** unified command-line tool (replacing the deprecated **toktx**) is covered with its **create**, **encode**, **validate**, and **info** subcommands.
