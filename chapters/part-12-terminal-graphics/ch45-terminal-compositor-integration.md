...
9. [Colour Space, HDR, and Terminal Graphics](#9-colour-space-hdr-and-terminal-graphics)
...
This chapter traces the full path of a frame of terminal output from the GPU render call down to the display engine's scanout, grounding each protocol and kernel mechanism in the concrete implementation choices that modern terminals make. Section 1 establishes how a terminal registers itself as a **Wayland** client, negotiating **xdg_toplevel** window management and the configure–resize cycle that drives **GBM** surface reallocation. Section 2 covers **EGL** context acquisition via **eglGetPlatformDisplayEXT()**, **GBM** buffer allocation with **gbm_surface_create_with_modifiers2()**, and the double-buffering model that governs **wl_buffer** release. Section 3 explains **DRM** format modifier negotiation through **zwp_linux_dmabuf_v1** and the **zwp_linux_dmabuf_feedback_v1** tranche mechanism, including the significance of modifiers such as **DRM_FORMAT_MOD_LINEAR**, **I915_FORMAT_MOD_4_TILED**, and **AMD_FMT_MOD_DCC** for zero-copy scan-out. Section 4 follows the GPU render loop from **eglSwapBuffers()** through **gbm_surface_lock_front_buffer()**, **gbm_bo_get_fd()**, **zwp_linux_buffer_params_v1**, and **wl_surface.commit()** to the compositor's import via **eglCreateImageKHR()** or **VkImportMemoryFdInfoKHR**. Section 5 covers the CPU-path taken by terminals such as **foot**, which use **pixman**, **memfd_create()**, and **wl_shm** instead of **DMA-BUF**, and discusses the contexts where this path is preferable. Section 6 addresses explicit synchronisation: the history of implicit **dma_fence** fencing, the **NVIDIA** **GSP** firmware problem that motivated **wp_linux_drm_syncobj_manager_v1**, and the acquire/release point protocol that eliminates race conditions on all hardware. Section 7 examines compositor-side processing: damage merging, **DMA-BUF** import and reuse, and the conjunction of conditions required for plane promotion — modifier compatibility with the **IN_FORMATS** plane property, absence of compositor effects, and single-output geometry alignment. Section 8 describes the **KMS** atomic commit that delivers the frame to the display engine, covering **drmModeAtomicAddProperty()**, the **FB_ID**, **IN_FENCE_FD**, and **SRC_\*** plane properties from **libdrm**, the display pipeline's degamma/CTM/gamma stages, and **wp_presentation** feedback for frame-pacing against **VRR** displays. Section 9 examines colour space and **HDR** constraints arising from **Sixel**, **Kitty**, and **iTerm2**'s implicit **sRGB** assumption, the **wp_color_management_v1** protocol, and what a future **Kitty** **HDR** extension would need to signal. Section 10 analyses the security model: the privilege boundary enforced by **DRM** render nodes (`/dev/dri/renderDN`), the kernel's **DMA-BUF** reference-counting, **Flatpak** `--device=dri` grants, and the sandboxing implications of the **Kitty** graphics protocol's **shm_open** transmission mode.
...
```c
/* Handling zwp_linux_dmabuf_feedback_v1 tranche events — typical terminal pattern */
/* Compositor: src/egl/drivers/dri2/platform_wayland.c in Mesa */
...
Presentation timing feedback closes the loop back to the terminal. The `wp_presentation` protocol allows the compositor to report when a committed buffer actually appeared on screen, with nanosecond-resolution timestamps, refresh interval, and sequence number. The terminal registers a `wp_presentation_feedback` object for each committed surface and receives a `presented` event carrying the VBLANK timestamp of the frame in which the buffer became visible, the output's refresh duration (the reciprocal of the refresh rate in nanoseconds), and the presentation flags (which indicate whether the presentation was synchronised to a hardware clock). The terminal uses this timestamp to pace its render loop: after receiving `presented` for frame N, it begins rendering frame N+1 immediately, aiming to complete and commit before the next VBLANK. For a 165 Hz display the VBLANK interval is approximately 6.06 ms; a terminal with a full sub-millisecond render budget (achievable with tight damage tracking per Chapter 44) has ample margin. The sequence number in the `presented` event can detect missed frames — a jump of more than one sequence number means the terminal committed too late and the compositor displayed an older buffer for an extra frame.

For variable-refresh-rate displays (via VRR or FreeSync, described in Chapter 3), the refresh duration in the `presented` event varies between the minimum and maximum refresh interval of the display. A terminal that paces itself on the `presented` timestamp automatically adapts to the variable rate without any VRR-specific code. This is one of the compelling advantages of the `wp_presentation` feedback model over the older `wl_surface.frame` callback, which provides a timestamp but not the refresh duration.

---

## 9. Colour Space, HDR, and Terminal Graphics
...
**Chapter 3** (Advanced Display Features): The `wp_color_management_v1` protocol discussed in Section 9, VRR/FreeSync interaction with `wp_presentation` in Section 8, and the HDR surface signalling opportunity for Kitty graphics all originate in the mechanisms described there.
...
**Chapter 23** (Legacy and Sandboxed App Support): Flatpak `--device=dri` grants, the xdg-desktop-portal DRM access model, and the security constraints on sandboxed terminals in Section 10 are rooted in the portal architecture described there.

**Chapter 26** (Hardware Video): The HDR metadata model used by VA-API video surfaces — `wl_surface` colour space advertising — is the analogue that a future Kitty HDR extension would replicate (Section 9).

**Chapter 38** (PipeWire and the Video Session Layer): Screen capture of terminal content via the xdg-desktop-portal screen-share portal (Section 10) uses the PipeWire session layer described there.

**Chapter 44** (Terminal GPU Rendering Architectures): The glyph atlas construction, image texture management, and damage tracking techniques described there are the operations that produce the GPU commands consumed at the start of Section 4's render loop.
