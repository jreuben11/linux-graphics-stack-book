...
1. [The Pixel-in-Terminal Problem](#1-the-pixel-in-terminal-problem)
   - [1.1 What is Sixel?](#11-what-is-sixel)
   - [1.2 What is the Kitty Graphics Protocol?](#12-what-is-the-kitty-graphics-protocol)
   - [1.3 What is the iTerm2 Inline Images Protocol?](#13-what-is-the-iterm2-inline-images-protocol)
...
Sixel is a raster image encoding format designed to transmit pixel graphics over a serial byte stream using printable ASCII characters. The name derives from "six pixels": the fundamental atom of the encoding is a single ASCII character representing a vertical strip of six pixels as a 6-bit bitmask. DEC shipped Sixel as part of the graphics mode of its VT240 and VT340 hardware terminals in the 1980s, where it served as the mechanism for loading raster images into a dedicated pixel framebuffer separate from the character cell raster.

A Sixel image stream is wrapped in a DCS (Device Control String) escape sequence, introduced by `ESC P` and terminated by `ESC \`. Within the stream, palette colour registers are defined with the `#` command, pixel bands are encoded as sequences of Sixel characters, and run-length repeat counts compress columns of identical colour. Because Sixel is fundamentally a palette-indexed format, source images with more than 256 distinct colours must be quantised to a palette before encoding, introducing visible banding on photographic content. There is no native transparency channel.

In the Linux graphics stack, Sixel operates entirely within the terminal emulator layer: the DCS sequence is intercepted by the terminal's escape sequence parser, the pixel data is decoded into a texture or pixel buffer, and the result is composited over the cell grid at the current cursor position. Sixel has the widest support of any pixel protocol across hardware serial terminals, SSH sessions to legacy servers, and modern terminal emulators including xterm, foot, mlterm, and WezTerm. Its structural limitations — the 256-colour ceiling, absence of alpha transparency, high bandwidth verbosity, and scrollback synchronisation complexity — are intrinsic to the palette-indexed model and cannot be resolved within the protocol.

### 1.2 What is the Kitty Graphics Protocol?

The Kitty Graphics Protocol is a modern terminal pixel protocol designed explicitly for GPU-accelerated terminal renderers. It provides a stateful client-server model in which the terminal maintains a persistent image store: a client transmits an image once and assigns it a 32-bit image ID, then issues lightweight placement commands that reference the stored image by ID rather than retransmitting pixel data. This mirrors the GPU texture object model — the image upload corresponds to a texture allocation and data transfer, and each placement corresponds to a textured quad draw call.

The protocol uses APC (Application Program Command) escape sequences (`ESC _ G ... ESC \`), chosen because APC strings pass silently through terminals that do not implement the protocol. Pixel data may be transmitted as raw RGB or RGBA, as a complete PNG file, or with zlib compression applied to raw pixels. The data may be delivered inline in the escape sequence payload, via a file path, or via a POSIX shared memory object for zero-copy same-machine transfers. Chunked transmission allows large images to be split across multiple escape sequences to avoid overflowing intermediate buffers in multiplexers.

Beyond image storage and placement, the protocol defines source-rectangle cropping, sub-cell pixel offsets, z-index layering for rendering above or below the text plane, animation frames with configurable inter-frame timing, and a Unicode placeholder mechanism. The placeholder mechanism encodes row, column, and image ID as combining diacritics on the private-use codepoint U+10EEEE, allowing multiplexers to move image cells through the cell buffer without understanding the graphics protocol. The Kitty Graphics Protocol is implemented in kitty, WezTerm, and several other GPU-accelerated terminals as of 2025–2026.

### 1.3 What is the iTerm2 Inline Images Protocol?

The iTerm2 Inline Images Protocol is a stateless terminal pixel protocol that encodes an image as a single OSC (Operating System Command) escape sequence. The entire image payload — a base64-encoded file in any format the terminal recognises, typically PNG or JPEG — is embedded in one OSC 1337 sequence along with parameters controlling display width, height, aspect ratio preservation, and inline display mode. There is no server-side image store, no image IDs, no placement persistence, and no deletion command: each escape sequence is a self-contained, one-shot image display request.

The stateless design makes the protocol inherently safe across multiplexer boundaries. Because the terminal accumulates no state that must be synchronised when a session is attached or detached, the iTerm2 protocol passes through tmux and GNU screen without the fragile state-tracking required by the Kitty protocol. This makes it the preferred choice for remote SSH sessions and for tooling that targets a broad range of terminal environments without control over which terminal is in use. The trade-off for simplicity is a reduced feature set: no animation, no z-index layering, no source-rectangle cropping, and no server-side deduplication of repeated images.

The protocol originated as a macOS-specific extension and is supported on Linux by WezTerm and partially by Konsole. Version 3.5 of the specification introduced a multipart transmission mechanism using `MultipartFile=`, `FilePart=`, and `FileEnd` parameters to work around length limits imposed by multiplexers on OSC sequences. In the Linux graphics stack, the iTerm2 protocol occupies the same position as Sixel and Kitty: escape sequence interception in the terminal emulator, decode into a texture or pixel buffer, and compositing over the cell grid — but with no persistent state on the terminal side beyond the duration of the single rendering operation.
...
## Strategic Outlook: Will Terminal Pixel Protocols Converge?

The three-protocol landscape described in this chapter imposes a concrete and measurable cost on application developers. A tool such as **ranger**, **yazi**, or a Neovim image plugin (e.g., **image.nvim**) must maintain detection logic, encoding backends, and placement strategies for all three protocols independently. The **tmux** passthrough path (`allow-passthrough`) only partially alleviates this: it forwards bytes it does not parse, which helps Sixel and iTerm2 but not Kitty's stateful image store, whose server-side object model tmux cannot proxy transparently. The practical result is that every image-aware TUI application carries a multi-protocol fallback stack — Kitty first, iTerm2 second, Sixel third, Unicode block characters as the final fallback — and each layer must be independently tested against each terminal that might be in the call chain. This is not a small maintenance burden, and it grows as new terminals ship partial implementations of one or more protocols.

The single largest barrier to convergence is **VTE**. The GTK terminal widget that powers **GNOME Terminal**, **Tilix**, and several other GNOME-ecosystem terminals has Sixel support gated behind the `-Dsixel=true` build flag, but has not shipped Kitty or iTerm2 support, and the VTE maintainers have historically been sceptical of in-band pixel protocols on architectural grounds — the argument being that pixel graphics belong in a Wayland sub-protocol between the terminal and compositor, not in the escape-sequence stream. This position is technically coherent but its practical effect is to block the GNOME desktop ecosystem from participating in the GPU-accelerated protocol tier. Until VTE ships Kitty support with that flag enabled by default in major distributions, any application that wants to reach GNOME Terminal users must carry the Sixel fallback unconditionally.

From a protocol-design standpoint, the technical superiority of the Kitty Graphics Protocol is not disputed. Named image IDs with explicit lifetimes, placement IDs that survive cell-grid mutations, sub-cell pixel offsets, z-index ordering, RGBA transparency, animation frames, and POSIX shared memory zero-copy delivery constitute a qualitatively different capability tier from Sixel's palette-indexed band encoding. The installed base arithmetic, however, runs in Sixel's direction: every xterm-derived terminal on every distribution, including those shipped to embedded and enterprise systems that will not be updated for a decade, understands Sixel. This asymmetry is not resolving; it is stabilising. Kitty is the correct protocol for a GPU-accelerated local terminal session. Sixel is the correct protocol for any environment where the terminal inventory is not known in advance.

The realistic convergence path is therefore dual-track rather than singular. **Kitty** is becoming the de facto standard for the GPU-accelerated terminal tier — **kitty**, **WezTerm**, **Ghostty**, and prospectively **foot** — while **Sixel** retains its role as the universal compatibility fallback for SSH sessions, legacy servers, and hardware terminals. The **freedesktop.org terminal-wg**, despite having been active since 2018, has not produced a binding specification for image protocols, and there is no credible evidence that a new formal standard is imminent. The **Good Image Protocol** remains the most technically complete alternative proposal, but with only one known implementer after six years it has not crossed the adoption threshold that would make it a safe primary target. The most probable long-term outcome is not a new standard displacing the existing three protocols but rather an implicit two-tier equilibrium: Kitty for capable terminals, Sixel for everything else, with VTE's eventual adoption of Kitty — if and when it occurs — constituting the moment at which the Kitty tier achieves sufficient coverage to be treated as universally available on the Linux desktop.
