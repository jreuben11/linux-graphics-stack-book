...
Nova is the most structurally significant change in Linux's NVIDIA driver landscape since the 2022 nvidia-open release. Where the nvidia-open release opened the CPU-side stub of NVIDIA's existing proprietary driver, Nova is a clean-sheet reimplementation: a two-component Rust driver that makes the GSP-RM firmware the sole source of hardware authority for Turing and later GPUs. It was merged in its initial form into Linux 6.15 (May 2025), marking the first Rust-written Direct Rendering Manager driver in the mainline kernel — a milestone both for the Rust-in-Linux project and for open NVIDIA GPU support. This chapter is directed at kernel driver developers and Rust-in-kernel contributors who need to understand Nova's architecture at the source level; graphics application developers will find the chapter useful for understanding the kernel interface that the Mesa NVK Vulkan driver is expected to eventually target.
