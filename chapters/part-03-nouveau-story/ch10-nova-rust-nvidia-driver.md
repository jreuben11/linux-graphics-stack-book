...
Section 1 examines the design motivation for **Nova**: the accumulated complexity of **nvkm** (Nouveau's internal hardware abstraction layer) across GPU generations from **NV04** through **Ada Lovelace**, the opportunity that **GSP-RM** firmware presents as a clean abstraction boundary for **Turing**+ hardware, the maturity of the **Rust-for-Linux** project as a compelling fit for reference-counted **GEM** buffer objects and device lifetimes, and the architectural decision to split **Nova** into two separate kernel modules — **nova-core** (`drivers/gpu/nova-core/`) and **nova-drm** (`drivers/gpu/drm/nova/`) — connected via the **Linux Auxiliary Bus**.

Section 2 covers **nova-core** in depth: its source tree layout under `drivers/gpu/nova-core/`, the **PCI** probe and GPU identification flow via `NV_PMC_BOOT_0` and `NV_PMC_BOOT_42` **MMIO** registers and the `Chipset` enum, firmware versioning against a pinned **linux-firmware** version (`570.144`), the multi-stage **GSP** boot sequence (VBIOS parsing, firmware loading, **WPR2** framebuffer layout, **FWSEC-FRTS** execution, **GSP** falcon **LIBOS** boot, booter execution on the **SEC2** falcon, **GspSequencer**, and command-queue establishment), the dual circular-buffer command queue architecture in `gsp/cmdq.rs` using **DMA**-coherent `GspMem`, **DebugFS** exposure of the three **GSP** log buffers (`loginit`, `logintr`, `logrm`) via `debugfs::Scope`, and the `Falcon<E: FalconEngine>` generic abstraction covering both **DMA** and **PIO** firmware transfer modes and per-architecture **FalconHal** trait dispatch.

Section 3 covers **nova-drm**: its registration as an **auxiliary device** driver binding to **nova-core**'s auxiliary device, the `NovaDriver` implementation of `drm::Driver` and its three current ioctls (`NOVA_GETPARAM`, `NOVA_GEM_CREATE`, `NOVA_GEM_INFO`), the `gem::Object<NovaObject>` **GEM** buffer object implementation with **ARef**-based reference counting, and the deliberately minimal current feature set — a render node at `/dev/dri/renderDN` with no command submission, no **GPUVM**, no **drm_syncobj**, no display/**KMS**, and no **PRIME**/**DMA-BUF** support.

Section 4 analyses the **Rust** implementation techniques that distinguish **Nova** from its **C**-based counterparts: the **DRM** **Rust** abstraction layer types (`drm::Device<T>`, `drm::gem::Object<T>`, `drm::File<T>`, `auxiliary::Driver`), typed **MMIO** register access and the `bounded_enum!` macro in `bitfield.rs` for validated hardware enum conversions, the `PinInit` / `try_pin_init!` in-place initialization pattern for pinned structs, memory safety discipline over **DMA**-coherent shared memory in `Cmdq`, the `FirmwareObject<F, S>` typestate pattern that enforces firmware signature patching at compile time, and a concrete comparison of these techniques against **Nouveau**'s **C** model for reference counting, error-path cleanup, and lock-discipline documentation.

Section 5 covers GPU generation scope — **Turing** (**TU102**–**TU116**, **SM 7.5**), **Ampere** (**GA100**–**GA107**, **SM 8.x**), and **Ada Lovelace** (**AD102**–**AD107**, **SM 8.9**) — and what **Nova** explicitly does not support (pre-**Turing** hardware, **Hopper**/**GH100**, **Blackwell**/**GB100**/**GB202**, display/**KMS**, and **CUDA**). A kernel-version history table traces **Nova** milestones from **Linux 6.15** through **Linux 7.2**, and a comparison with **nvidia-open** clarifies that both drivers consume the same **GSP-RM** firmware blobs but share no CPU-side code or abstraction model.

Section 6 situates **Nova** in the broader **Rust**-in-**DRM** landscape: the dedicated `drm-rust-next` development tree, the parallel **ARM** **Tyr** driver for **Mali CSF** hardware developed by Collabora, and the two-track contribution workflow spanning **DRM** maintainer review and **Rust-for-Linux** maintainer review.

Section 7 addresses co-existence with **Nouveau** at runtime, the feature parity milestones required before **Nova** can become the default open driver for **Turing**+ hardware (GPU command submission, **GPUVM** integration via `drm_gpuvm`, **drm_syncobj** timeline synchronization, display/**KMS**, and **NVENC**/**NVDEC** video engine support), the intended **NVK** backend transition once **nova-drm** reaches **UAPI** stability with `DRM_NOVA_EXEC` and **GPUVM** bind ioctls, and the distinction between the internal module names (`nova_core.ko`, `nova.ko`) and the informal project names used in mailing-list discussions.
