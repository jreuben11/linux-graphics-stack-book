...
This chapter traces that methodology from its origins. It begins with the historical context: why NVIDIA's proprietary driver was intolerable in a mainline kernel distribution, and what the community attempted before reverse engineering became the explicit plan. It then dives deeply into the two central technical instruments — mmiotrace for capturing MMIO register traffic and Envytools for decoding and organising the results — before turning to how that accumulated knowledge is systematised in the hardware register database (HWDB). The chapter closes with an honest treatment of the legal framework that constrains this work and the ethical tensions that have intensified as NVIDIA has begun providing partial official support. After reading this chapter you will understand the full capture-to-analysis pipeline well enough to run traces yourself, and you will understand how the symbolic register knowledge in the Envytools XML database becomes the C constants scattered throughout the kernel driver source.
