...
Section 2 dives into the primary technical instrument for capturing hardware behaviour: **mmiotrace**. It explains how **NVIDIA** GPUs expose their register space to the host CPU through the **PCI** **BAR** (Base Address Register) mechanism — **BAR0** as the primary control register space, **BAR1** as the framebuffer aperture — and how **mmiotrace** exploits the CPU's virtual memory hardware to intercept every read and write to those **MMIO** regions via a page-fault tracing mechanism developed by Pekka Paalanen and integrated into the mainline kernel in **Linux 2.6.31**. The section covers:

- **mmiotrace log format** — records direction, width, address, value, instruction pointer, and timestamp for each register access
- **CONFIG_MMIOTRACE activation** — enabled through the **ftrace** infrastructure; requires all but one CPU offline during a trace on SMP systems
- **valgrind-mmt** — a fork of the **Valgrind** dynamic instrumentation framework that captures GPU pushbuffer contents by intercepting **mmap()** calls for command buffers
- **demmt** — consumes both **mmiotrace** and **valgrind-mmt** output to produce human-readable annotations

Section 3 examines **Envytools** — the repository at `https://github.com/envytools/envytools` that is the knowledge base those observations accumulate into. Its principal components are:

- **rnndb/** — the rules-ng-ng XML register database describing every characterised GPU register (address within **BAR0**, bitfield semantics) for GPU families **NV50**, **NVC0**, **NVE0**, **GM200**, **GP100**, **GV100**, **TU100**, and **GA100**; coverage is strongest for **NV50** through **Maxwell**, with **Ampere** (**GA100**) and **Ada Lovelace** (**AD102**) significantly incomplete
- **headergen2** — generates C `#define` header files from the XML, producing constants used throughout the **nvkm** kernel driver, closing the loop from raw observation to symbolic driver code
- **nvkm_rd32 / nvkm_wr32** — access macros defined in `drivers/gpu/drm/nouveau/include/nvkm/core/device.h`, wrapping `ioread32_native()` and `iowrite32_native()` against the `ioremap()`'d **BAR0** base address
- **envydis** — a complete disassembler for the **Falcon** (FAst Logic CONtroller) microcontroller ISA and for GPU shader ISAs including **g80**, **gf100**, **gk110**, and **gm107**; **envyas** is the complementary assembler
- **demmt** — decodes **NV50_3D** method-value pairs from **valgrind-mmt** traces using the **rnndb** symbolically, producing human-readable pushbuffer analysis

The **Falcon** processor runs multiple firmware instances per GPU: **PDAEMON** (power management daemon), **PGRAPH CTX** (graphics context switching), **PMU** (performance monitoring unit), and the Video Processor (**VP**).
...
- `src/nouveau/headers/` — in the **Mesa** source tree for **NVK**

It explains the categories of hardware knowledge that go beyond register names:

- **Timing constraints** — operations such as **PLL** programming require writing registers in sequence and waiting for a lock signal; the correct polling register and minimum wait time must be reverse-engineered from observed driver behaviour
- **Engine reset sequences** — time-dependent procedures for recovering a stalled GPU engine; incorrect timing causes the engine to appear recovered while producing incorrect results
- **Silicon errata** — hardware bugs present in specific chip steppings, tracked through stepping identifiers read from **NV_PMC_BOOT_0** (e.g., the multiple NV50 A0/A1/A2 revisions each required different workarounds)

The section also examines the hardware tables in the **Mesa** Gallium-based nouveau classic driver under `src/gallium/drivers/nouveau/`, including the **nv50_ir/** shader compiler backend whose scheduling constraints and performance characteristics were entirely inferred from benchmarking. The narrative documentation at `https://envytools.readthedocs.io`, written in **reStructuredText** and built with **Sphinx**, constitutes the closest equivalent to a hardware reference manual the Nouveau community has produced.

Section 5 covers community structure and key contributors across four phases:

- **Founding phase (2005–2008)** — Marchesin, Xavier Chantry, and Pekka Paalanen establishing the **mmiotrace** methodology and early **KMS** (Kernel Mode Setting) integration
- **Ben Skeggs era (2009–2023)** — Skeggs architected the **nvkm** refactor and drove GPU frequency scaling support
- **2012 Aalto University incident** — Linus Torvalds's public criticism prompted NVIDIA to begin piecemeal cooperation including signing firmware blobs for Kepler
- **Current era (2020–present)** — Danilo Krummrich's memory management rewrite and Faith Ekstrand's **NVK** Vulkan driver, which achieved **Vulkan 1.3** conformance on **Turing**, **Ampere**, and **Ada** hardware and was enabled by default in **Mesa 24.0**

Section 6 provides an honest treatment of the legal framework and ethical tensions. It explains why the classical "clean-room" doctrine — used by **Phoenix BIOS** and **Wine** — does not strictly apply to **Nouveau**, since the same developers who run traces also write driver code. The section covers:

- **17 U.S.C. §1201(f) (DMCA)** — US interoperability protection, supported by Ninth Circuit precedents in *Sega v. Accolade* and *Sony v. Connectix*
- **Article 6, Directive 2009/24/EC** — the equivalent EU protection under the Computer Programs Directive
- **NVIDIA EULA prohibition** — the reverse-engineering ban in NVIDIA's end-user licence agreement and why no legal action has been brought against the project
- **GSP-RM ethical question** — whether delegating hardware management to NVIDIA's signed firmware blob rather than reverse-engineering register interfaces means the driver is genuinely open source
...
- **Hopper and Blackwell GSP-RM enablement in mainline Nouveau**: In May 2025, Ben Skeggs posted a 62-patch series adding Nouveau support for NVIDIA GH100 (Hopper), GB10x, and GB20x (Blackwell) GPUs via the GSP-RM firmware path using the 570-series signed firmware blobs. Merging and stabilising this work is the project's primary near-term goal. [Source](https://www.phoronix.com/news/NVIDIA-Nouveau-Hopper-Blackwell)
- **Expanded Hopper and Blackwell open-source header files**: NVIDIA continues releasing additional header files for Hopper and Blackwell architectures, extending the register-name coverage available to Nouveau and NVK without requiring mmiotrace-based inference. These headers complement the rnndb for pre-GSP generations. [Source](https://news.slashdot.org/story/25/07/21/207245/nvidia-makes-more-hopper-blackwell-header-files-open-source)
- **Nova (Rust NVIDIA driver) Hopper/Blackwell preparatory work**: NVIDIA engineers are actively upstreaming preparatory patches for Hopper and Blackwell GPU support in the Nova Rust kernel driver, which is intended to eventually supersede both the proprietary nvidia.ko and the reverse-engineered Nouveau for supported hardware generations. [Source](https://www.phoronix.com/news/Hopper-Blackwell-Nova-Prep)
- **NVK Vulkan conformance expansion**: NVK achieved Vulkan 1.3 conformance on Turing, Ampere, and Ada hardware in Mesa 24.0; near-term work targets bringing Hopper-class hardware to full conformance as GSP-RM enablement matures. Note: specific timeline needs verification against Mesa milestone planning.
- **rnndb maintenance for Maxwell and Pascal**: For the older generations still in active deployment, the Envytools rnndb continues to receive corrections as NVK's conformance testing surfaces incorrect or incomplete bitfield definitions; these are low-drama but important for the accuracy of drivers running on pre-Turing hardware. Note: needs verification for specific outstanding entries.

### Medium-term (1–3 years)

- **GSP-RM message protocol partial documentation**: As NVIDIA publishes more of its open kernel module source and as Nova development proceeds, the high-level GSP-RM RPC message protocol that replaces direct register programming on Turing and later hardware is expected to become better documented. This will reduce the reliance on reverse-engineered inference for the GSP message interface and make the ethical concern about opaque firmware blobs less acute. [Source](https://developer.nvidia.com/blog/nvidia-transitions-fully-towards-open-source-gpu-kernel-modules.md/)
- **Envytools Blackwell (GB10x/GB20x) rnndb entries**: As GSP-RM enablement for Blackwell matures and NVIDIA releases additional headers, the community will backfill Blackwell register entries into the rnndb XML for the subset of registers still exercised by non-GSP paths (display engine, power management basics). The rnndb coverage for Ampere through Ada currently remains significantly incomplete relative to the older Maxwell/Pascal entries. [Source](https://github.com/envytools/envytools)
- **Nova replacing Nouveau for new-generation hardware**: NVIDIA's stated trajectory is that Nova — the modern Rust-based upstream driver — will become the preferred driver for Hopper and Blackwell hardware, with the reverse-engineering-based Nouveau methodology remaining relevant only for pre-Turing hardware that cannot run GSP-RM firmware. This transition will gradually reduce the scope where mmiotrace-based reverse engineering is the primary methodology. [Source](https://www.phoronix.com/news/Hopper-Blackwell-Nova-Prep)
- **demmt and valgrind-mmt tooling refresh**: The valgrind-mmt and demmt toolchain has not received significant updates for Ada Lovelace and later pushbuffer formats. Medium-term work will be needed to extend demmt's NV50_3D method decoding tables to cover the method-value pairs used in Ada and Hopper command streams if direct command-stream analysis is required for those generations. Note: needs verification against current demmt development activity.

### Long-term

- **Archival and knowledge preservation for pre-Turing hardware**: As Turing and later hardware becomes the GSP-RM baseline, the decades of mmiotrace-based register knowledge for NV50 through Maxwell represents irreplaceable documentation of hardware that no vendor will ever document. Long-term, the Envytools rnndb and envytools.readthedocs.io narrative documentation serve an archival function — preserving knowledge of hardware whose proprietary driver support will eventually be discontinued. Ensuring this knowledge remains discoverable and accurate is a community responsibility independent of any new feature work.
- **Automated trace analysis tooling**: The manual labour of reading mmiotrace logs and cross-referencing against rnndb remains high. A long-term direction is more automated analysis — using machine learning or symbolic execution to identify register patterns across multiple traces and propose rnndb entries. No concrete project currently pursues this, but it represents the natural evolution of the methodology if reverse engineering of new hardware (e.g., hypothetical post-Blackwell generations where documentation is again withheld) were to become necessary. Note: speculative.
- **Formal verification of rnndb register semantics**: As the rnndb grows and is used by both kernel drivers and Mesa, the risk of a subtly incorrect bitfield definition causing a correctness bug increases. A long-term goal is formal or semi-formal verification of rnndb entries against observed hardware behaviour using systematic test harnesses — similar in spirit to dEQP conformance testing but targeting register-level correctness rather than API conformance. Note: speculative direction, no concrete RFC as of 2026.
