...
**Falcon** microcontrollers are a second major architectural axis. Starting around the **NV98** generation, NVIDIA embedded small **Falcon** **RISC**-like processors inside the GPU die for power management, clock sequencing, context save/restore, and firmware authentication. The relevant **Falcon** instances are:
- **PMU** — power management
- **PDAEMON** — pre-Kepler power management
- **GR CTX Falcon** — graphics context save/restore
- **SEC2** — firmware authentication on **Turing**+
- **GSP** — full GPU firmware stack

The **nvkm_falcon** abstraction layer in **drivers/gpu/drm/nouveau/nvkm/falcon/** provides **nvkm_falcon_fw_boot()** to orchestrate the reset–load–boot sequence common to all **Falcon** processors, while generation-specific vtables handle differing register layouts.

This chapter proceeds from the macro to the micro, covering:
- **nvkm_device / nvkm_subdev / nvkm_engine / nvkm_object hierarchy** — the three-tier object model
- **Falcon microcontrollers** — NVIDIA's embedded engine controllers for engine management
- **channel and pushbuffer submission**
- **drm_sched GPU scheduler integration**
- **GPU memory management** — via **TTM** and **drm_gpuvm**
- **nvkm_device_chip chipset descriptor tables** — per-family variation and key architectural transitions from **NV04** through **AD100/Ada**, including the firmware-signing requirement introduced on **Maxwell**, the five-level **MMU** arriving on **Pascal**, and the **GSP-RM** dependency beginning on **Ampere**
- **DRM/KMS driver interface** — **nouveau_driver** registration in **nouveau_drm.c**, the component framework that separates the display subsystem, and the full ioctl surface from the legacy **DRM_NOUVEAU_GEM_PUSHBUF** path to the modern **DRM_NOUVEAU_VM_BIND** and **DRM_NOUVEAU_EXEC** ioctls that power **NVK**
