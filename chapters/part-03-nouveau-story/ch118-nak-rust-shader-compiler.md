...
- [1. Why a New Compiler: The Case Against nv50\_ir](#1-why-a-new-compiler-the-case-against-nv50_ir)
  - [1.1 What is NAK?](#11-what-is-nak)
  - [1.2 What is NIR?](#12-what-is-nir)
  - [1.3 What is SASS?](#13-what-is-sass)
...
NAK (the Nvidia Awesome Kompiler) is the NVIDIA GPU shader compiler backend for the open-source Mesa graphics stack, written in Rust and merged into Mesa 24.0 in February 2024. It occupies the final compilation stage in the NVK Vulkan driver pipeline: it accepts NIR (Mesa's common shader intermediate representation) as input, performs NVIDIA-specific optimization and code generation, and emits SASS (Streaming ASSembly) binary blobs ready for the GPU's command processor to load. Before NAK, the equivalent role was filled by the `nv50_ir` compiler, a C++ codebase accumulated around the Gallium3D stack over many years that lacked support for GPU generations from Turing (sm75) onwards and had a register allocator with hard failure modes at high register pressure. NAK was designed from scratch with a clean SSA-throughout architecture, a principled NIR handoff boundary, and Rust's tagged-union enum types and borrow checker providing structural guarantees that C++ could not economically deliver. Within Mesa it is the first GPU compiler backend written in Rust, and its integration established the build system conventions (Meson Rust support), FFI boundary design (seven C-callable entry points generated with `bindgen`), and minimum Rust version (1.82.0) that later Rust backends followed. The chapter traces NAK from its IR design through its optimization passes, register allocator, ISA encoders for Maxwell through Hopper, and the NVK entry point `nak_compile_shader()` that connects shader compilation to the Vulkan pipeline. Source: `src/nouveau/compiler/nak/` in the Mesa repository at `gitlab.freedesktop.org/mesa/mesa`.

### 1.2 What is NIR?

NIR (New Intermediate Representation) is Mesa's common shader intermediate representation, used as the canonical form through which all Mesa GPU drivers receive shaders for compilation. It operates in Static Single Assignment form, carries explicit type information (base type, number of components, bit-width), and is designed so that GPU-independent optimization passes — algebraic simplification, copy propagation, dead code elimination, loop unrolling, scalar lowering — run once in NIR and benefit every downstream driver backend. A Vulkan shader's journey through Mesa begins with SPIR-V bytecode converted to NIR by `spirv_to_nir`; the resulting NIR then passes through a driver-agnostic optimization loop before the driver-specific backend receives it. NAK's entry point `nak_compile_shader()` receives a NIR shader that has already been through all required NIR lowering passes mandated by NVK; inside NAK, `from_nir.rs` translates each NIR instruction into one or more NAK IR instructions, preserving SSA structure. The NIR handoff boundary is a deliberate design choice: heavy optimization work shared with other drivers stays in NIR and C, while only NVIDIA-specific optimization, uniform-register promotion, register allocation, and SASS encoding happen in Rust. The NIR pass API and instruction set are defined in `src/compiler/nir/` in the Mesa source tree, with documentation at `docs.mesa3d.org/nir.html`.

### 1.3 What is SASS?

SASS (Streaming ASSembly) is the native machine-code instruction set executed by NVIDIA GPU shader processors. It is distinct from PTX (Parallel Thread Execution), which is NVIDIA's documented virtual ISA; PTX is compiled to SASS by the proprietary driver at runtime, while SASS is the actual binary encoding that GPU hardware decodes and executes directly. NVIDIA does not publicly document the SASS ISA, so Mesa's SASS encoders were developed through reverse engineering of hardware behavior, study of open tools such as Envytools (`github.com/envytools/envytools`), and analysis of PTX-to-SASS outputs produced by NVIDIA's proprietary compiler. The encoding format changed substantially across GPU generations: Maxwell (sm50/sm52), Pascal (sm60/sm61), and pre-Volta Turing (sm70 refers to Volta here) use 64-bit instruction words, while Volta (sm70) and all later generations use 128-bit instruction words where the upper 64 bits encode the instruction and the lower 64 bits carry embedded scheduling control (latency counts, dependency wait masks, and yield flags). NAK implements separate encoder modules for each supported ISA variant: `sm50_encode.rs`, `sm70_encode.rs`, `sm75_encode.rs`, `sm80_encode.rs`, `sm89_encode.rs`, and `sm90_encode.rs`. The encoder output is a flat byte array prepended with an 80-byte Shader Program Header (SPH) that the NVK driver passes to the kernel for direct upload to the GPU.
...
Tinygrad's NIR path represents one instance of a broader pattern where NAK serves as the terminal compilation step for ML and compute workloads originating well above the Mesa layer. Two other instances follow the same structural logic:

**rusticl → NAK.** Mesa's `rusticl` (Chapter 25) is an OpenCL 3.0 implementation written in Rust that sits on top of Gallium pipe drivers. On NVK hardware, the path is: OpenCL kernel source → `rusticl` → NIR → NAK → NVIDIA SASS. The OpenCL application never calls into the NVIDIA proprietary ICD; the entire stack is Mesa. `rusticl` achieved Khronos OpenCL 3.0 conformance on radeonsi/iris in Mesa 24.x; NVK/NAK conformance is the natural next milestone as NVK matures.

**IREE → SPIR-V → NIR → NAK.** IREE (Intermediate Representation Execution Environment, Chapter 91) is an MLIR-based ML deployment compiler and runtime that can target Vulkan compute. Its compilation path emits SPIR-V from MLIR's `linalg` and `vector` dialects; the Vulkan runtime submits that SPIR-V to `spirv_to_nir()`, then NAK compiles to SASS. JAX and XLA-based models deployed through IREE follow this same route. Unlike Tinygrad's direct NIR emission, the IREE path goes through SPIR-V as an intermediate, adding a translation step but gaining compatibility with any Vulkan compute implementation. For both paths the quality of SASS the model's kernels execute at is determined entirely by NAK.
...
## Strategic Outlook: NAK, Rust, and the Future of GPU Compiler Infrastructure in Mesa

ACO established a thesis in 2019: for GPU ISA targets, a Mesa-maintained custom compiler beats a general-purpose LLVM backend on every metric that matters — compile latency, occupancy quality, VGPR pressure management, and developer iteration speed. The LLVM AMDGPU backend was functional; it simply could not be shaped around the RADV team's specific constraints quickly enough to matter. ACO, written in C++, won on all four axes. Within eighteen months it became RADV's default. That demonstration cleared the psychological and political path for subsequent custom compilers.

NAK is the second data point in that argument, and the first in Rust. Its predecessor was not LLVM but nv50\_ir — sixty thousand lines of bespoke C++ carrying design decisions from the Fermi era, with a register allocator that hard-failed under pressure rather than spilling, no concept of Turing's uniform register file, and an ISA encoder incompatible with Volta's 128-bit instruction word format. The argument for a clean-sheet NAK was structural, not ideological. But the choice of Rust added something ACO's C++ implementation could not guarantee: the borrow checker converts compiler data-structure discipline from a social convention into a compile-time requirement. Use-after-free in a register allocator, iterator invalidation during a pass that both reads and rewrites its instruction list, aliasing between a phi's source and its destination — these are the failure modes that make GPU compilers hard to extend safely. Rust's ownership model addresses them at the type level. Faith Ekstrand described the borrow checker as "a code review buddy"; what that means in practice is that the entire class of nv50\_ir's hard-failure allocation bugs cannot be introduced into NAK by construction.

If NAK continues to close the gap with NVIDIA's proprietary compiler on SASS quality — shader instruction count, register allocation efficiency, spill rate on register-heavy compute kernels — it will confirm what ACO already showed: that GPU ISA compilers built close to the hardware, owned by the driver team, and iterated inside the same repository as the driver itself outperform LLVM's general-purpose architecture on the constraints that GPU workloads impose. Wave-level scheduling, VGPR pressure management, and occupancy-aware register allocation are not optimisations LLVM is designed to reason about natively. NAK and ACO are not edge cases; they are the Mesa community's answer to that mismatch. Intel's BRW backend (the compiler for the GEN EU ISA) completes the picture from a different direction — Intel graphics codegen was never through LLVM to begin with, making BRW the existence proof that per-vendor ISA-aware Mesa compilers predate ACO's explicit LLVM-replacement argument by nearly a decade.

Whether Rust will expand beyond NAK inside Mesa is not yet settled, but the evidence leans toward "yes, and not just for NVIDIA." KRAID (merged June 2026, ARM Mali Valhall/v9) explicitly modelled its IR on NAK and represents a non-NVIDIA Rust shader compiler in Mesa's userspace stack. The Nova kernel driver (the Rust-language successor to nvkm, developed under Rust-for-Linux) is a separate trajectory — kernel DRM Rust rather than Mesa userspace Rust — but together they indicate that the graphics stack is developing two reinforcing Rust fronts simultaneously. ACO and BRW remain C++ and face no stated plan for rewriting; the counter-evidence matters. The more accurate claim is that Rust has established itself as the default language for *new* GPU compiler backends in Mesa, while existing C++ compilers evolve in place. That is a quieter but more durable shift: the next blank-sheet compiler problem in Mesa is more likely to start in Rust than in C++, regardless of the vendor.
