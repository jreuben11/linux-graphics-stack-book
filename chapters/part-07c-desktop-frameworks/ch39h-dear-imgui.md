...
- **Legacy backend init signatures continue to be retired.** The in-progress 1.93.0 release removes backend initialization entry points that predate the dynamic texture-atlas protocol (for example the pre-`ImGuiBackendFlags_RendererHasTextures` form of `ImGui_ImplDX12_Init()`), following the same multi-release deprecation window described above. Expect further removals of this kind in upcoming point releases as the remaining pre-1.92 backend APIs age out. [Source](https://github.com/ocornut/imgui/blob/master/docs/CHANGELOG.txt)
- **`LiveEdit` default change still settling.** v1.92.9 flipped the default so dragging or typing into `DragXXX`/`SliderXXX`/`InputScalar` widgets no longer writes intermediate values to the backing variable while editing is in progress (`ImGuiItemFlags_LiveEditOnInputScalar` now off by default). This is a behavioral default change to widgets covered in this chapter's examples; applications and third-party widget extensions that assumed the old live-write semantics should expect continued follow-up fixes. [Source](https://github.com/ocornut/imgui/releases/tag/v1.92.9)
- **SDL_GPU3 backend tracking an evolving upstream API.** `imgui_impl_sdlgpu3.cpp` has changed its `ImTextureID` representation and render-state/sampler-callback structs across several recent point releases as SDL's own GPU API stabilises. On Linux, where SDL_GPU commonly targets Vulkan, applications using this backend should expect continued source churn rather than ABI/API stability release to release. [Source](https://github.com/ocornut/imgui/blob/master/docs/CHANGELOG.txt)

### Medium-term (1-3 years)

- **The dynamic texture-atlas protocol displacing the static upload model.** All standard renderer backends — Vulkan, OpenGL2/3, SDL_GPU3, SDLRenderer2/3, WebGPU, Metal, and the DirectX family — now implement `ImGuiBackendFlags_RendererHasTextures`, and the legacy `ImGui_ImplXXXX_CreateFontsTexture()` / `DestroyFontsTexture()` entry points have already been removed from backends that had them. The single-upload static atlas model described in §3.2 is being phased out as a supported path rather than merely offered as an alternative alongside the newer one. [Source](https://github.com/ocornut/imgui/blob/master/docs/CHANGELOG.txt)
- **Docking/multi-viewport stays a parallel branch, not a merge candidate.** The project's own documentation continues to describe `docking` as a branch that is "kept in sync with master regularly," not as a feature queued for merge. The Wayland absolute-window-positioning mismatch noted in §10 for multi-viewport is therefore likely to remain a standing limitation of the branch rather than something resolved by a near-term merge into `master`. [Source](https://github.com/ocornut/imgui/blob/docking/docs/README.md)
- **WebGPU backend gaining a lighter-weight native option.** `imgui_impl_wgpu.cpp` added support for WGVK, a lightweight native WebGPU implementation, as an alternative to linking the full Dawn or wgpu-native stacks. For debug-overlay and tool use cases — this chapter's core audience — that lowers the dependency footprint of adding a WebGPU-backed ImGui overlay compared to pulling in Dawn (Ch35). [Source](https://github.com/ocornut/imgui/blob/master/docs/CHANGELOG.txt)

### Long-term

- **No v2.0, and the deprecation-cycle pattern is likely to continue rather than culminate in a breaking rewrite.** Consistent with the versioning and vendoring policy above, the project has not announced a major-version rearchitecture. The steady cadence of scoped, multi-release-noticed removals still landing in the 1.93.0 WIP branch is the same mechanism the project has used for years to evolve the API, and nothing in current release activity suggests a shift toward a breaking v2. [Source](https://github.com/ocornut/imgui/blob/master/docs/CHANGELOG.txt)
- **Full docking/viewport merge into `master` is not currently committed to.** No public roadmap statement folds `docking` into `master`. Given how long the split has persisted and that the branch is described as kept "in sync" rather than "merging soon," Linux application authors who need docking or multi-viewport should plan to keep tracking the separate branch — and its Wayland positioning caveat — indefinitely rather than expecting first-class docking support to land in `master` on any particular horizon. [Source](https://github.com/ocornut/imgui/blob/docking/docs/README.md)
