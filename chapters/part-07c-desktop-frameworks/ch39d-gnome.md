...
- [1. GNOME Architecture Overview](#1-gnome-architecture-overview)
  ...
  - [1.4 What is Mutter?](#14-what-is-mutter)
  - [1.5 What is GObject?](#15-what-is-gobject)
  - [1.6 What is GJS?](#16-what-is-gjs)
...
Mutter is the compositor and window manager at the base of the GNOME desktop. It is distributed as a shared library, `libmutter`, rather than a standalone binary; the GNOME Shell process links against it and calls into its C API to become both the Wayland compositor and, when running under X11, the compositing window manager. Mutter owns the display hardware: in a Wayland session it opens DRM device nodes directly, programs the kernel modesetting (KMS) subsystem to drive CRTC modes, scanout, and page-flipping, and manages framebuffers through GBM. It implements a full Wayland server — creating the `wl_display`, advertising global interfaces (`wl_compositor`, `xdg_wm_base`, `zwp_linux_dmabuf_v1`, `wp_linux_drm_syncobj_v1`), and accepting surface commits from GTK4 and other Wayland clients — while simultaneously running the OpenGL scene graph, via Cogl, that composites those surfaces onto the screen. The retained scene graph is provided by Clutter, a toolkit vendored into the Mutter source tree. GNOME Shell's desktop UI — panel, overview, workspace switcher — is built as Clutter actors on the same stage that Mutter drives, which is why the compositor and the shell UI share a single process. §2 works through the key Mutter subsystems: the `MetaKms*` KMS abstraction, the Cogl and Clutter rendering pipeline, window management objects (`MetaWindow`, `MetaStack`), the embedded Wayland server, and the colour management and HDR output paths.

### 1.5 What is GObject?

GObject is a C-language type system and object model provided by GLib, the foundational utility library on which all GNOME infrastructure is built. Because ISO C has no native classes, interfaces, signals, or reference counting, GObject provides these through a set of macros and runtime bookkeeping: every type registers itself with the type system at load time, declares a class structure (vtable) and an instance structure, and inherits from `GObject` to gain reference-count-based memory management, a property system with change notification, and a signal mechanism analogous to Qt signals/slots. The relevance to the graphics stack is concrete: GTK4 widgets, Clutter actors, GIO streams, D-Bus proxies generated by `gdbus-codegen`, and Mutter's own `MetaDisplay`, `MetaKms`, and `MetaWindow` are all `GObject` subclasses. GObject also defines the bridge between C libraries and higher-level languages. Each GObject-based library ships `.gir` (GObject Introspection XML) and `.typelib` (compiled binary) metadata describing its types and methods; this metadata allows GJS, PyGObject, and gtk-rs to call the same C API without hand-written bindings. §1.3 introduces GObject in the context of the GNOME language split, and §5.6 covers GObject Introspection internals.

### 1.6 What is GJS?

GJS is a JavaScript runtime for GNOME built on Mozilla's SpiderMonkey engine. It exposes GObject-based C libraries — GLib, Gio, Clutter, Mutter, GTK4, and the rest — to JavaScript through GObject Introspection, allowing JavaScript code to instantiate GObject instances, connect to signals, and call C functions directly, with automatic reference-count management integrated into the garbage collector. Within the GNOME desktop, GJS's most important role is running GNOME Shell itself: the entire desktop UI under `js/ui/` is a GJS application that boots inside the `gnome-shell` process, which also links `libmutter`, and manipulates Clutter actors, D-Bus proxies, and Mutter window-management objects from JavaScript. GNOME Shell extensions use the same runtime: an extension is a JavaScript module loaded into the running Shell, able to add Clutter actors to the stage, intercept Mutter signals, or export D-Bus interfaces. Because JavaScript runs in-process with the compositor, extension faults can destabilise the session, which is the reason the extension review process exists. §5 covers the GJS architecture — SpiderMonkey integration, typelib imports, the GLib main loop and its async model — and compares GJS with PyGObject and gtk-rs for application development.
...
GtkBuilder consumes `.ui` files — XML documents that describe widget trees. These files were historically generated by GUI editors (Glade) and are difficult to write or review by hand: they are verbose, all property values are untyped strings, and typos in property names go undetected until runtime. **Blueprint** (`.blp`) is a typed, human-writable alternative that `blueprint-compiler` transpiles to `.ui` at build time. The GTK runtime never sees Blueprint; it receives identical `.ui` output. [Source](https://jwestman.pages.gitlab.gnome.org/blueprint-compiler/)

#### Why Blueprint

| Problem with raw `.ui` XML | Blueprint solution |
|-----------------------------|-------------------|
| Property values are strings — typos silently compile | Typed schema validated at compile time |
| Signal handler names are unchecked strings | Handler names validated against GIR signatures |
| Translation requires `translatable="yes"` per attribute | `_("…")`, `C_("ctx", "…")`, `ngettext()` are first-class syntax |
| Verbose structure with repeated closing tags | Brace-delimited blocks, 3–5× shorter than equivalent XML |
| No IDE completion (XML schema coverage is incomplete) | Language server (`blueprint-language-server`) provides full completion and hover |

#### Language Anatomy

A `.blp` file opens with `using` declarations that anchor the type schema version, then defines top-level resources (menus, constants) and a `template` block that becomes the root widget class:
...
// Named menu — referenced by id anywhere in this file
menu primary_menu {
  section {
    item { label: _("Preferences"); action: "app.preferences"; }
    item { label: _("Keyboard Shortcuts"); action: "win.show-help-overlay"; }
    item { label: _("About My App"); action: "app.about"; }
  }
}

// Template: declares the widget class this file describes.
// $MyWindow = the C type MyWindow (GType registered with G_DEFINE_FINAL_TYPE).
template $MyWindow : Adw.ApplicationWindow {
  title: _("My Application");
  default-width:  900;
  default-height: 600;

  // Root layout provided by AdwApplicationWindow via AdwToolbarView
  Adw.ToolbarView {
    [top]                         // child-type annotation: top toolbar slot
    Adw.HeaderBar header_bar {   // named — accessible via gtk_widget_get_template_child()
      [end]
      Gtk.MenuButton {
    ...
    content: Gtk.ScrolledWindow {   // 'content:' = named slot property
      Gtk.ListView list_view {      // named child — wired up in instance_init
...
Child-type annotations (`[top]`, `[end]`, `[start]`, `[overlay]`) map to the `gtk_XXX_add_XXX()` family. Properties of child objects use colon syntax; slot properties (those that accept a widget) end with `:`. Anonymous children (no id) are inaccessible from C; add an id only when implementation code needs a direct reference.

#### Adaptive Navigation with `AdwNavigationView`

Modern GNOME apps use `AdwNavigationView` for hierarchical navigation that collapses on narrow widths, and `AdwNavigationSplitView` or `AdwOverlaySplitView` for sidebar-plus-content layouts. Blueprint expresses both:
    ...
    sidebar: Adw.NavigationPage {
      ...
      Adw.ToolbarView {
        ...
        content: Gtk.ScrolledWindow {
          Gtk.ListBox sidebar_list { selection-mode: single; }
        };
      }
    };

    // Content pane: pushed or side-by-side depending on width
...
`AdwNavigationSplitView` automatically collapses to a single-pane navigation stack on widths below `min-sidebar-width`; the sidebar becomes the root page and pushing the content page slides it in. No width-change signal handling is needed in C.

#### Preferences Dialogs

GNOME preferences follow a standardised structure: `Adw.PreferencesDialog` contains `Adw.PreferencesPage`s, each with `Adw.PreferencesGroup`s and `Adw.ActionRow`/`Adw.SwitchRow`/`Adw.ComboRow`/`Adw.SpinRow` children. Blueprint expresses this concisely:
...
template $MyPreferencesDialog : Adw.PreferencesDialog {
  title: _("Preferences");
  search-enabled: true;
  ...
  Adw.PreferencesPage {
    ...
    Adw.PreferencesGroup {
      ...
      Adw.SwitchRow dark_mode_row {
        title: _("Dark Mode");
        subtitle: _("Use the dark variant of the current theme.");
        // Bind to GSettings key via a GSettings-backed model (wired in C)
      }
    ...
    Adw.PreferencesGroup {
      ...
      Adw.SpinRow {
        ...
        subtitle: _("Minutes between automatic saves. 0 to disable.");
        adjustment: Gtk.Adjustment {
          lower: 0; upper: 60; step-increment: 1; value: 5;
        };
        numeric: true;
        climb-rate: 1;
      }
...
#### Signal Connections and Property Binding

Signals use `=>` and a `$method_name()` reference to a method on the template class; Blueprint validates the handler signature against GIR at compile time. `bind` introduces a one-way `GtkExpression` binding that updates the target whenever the source property changes:

```blp
template $SearchWindow : Adw.ApplicationWindow {
  Gtk.SearchEntry search_entry {
    placeholder-text: _("Search…");
    // Signal: search-changed(GtkSearchEntry*) — $on_search is a method on SearchWindow
    search-changed => $on_search();
    stop-search    => $on_search_stopped();
    accessibility { label: _("Search files"); }
  }

  Gtk.Label result_count {
    // One-way bind: updates whenever search_entry.text changes.
    // transform-to calls format_count(GBinding*, GValue*, GValue*, gpointer) → gboolean
    label: bind search_entry.text transform-to $format_count();
  }

  Adw.AlertDialog confirm_close_dialog {
    heading: _("Discard Search?");
    body:    _("The current search will be lost.");
    responses [
      cancel:  _("Cancel"),
    ...
    response => $on_confirm_close_response();
  }
}
```

#### Meson and GResource Integration

The recommended pattern compiles all `.blp` files in a single `custom_target`, then lists the generated `.ui` files in the GResource bundle so they are embedded in the binary:

```meson
# meson.build (application sources section)

blueprint_compiler = find_program('blueprint-compiler', version: '>= 0.14')

# Compile all .blp files to .ui in the build directory
blueprints = custom_target('blueprints',
  input: files(
    'src/window.blp',
    'src/preferences-dialog.blp',
  ),
  output: ['window.ui', 'preferences-dialog.ui'],
  command: [
    blueprint_compiler, 'batch-compile',
    '@OUTDIR@', '@CURRENT_SOURCE_DIR@/src',
    '@INPUT@',
  ],
)
...
resources = gnome.compile_resources(
  'my-app-resources',
  'src/my-app.gresource.xml',
  source_dir:   ['src', meson.current_build_dir()],
  dependencies: blueprints,   # ensures .ui files exist before the bundle is built
)
```

The `batch-compile` subcommand (Blueprint ≥ 0.14) processes all inputs in one pass, reducing build overhead vs. one `custom_target` per file.

In `my-app.gresource.xml`, the `.ui` files are listed by their generated names (not `.blp`):
...
#### Tooling: Language Server and IDE Support

**`blueprint-language-server`** ships with `blueprint-compiler` and implements the Language Server Protocol. It provides:

- Property name completion (against the full GIR type schema for the declared GTK/Adw version)
- Signal name completion and hover showing the signal's parameter types
- Inline diagnostics for type mismatches, unknown properties, and missing `using` declarations
- Go-to-definition for named children

GNOME Builder enables the language server automatically for `.blp` files. VS Code and Neovim can use it via the generic LSP client with the command `blueprint-compiler lsp`. GNOME Workbench ships a live-preview environment that hot-reloads Blueprint files on save, allowing visual iteration without a full rebuild.

#### Limitations and Status

Blueprint is pre-1.0 (breaking syntax changes are possible between releases) but is considered production-ready for GNOME apps — GNOME Builder scaffolds all new projects with Blueprint, and all official GNOME HIG sample applications use it. It is distributed as part of the GNOME Platform SDK. Blueprint only supports GtkBuilder-hosted widget trees; it cannot describe GObject subclassing, GLib signal emission logic, or non-widget GObject trees — those remain in C, Rust, or JavaScript.

For the complete language reference — bind expression chains, `typeof<>`, `lookup` expressions, CSS style classes, the `condition` pseudo-property, and the full grammar — see Ch39c §13.3.
