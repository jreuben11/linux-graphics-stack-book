...
- [1. KDE Frameworks 6 (KF6) Architecture](#1-kde-frameworks-6-kf6-architecture)
  ...
  - [1.6 What is KDE Plasma?](#16-what-is-kde-plasma)
  - [1.7 What is KDE Frameworks 6 (KF6)?](#17-what-is-kde-frameworks-6-kf6)
  - [1.8 What is KWin?](#18-what-is-kwin)
  - [1.9 What is Kirigami?](#19-what-is-kirigami)
...
### 1.6 What is KDE Plasma?

KDE Plasma is a full desktop environment built on Qt6 and the KDE Frameworks library stack. It encompasses the compositor and window manager (KWin), the desktop shell process (plasmashell), a suite of system-settings panels, and a broad ecosystem of applications. The Plasma 6 release in February 2024 marked the transition from Qt5 and KF5 to Qt6 and KF6, and changed the default session from X11 to Wayland. As a desktop environment, Plasma is layered: the lowest layer is the compositor (KWin), which owns the display and implements the Wayland server protocol; above it sits plasmashell, which draws panels, the desktop, and system-tray applets called "plasmoids"; applications link against KF6 libraries and render into Wayland surfaces managed by KWin. The tight integration between these layers — all sharing Qt's event loop, QML engine, and D-Bus session bus — allows Plasma's desktop widgets to be written in the same QML dialect as application UIs, and allows the compositor to reload effects without restarting. Plasma is distinct from the KDE Frameworks libraries themselves: Plasma is the desktop product, while KF6 is the general-purpose library collection that Plasma depends on but which any Qt application can use independently.

### 1.7 What is KDE Frameworks 6 (KF6)?

KDE Frameworks 6 is a collection of over 70 add-on libraries built on top of Qt6, providing functionality that Qt itself does not supply: cascading XDG configuration files, network-transparent I/O, MIME-type handling, asynchronous job abstractions, translation utilities, and a hardware-abstraction layer for storage devices. KF6 replaces the monolithic `kdelibs` from the KDE 4 era, which was decomposed into independent libraries in the KF5 series beginning in 2014. Each framework is classified into one of four tiers based on what it is allowed to depend on: Tier 1 frameworks depend only on Qt and are suitable for use in any Qt application, while Tier 3 frameworks may pull in substantial portions of the KDE stack. This tier discipline is the primary mechanism that keeps individual KF6 libraries reusable outside of Plasma. Every framework ships as a separately installable CMake package under the `KF6::` namespace prefix, enabling fine-grained dependency declarations in consumer `CMakeLists.txt` files. The ECM (Extra CMake Modules) package sits at Tier 0, providing the shared CMake infrastructure used by every other framework. The stable API for each framework is published at `api.kde.org`. [Source](https://develop.kde.org/products/frameworks/)

### 1.8 What is KWin?

KWin is the display compositor and window manager of KDE Plasma. In the Wayland session it runs as the `kwin_wayland` binary, simultaneously acting as a Wayland server (implementing `wl_compositor`, `xdg_shell`, and many Wayland protocol extensions), a KMS/DRM display driver (programming scanout hardware directly through the kernel's DRM subsystem), and a compositing renderer (painting window surfaces, desktop effects, and decorations with OpenGL or, experimentally, Vulkan). An integrated XWayland instance handles legacy X11 clients transparently. KWin's rendering is organised around an item-based retained scene graph of surfaces and decorations; a `RenderLoop` per output schedules frames against the display's vblank timing; and an extensible effect system allows animations and filters to intercept every paint operation. KWin is distinct from wlroots-based compositors and Mutter (GNOME's compositor): it shares no library code with either, and instead builds directly on Qt's platform abstraction alongside its own OpenGL and DRM abstractions. The section of this chapter covering KWin (§2) focuses on internal architecture; the broader Wayland protocol landscape and multi-compositor comparisons are treated in Chapter 22. KWin's source is at `invent.kde.org/plasma/kwin`. [Source](https://invent.kde.org/plasma/kwin)

### 1.9 What is Kirigami?

Kirigami is a Tier 1 KDE Framework that provides an adaptive QML component library for building applications that run without modification on desktop, mobile (Plasma Mobile), and embedded form factors. The central concept is that a single QML codebase automatically reorganises its layout based on available screen width: on a wide screen a `PageRow` shows multiple pages side-by-side; on a narrow screen it collapses to a single-pane navigation stack. Kirigami exposes this adaptation through the `wideScreen` attached property and the `PageRow` component, while higher-level components such as `ApplicationWindow`, `Action`, `Card`, and `InlineMessage` provide a consistent interaction vocabulary across form factors. Kirigami is not a visual style or theme: it renders through Qt Quick's standard scene graph using whatever platform style Qt Quick Controls selects, delegates color tokens to `Kirigami.Theme` (which reads from the Plasma color scheme), and derives sizing constants from `Kirigami.Units` (based on desktop font metrics). The QML module identifier is `org.kde.kirigami`. Because Kirigami is Tier 1, it can be used in Qt applications that have no dependency on KWin, plasmashell, or the rest of KF6. Section 4 of this chapter covers the Kirigami component model in detail.
