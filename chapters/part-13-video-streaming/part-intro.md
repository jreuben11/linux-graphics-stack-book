...
**RTP (Real-time Transport Protocol, RFC 3550)** is the packet format that actually carries encoded audio/video payload once a session has been set up; it adds a sequence number and timestamp to each packet so a receiver can detect loss and reorder packets, but it does not itself negotiate sessions, retransmit lost packets, or provide congestion control. RTP is a payload carrier, not a standalone protocol — it is used underneath RTSP (session control) and WebRTC (SDP/ICE for negotiation), and is normally paired with its companion control protocol, **RTCP (RTP Control Protocol)**, which runs alongside the media stream to report reception quality (packet loss, jitter, round-trip time) back to the sender.
