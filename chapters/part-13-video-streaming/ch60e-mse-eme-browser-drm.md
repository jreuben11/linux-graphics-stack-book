...
MSE itself is container-agnostic; the actual segment format is defined in a companion spec, the ISO BMFF Byte Stream Format. It requires an **initialization segment** (an `ftyp` + `moov` box pair carrying codec configuration and no samples) followed by a stream of **media segments**, each a self-contained `moof` + `mdat` fragment addressable independently — the same fragmented-MP4 structure DASH and fMP4-packaged HLS already produce for CDN delivery (see [Chapter 60b §9](ch60b-video-streaming-protocols.md)). This is why a DASH/CMAF pipeline and an MSE player share a segment format end-to-end: the CDN-facing packaging step and the browser-facing append step consume the exact same bytes. [Source: ISO BMFF Byte Stream Format spec §2–3](https://www.w3.org/TR/mse-byte-stream-format-isobmff/)
