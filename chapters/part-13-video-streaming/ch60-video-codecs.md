...
The chapter is organised around four codec generations: H.264/AVC (ubiquitous, hardware-supported everywhere), H.265/HEVC (the dominant 4K/UHD codec), AV1 (royalty-free, increasingly hardware-accelerated), and VVC/H.266 (the emerging next generation). Each section covers both the conceptual algorithm and the Linux software encoder/decoder API. The GPU acceleration section ties the theoretical wavefront-parallelism models to the parallel decode paths exposed by VA-API (Ch26) and Vulkan Video (Ch50). The rate control section grounds the discussion in what AMD AMF and Intel Quick Sync actually implement in silicon, connecting to the NVENC and AMF deep-dives in Chapters 66 and 68.
...
Full-pixel block matching is insufficient for broadcast-quality prediction. H.264 implements **half-pixel and quarter-pixel (qpel)** interpolation:
...
H.264 separates the Video Coding Layer (VCL) from the Network Abstraction Layer (NAL). The VCL contains actual coding decisions; the NAL packetises them into self-describing units for network transport or file storage. The x264 API exposes NAL units via `x264_nal_t`:
