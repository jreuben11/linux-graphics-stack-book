...
- **libtorrent-rasterbar's 2.1.x branch continues rapid patch-level stabilization.** Point releases on both the RC_2_0 and RC_2_1 branches (v2.0.14 and v2.1.1) followed close on the heels of the v2.1.0/v2.0.13 releases discussed in §4 and §7, consistent with the library's pattern of active upkeep on its two most recent major branches rather than a slowdown after a major release. [Source: libtorrent releases](https://github.com/arvidn/libtorrent/releases)
- **WebTorrent has moved to a major-version line requiring modern Node.js.** v3.0.0 (superseding the v2.3.0 discussed in §8.1) raised the minimum supported runtime to Node 22+, part of a broader dependency-modernization push (ESM-only dependencies, updated `bittorrent-protocol`/`ut_metadata`) rather than a P2P-protocol change. [Source: WebTorrent v3.0.0 release notes](https://github.com/webtorrent/webtorrent/releases/tag/v3.0.0)
- **io_uring zcrx's DMA-BUF receive path (§12.3) has landed and is being hardened, not merely targeted.** Beyond the initial dmabuf-backed zero-copy receive support, subsequent kernel cycles added multi-area support, cgroup memory accounting, and a synchronous buffer-return fallback — incremental infrastructure work rather than net-new capability. [Source: Linux io_uring zcrx merge, kernel.org](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/commit/?id=a5c98e942457)
- **The nv-p2p deprecation from §12.4 is narrower in scope than a blanket CUDA 14.0 removal.** NVIDIA's current GPUDirect RDMA documentation scopes the CUDA 13.0 deprecation / CUDA 14.0 removal specifically to Tegra/Orin (L4T) platforms, and states that Blackwell-generation Tegra SoCs drop nv-p2p support outright in favor of upstream `pin_user_pages()`/`dma_map_sgtable()` — datacenter GPUDirect RDMA (the AJA/Deltacast capture-card path in §12.4) is not on the same removal timeline. [Source: NVIDIA GPUDirect RDMA documentation](https://docs.nvidia.com/cuda/gpudirect-rdma/index.html)

### Medium-term (1-3 years)
- **Device Memory TCP's driver and deployment surface is broadening past its original single-NIC reference implementation.** The Mellanox mlx5 driver has landed devmem/zcrx DMA-device plumbing and TX dma-unmap fixes, and the bind-rx control path has been extended to work from non-init user namespaces for container/netns use cases — both signs the mechanism is moving from a GVE/GKE-only demonstration toward general-purpose deployability, though still with no BitTorrent client wired into it. [Source: Linux net-next mlx5 devmem commits](https://git.kernel.org/pub/scm/linux/kernel/git/netdev/net-next.git/commit/?id=f1debf1a)
- **WebTransport for BitTorrent tracker signaling is an open, unimplemented proposal, not a committed replacement for WebSocket trackers (§9.1).** The request was explicitly redirected from the WebTorrent repository to `bittorrent-tracker`, and discussion there concluded WebTransport could reduce tracker-side connection overhead versus WebSocket but does nothing for the peer-to-peer data path itself, since WebTransport (like WebRTC data channels) has no browser-to-browser mode. [Source: webtorrent/bittorrent-tracker issue tracker](https://github.com/webtorrent/bittorrent-tracker/issues/360)
- **libtorrent's BitTorrent v2/hybrid-torrent code path is still being hardened rather than treated as a finished feature.** Open issues covering excessive merkle-tree memory/fastresume size for v2 and hybrid torrents, and requests to let a client prefer the v2 swarm when both are available, indicate the v1/v2 hybrid transition described in §2.6 remains an active engineering surface, not a settled corner of the protocol. [Source: libtorrent issue tracker](https://github.com/arvidn/libtorrent/issues/8012)

### Long-term
- **A community-proposed "BitTorrent Protocol Version 3/3.1" targeting SHA-1 retirement exists only as an unassigned, non-BEP feature request.** Authored outside the official BEP process (by the Tixati client's developer) and tracked as an open libtorrent issue, it proposes optional supplementary SHA2-256/SHA3-256 piece hashes and a proof-of-work anti-sniping mechanism layered on top of, not replacing, the existing v1/v2 hash formats from §2.6. No BEP number has been assigned and no major client has committed to implementing it, so whether BitTorrent's hash scheme evolves beyond BEP 52 remains genuinely open. [Source: Tixati BitTorrent Protocol v3 spec](https://tixati.com/specs/bittorrent/v3), [libtorrent issue tracker](https://github.com/arvidn/libtorrent/issues/8047)
- **Zero-copy network-to-GPU ingest is architecturally complete in the kernel but adoption-limited at the driver and application layers.** Both Device Memory TCP (§12.2) and io_uring zcrx (§12.3) now support DMA-BUF receive regions, but only a small set of NIC drivers expose the necessary header-split/flow-steering hardware path, and no BitTorrent client, media server, or streaming framework has wired a torrent-delivered bitstream into either mechanism. Absent a driver-ecosystem-wide push, this gap between "the kernel primitive exists" and "an application uses it for this workload" is likely to persist — the same pattern seen historically with DMA-BUF itself, which took years between kernel availability and universal userspace adoption.
