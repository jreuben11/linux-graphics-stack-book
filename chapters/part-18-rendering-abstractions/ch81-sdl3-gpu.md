...
1. [SDL3 and the GPU API: Filling the Gap](#1-sdl3-and-the-gpu-api-filling-the-gap)
   - [1.1 What is SDL3?](#11-what-is-sdl3)
   - [1.2 What is SDL_GPU?](#12-what-is-sdl_gpu)
   - [1.3 What is SPIR-V?](#13-what-is-spir-v)
   - [1.4 What Is Built on Top of SDL_GPU](#14-what-is-built-on-top-of-sdl_gpu)
...
The remaining sections of this chapter walk through the full **SDL_GPU** workflow in depth:

- **Section 2 — Device and window setup:** backend selection via **`SDL_CreateGPUDevice`** and **`SDL_ClaimWindowForGPUDevice`**, the **`SDL_GPUSupportsShaderFormats`** pre-check, and the extended **`SDL_CreateGPUDeviceWithProperties`** path.
- **Section 3 — Shaders and pipeline state:** compiling **SPIR-V** bytecode into **`SDL_GPUShader`** objects via **`SDL_GPUShaderCreateInfo`**; assembling a **`SDL_GPUGraphicsPipeline`** from **`SDL_GPUVertexInputState`**, **`SDL_GPURasterizerState`**, **`SDL_GPUMultisampleState`**, **`SDL_GPUDepthStencilState`**, and **`SDL_GPUGraphicsPipelineTargetInfo`**; configuring alpha blending via **`SDL_GPUColorTargetBlendState`**; depth testing with **`SDL_GPU_TEXTUREFORMAT_D32_FLOAT`**; and pushing per-draw constants with **`SDL_PushGPUVertexUniformData`** and **`SDL_PushGPUFragmentUniformData`** instead of descriptor-set-bound **UBO**s.
- **Section 4 — Textures and staging uploads:** **`SDL_GPUTexture`** and **`SDL_GPUSampler`** creation, and the staging-buffer upload workflow via **`SDL_GPUTransferBuffer`** and **`SDL_GPUCopyPass`**.
- **Section 5 — Buffers and indirect draw:** the same patterns applied to **`SDL_GPUBuffer`** objects for vertex data, index data (**`SDL_GPU_BUFFERUSAGE_INDEX`**), and **GPU**-driven indirect draw (**`SDL_GPU_BUFFERUSAGE_INDIRECT`**, **`SDL_GPUIndirectDrawCommand`**).
- **Section 6 — Command buffers and render passes:** the command buffer lifecycle (**`SDL_AcquireGPUCommandBuffer`**, **`SDL_SubmitGPUCommandBuffer`**, **`SDL_CancelGPUCommandBuffer`**); render pass recording with **`SDL_BeginGPURenderPass`** and **`SDL_GPUColorTargetInfo`**; viewport and scissor control via **`SDL_SetGPUViewport`** and **`SDL_SetGPUScissor`**; and GPU debug markers (**`SDL_PushGPUDebugGroup`**, **`SDL_InsertGPUDebugLabel`**) that integrate with tools such as **RenderDoc** and **Nsight** via **`VK_EXT_debug_utils`**.
- **Section 7 — Compute passes:** creating a **`SDL_GPUComputePipeline`** from a compute **SPIR-V** shader, dispatching work with **`SDL_DispatchGPUCompute`**, and mixing compute and graphics passes within a single command buffer with automatic pipeline barriers.
- **Section 8 — Swapchain and presentation:** swapchain management via **`SDL_WaitAndAcquireGPUSwapchainTexture`** and presentation modes (**`SDL_GPU_PRESENTMODE_VSYNC`**, **`SDL_GPU_PRESENTMODE_MAILBOX`**, **`SDL_GPU_PRESENTMODE_IMMEDIATE`**), including **HDR** output via **`SDL_GPUSwapchainComposition`** (**`SDL_GPU_SWAPCHAINCOMPOSITION_HDR10_ST2084`**, **`SDL_GPU_SWAPCHAINCOMPOSITION_HDR_EXTENDED_LINEAR`**).
- **Section 9 — Resource lifetime and synchronization:** the **`SDL_ReleaseGPU*`** functions, deferred destruction semantics, the `cycle` boolean for hazard-free frequent updates, and explicit **CPU**–**GPU** synchronization via **`SDL_GPUFence`** and **`SDL_WaitForGPUFences`**.
- **Section 10 — Comparison:** a structured comparison of **SDL_GPU** against raw **Vulkan** and **WebGPU**/**wgpu** across boilerplate, portability, shader formats, descriptor models, memory management, and advanced feature support.
- **Section 11 — Linux-specific behavior:** **Vulkan** as the sole backend; surface creation choosing between **`VK_KHR_wayland_surface`** and **`VK_KHR_xlib_surface`** based on the display server; the underlying **Mesa** drivers (**RADV**, **ANV**, **NVK**) and NVIDIA proprietary; enabling **`VK_LAYER_KHRONOS_validation`** validation layers via the `debug_mode` flag; the fact that **GBM** and **EGL** are not used by the **Vulkan** backend; relevant environment variables (**`SDL_VIDEODRIVER`**, **`SDL_GPU_DRIVER`**, **`VK_INSTANCE_LAYERS`**, **`VK_ICD_FILENAMES`**); and the status of the in-progress **WebGPU**/**Emscripten** community backend.

### 1.1 What is SDL3?

SDL (Simple DirectMedia Layer) is an open-source, cross-platform C library providing a unified API for window creation, input handling, audio, timers, and file I/O. SDL3 is a major version break from SDL2: it revises the event model, modernises the build system to CMake, and adds the new `SDL_gpu.h` GPU API while dropping the legacy SDL2 renderer's OpenGL path for GPU work. SDL3 targets Linux, Windows, macOS, iOS, Android, and several console platforms through a single source tree. On Linux, SDL3 integrates with the Wayland or X11 display server at startup (selected via `SDL_VIDEODRIVER`) and reaches the GPU through the Vulkan ICD loader (`libvulkan.so.1`), which dispatches to Mesa drivers such as RADV, ANV, and NVK, or to the NVIDIA proprietary driver. SDL3 headers and the shared library are distributed as `libsdl3-dev` on Debian/Ubuntu derivatives, or can be built from source at [https://github.com/libsdl-org/SDL](https://github.com/libsdl-org/SDL). For graphics application developers on Linux, SDL3 manages window surfaces, swapchain creation, and Vulkan instance setup that would otherwise require hundreds of lines of boilerplate. This chapter focuses exclusively on the GPU subsystem; SDL3's windowing, event, audio, and I/O subsystems are documented in the SDL wiki and fall outside this chapter's scope.

### 1.2 What is SDL_GPU?

SDL_GPU is the GPU abstraction layer introduced in SDL3, declared in `SDL_gpu.h` and stabilised in SDL 3.2.0 (January 2025). It occupies the design space between raw Vulkan — maximally expressive but verbose — and a full game engine — portable but opaque. SDL_GPU exposes the core GPU programming model (pipelines, command buffers, render passes, compute dispatches, texture uploads) through approximately 80 entry points, all prefixed `SDL_GPU` or operating on `SDL_GPU*` handle types. The API design is inspired by Apple Metal: resources are identified by opaque handles returned at creation time, command buffers are acquired from the device and submitted in a linear sequence, pass boundaries are delimited by explicit begin/end calls, and shader-resource binding is declared per-pipeline via slot counts rather than through descriptor pool allocators and layout objects. On Linux, SDL_GPU translates every call into Vulkan 1.0-compatible operations via an internal Vulkan backend (`src/gpu/vulkan/SDL_gpu_vulkan.c` in the SDL source tree). The pass model — copy pass, compute pass, render pass — drives automatic pipeline barrier insertion, so developers do not write explicit image-layout transitions or access-mask pairs. SDL_GPU is not a scene graph, a material system, or a shader compiler; it is a thin, portable command-buffer API that removes Vulkan boilerplate without concealing the GPU execution model. [Source: SDL_gpu.h, release-3.2.14](https://github.com/libsdl-org/SDL/blob/release-3.2.14/include/SDL3/SDL_gpu.h)

### 1.3 What is SPIR-V?

SPIR-V (Standard Portable Intermediate Representation — Vulkan) is the binary shader intermediate language defined by the Khronos Group as the required shader input for Vulkan. Rather than submitting GLSL or HLSL source text to the driver at runtime — a practice that caused inter-driver divergence and unpredictable pipeline-creation stalls in the OpenGL era — Vulkan accepts pre-compiled SPIR-V bytecode that each driver translates to native GPU ISA during pipeline creation. SPIR-V is a typed, structured, SSA (static single-assignment) IR encoded as a stream of 32-bit words. It describes a shader's entry points, interface variables, descriptor bindings, and arithmetic instructions in a driver-neutral form. The Khronos reference compiler `glslang` ([https://github.com/KhronosGroup/glslang](https://github.com/KhronosGroup/glslang)) compiles GLSL to SPIR-V; `dxc` (DirectX Shader Compiler) compiles HLSL; the `naga` library used by wgpu compiles WGSL. Offline tools such as `spirv-opt` and `spirv-reflect` can validate, optimise, and inspect the IR before it ever reaches a driver. Within SDL_GPU on Linux, SPIR-V bytecode is the only accepted shader format: the `SDL_GPU_SHADERFORMAT_SPIRV` flag must be passed to `SDL_CreateGPUDevice`, and the bytecode blob is supplied via `SDL_GPUShaderCreateInfo.code` directly to the Vulkan backend, which wraps it in a `VkShaderModule`. The separation of offline shader compilation from driver-side GPU compilation is a key reason SDL_GPU can keep pipeline-creation latency predictable on Linux. [Source: Khronos SPIR-V specification](https://registry.khronos.org/SPIR-V/)

### 1.4 What Is Built on Top of SDL_GPU

SDL_GPU stays deliberately thin — no scene graph, no material system, no shader language of its own — so unlike Filament (Chapter 83) or bgfx (Chapter 84), which ship their own render-graph and material layers, SDL_GPU's ecosystem is still small and mostly solves *adjacent* problems rather than hiding the explicit API itself:

- **Shader tooling, not shading abstraction.** [`SDL_shadercross`](https://github.com/libsdl-org/SDL_shadercross) is the official companion library that cross-compiles HLSL or SPIR-V source into whichever binary format each backend needs (SPIR-V for Vulkan, DXBC/DXIL for D3D12, MSL for Metal), either offline via a command-line tool or at runtime by linking against `SDL_shadercross.h`. It solves the "one shader source, three backends" problem but leaves pipeline and pass authoring exactly as explicit as raw SDL_GPU. [Source: SDL_shadercross README](https://github.com/libsdl-org/SDL_shadercross) — see also the introductory post from Moonside Games. [Source: Introducing SDL_shadercross](https://moonside.games/posts/introducing-sdl-shadercross/)
- **A dedicated shading language is in progress.** [`SDL_shader_tools`](https://github.com/libsdl-org/SDL_shader_tools) is an early-stage libsdl-org project building "SDLSL," a GLSL/HLSL-like language with a single cross-platform bytecode target, plus a `sdl-shader-compiler` command-line tool. As of this writing it has a parser and semantic analysis but is not yet a finished, shipping compiler. [Source: SDL_shader_tools README-SDL_gpu.md](https://github.com/libsdl-org/SDL_shader_tools/blob/main/docs/README-SDL_gpu.md)
- **Immediate-mode UI, not a 3D abstraction.** Dear ImGui ships an official `imgui_impl_sdlgpu3` renderer backend. It is notable for a two-stage draw model unique among ImGui backends: because SDL_GPU forbids copy operations inside an active render or compute pass, the app must call `ImGui_ImplSDLGPU_PrepareDrawData` to upload vertex/index data via a copy pass *before* opening the render pass that calls `ImGui_ImplSDLGPU_RenderDrawData`. [Source: imgui_impl_sdlgpu3.cpp](https://github.com/ocornut/imgui/blob/master/backends/imgui_impl_sdlgpu3.cpp)
- **No established scene-graph or material engine yet.** Because SDL_GPU only stabilised in SDL 3.2.0 (January 2025), no widely adopted higher-level 3D engine is built *on top of* it the way Filament or bgfx supply their own backend-abstracted material systems; community projects (such as `SDL_gpu_examples`, a reference-implementation collection rather than a library) currently fill the gap with example code instead of a reusable abstraction layer. [Source: SDL_gpu_examples](https://github.com/TheSpydog/SDL_gpu_examples)
- **Naming caveat:** a same-named but unrelated project, [`sdl-gpu` by grimfang4](https://github.com/grimfang4/sdl-gpu), predates SDL3 and is a higher-level 2D-only rendering replacement for SDL2's `SDL_Render` API, built on OpenGL rather than SDL3's Vulkan/Metal/D3D12 `SDL_gpu.h`. It is unrelated to the API this chapter covers. [Source: grimfang4/sdl-gpu README](https://github.com/grimfang4/sdl-gpu/blob/master/README.md)
