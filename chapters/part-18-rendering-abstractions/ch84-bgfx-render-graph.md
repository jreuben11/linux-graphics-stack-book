...
1. [The Abstraction Spectrum](#1-the-abstraction-spectrum)
   - [1.2 What is bgfx?](#12-what-is-bgfx)
   - [1.3 What is a Cross-Platform Rendering Abstraction?](#13-what-is-a-cross-platform-rendering-abstraction)
   - [1.4 What is the Frame Graph Pattern?](#14-what-is-the-frame-graph-pattern)
...
bgfx is a BSD-2-clause-licensed, cross-platform rendering library that presents a single C++ API mapping to eight backend graphics APIs: Vulkan, OpenGL 3.1+, OpenGL ES 2/3.1, Direct3D 11 and 12, Metal, WebGL, and WebGPU. The library is structured as a "Bring Your Own Engine/Framework" tool — it takes responsibility for GPU command submission, resource lifecycle, and shader cross-compilation while imposing no scene graph, asset pipeline, or window management. On Linux, bgfx defaults to the Vulkan backend through the `bgfx::RendererType::Vulkan` enum value, internally managing `VkInstance`, `VkDevice`, swap chain images, and descriptor pool caches without exposing those Vulkan objects to the application.

The central architectural choice in bgfx is the view model: instead of explicit render passes, an application configures numbered views (0–255) with associated framebuffers, clear operations, and sort modes, then submits draw calls tagged to a view ID. At `bgfx::frame()`, the render thread radix-sorts all submissions by view and by an intra-view sort key, then translates each view into a `vkCmdBeginRenderPass`/`vkCmdEndRenderPass` pair. This model reduces the API surface area compared to raw Vulkan while remaining efficient enough for production games and tools. bgfx trades Vulkan-specific capabilities — ray tracing, mesh shaders, variable rate shading — for genuine multi-backend portability, making it appropriate when the target device matrix spans Vulkan, Metal, and legacy OpenGL ES hardware simultaneously.

### 1.3 What is a Cross-Platform Rendering Abstraction?

A cross-platform rendering abstraction, also called a graphics hardware abstraction layer (HAL), is a library or API layer that presents a uniform programming interface to the application while mapping that interface to multiple underlying GPU APIs at runtime. The motivation is practical: GPU-accelerated software must run on Linux with Vulkan, macOS with Metal, Windows with Direct3D 12 or 11, Android with Vulkan or OpenGL ES, and web browsers with WebGPU or WebGL. Writing and maintaining separate rendering backends for each target is expensive; a rendering abstraction amortises that cost by providing a single shader compilation pipeline, a unified resource management model, and a single draw submission path.

The tradeoff is always expressiveness versus portability. An abstraction that targets the lowest common denominator across all backends necessarily hides capabilities that exist only in some APIs — Vulkan ray tracing extensions, Direct3D 12 work graphs, Metal argument buffers. The abstraction spectrum runs from thin helpers that simply reduce Vulkan ceremony (volk, VMA, vk-bootstrap) through mid-level HALs that add backend-switching (bgfx, SDL3 GPU, LLGL, IGL) to full-featured rendering layers that also provide material systems and post-processing effects (Filament, wgpu). This chapter focuses on the mid-level band, where the application retains control over shaders and render-pass structure while outsourcing backend dispatch to the library.

### 1.4 What is the Frame Graph Pattern?

The frame graph pattern is a data-driven design pattern for organising a multi-pass rendering pipeline. Rather than writing imperative code that explicitly allocates GPU resources, inserts synchronisation barriers, and calls render passes in a fixed order, a frame graph describes each pass declaratively: which resources it reads, which it writes, and what GPU commands it executes. The frame graph system then analyses the declared dependencies, constructs a directed acyclic graph (DAG) of passes and resources, culls passes whose outputs are not consumed by any downstream pass or the final swapchain image, computes each resource's lifetime across the frame, and automatically inserts the correct `VkImageMemoryBarrier` and `VkBufferMemoryBarrier` calls at pass boundaries.

Two practical advantages follow from this analysis. First, because the system knows when each transient resource — a G-buffer texture, a shadow map, an SSAO half-resolution buffer — is last read, it can alias distinct resources to overlapping regions of a single `VkDeviceMemory` allocation, reducing per-frame VRAM consumption by 40–50% in typical deferred pipelines. Second, adding a new render pass does not require manually auditing existing barrier insertions for correctness; the frame graph recomputes barriers globally from the updated dependency graph. The pattern was documented in a 2017 GDC presentation on extensible rendering architecture and is now used in Filament's `FrameGraph`, Unreal Engine's Render Dependency Graph (RDG), Bevy's RenderGraph, and Granite's render pass system.
