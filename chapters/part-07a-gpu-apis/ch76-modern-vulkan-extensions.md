...
- **`VK_KHR_dynamic_rendering`** (Vulkan 1.3 core) — replace `VkRenderPass`/`VkFramebuffer` with per-frame `vkCmdBeginRendering` + `VkRenderingInfo`.
- **`VK_EXT_descriptor_indexing`** (Vulkan 1.2 core) — bindless resource access via a single large descriptor array indexed by push-constant integers.
- **`VK_EXT_mesh_shader`** — replace vertex/tessellation/geometry stages with GPU-compute-like task + mesh shader stages for GPU-driven culling and geometry amplification.
- **`VK_KHR_cooperative_matrix`** — SPIR-V `coopMatMulAdd` operations that map to tensor cores / matrix engines for ML inference and neural upscaling.
- **`VK_EXT_shader_object`** — compile individual `VkShaderEXT` objects; set all graphics state dynamically; no `VkPipeline` objects required.
- **`VK_EXT_graphics_pipeline_library`** — decompose `VkPipeline` into four independently compiled segments, linked fast at draw time to eliminate shader-compilation stutter.
- **`VK_EXT_extended_dynamic_state3`** — make ~33 additional rasterisation, blend, and multisample states dynamic; implicitly required by shader objects.
