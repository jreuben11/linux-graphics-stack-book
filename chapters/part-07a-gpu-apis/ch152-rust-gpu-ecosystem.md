...
- **`spirv-unknown-vulkan1.x` into the official target tier**: The target triples `spirv-unknown-vulkan1.1`, `spirv-unknown-vulkan1.2`, and `spirv-unknown-vulkan1.3` **already exist** in the rust-gpu project — but they are not official rustc targets. They are generated at build time by `spirv-builder` (in `crates/rustc_codegen_spirv-types/src/target_spec.rs`) as JSON target spec files, and only function when `librustc_codegen_spirv.so` is loaded via `-Zcodegen-backend`. They do not appear in `rustc --print target-list` and have no official Rust project tier status. Since Rust 1.94, JSON target specs additionally require `-Zjson-target-spec` (an unstable flag), making this path nightly-only and fragile across toolchain updates. The long-term aspiration is for SPIR-V targets to enter the official rustc tier policy — removing the dependency on the external codegen backend ABI (`__rustc_codegen_backend`) and the `-Z` flag requirement. No RFC or leadership-council discussion for this exists as of mid-2026.
