...
ARM and embedded GPU drivers inhabit the same DRM driver framework as their x86 desktop counterparts, but the environment surrounding them is fundamentally different. Where a discrete GPU announces itself over PCI Express, negotiates BARs, and manages its own VRAM, an embedded GPU is baked into a System-on-Chip fabric: discovered through Device Tree or ACPI tables, powered and clocked through shared SoC power controllers, and relying on the system's IOMMU — the ARM SMMU — to provide the address translation that PCI-attached GPUs handle with hardware GARTs. Understanding this environmental difference is the prerequisite to understanding why ARM GPU drivers are written the way they are.

This chapter examines the four main open-source ARM GPU kernel driver families that have reached mainline Linux. Lima covers the Mali-400 and Mali-450 Utgard-architecture GPUs, a fixed-function vertex and programmable fragment design that still ships in Allwinner and Rockchip SoCs. Panfrost handles the Mali Midgard (T6xx, T7xx, T8xx) and Bifrost (G31 through G76) generations, which introduced a unified shader model and tiled rendering. Panthor is the newest of the three Mali drivers, targeting the Valhall CSF architecture (Mali-G310 through Immortalis-G720) and requiring a complete architectural break from Panfrost because the Valhall Command Stream Frontend replaced the Job Manager hardware model with firmware-managed command queues. The MSM driver covers Qualcomm's Adreno GPU family alongside the Snapdragon display subsystem, making it the largest and most complex ARM GPU driver in the tree. Finally, the Asahi DRM driver for Apple Silicon AGX represents an entirely different category: a Rust-language driver for completely undocumented hardware, produced by the Asahi Linux project through reverse engineering of macOS.
