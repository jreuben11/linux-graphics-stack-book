...
- [1. Embedded vs. x86: Platform Driver Model](#1-embedded-vs-x86-platform-driver-model)
  - [1.1 What is a System-on-Chip (SoC)?](#11-what-is-a-system-on-chip-soc)
  - [1.2 What is Device Tree?](#12-what-is-device-tree)
  - [1.3 What is the Platform Driver Model?](#13-what-is-the-platform-driver-model)
...
This chapter examines the five main open-source **ARM** GPU kernel driver families that have reached mainline **Linux**:

- **Lima** — covers the **Mali-400** and **Mali-450 Utgard**-architecture GPUs found in **Allwinner** and **Rockchip** SoCs, with a two-processor model (**GP** geometry processor and **PP** pixel processor array), a `drm_gpu_scheduler` per pipe, and a Mesa pairing of the **Lima Gallium3D** driver providing **OpenGL ES 2.0**.
- **Panfrost** — handles the **Mali Midgard** (**T6xx**/**T7xx**/**T8xx**) and **Bifrost** (**G31** through **G76**) generations, implementing a **Job Slot** submission model, per-context **Mali MMU** address spaces managed via the **io_pgtable** library in **ARM-LPAE** format, **GEM** objects backed by `drm_gem_shmem_object` with a `madvise`-controlled memory shrinker, **DEVFREQ**-based **DVFS** with `simple_ondemand` governor, and a Mesa pairing of the **Panfrost Gallium3D** driver (OpenGL ES 3.1 on **Mali-G52** and **Mali-G610**).
- **Panthor** — the newest Mali driver, targeting the **Valhall CSF** architecture (**Mali-G310** through **Immortalis-G720**) and requiring a complete architectural break from **Panfrost**: the **Valhall Command Stream Frontend** replaced the **Job Manager** model with a firmware-managed microcontroller that consumes ring buffers and doorbells rather than job-slot register writes; uses **drm_gpuvm** for GPU VM management and pairs with the **PanVK** Vulkan driver in **Mesa** for **Vulkan 1.3** on **Mali-G610** and newer hardware.
- **Tyr** — merged in Linux 6.18, a **Rust**-language reimplementation of the same **CSF** driver, the first **GPU** driver in mainline written in **Rust**, developed jointly by **Collabora**, **Arm**, and **Google** and built on the **rust/kernel/drm/** bindings layer.
- **MSM / freedreno** — covers **Qualcomm Adreno** GPUs (**Adreno 2xx** through **Adreno 7xx**) and unlike every other driver discussed here also manages the **Snapdragon** display subsystem (**MDP4**, **MDP5**, **DPU**) under the same `drm_driver`; the per-generation files (`a3xx_gpu.c` through `a7xx_gpu.c`) implement an `adreno_gpu_funcs` vtable; **PM4** command packets fill `msm_ringbuffer` ring buffers submitted via `DRM_IOCTL_MSM_GEM_SUBMIT`; **Adreno** firmware and **PFP** microcode are loaded via `request_firmware` from `linux-firmware`; the security-critical **zap shader** crosses into **TrustZone** via `qcom_scm_pas_init_image` to configure **SMMU** stage-2 translations; **DEVFREQ** OPP transitions are accompanied by Qualcomm **interconnect** framework (`icc_set_bw`) DDR bandwidth votes; the display side covers **MIPI DSI** (`msm_dsi`) and **DisplayPort** (`msm_dp`) encoders; and the Mesa pairing is **freedreno Gallium3D** for all generations plus **Turnip** for **Vulkan 1.3** on **Adreno 6xx** (default on **AArch64** since **Mesa 25.1**).
- **Asahi** — the DRM driver for **Apple Silicon AGX** (M1/M2/M3/M4) produced by the **Asahi Linux** project through reverse engineering of **macOS**: the **AGX** architecture is a **Tile-Based Deferred Renderer** (**TBDR**) with separate geometry and fragment phases, **parameter buffers** in system **RAM**, and a **Unified Memory Architecture** (**UMA**) with no discrete **VRAM**; the reverse engineering methodology centres on the **m1n1** hypervisor for **MMIO** tracing of **macOS** GPU driver activity, supplemented by **Python** scripts that reconstruct data structure layouts from **DMA** memory snapshots; the **Rust** driver at `drivers/gpu/drm/asahi/` uses `rust/kernel/drm/` bindings and exposes `asahi_vm` per-context GPU VA spaces, `asahi_queue` firmware ring buffers, and `asahi_gem` system-RAM-backed objects; the **AGX** firmware blob is extracted from **macOS IPSW** archives by `asahi-installer` and loaded via `request_firmware`; as of **Linux 6.16** only the **uAPI** header (`include/uapi/drm/asahi_drm.h`) has been merged into mainline; and the userspace stack comprises the **Mesa Asahi Gallium3D** driver (OpenGL ES 3.1 / OpenGL 4.6), the **Honeykrisp** Vulkan driver targeting **Vulkan 1.3**, and **rusticl** OpenCL.

The chapter also covers cross-cutting topics shared by all drivers:

- **Embedded display subsystem integration** — **MIPI DSI** panels, **drm_panel_funcs** callbacks, **DSI** video and command modes, bridge chips (**DSI-to-eDP** converters such as **Analogix ANX7625**, **Parade PS8640**), and the **drm_writeback_connector** for capture.
- **Power management** — **DEVFREQ**, `operating-points-v2` **OPP** tables, `thermal_cooling_device` registration for thermal throttling, **Generic Power Domain** dependency ordering, and the `pm_runtime` suspend/resume sequence.
- **System-level IOMMU and memory protection** — the **ARM SMMU** architecture, the distinction between the GPU's internal **MMU** and the system **SMMU**, **IOMMU groups** and **DMA-BUF** import/export as the controlled access gate between isolation domains, **SMMU** fault handling via `iommu_set_fault_handler`, and **CMA** fallback on **SMMU**-bypass SoCs.
- **Embedded-platform firmware loading** — the `firmware-name` **Device Tree** property, **Panthor CSF** firmware boot sequence and binary header validation, **Adreno** firmware and the **zap shader** security flow, **AGX** firmware extraction, and power-domain sequencing pitfalls on **Rockchip RK3399**.
...
A System-on-Chip (SoC) integrates a CPU, GPU, memory controller, display engine, video codec, I/O peripherals, and power management circuitry onto a single piece of silicon. Unlike a desktop PC — where a CPU sits in a socket, a discrete GPU plugs into a PCIe slot, and DRAM occupies separate DIMM slots — an SoC wires all these components together on-die or through a high-bandwidth internal interconnect. This architecture dominates mobile phones, tablets, single-board computers (Raspberry Pi, Rockchip boards), and increasingly ARM-based laptops (Qualcomm Snapdragon X Elite, Apple Silicon). The Linux kernel treats each functional block of an SoC as a separate device described in the Device Tree or ACPI tables; the GPU is one such block. Because the GPU shares the same physical DRAM as the CPU and has no discrete VRAM, memory management takes a different form: GPU buffers are ordinary system memory pages, and address translation is handled by the SoC's IOMMU (the ARM SMMU) rather than by GPU-internal hardware. The implications flow through every aspect of the driver: power management must coordinate with SoC-wide power domains, clock control is handled by the SoC clock framework, and the driver probe sequence cannot begin until the power domain and clock dependencies declared in the Device Tree are satisfied. Every GPU driver covered in this chapter — Lima, Panfrost, Panthor, MSM, and Asahi — operates in an SoC context and must navigate these shared-silicon constraints before it can submit a single draw call.

### 1.2 What is Device Tree?

Device Tree (DT) is a data structure used by the Linux kernel to describe hardware that cannot enumerate itself at runtime. Where a PCI device announces its vendor and device IDs in configuration space registers, an SoC peripheral has no such self-description mechanism: its register base address, interrupt lines, clock inputs, power domain membership, and IOMMU connections are fixed in silicon and must be communicated to the kernel through an external description. The Device Tree specification defines a hierarchical text format (`.dts` source compiled to `.dtb` binary) in which each hardware node carries a `compatible` string that the kernel matches against driver `of_device_id` tables. For an ARM GPU, the DT node encodes the MMIO `reg` range, the `interrupts` array, the `clocks` handle, the `power-domains` reference, and an `operating-points-v2` OPP table for DVFS. At boot, the firmware (U-Boot or EFI) passes a flattened DTB to the kernel, which parses it and calls each matching driver's `probe` function with a `platform_device` wrapping that node. Device Tree binding documentation for GPU drivers lives under `Documentation/devicetree/bindings/gpu/` in the kernel source tree; each binding specifies which properties are required, optional, or deprecated. Without a correct and complete DT node, the driver does not bind and the GPU is invisible to userspace. Device Tree is the authoritative hardware description for nearly all ARM SoC Linux drivers, including every GPU driver covered in this chapter; ACPI replaces it only on a subset of Qualcomm Snapdragon laptop platforms.

### 1.3 What is the Platform Driver Model?

The platform driver model is the Linux kernel subsystem that manages devices discovered through Device Tree or ACPI rather than through self-describing buses such as PCI or USB. When the kernel processes a Device Tree at boot, it creates a `platform_device` for each hardware node and attempts to match it against registered `platform_driver` entries using the `of_device_id` compatible-string table. On a match, the kernel calls the driver's `probe` function, which receives a `platform_device` handle giving access to all DT-declared resources: MMIO regions (`platform_get_resource`, `devm_ioremap_resource`), IRQ lines (`platform_get_irq_byname`), clocks (`devm_clk_get`), power domains (`dev_pm_domain_attach`), and voltage regulators (`devm_regulator_get`). Device-managed (`devm_*`) variants of these calls bind resource lifetime to the device, ensuring cleanup on probe failure or driver removal without explicit error-path unwinding. Every ARM GPU driver in mainline Linux — Lima, Panfrost, Panthor, MSM, and Asahi — enters the kernel through `platform_driver.probe`. The platform driver model also integrates with `pm_runtime` for fine-grained idle power management, enabling the driver to gate clocks and power domains when the GPU is idle and restore them transparently when work arrives. The `platform_driver` structure is defined in `include/linux/platform_device.h`; a driver registers it with `module_platform_driver()` or calls `platform_driver_register()` directly. Understanding this model is a prerequisite for reading any ARM GPU driver's probe function or its Device Tree binding documentation in the kernel source.
