...
- **Cerebras and Groq remain outside this picture entirely**: Cerebras's CS-3 wafer-scale system is not even a local PCIe device — it attaches to its host CPU cluster over the network rather than a PCI bus, and is controlled through a userspace Apptainer/Singularity container SDK (`sdk-cbcore-*.sif`), with no kernel-level driver surface to speak of. [Source: Cerebras SDK installation guide](https://sdk.cerebras.net/installation-guide) Groq's GroqCard is a conventional PCIe Gen4 ×16 card, but its "GroqWare Suite" exposes chip control through a proprietary userspace library rather than a kernel module registered against `ACCEL_MAJOR` — there is no public indication either vendor has pursued `drivers/accel/` inclusion. [Source: Groq — Inside the LPU](https://groq.com/blog/inside-the-lpu-deconstructing-groq-speed) Both are best understood as datacenter inference/training systems adjacent to, but currently outside, the `accel` subsystem this chapter documents.
