...
The Linux kernel contains three major GPU driver families for x86 hardware: amdgpu for AMD Radeon products from the GCN 2nd-generation architecture onward, the i915/Xe pair for Intel integrated and discrete graphics, and nouveau alongside NVIDIA's own open-source kernel module for NVIDIA GPUs. Despite the enormous differences in hardware architecture — AMD's IP block decomposition, Intel's GuC firmware-mediated submission, NVIDIA's GSP-RM offload model — all three conform to the same kernel-side contract. They register with the DRM subsystem, implement the GEM memory management interface, drive the GPU command scheduler, and expose a KMS display pipeline. Chapter 1 introduced that contract in the abstract; this chapter makes it concrete.

The central goal here is to give readers a working model of what a production DRM driver actually does: how it discovers hardware over PCI, loads and authenticates firmware, maps MMIO regions, manages buffer objects through TTM or GEM, schedules GPU work via `drm_gpu_scheduler`, and tears down cleanly when the device is removed or suspended. By examining all three driver families in parallel, it becomes clear which design choices are imposed by hardware constraints and which reflect architectural preferences that could have been made differently. Sections covering format modifiers, firmware loading, runtime power management, GPU reset, GPU virtualisation with virtio-gpu, and AMD's Heterogeneous Memory Management complete the chapter, giving readers the full lifecycle of a GPU device from boot to compute dispatch.
