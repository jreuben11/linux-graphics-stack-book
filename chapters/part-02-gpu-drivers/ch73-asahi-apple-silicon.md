...
Section 1 surveys the **AGX** hardware architecture in depth:

- **G13–G14 GPU generations** — span **M1** and **M2** family **SoC**s: **T8103**, **T6001** (M1 Max), **T6002** (M1 Ultra), **T8112**, **T6020**/**T6021**/**T6022**
- **TBDR** (Tiled Deferred Rendering) — tile-based architecture with **PowerVR** lineage; pipeline split between:
  - **TA** (Tile Accelerator) pass: **VDM** (Vertex Data Master), **PPP** (Primitive Processing Pipeline), **TVB** (Tiled Vertex Buffer)
  - **ISP** (Image Synthesis Processor) pass: fragment shading through an on-chip tilebuffer
- **USC** (Unified Shader Cores) — execute all shader types using the **G13G ISA**
- **UMA** (Unified Memory Architecture) — no discrete **VRAM**; all allocations in shared **LPDDR5**/**LPDDR5X** system RAM

Section 2 covers the reverse engineering methodology that made the driver possible:

- **`m1n1`** — bare-metal hypervisor intercepting **MMIO** register traffic from macOS, used for non-GPU **SoC** peripherals
- **IOKit interception** via **`DYLD_INSERT_LIBRARIES`** — hooks memory allocation, command buffer creation, and submission to capture **GPU**-visible memory at the moment of dispatch; implemented as the **`wrap`** library and **`agxdecode`** tool in **Mesa**
- Comparison with **Nouveau**'s **`LD_PRELOAD`** wrapping, **`mmt-replay`**, and **envytools** disassembly — the key distinction is that **AGX** puts scheduling, preemption, and fault recovery into co-processor firmware rather than hardware registers

Section 3 examines Asahi Lina's **`drm/asahi`** Rust kernel driver — the first **Rust** driver in the **DRM** subsystem — covering:

- **Rust** safety rationale — chosen for safety over the complex firmware **ABI** (more than 70 packed struct types)
- Module structure: **`driver.rs`**, **`gpu.rs`**, **`mmu.rs`**, **`gem.rs`**, **`channel.rs`**, **`microseq.rs`**, **`fw/`**, **`hw/`**, and more
- **`AsahiDriver`** / **`drm::drv::Driver`** — initialisation pattern with per-chip **Device Tree** compatible strings
- **UAT** (Unified Address Translator) — GPU **MMU** using **ARM64**-compatible page tables with 16 KiB pages and 64 VM contexts
- Work submission to the **ASC** (Apple Silicon Coprocessor) via **RTKit** channel ring buffers in **`channel.rs`** and firmware microsequences in **`microseq.rs`**; crash recovery via **`crashdump.rs`**

Section 4 details the **Asahi UAPI** — accepted into mainline **Linux** as **`include/uapi/drm/asahi_drm.h`** without the driver, a first for the **DRM** subsystem — covering the full IOCTL sequence:

- **`DRM_IOCTL_ASAHI_GET_PARAMS`** — returns **`struct drm_asahi_params_global`**
- **`DRM_IOCTL_ASAHI_VM_CREATE`** / **`struct drm_asahi_vm_create`** — explicit **GPU** VM management
- **`DRM_IOCTL_ASAHI_GEM_CREATE`** / **`struct drm_asahi_gem_create`** — buffer allocation with the **`DRM_ASAHI_GEM_WRITEBACK`** flag
- **`DRM_IOCTL_ASAHI_VM_BIND`** / **`struct drm_asahi_gem_bind_op`** — sparse binding via **`DRM_ASAHI_BIND_SINGLE_PAGE`**
- **`DRM_IOCTL_ASAHI_QUEUE_CREATE`** — queue creation with priority and **`usc_exec_base`**
- **`DRM_IOCTL_ASAHI_SUBMIT`** / **`struct drm_asahi_submit`** — flat command buffers with explicit **DRM syncobj** synchronisation; the central **`struct drm_asahi_cmd_render`** carries **`vdm_ctrl_stream_base`**, **`bg`**/**`eot`** tile programs, tile dimensions, depth/stencil buffers, and **`drm_asahi_timestamps`**

Section 5 covers the **Mesa Asahi Gallium** driver in **`src/gallium/drivers/asahi/`** and **`src/asahi/`**, including:

- **`agx_context`** / **`agx_draw_vbo`** state tracker — orchestrates eight **AGX** hardware units: **VDM**, **PPP**, **ISP**, **USC**, **UVS**, **PDM**, **PBE**, **CDM**
- **NIR** compiler backend in **`src/asahi/compiler/`** — handles varying lowering to **`st_var`** / **`ldcf`** / **`iter`** instructions, uniform lowering to the 256-entry uniform register file, image layout selection (strided linear, GPU-tiled, twiddled), and lowering of **OpenGL** transform feedback, geometry shaders, and tessellation evaluation shaders to **compute** dispatches
- **OpenGL 4.6** conformance bridging — buffer robustness via unsigned-minimum clamping, image robustness by forcing out-of-bounds texture coordinates, and clip control via a vertex shader epilogue remapping **clip-space Z**

Section 6 covers **Honeykrisp** (**`src/asahi/vulkan/`**), the **Vulkan** driver for Apple Silicon on Linux, forked from **NVK** (Faith Ekstrand's **Mesa** **Vulkan** driver for **NVIDIA**) and adapted for **TBDR** semantics:

- **Render pass operations** — **`VK_ATTACHMENT_LOAD_OP_*`** / **`VK_ATTACHMENT_STORE_OP_*`** mapped to synthesised **BG** (background) and **EoT** (end-of-tile) **USC** programs in **`struct drm_asahi_bg_eot`**
- **Dynamic state** — handled via prolog/epilog shader splits, conditional code, precompiled variants, and indirection tables to avoid pipeline stutter
- **Descriptor sets** — adapted for **UMA** hardware formats
- **`VK_EXT_image_drm_format_modifier`** — enables zero-copy **WSI** with **DMA-BUF** sharing to the **Wayland** compositor
- **Conformance timeline** — **Vulkan 1.3** (June 2024), **Vulkan 1.4** (late 2024), sparse binding in **Mesa 25.1** enabling **VKD3D-Proton** **DX12_0**
- **Hardware tessellation** — M1 tessellator too limited for **OpenGL**/**Vulkan** **TCS**/**TES** semantics; emulated via compute dispatches

Section 7 covers the **DCP** (Display Control Processor), an **ASC** embedded **ARM64** core running **RTKit**, including:

- **DCP hardware responsibilities** — pixel clocking, timing, **DPLL** programming, **DisplayPort** link training, cursor planes, brightness control
- **CPU communication protocol** — layered: **`apple-mailbox`** (96-bit **ASC** messages) → **`apple-rtkit`** → **EPIC** (Endpoint Interface Client) service calls
- **`drm/apple/`** **KMS** driver — implements atomic **DRM/KMS** modesetting delivering page flipping, **VSync**, brightness control, 120 Hz **ProMotion**, audio over **DisplayPort**/**HDMI**, and experimental external display via **USB-C** (the "fairydust" branch, coordinating **DCP**, **DPXBAR**, **ATCPHY**, and **ACE**)
- **VRR** (Variable Refresh Rate) — requires a full modeset to toggle, a known limitation compared to **i915** or **amdgpu** that program timing registers directly

Section 8 examines the implications of the **UMA** model:

- **Shared memory pool** — no **VRAM**/**GTT** split; every **`DRM_IOCTL_ASAHI_GEM_CREATE`** allocation draws from the shared **LPDDR5**/**LPDDR5X** pool, competing for bandwidth among **CPU** cores, **AGX GPU** clusters, **Neural Engine**, **ISP**, and video encode/decode engines
- **Zero-copy DMA-BUF sharing** — **`drm_gem_prime_import`** between the **GPU** and the camera **ISP** or video decoder is structurally zero-copy
- **Cache coherence** — hardware **cache-coherent** between **CPU** and **GPU**; no **`drm_clflush_sg`** calls required
- **Sparse binding primitive** — **`DRM_ASAHI_BIND_SINGLE_PAGE`** with **`DRM_ASAHI_FEATURE_SOFT_FAULTS`** provides the hardware primitive for **Vulkan** sparse binding

Section 9 summarises conformance, performance, and upstreaming status:

- **Conformance achieved** — **OpenGL 4.6**, **OpenGL ES 3.2**, **OpenCL 3.0**, **Vulkan 1.4**
- **Performance data** — 355 GB/s buffer clear on M1 Ultra; 100 million draw calls/second in **`vkoverhead`**; **AAA** gaming via **FEX** + **Wine** + **DXVK**/**VKD3D-Proton** + **muvm**
- **Upstreaming status** — **UAPI** in mainline; **Gallium** and **Honeykrisp** fully upstream in **Mesa**; **`drm/asahi`** kernel driver (~21,000 lines) remains out-of-tree; project leadership transitions of 2025 noted
- **Future work** — **M3**/**M4** GPU acceleration awaits reverse engineering of the **G15**/**G16** architecture (hardware ray tracing and **Dynamic Caching**)
