...
This chapter covers the full vertical slice from hardware topology to application API. It begins with the physical design of hybrid graphics—how MUX chips route display signals and when they are absent—then descends into the kernel PRIME infrastructure that enables cross-device buffer sharing via DMA-BUF file descriptors. From there it covers Mesa's `DRI_PRIME` and Vulkan device-selection layers, NVIDIA's proprietary render-offload mechanism, and the reverse-PRIME path for scan-out. On the hardware-policy side, AMD SmartShift's kernel sysfs interface, ASUS's `supergfxctl` daemon, and System76's `system76-power` tool are examined. Finally, the chapter surveys multi-GPU rendering from the now-defunct SLI/CrossFire era through Vulkan device groups, and explains the kernel `p2pdma` framework that underpins GPU-to-GPU peer DMA for machine learning workloads.
