...
### 7.2 Cheat Taxonomy: What the User-Space Gap Enables

Section 7.1 described the gap abstractly — no `ObRegisterCallbacks` equivalent, no cross-process handle interception. Concretely, that gap is what makes each of the major cheat categories tractable in user space, and understanding the technique class is what lets an anti-cheat engine (or a server-side validation layer) target the right signal. This section surveys the categories at an architectural level — the mechanism and what it exposes to a detector — without providing working exploit code; the referenced sources ([FairplayLab Bible](https://bible.fairplaylab.org/), Cano's *Game Hacking* [Source](https://nostarch.com/gamehacking)) go into full implementation detail for readers doing anti-cheat engineering.

**Memory editors.** The foundation for most other categories. A separate process reads and writes the target's address space via `process_vm_readv(2)`/`process_vm_writev(2)` or by opening `/proc/PID/mem` directly, then locates the addresses of interest through signature/pattern scanning (searching mapped pages for a byte pattern anchored to a known instruction sequence, the technique Cheat Engine popularized). On Windows, `PsSetLoadImageNotifyRoutine` and `ObRegisterCallbacks` let a kernel-mode anti-cheat driver strip `PROCESS_VM_READ`/`PROCESS_VM_WRITE` rights from untrusted handles opened against the game process; Linux has no equivalent kernel hook available to a user-space library, only the game's own `PR_SET_DUMPABLE` self-restriction, which a user-space AC can set but never enforce against a determined, privileged local user. [Source](https://tulach.cc/the-issue-of-anti-cheat-on-linux/)

**Aimbots.** Two distinct generations, with different detection surfaces. The classic form reads the entity list and view/projection matrices out of process memory (the same structures a memory editor locates by pattern scanning), computes the screen-space or world-space aim vector to the nearest valid target, and injects a synthetic mouse movement — historically via a global input hook or, on Linux, potentially via `/dev/uinput` (the same virtual-input-device interface legitimate tools like `ydotool` use for automation). Because this class touches game memory and the input stack, user-space AC can flag it through memory-region hashing and input-timing analysis (aim corrections that are too fast, too linear, or too precisely centered on a hitbox). The newer generation is a **vision-based aimbot**: it never touches game memory or process internals at all, instead running an object-detection model (commonly YOLO-family) over a live screen capture — sometimes fed through an external capture card specifically to keep the model's input path off the gaming PC entirely — and driving aim through the OS input layer only. Because no game memory or process is touched, kernel- and user-space anti-cheat that scans process state is structurally blind to it; academic detection work instead targets the model itself, e.g. injecting adversarial "honeytoken" visual patches into rendered frames that a YOLO-based aimbot misclassifies at over 90% detection rate while being imperceptible to a human player. [Source](https://arxiv.org/abs/2606.07650) [Source](https://arxiv.org/pdf/2606.25734) This is the strongest argument in the whole chapter for why server-side behavioral telemetry (Section 7.4) is not a fallback but the durable long-term defense: it is one of the only detection strategies that generalizes across memory-based and vision-based aimbots alike, and across Windows and Linux clients equally, since it inspects outcomes rather than client internals.

**Wallhacks / ESP.** Reads entity position and visibility data out of memory (again, the same pattern-scanning foundation) and renders it as an overlay — boxes, skeletons, or a radar — on top of, or instead of, the game's normal geometry occlusion. Two architectures exist. An *external* overlay runs as a separate transparent, click-through window and never injects into the game process, which removes it from the process-memory-hashing detection surface that EAC/BattlEye rely on. An *in-process* overlay hooks the graphics API's present call — `vkQueuePresentKHR` for a Vulkan title, the DXGI present chain for D3D — to draw its own geometry each frame before the swapchain image is presented. This is architecturally identical to what legitimate tools do: RenderDoc's Vulkan layer mechanism and MangoHud both interpose on the same `vkGetInstanceProcAddr`/`vkCreateInstance` dispatch chain to draw a performance overlay, using the `VK_LAYER_*` implicit-layer JSON manifest mechanism the Vulkan loader was designed to support. [Source](https://renderdoc.org/vulkan-layer-guide.html) A wallhack's render hook uses the identical interposition point but is typically installed via DLL/`.so` injection and IAT or vtable patching rather than a registered Vulkan layer, since an unexpected entry in `VK_INSTANCE_LAYERS` or the layer manifest search path is exactly the kind of anomaly a scanner would flag. [Source](https://guidedhacking.com/threads/dll-hijacking-vulkan-hook-tutorial-quake-2-hack.13518/) Anti-cheat detection therefore centers on module and layer enumeration (unexpected loaded libraries, unexpected implicit layers), and on hashing the function-pointer tables (`VkQueueFunctions`, `vtable`s, IAT entries) that a hook must overwrite to intercept the present call.

**Speed hacks.** Hook the timing primitives a game's fixed- or variable-timestep loop reads to compute elapsed time per frame — `GetTickCount`/`QueryPerformanceCounter`/`timeGetTime` on Windows, or `clock_gettime(2)`/`gettimeofday(2)` for a Linux-native title — via IAT hooking (or PLT/GOT patching for an ELF binary), and return a scaled delta so the game integrates movement, cooldowns, or animation faster (or slower) than real time. Cheat Engine's open-source `speedhack2.pas` is the canonical reference implementation of the technique. [Source](https://github.com/cheat-engine/cheat-engine/blob/master/Cheat%20Engine/speedhack2.pas) This is precisely the attack that Section 7.4's server-side **movement validation** is designed to catch: because the manipulation happens entirely on the client's notion of elapsed time, a server that independently timestamps each update and rejects position deltas inconsistent with its own clock defeats a speed hack regardless of how the client's timing functions were hooked — one of the few cheat categories where a purely server-authoritative, platform-agnostic check is close to a complete fix.

**Cheat development tooling.** No vendor sells a single unified "cheat SDK," but a small set of open-source libraries — mostly built for legitimate reverse-engineering, security research, or generic UI/graphics work — has become the de facto foundation that community "cheat bases" are built on, which is exactly why anti-cheat engines fingerprint them by name and behavior rather than treating every cheat as a bespoke unknown:

- **Present-hooking libraries**, e.g. `kiero`, provide a ready-made hook into the DirectX 9–12/OpenGL/Vulkan present call so a cheat author does not have to re-derive the swapchain dispatch interposition described above for every title. [Source](https://github.com/Rebzzel/kiero)
- **Generic inline-hooking libraries**, e.g. MinHook (trampoline-based x86/x64 function hooking) and Microsoft Detours, intercept arbitrary game functions by address rather than a fixed graphics API entry point, and are the hooking layer under most present hooks as well as under memory-editor and speedhack implementations. [Source](https://deepwiki.com/gmh5225/UE-UE4-Cheat-Source-Code/2.1-injection-and-hooking-system)
- **Dear ImGui** is the near-universal choice for cheat menu UI: it renders into a vertex/index buffer the host application's own renderer submits, so once a present hook exists, drawing a menu or ESP overlay is a matter of calling ImGui's immediate-mode API each frame rather than writing a renderer from scratch. Its buffer layout and draw-call signature are themselves a detectable fingerprint distinct from the game's own UI draws. [Source](https://guidedhacking.com/threads/guide-to-imgui-hooks-for-game-hack-menus.13993/)
- **Frida**, a dynamic binary instrumentation toolkit built for mobile and general security research, is increasingly repurposed as a scriptable, cross-platform alternative to Cheat Engine: `frida-gum`'s `Interceptor` and memory-scanning APIs can hook functions and read/write process memory from a JavaScript-driven script without a bespoke injector, which is attractive for Linux-native or cross-platform titles where Windows-only tooling doesn't apply. [Source](https://tickelton.gitlab.io/articles/cheating-with-frida/)
- **Engine-specific SDK dumpers** turn the memory-editor foundation from a per-title reverse-engineering project into a largely mechanical one wherever a game shares a common engine. For Unreal Engine 4/5 titles, tools such as Dumper-7 and UE4SS walk the engine's own reflection metadata (`UClass`/`UProperty` tables) at runtime and emit ready-to-use C++ headers with correct struct offsets and function addresses for that exact game build. [Source](https://github.com/Encryqed/Dumper-7) For Unity IL2CPP titles, Il2CppDumper reconstructs equivalent class, field, and method metadata from the compiled binary and `global-metadata.dat`. [Source](https://github.com/Perfare/Il2CppDumper) Because so much of the shooter and MMO catalog is built on one of these two engines, these dumpers are a large part of why a new title's cheat often appears within days of release: the hard part (locating structures by hand) is already automated by the engine SDK dumper, leaving only the game-specific gameplay logic to write.

This is also where anti-cheat detection gets concrete leverage: EAC, BattlEye, and VAC-style scanners maintain signature databases for exactly these components — the ImGui vertex-buffer shape, `kiero`/MinHook's trampoline byte patterns at hooked addresses, `frida-server`'s default port and thread/module naming, and injected UE4SS or Dumper-7 module names — rather than needing to understand each downstream cheat's game-specific logic individually. A cheat built entirely from custom, unpublished code is comparatively rare precisely because this shared tooling makes reuse so much cheaper than building from scratch.

**Other categories worth naming for completeness of the threat model:**

- **Kernel-level cheats via BYOVD (Bring Your Own Vulnerable Driver).** Rather than authoring a malicious driver (which would fail Windows driver-signing requirements), the cheat loads a legitimately signed but exploitable third-party driver and uses its own vulnerable IOCTLs to obtain arbitrary kernel read/write, from which it can hide the cheat process from `/proc`-equivalent enumeration or directly patch anti-cheat detection routines. The irony is instructive: Genshin Impact's own kernel-mode anti-cheat driver, `mhyprot2.sys`, was itself repeatedly weaponized this way — including by ransomware actors terminating EDR processes — because it remained installed and validly signed long after the game exited. [Source](https://thehackernews.com/2022/09/ransomware-attackers-abuse-genshin.html) This is the exact scenario Section 7.1's "kernel modules can hide themselves" gap describes, and it is the concrete incident kernel-anti-cheat vendors point to when arguing for driver allow-listing and Secure Boot enforcement.
- **Input-layer manipulation** (no-recoil, no-spread, macro/rapid-fire bots). Rather than editing memory, these suppress or synthesize input events — filtering out a weapon's recoil-compensating mouse movement, or replaying a fixed macro. On Linux this maps directly onto `evdev`/`uinput`, the same virtual-input-device layer legitimate remapping and accessibility tools use. Because no process memory is touched, detection is entirely behavioral: input timing that is too metronomic, or mouse deltas with statistically inhuman precision.
- **Network/packet manipulation.** Client-side packet spoofing, replay, or throttling (the classic "lag switch") against a game that trusts unauthenticated client state. This is a direct restatement of the case for the server-authoritative architecture in Section 7.4 — it is not a Linux-specific concern, but it is a category anti-cheat alone cannot fix without server-side corroboration.

### 7.3 Linux IMA and Hypothetical Kernel Module Anti-Cheat
...
### 7.4 Server-Side Mitigation as the Viable Alternative
...
- **EAC ARM Linux adoption by publishers.** Epic Games shipped ARM support for Easy Anti-Cheat in EOS SDK 1.17.1.3 (August 2025), extending the native `.so` to ARM64 Linux hosts. The near-term work is pushing publishers to update their EAC SDK version and re-enable the ARM+Linux platform flag — the technical enablement exists but opt-in rates lag behind x86-64. [Source](https://www.tomshardware.com/video-games/pc-gaming/epic-games-adds-easy-anti-cheat-for-arm-devices-feature-unlocks-support-for-windows-linux-and-switch-2)
- **Proton EasyAntiCheat and BattlEye Runtime hardening.** Valve continues to iterate on the Proton anti-cheat bridge runtimes (Steam tools `1826330` and `1161040`). Near-term releases are expected to address the recurring version-mismatch failures seen when EAC SDK versions outpace the runtime, reducing the tail of games that fall into a broken state after a publisher SDK bump. [Source](https://github.com/ValveSoftware/Proton/issues/6740)
- **GTA V BattlEye opt-in resolution.** Rockstar's September 2024 enablement of BattlEye for GTA Online broke Linux play; community pressure and Valve liaison work is ongoing to push Rockstar to enable the Linux BattlEye platform flag, a step that requires no code changes. Note: needs verification — Rockstar has not confirmed a timeline. [Source](https://www.gamingonlinux.com/2024/09/grand-theft-auto-v-gets-battleye-anti-cheat-breaks-online-play-on-steam-deck-linux/)
- **ntsync-aware anti-cheat testing.** With `ntsync` merged in Linux 6.14, Valve's QA is expanding Steam Deck compatibility validation to cover EAC and BattlEye behaviour under the new NT synchronisation primitives, ensuring the threading model changes do not introduce false-positive detections or crashes. [Source](https://www.phoronix.com/news/Linux-6.14-NTSYNC)

### Medium-term (1–3 years)

- **Linux IMA-based boot attestation for user-space anti-cheat.** The Linux Integrity Measurement Architecture (IMA) already extends TPM PCRs with hashes of loaded kernel modules and executables. A medium-term direction is an agreed protocol — between anti-cheat vendors and the Proton/SteamOS stack — where the Proton anti-cheat runtime can present a signed IMA measurement log to the game's EAC/BattlEye backend as a partial substitute for Windows Secure Boot attestation. The Trusted Computing Group has published guidance on using TPM 2.0 for gaming integrity. [Source](https://trustedcomputinggroup.org/using-a-tpm-to-bolster-anti-cheat-measures-in-gaming/) Note: no formal EAC/BattlEye commitment to this protocol has been announced.
- **AI/ML-based server-side behavioral detection as the primary Linux defence.** Industry analysis indicates the anti-cheat field is shifting detection weight toward server-side ML heuristics (network packet timing, aim trajectory analysis, hit-rate statistics) that are OS-agnostic. For Linux, where kernel-level attestation is structurally unavailable, this shift is particularly significant: it would allow anti-cheat vendors to accept a weaker client-side posture on Linux while compensating with stronger server-side signal. [Source](https://arxiv.org/pdf/2512.21377)
- **Vanguard user-mode AI protection research.** Riot has investigated shifting portions of Vanguard's detection work into attested user-mode sandboxes powered by ML inference, which could in principle reduce dependence on ring-0 access. Whether this eventually enables a Linux path depends on whether Linux user-space attestation (via IMA + TPM) satisfies Riot's security bar. Riot has made no public commitment to Linux. [Source](https://klizosolutions.medium.com/user-mode-ai-protectionbeyond-the-kernel-vanguards-planned-shift-to-user-mode-ai-protection-d074139333ad) Note: needs verification.
- **XIGNCODE3 and nProtect GameGuard successor evaluation.** Korean anti-cheat vendors whose products currently block Linux entirely are watching Steam Deck market share growth. Medium-term pressure from Korean publishers whose games are distributed on Steam may drive evaluation of user-space Linux ports comparable to what EAC and BattlEye have shipped. No public announcements have been made.

### Long-term

- **Confidential computing / TEE-based anti-cheat attestation on Linux.** Long-term architectural research explores using Intel TDX or AMD SEV-SNP trusted execution environments to provide a hardware-rooted attestation anchor on Linux without requiring a traditional kernel-mode driver. A game client running inside a confidential VM could attest its own integrity using a virtual TPM (vTPM). This would address Riot's stated objection that Linux cannot attest boot state, though it would impose hardware requirements and latency overhead. [Source](https://arxiv.org/pdf/2303.16463) Note: no gaming vendor has announced a production deployment of this model.
- **Kernel-level Linux anti-cheat module (community or vendor).** A theoretically possible but politically fraught direction is a signed, open-source Linux kernel module providing a subset of the protected-process and handle-stripping semantics that Windows ring-0 anti-cheat relies upon. This would require out-of-tree or upstream kernel patches, mandatory module signing enforcement in the gaming distribution, and acceptance from the Linux kernel community — all of which face substantial opposition on philosophical grounds. Valve has explicitly declined to pursue this path. [Source](https://www.phoronix.com/news/Easy-Anti-Cheat-Linux/)
- **Native Linux Riot Games clients.** The only path to Valorant and League of Legends on Linux that satisfies Vanguard's security requirements is a first-party native Linux client from Riot, which would allow Riot to ship a Linux kernel module or attestation agent under their own signing infrastructure. As of mid-2026, Riot has not announced any such plan, and the technical and business investment required is substantial. [Source](https://leagueoflinux.org/faq/vanguard/)
...
- FairplayLab Bible — cheat technique and detection reference: [FairplayLab Bible](https://bible.fairplaylab.org/)
- Nick Cano, *Game Hacking: Developing Autonomous Bots for Online Games* (No Starch Press, 2016): [nostarch.com/gamehacking](https://nostarch.com/gamehacking)
- RenderDoc — Vulkan layer interposition mechanism: [Brief guide to Vulkan layers](https://renderdoc.org/vulkan-layer-guide.html)
- GuidedHacking — Vulkan present-hook technique: [DLL Hijacking & Vulkan Hook Tutorial](https://guidedhacking.com/threads/dll-hijacking-vulkan-hook-tutorial-quake-2-hack.13518/)
- Cheat Engine source — canonical speedhack implementation: [speedhack2.pas](https://github.com/cheat-engine/cheat-engine/blob/master/Cheat%20Engine/speedhack2.pas)
- The Hacker News — ransomware abuse of Genshin Impact's `mhyprot2.sys` anti-cheat driver (September 2022): [Ransomware Attackers Abuse Genshin Impact Anti-Cheat System](https://thehackernews.com/2022/09/ransomware-attackers-abuse-genshin.html)
- arXiv — Detecting Aimbot Cheaters in MOGs: [arXiv:2606.07650](https://arxiv.org/abs/2606.07650)
- arXiv — Shoot the Honey, Cloak the Player: adversarial-patch detection of visual/vision-based cheating: [arXiv:2606.25734](https://arxiv.org/pdf/2606.25734)
- kiero — universal D3D9-D3D12/OpenGL/Vulkan present hook: [Rebzzel/kiero](https://github.com/Rebzzel/kiero)
- DeepWiki — UE4 cheat source injection/hooking system architecture (Kiero + MinHook + ImGui): [Injection & Hooking System](https://deepwiki.com/gmh5225/UE-UE4-Cheat-Source-Code/2.1-injection-and-hooking-system)
- GuidedHacking — ImGui cheat menu hooking guide: [Guide to ImGui Hooks for Game Hack Menus](https://guidedhacking.com/threads/guide-to-imgui-hooks-for-game-hack-menus.13993/)
- tickelton — Using Frida as a scriptable cheat engine: [Cheating with Frida](https://tickelton.gitlab.io/articles/cheating-with-frida/)
- Dumper-7 — Unreal Engine SDK generator: [Encryqed/Dumper-7](https://github.com/Encryqed/Dumper-7)
- Il2CppDumper — Unity IL2CPP reverse-engineering tool: [Perfare/Il2CppDumper](https://github.com/Perfare/Il2CppDumper)
