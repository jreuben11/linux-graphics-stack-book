...
**The server auto-starts by default.** The addon's `register()` function — called every time Blender launches with the addon enabled, not just on install — reads a Scene `BoolProperty` named `blendermcp_auto_start_server` (registered in the same `register()` call, `default=True`) and, when it is `True`, constructs a `BlenderMCPServer` and calls `.start()` unconditionally before the addon's panel is ever drawn ([`addon.py`](https://github.com/ahujasid/blender-mcp/blob/main/addon.py), `register()`). The N-panel's "Connect to MCP server" button (§2.3) only matters for a user who has flipped the "Auto-Start Server" checkbox off, or who wants to change the port before connecting. In practice this means both the community addon and the official server bring the TCP listener up on every Blender launch once installed and enabled — this is not a differentiator between them (§14.4 revisits this for headless pipelines).
...
> | Auto-start TCP server on Blender launch | yes — `register()` starts the server unconditionally by default (§2.1) | yes, via an Auto Start preference (§14) |
...
`uvx` (from the `uv` package manager) creates an ephemeral virtual environment, installs `blender-mcp` from PyPI, and executes the server — no manual `pip install` required. Because the addon's `register()` starts its TCP server automatically by default (§2.1), no N-panel interaction is needed after enabling the addon — Claude Code can call Blender tools as soon as Blender has finished launching. The "Connect to MCP server" button in the N-panel's "BlenderMCP" tab is only needed if Auto-Start has been disabled in the addon's preferences.
...
- [`alexisrolland/ComfyUI-Blender`](https://github.com/alexisrolland/ComfyUI-Blender) (GPL-3.0, Blender 5.0-compatible as of v4.0.0) is the closest existing equivalent to §12.4's combined workflow, packaged as a single add-on. A workflow is authored in ComfyUI using the project's own Blender-specific I/O nodes (`blender_input_load_image`, `blender_input_load_mask`, `blender_input_load_3d`, `blender_output_save_image`, `blender_output_save_glb`, among others), then exported as API-format JSON and imported into the Blender add-on, which auto-generates a UI panel from the workflow's inputs and outputs. On the Blender side, the add-on ships its own render operators — `render_depth_map`, `render_lineart`, `render_preview`, `render_view` — plus `create_material`/`project_material` to bring a generated image back as a material projected onto the mesh. This is a complete scene → control-pass → diffusion → reimport loop, built specifically around Blender's compositor rather than a generic image round-trip.
  > **Install the packaged release, not the repo source.** The repository root is a *server-side* ComfyUI custom node (`custom_nodes/ComfyUI-Blender`) whose I/O nodes import `comfy.samplers` and other modules that only exist inside a real ComfyUI install — zipping the repo and installing that zip as a Blender add-on fails with `ModuleNotFoundError` the moment the add-on tries to register. The installable Blender add-on is a separate artifact published on the repo's [GitHub Releases](https://github.com/alexisrolland/ComfyUI-Blender/releases) page (e.g. `comfyui_blender_v4.5.1.zip`); install *that* file from Preferences → Add-ons, and drop the custom-node portion of the repo into your ComfyUI server's `custom_nodes/` directory separately.
...
The official Blender Lab MCP server (v1.0.0) includes an **Auto Start** preference in its Extension settings that automatically starts the TCP server when Blender opens in interactive mode. As of §2.1, this is no longer a differentiator from the community addon — `ahujasid/blender-mcp`'s `register()` also starts its server unconditionally by default — so either server reaches the same "no panel click needed" state for interactive, non-headless sessions. What neither server changes is the headless constraint above: Auto Start still depends on Blender's windowing-system event loop to pump `bpy.app.timers`, so it has no effect under `--background`.

### 14.5 Flatpak Packaging Gotchas

Flatpak is Blender's own recommended distribution channel on Linux (the download page at [blender.org/download](https://www.blender.org/download/) links it alongside the tarball), and it sandboxes the application in ways that trip up scripted add-on installs and MCP/AI-tool pipelines that were only ever tested against a tarball or distro-packaged Blender.

**`/app` is read-only.** The flatpak's bundled Python interpreter and standard library live under `/app`, which is mounted read-only inside the sandbox — `pip install` (or an add-on's own runtime dependency installer, as several of this chapter's AI add-ons use, e.g. Dream Textures' `.python_dependencies`, §12.1) cannot write there and fails outright if pointed at the interpreter's own `site-packages`. Runtime-installed Python packages must instead target a writable, per-user directory that Blender's `sys.path` already includes: `~/.var/app/org.blender.Blender/config/blender/<version>/scripts/addons/modules` (the flatpak's mapping of the ordinary `scripts/addons/modules` add-on search path, under flatpak's per-app data prefix rather than `~/.config/blender`). Any install instructions in this chapter that assume a plain `pip install <package>` into Blender's own interpreter need the `--target` flag pointed at that path instead when Blender is the flatpak build:

```bash
# Flatpak Blender: install a runtime dependency for an add-on, e.g. version 5.1
pip install --target ~/.var/app/org.blender.Blender/config/blender/5.1/scripts/addons/modules some-package
```
...
- **3D-Print Toolbox** (`object_print3d_utils`) provides dedicated check operators for non-manifold edges, intersecting faces, and wall thickness, plus a **Make Manifold** operator that attempts automatic repair of bad normals, holes, and empty geometry ([3D Print Toolbox — Blender Manual](https://docs.blender.org/manual/en/4.0/addons/mesh/3d_print_toolbox.html), [source](https://github.com/blender/blender-addons/blob/main/object_print3d_utils/__init__.py)). Since Blender 4.2's addon-to-extension reorganisation it is no longer bundled — it does not appear in `addon_utils.modules()` on a stock 4.2+/5.x install — and must be installed from [extensions.blender.org/add-ons/print3d-toolbox](https://extensions.blender.org/add-ons/print3d-toolbox/) rather than merely enabled in Preferences → Add-ons.
...
- [3D Print Toolbox — Blender Manual](https://docs.blender.org/manual/en/4.0/addons/mesh/3d_print_toolbox.html) — Non-manifold/intersection/thickness checks and Make Manifold repair, installed via [extensions.blender.org](https://extensions.blender.org/add-ons/print3d-toolbox/) on 4.2+
