# Chapter 244e: FOSS Simulation-Game Frameworks — Data Models, Rules Engines, and Modding as SDKs

> **Part**: Part XI — Engines and Creative Tools
> **Audience**: Systems and application developers evaluating a data-driven rules engine and modding architecture for a domain-specific simulation — 4X/strategy-game developers, and readers of Ch244c/Ch244d looking for the data-model half of that story rather than the rendering half.
...
- [7. Brief Survey: FreeCol, FIFE/Unknown Horizons, Wesnoth WML, Widelands, ServUO, Oolite, Pioneer](#7-brief-survey-freecol-fifeunknown-horizons-wesnoth-wml-widelands-servuo-oolite-pioneer)
...
| **Widelands** Lua data + production-program DSL | Lua tables passed to `wl.Descriptions():new_tribe`/`new_productionsite_type`/`new_worker_type` etc., with per-building behavior expressed as a small engine-recognized verb vocabulary (`consume=`, `produce=`, `sleep=`, `animate=`, `return=…`) inside a `programs` table [Source](https://github.com/widelands/widelands/blob/master/data/tribes/buildings/productionsites/barbarians/bakery/init.lua) | Yes, and unusually completely: a tribe's full roster of wares, workers, buildings, and immovables is listed in Lua (`wares_order`, `workers_order`, `buildings`) with no hardcoded per-tribe content in C++ [Source](https://github.com/widelands/widelands/blob/master/data/tribes/initialization/barbarians/units.lua); the official add-on manager has a dedicated `tribes` category — "a set of scripts that modify tribe units, namely wares, workers, buildings, immovables, ships, or tribes" — for shipping new tribes without touching the engine [Source](https://www.widelands.org/documentation/add-ons/) | High for content, but bounded by the production-program vocabulary: a mechanic not expressible as one of the engine's recognized program actions still needs a C++ change | Host-authoritative deterministic lockstep, not a rules server: `GameHost`/`GameClient` relay `PlayerCommand`s and cross-check periodic sync reports rather than exchanging ruleset state [Source](https://github.com/widelands/widelands/blob/master/src/network/gamehost.h) | Low to moderate: declarative data plus a bounded DSL, but the add-on system has ten distinct typed categories (`tribes`, `world`, `script`, `map_generator`, `campaign`, …) an author must target correctly, and multiple add-ons compose by plain top-to-bottom execution order in the add-on manager, with no dependency or conflict-detection mechanism documented beyond that ordering [Source](https://www.widelands.org/documentation/add-ons/) |
| **ServUO** `EventSink` C# events + separately-compiled `Scripts/` project | XML data files (`Data/SpawnDefinitions.xml`, `Data/Regions.xml`) for content that fits a fixed schema, plus a `Scripts/Scripts.csproj` tree of C# source compiled to `Scripts.dll` by an automatic `dotnet build` on every server start [Source](https://raw.githubusercontent.com/ServUO/ServUO/master/Server/ScriptCompiler.cs); a static `Server.EventSink` class declares C# events (`Login`, `Speech`, `PlayerDeath`, …) that core network/game code raises and that `Scripts/` handlers subscribe to [Source](https://raw.githubusercontent.com/ServUO/ServUO/master/Server/EventSink.cs) | Yes, but not at the data-format level: the whole `Scripts/` tree is full C#, rebuilt as its own project separate from `Server/` and reloaded via `Assembly.LoadFrom`, so a modder never touches or rebuilds the core engine assembly, only the content project [Source](https://raw.githubusercontent.com/ServUO/ServUO/master/Server/ScriptCompiler.cs) | Effectively unbounded — it is general-purpose C#, closer to Thousand Parsec's compiled ruleset modules (§5) than to any text/YAML/Lua-data format in this table — but reachable through an ordinary edit-and-restart loop rather than a separate toolchain, since `Scripts.csproj` builds with the same `dotnet` already required to build ServUO itself | Single authoritative server process; the classic UO client and third-party clients such as ClassicUO (§7) are thin network renderers with no rules logic, communicating over UO's proprietary client/server protocol rather than a project-defined one | Low for XML-schema content (spawns, regions); high for anything routed through `EventSink`, which requires reading C# event-arg types and the `ScriptCompiler.Invoke("Configure")`/`Invoke("Initialize")` reflection convention that scans every loaded assembly for matching static methods [Source](https://raw.githubusercontent.com/ServUO/ServUO/master/Server/ScriptCompiler.cs) |

Reading down the "moddable without recompiling" column against the "maintenance burden" column shows the trade-off directly: the two projects whose primary modding path is plain data-file editing (OXCE's YAML, and Freeciv's `.ruleset` text for most day-to-day balance work) are also the two still under active development, while the project offering the most powerful modding surface (Thousand Parsec, compiled swappable rulesets) is the one that stopped. ServUO complicates that reading rather than confirming it: its modding surface is also compiled code, not data, yet the project remains active — the difference is that `Scripts/` is compiled automatically as part of every server start rather than requiring a separate release-engineering step the way `tpserver-cpp`'s `dlopen`ed modules did, which suggests it was the *toolchain friction* around Thousand Parsec's approach, not the fact of compiled rulesets per se, that correlates with the project stalling. OpenCiv3 and Freeciv both also embed Lua, and comparing them is instructive precisely because it isolates the code-vs-data axis from the active-vs-dead one: OpenCiv3 exposes its entire `C7GameData` object graph to mod-authored Lua by reflection, while Freeciv's Lua reaches the engine only through the curated, `tolua`-bound `api_edit_*`/`api_server_*` surface and only in response to named signals (§3.4) — a hand-curated API versus an open reflection surface, not "no code" versus "code." OXCE's YAML merging remains the one format in this chapter with no code path at all for a mod author to execute. Widelands sits closer to OXCE and Freeciv than to OpenCiv3 on this axis despite using Lua: its `programs` tables are declarative data referencing a fixed, engine-defined verb vocabulary, not general Lua logic reaching into engine internals the way OpenCiv3's reflected `C7GameData` access does — the DSL, not the host language, is what bounds it. The client/server column is a separate axis again: it tracks with neither modding power nor project health — the actively developed Freeciv is network-split and the actively developed OXCE is not — which is the point of listing it separately rather than folding it into "maintenance burden." All of this is addressed more generally for engine modding across languages in Ch244d.

---

## 7. Brief Survey: FreeCol, FIFE/Unknown Horizons, Wesnoth WML, Widelands, ServUO, Oolite, Pioneer
...
**Widelands** (`github.com/widelands/widelands`, GPL-2.0, C++/SDL2/OpenGL, v1.3.1 as of 2026-02-22) is a Settlers-style economic-simulation RTS whose entire per-tribe content — wares, workers, buildings, immovables, and the production chains connecting them — is Lua data with no hardcoded C++ equivalent. A tribe's roster is declared once in `wl.Descriptions():new_tribe{ wares_order = {...}, workers_order = {...}, buildings = {...} }` [Source](https://github.com/widelands/widelands/blob/master/data/tribes/initialization/barbarians/units.lua), and each building is its own `new_productionsite_type` call whose behavior is a `programs` table of string actions drawn from a fixed engine vocabulary — `consume=water:3 wheat:3`, `sleep=duration:20s800ms`, `animate=working duration:20s`, `return=skipped unless economy needs barbarians_bread` — rather than arbitrary Lua control flow [Source](https://github.com/widelands/widelands/blob/master/data/tribes/buildings/productionsites/barbarians/bakery/init.lua). This puts Widelands well past OpenCiv3 on "how much of the game is Lua" while staying well short of it on "how much of that Lua is general-purpose": the vocabulary is small, engine-defined, and not Turing-complete by design. The add-on manager formalizes this as a first-class distribution mechanism, with a `tribes` category defined as "a set of scripts that modify tribe units, namely wares, workers, buildings, immovables, ships, or tribes," alongside separate `world`, `script`, `map_generator`, `campaign`, `win_condition`, `starting_condition`, `ui_plugin`, and `theme` categories, each with its own schema; installed add-ons execute top-to-bottom in a reorderable list, with no dependency graph or per-field conflict detection documented beyond that ordering [Source](https://www.widelands.org/documentation/add-ons/). Multiplayer is host-authoritative deterministic lockstep rather than a rules-serving process: `GameHost` relays each client's `PlayerCommand`s to the others and periodically compares `sync_report`s to catch desyncs, so — unlike Freeciv's `civserver` — the ruleset itself is never transmitted or arbitrated by a server at runtime [Source](https://github.com/widelands/widelands/blob/master/src/network/gamehost.h). Rendering is SDL2 windowing over a genuine OpenGL pipeline, not software blitting: `src/graphic/gl/` holds dedicated GLSL-driven programs for terrain, roads, dithering, and line/rect fills [Source](https://github.com/widelands/widelands/tree/master/src/graphic/gl), and a separate `build_texture_atlas.cc`/`.h` pair packs sprites into shared texture atlases [Source](https://github.com/widelands/widelands/blob/master/src/graphic/build_texture_atlas.cc).

**ServUO** (`github.com/ServUO/ServUO`, GPL-2.0, C#, 753 stars, pushed as recently as 2026-08-04 [Source](https://api.github.com/repos/ServUO/ServUO)) is a server emulator for *Ultima Online* — it reimplements the server half of a still-commercially-live MMO, which puts it in a different legal and architectural position from every other project in this chapter's survey, all of which reimplement or extend abandoned or FOSS-native games. §6's table already covers its two-tier extensibility model in full: XML data files for fixed-schema content (spawns, regions) plus a `Scripts/` tree of ordinary C# compiled into `Scripts.dll` by an automatic `dotnet build` at every server start, kept structurally separate from the `Server/` core assembly [Source](https://raw.githubusercontent.com/ServUO/ServUO/master/Server/ScriptCompiler.cs). The piece worth adding here is the event surface that `Scripts/` code actually hooks: `Server/EventSink.cs` declares dozens of C# events on a static class — `Login`, `Speech`, `PlayerDeath`, `CreatureDeath`, and more — each raised by core networking or game-logic code through a matching `Invoke*` method, for example `InvokeLogin(LoginEventArgs e)` firing the `Login` event from `Server/Network/PacketHandlers.cs` [Source](https://raw.githubusercontent.com/ServUO/ServUO/master/Server/EventSink.cs). A `Scripts/` file subscribes with plain C# delegate syntax at startup, as `Scripts/Misc/LoginStats.cs` does: `EventSink.Login += new LoginEventHandler(EventSink_Login);` inside a static `Initialize()` method that `ScriptCompiler.Invoke("Initialize")` discovers by reflection across every loaded assembly [Source](https://raw.githubusercontent.com/ServUO/ServUO/master/Scripts/Misc/LoginStats.cs). The result is architecturally closer to a plugin/event-bus system than to any of this chapter's data-driven rulesets: server behavior a mod wants to change is reached by subscribing to a named signal rather than filling in a text or YAML template, and the ceiling is whatever C# can express rather than a bounded vocabulary. Community "distro" packages layer further content over a stock ServUO checkout this same way — Nerun's Distro, one long-standing example, ships as spawn-definition data plus a `Premium Spawner` script package dropped into an existing `Data/`/`Scripts/` tree rather than as a fork of `Server/` [Source](https://github.com/cmileto/servuo-nerun-distro); its last push was 2018-03-27, so it is cited here as an illustration of the pattern rather than as evidence of current activity. **ClassicUO** (`github.com/ClassicUO/ClassicUO`, BSD-2-Clause, C#, 719 stars) is worth one contrasting sentence rather than treatment of its own: it is a from-scratch open-source *client* for the same game ServUO serves, and its `ClassicUO.Renderer` module is a conventional 2D sprite-batcher (`Batcher2D.cs`, texture atlases under `Arts/`, a hue-translation shader for UO's palette-swap color system) rather than an architecturally distinctive rendering pipeline in the sense this book otherwise looks for [Source](https://github.com/ClassicUO/ClassicUO/tree/main/src/ClassicUO.Renderer).

**Oolite** and **Pioneer** are both worth stating plainly up front: they are original space-trading games inspired by David Braben and Ian Bell's *Elite*, not reimplementations of it — unlike, for instance, this chapter's other survey entries, neither ships Elite's actual assets, save format, or licensed code. **Oolite** (`github.com/OoliteProject/oolite`, Objective-C/OpenGL) dual-licenses its code under GPL-2.0 and its artwork/config data under CC BY-NC-SA 3.0, stated explicitly in its own `LICENSE.md`: "This work is licensed under the GNU General Public License version 2... Additionally, all artwork – 3D models, images and sounds – included in the work, as well as configuration files, are also licensed under the [CC BY-NC-SA 3.0] License" [Source](https://raw.githubusercontent.com/OoliteProject/oolite/master/LICENSE.md) — GitHub's own SPDX detector reports this as `NOASSERTION` because a dual code/asset license does not map onto a single SPDX identifier, not because the license is actually ambiguous. Its content-authoring surface is OXPs ("Oolite eXpansion Packs") scripted in JavaScript against a SpiderMonkey-backed engine, exposed to scripts through a global `worldScripts` object that `InitOOJSWorldScripts` registers on the JS global [Source](https://raw.githubusercontent.com/OoliteProject/oolite/master/src/Core/Scripting/OOJSWorldScripts.m); a script hooks named engine events by assigning functions to well-known properties, as the bundled `oolite-contracts-cargo.js` does with `this.startUp = function() {...}` and `this.shipWillLaunchFromStation = function() {...}` [Source](https://raw.githubusercontent.com/OoliteProject/oolite/master/Resources/Scripts/oolite-contracts-cargo.js) — the same named-callback shape as Freeciv's `signal.connect`-hooked Lua (§3.4), but in JavaScript and without an intervening curated-API layer between the callback and the engine's native JS bindings. **Pioneer** (`github.com/pioneerspacesim/pioneer`, C++/OpenGL, GPL-3.0 — confirmed against `licenses/GPL-3.txt` in the repository itself rather than GitHub's `null` API field, and displayed as a GPLv3 badge in its own README [Source](https://raw.githubusercontent.com/pioneerspacesim/pioneer/master/README.md)) drives both event handling and galaxy content generation through Lua. Gameplay modules register on named engine signals through an `Event.Register` call, exactly mirroring Freeciv's and Oolite's named-hook pattern: `data/modules/CargoRun/CargoRun.lua` ends with a block of registrations including `Event.Register("onEnterSystem", onEnterSystem)` and `Event.Register("onGameStart", onGameStart)` [Source](https://raw.githubusercontent.com/pioneerspacesim/pioneer/master/data/modules/CargoRun/CargoRun.lua). More distinctively, star systems themselves are Lua data built through a fluent object API rather than a flat table: `data/systems/00_sol.lua` constructs Sol with `CustomSystem:new('Sol', { 'STAR_G' })` and chained method calls like `:govtype(...)`/`:lawlessness(...)`, then builds each planet as a separate `CustomSystemBody:new(...)` object with its own chained physical parameters (`:radius`, `:mass`, `:semi_major_axis`, `:eccentricity`, and so on) [Source](https://raw.githubusercontent.com/pioneerspacesim/pioneer/master/data/systems/00_sol.lua) — procedural galaxy generation exposed as authorable Lua data, the same "content as data passed to engine-defined constructors" shape §6 uses to characterize Widelands' tribe definitions, applied here to star systems instead of buildings.
...
**Chapter 244d — Modding Architectures** covers the general engineering problem this chapter's frameworks each answer once, for one domain: embedded Lua as a modding substrate (§2.4's OpenCiv3/MoonSharp is a second, independently engineered instance of the `mlua`-style embedded-interpreter pattern Ch244d surveys generically, though built on MoonSharp rather than `mlua`; Freeciv's `tolua`-bound `api_edit_*`/`api_server_*` surface, §3.4, is a third, hand-curated instance of the same pattern), and data-driven rulesets more broadly as the domain-specific, code-free counterpart to Ch244d's scripting-and-sandboxing survey. Where Ch244d asks what an extension is allowed to *reach* — the file descriptor, the GPU, another mod's private state — OXCE's declarative YAML merging (§4) sidesteps that question by never running arbitrary code in the first place. OpenCiv3 and Freeciv both reopen it, but at different points on the same axis: a mod script reaching live engine objects through OpenCiv3's `GAME_DATA()` is a trust decision with almost no boundary, while Freeciv's `signal.connect`-hooked Lua is scoped to whatever the curated `api_edit_*`/`api_server_*` functions choose to expose — the hand-curated-binding pattern Ch244d treats as the norm, against which OpenCiv3's reflection-based approach is the outlier.

**Chapter 244c — Open-Source 2D Simulation-Game Engines** is this chapter's companion "reimplementation" chapter, there scoped to rendering architecture for a related family of projects and here scoped to rules and data-model architecture. Read together, the two chapters cover the same genre of open-source strategy/simulation project from its two structural halves: how it draws itself, and what it is actually simulating underneath.

**Chapter 244b — AI Agents in Games** covers the classical game-AI techniques (navmeshes, behaviour trees, GOAP, utility systems) and their modern/LLM-driven successors that sit underneath opponent implementations like Freeciv's `ai/classic/classicai.c` and OpenCiv3's `C7Engine/AI/PlayerAI.cs` (§3.5) — this chapter deliberately stops at naming those AI modules as engine-native, non-data-driven code, and defers the AI architecture itself to Ch244b. Ch244b's §3.5 also covers Charisma.ai's LLM-backed "structured story graph" as a hybrid, runtime-generated alternative to the ahead-of-time-compiled Ink/Yarn Spinner model this chapter's §12 surveys — the two sit at opposite ends of the same authoring problem, one compiling a writer's fixed branches and the other generating branches at runtime around a graph skeleton.

**Chapter 244f — Artificial Life on the GPU** is a sibling Part XI chapter covering a different class of simulation software — deliberately GPU-compute-focused where this chapter is deliberately non-graphics. The two chapters share little architectural overlap but sit side by side in the Part XI survey of simulation-oriented software; Ch244f links back to this chapter as its data-driven, rules-engine-first counterpart.
...
- [GitHub — widelands/widelands](https://github.com/widelands/widelands) — GPL-2.0, v1.3.1 released 2026-02-22 (§6, §7)
- [Widelands `data/tribes/initialization/barbarians/units.lua` (master)](https://github.com/widelands/widelands/blob/master/data/tribes/initialization/barbarians/units.lua) — `wl.Descriptions():new_tribe`, `wares_order`/`workers_order`/`buildings` tables listing a tribe's full content roster in Lua (§6, §7)
- [Widelands `data/tribes/buildings/productionsites/barbarians/bakery/init.lua` (master)](https://github.com/widelands/widelands/blob/master/data/tribes/buildings/productionsites/barbarians/bakery/init.lua) — `new_productionsite_type`, `programs.main.actions` string-verb production DSL (`consume=`, `sleep=`, `animate=`, `return=`) (§6, §7)
- [Widelands documentation — Add-Ons](https://www.widelands.org/documentation/add-ons/) — Ten typed add-on categories including `tribes` ("modify tribe units, namely wares, workers, buildings, immovables, ships, or tribes"); top-to-bottom execution order (§6, §7)
- [Widelands `src/network/gamehost.h` (master)](https://github.com/widelands/widelands/blob/master/src/network/gamehost.h) — `send_player_command`, `request_sync_reports`/`check_sync_reports`, host-relayed deterministic-lockstep multiplayer (§6, §7)
- [Widelands `src/graphic/gl/` directory listing (master)](https://github.com/widelands/widelands/tree/master/src/graphic/gl) — `terrain_program`, `road_program`, `dither_program`, `fill_rect_program` — SDL2-windowed OpenGL rendering, not software blitting (§7)
- [Widelands `src/graphic/build_texture_atlas.cc` (master)](https://github.com/widelands/widelands/blob/master/src/graphic/build_texture_atlas.cc) — Build-time sprite texture-atlas packing, separate from the `gl/` runtime shader programs (§7)
- [GitHub — ServUO/ServUO](https://github.com/ServUO/ServUO) — GPL-2.0, 753 stars, pushed 2026-08-04 (§6, §7)
- [ServUO `Server/EventSink.cs` (master)](https://raw.githubusercontent.com/ServUO/ServUO/master/Server/EventSink.cs) — Static `EventSink` class, C# event declarations (`Login`, `Speech`, `PlayerDeath`, …), `InvokeLogin` and other `Invoke*` raise methods (§6, §7)
- [ServUO `Server/ScriptCompiler.cs` (master)](https://raw.githubusercontent.com/ServUO/ServUO/master/Server/ScriptCompiler.cs) — `Scripts.csproj` built separately from `Server/` via `dotnet build` at every server start, `Assembly.LoadFrom`, `Invoke("Configure")`/`Invoke("Initialize")` reflection convention, `CallPriorityComparer` (§6, §7)
- [ServUO `Scripts/Misc/LoginStats.cs` (master)](https://raw.githubusercontent.com/ServUO/ServUO/master/Scripts/Misc/LoginStats.cs) — `EventSink.Login += new LoginEventHandler(...)` subscription inside a static `Initialize()` method (§7)
- [GitHub — cmileto/servuo-nerun-distro](https://github.com/cmileto/servuo-nerun-distro) — GPL-2.0, last pushed 2018-03-27, spawn-data/Premium-Spawner overlay on a stock ServUO tree (§7)
- [GitHub — ClassicUO/ClassicUO](https://github.com/ClassicUO/ClassicUO) — BSD-2-Clause, 719 stars (§7)
- [ClassicUO `src/ClassicUO.Renderer/` directory listing (main)](https://github.com/ClassicUO/ClassicUO/tree/main/src/ClassicUO.Renderer) — `Batcher2D.cs`, hue-translation shader, conventional 2D sprite-batching pipeline (§7)
- [GitHub — OoliteProject/oolite](https://github.com/OoliteProject/oolite) — dual-licensed, Objective-C/OpenGL, original Elite-inspired game (§7)
- [Oolite `LICENSE.md` (master)](https://raw.githubusercontent.com/OoliteProject/oolite/master/LICENSE.md) — GPL-2.0 for code, CC BY-NC-SA 3.0 for artwork/config data, manually verified after GitHub's license API returned `NOASSERTION` (§7)
- [Oolite `src/Core/Scripting/OOJSWorldScripts.m` (master)](https://raw.githubusercontent.com/OoliteProject/oolite/master/src/Core/Scripting/OOJSWorldScripts.m) — `InitOOJSWorldScripts`, global `worldScripts` JS object registration (§7)
- [Oolite `Resources/Scripts/oolite-contracts-cargo.js` (master)](https://raw.githubusercontent.com/OoliteProject/oolite/master/Resources/Scripts/oolite-contracts-cargo.js) — `this.startUp`/`this.shipWillLaunchFromStation` named-callback OXP script (§7)
- [GitHub — pioneerspacesim/pioneer](https://github.com/pioneerspacesim/pioneer) — GPL-3.0, C++/OpenGL, original Elite-inspired game (§7)
- [Pioneer `README.md` (master)](https://raw.githubusercontent.com/pioneerspacesim/pioneer/master/README.md) — GPLv3 license badge, manually confirmed after GitHub's license API returned `null` (§7)
- [Pioneer `licenses/GPL-3.txt` (master)](https://github.com/pioneerspacesim/pioneer/blob/master/licenses/GPL-3.txt) — Full GPL-3.0 license text bundled in-repo (§7)
- [Pioneer `data/modules/CargoRun/CargoRun.lua` (master)](https://raw.githubusercontent.com/pioneerspacesim/pioneer/master/data/modules/CargoRun/CargoRun.lua) — `Event.Register("onEnterSystem", ...)`/`Event.Register("onGameStart", ...)` mission-module hooks (§7)
- [Pioneer `data/systems/00_sol.lua` (master)](https://raw.githubusercontent.com/pioneerspacesim/pioneer/master/data/systems/00_sol.lua) — `CustomSystem:new(...)`/`CustomSystemBody:new(...)` fluent builder for procedural galaxy content (§7)
...
- [Freeciv `fc_version` (`S3_3`)](https://raw.githubusercontent.com/freeciv/freeciv/S3_3/fc_version) — `PATCH_VERSION="94"`, `VERSION_LABEL="-alpha2+"`, `NEXT_STABLE_VERSION="3.3.0"`, `RELEASE_MONTH=0` (Roadmap)
- [Freeciv `fc_version` (`main`)](https://raw.githubusercontent.com/freeciv/freeciv/main/fc_version) — `MAIN_VERSION=3.4`, `NEXT_STABLE_VERSION="3.4.0"`, `IS_DEVEL_VERSION=1`, `RELEASE_MONTH=0` with the comment "0 to disable display of release month until we can make better estimate" (Roadmap)
- [Freeciv `server/ruleset/rscompat.c` (`main`)](https://raw.githubusercontent.com/freeciv/freeciv/main/server/ruleset/rscompat.c) — `if (info->version < RSFORMAT_3_4)` migration branches for user flags and effect names; `rscompat_effect_name_3_4()` (§9.1, Roadmap)
- [Freeciv `data/classic/game.ruleset` (`main`)](https://raw.githubusercontent.com/freeciv/freeciv/main/data/classic/game.ruleset) — `format_version = 50` (§9.1, Roadmap)
- [Freeciv `data/classic/game.ruleset` (`S3_3`)](https://raw.githubusercontent.com/freeciv/freeciv/S3_3/data/classic/game.ruleset) — `format_version = 40` (§9.1, Roadmap)
- [Freeciv `data/classic/game.ruleset` (`S3_2`)](https://raw.githubusercontent.com/freeciv/freeciv/S3_2/data/classic/game.ruleset) — `format_version = 30` (§9.1, Roadmap)
- [freeciv-web commits on `develop`](https://github.com/freeciv/freeciv-web/commits/develop) — Most recent `develop` commit dated 2025-05-02 (§3.3, Roadmap)
- [Wesnoth `changelog.md` (`master`)](https://raw.githubusercontent.com/wesnoth/wesnoth/master/changelog.md) — Heading `## Version 1.19.26+dev`; WML additions `[default_frame]`, `priority` on abilities and weapon specials, `[overwrite_abilities]`, deprecation of `overwrite_specials` (§7, Roadmap)
- [Wesnoth branch list](https://github.com/wesnoth/wesnoth/branches/all) — Stable branches stop at `1.18`; no `1.20` branch exists (Roadmap)
- [Wesnoth milestone 1.21](https://github.com/wesnoth/wesnoth/milestone/29) — Opened 2026-04-27 for the post-1.20 development series (Roadmap)
- [OpenCiv3 milestone "[DUTCH] Technology and Research"](https://github.com/C7-Game/OpenCiv3/milestone/17) — Open milestone covering the tech tree and automatic research (Roadmap)
- [OpenCiv3 milestone "[CRT] Architectural Improvements"](https://github.com/C7-Game/OpenCiv3/milestone/11) — "Figure out what a mod will look like, ship one, even if it's super small" listed as outstanding (§10.2, Roadmap)
- [OpenCiv3 milestone "[CRT] Save/Load"](https://github.com/C7-Game/OpenCiv3/milestone/8) — "This will likely not mean you can load Carthage saves forever" (§9.3, Roadmap)
- [OpenCiv3 release `v0.3-dutch-preview-1`](https://github.com/C7-Game/OpenCiv3/releases) — Dated 2025-12-07; introduces standalone mode with placeholder graphics, removing the Civilization III media requirement (Roadmap)
- [openciv3.org](https://openciv3.org/) — "under active development and currently in an early pre-alpha state"; "a rudimentary playable game but lacking many mechanics and late-game content" (Roadmap)
- [OXCE releases (MeridianOXC/OpenXcom)](https://github.com/MeridianOXC/OpenXcom/releases) — Extended 8.6.2 (2026-07-25) and 8.6.3 (2026-08-12); ruleset-visible additions including voice sets, a "Not purchasable" equipment filter, and per-craft-type armor group limits (§9.2, Roadmap)
...
- **Wesnoth branches 1.20 stable and freezes a revised WML API.** The `master` changelog reads `## Version 1.19.26+dev` and the newest stable branch is still `1.18`, so 1.20 has not yet branched; the project has nonetheless already opened a `1.21` milestone for the series after it, dating the 1.20 cut to the near term [Source](https://github.com/wesnoth/wesnoth/milestone/29). The 1.19 development series has been actively changing WML — adding `[default_frame]` in `[unit_type]`, `priority` on abilities and weapon specials, and `[overwrite_abilities]`, while deprecating `overwrite_specials` — so 1.20 will freeze a materially different content-authoring surface from 1.18 [Source](https://raw.githubusercontent.com/wesnoth/wesnoth/master/changelog.md) (§7, §8.4).
- **Freeciv 3.3.0 leaves alpha with ruleset `format_version = 40`.** The `S3_3` branch carries `PATCH_VERSION="94"`, `VERSION_LABEL="-alpha2+"` and `NEXT_STABLE_VERSION="3.3.0"`, placing it in the late-alpha run-up to a stable release [Source](https://raw.githubusercontent.com/freeciv/freeciv/S3_3/fc_version). Every third-party ruleset that targets 3.2's `format_version = 30` will need the `rscompat` migration path described in §9.1 to load against 3.3's `format_version = 40` [Source](https://raw.githubusercontent.com/freeciv/freeciv/S3_2/data/classic/game.ruleset) [Source](https://raw.githubusercontent.com/freeciv/freeciv/S3_3/data/classic/game.ruleset) (§3.2, §9.1).
- **OpenCiv3 completes the technology and research milestone.** The open `[DUTCH] Technology and Research` milestone covers the tech tree and automatic research generation — the last major Civilization III subsystem still absent from the C7GameData model described in §2 [Source](https://github.com/C7-Game/OpenCiv3/milestone/17). It follows `v0.3-dutch-preview-1` (2025-12-07), which made the engine runnable in standalone mode with placeholder graphics and thereby removed the original-media dependency that had gated distribution [Source](https://github.com/C7-Game/OpenCiv3/releases). The project describes itself as "under active development and currently in an early pre-alpha state", offering "a rudimentary playable game but lacking many mechanics and late-game content" [Source](https://openciv3.org/) (§2, §8.1).
- **OXCE continues shipping monthly point releases that widen the `.rul` surface.** Extended 8.6.2 (2026-07-25) and 8.6.3 (2026-08-12) added ruleset-visible keys such as voice sets, a "Not purchasable" equipment filter, and per-craft-type armor group limits, with no published roadmap beyond the release cadence itself [Source](https://github.com/MeridianOXC/OpenXcom/releases) (§4, §9.2).

### Medium-term (1–3 years)
- **Freeciv 3.4 and a second consecutive ruleset format bump.** The `main` branch declares `MAIN_VERSION=3.4` and `NEXT_STABLE_VERSION="3.4.0"` [Source](https://raw.githubusercontent.com/freeciv/freeciv/main/fc_version), and its `classic` ruleset already carries `format_version = 50` [Source](https://raw.githubusercontent.com/freeciv/freeciv/main/data/classic/game.ruleset). Migration code for that jump is in tree — `rscompat.c` guards user-flag and effect-name conversions behind `if (info->version < RSFORMAT_3_4)` — confirming that the compatibility burden analysed in §9.1 is being paid forward one major release at a time rather than eliminated [Source](https://raw.githubusercontent.com/freeciv/freeciv/main/server/ruleset/rscompat.c) (§9.1).
- **OpenCiv3's mod packaging format remains an open design question.** The `[CRT] Architectural Improvements` milestone still lists "Figure out what a mod will look like, ship one, even if it's super small" as outstanding work [Source](https://github.com/C7-Game/OpenCiv3/milestone/11), and the prototype-mod issue has been open since 2022 [Source](https://github.com/C7-Game/OpenCiv3/issues/133). Until that resolves, the MoonSharp scripting hook described in §8.1 has no distribution story to attach to, which is the gap identified in §10.2 (§8.1, §10.2).
- **Save compatibility across OpenCiv3 milestones is explicitly not guaranteed.** The `[CRT] Save/Load` milestone description states that shipping save/load "will likely not mean you can load Carthage saves forever" — a project-stated position that matches the §9.3 analysis of pre-1.0 engines treating the save format as internal state rather than a contract [Source](https://github.com/C7-Game/OpenCiv3/milestone/8) (§9.3).
- **FreeCol continues rolling development builds without cutting a stable tag.** The last tagged stable release is v1.2.0 (2024-07-23), while a `latest` "Development Build" prerelease is regenerated per commit and remains current [Source](https://github.com/FreeCol/freecol/releases). Two-plus years of daily commits without a stable tag means the rulesets and specification files in §7 are effectively versioned by git revision, not by release (§7, §10.4).
- **Base OpenXcom and OXCE diverge further.** Upstream `src/version.h` still declares `OPENXCOM_VERSION_SHORT "1.0"` with a `" Dev"` git suffix, unchanged since the 2014 release, and the project publishes no tagged builds or milestones [Source](https://raw.githubusercontent.com/OpenXcom/OpenXcom/master/src/version.h). OXCE's monthly cadence therefore makes the fork, not the parent, the de facto target for the ruleset ecosystem described in §4 (§4, §9.2).

### Long-term
- **Thousand Parsec is permanently inactive; its protocol-first ruleset model has no successor.** The last push to the server implementation `tpserver-cpp` was 2011-05-03 and the last push to any code repository in the organisation was 2017-08-28; no repository has been archived, so the project is dormant rather than formally wound down [Source](https://github.com/orgs/thousandparsec/repositories). Nothing in the surveyed projects revives the compiled swappable-ruleset-module design of §5 — the durable approaches are all plain-text or YAML rulesets loaded at runtime (§5, §10.3).
- **The web bridge is the least maintained layer of the Freeciv ecosystem.** freeciv-web's `develop` branch has had no commit since 2025-05-02 [Source](https://github.com/freeciv/freeciv-web/commits/develop). As §3.3 argues, freeciv-web is a bridge over an unmodified rules engine rather than a reimplementation, so the core game continues to advance through 3.3 and 3.4 regardless — but the browser client will lag each ruleset format bump until someone re-syncs it (§3.3).
- **None of the surveyed projects publishes a dated long-term roadmap.** Freeciv is the only one whose build system carries a release-date field at all, and it is explicitly disabled: `fc_version` sets `RELEASE_MONTH=0`, annotated "0 to disable display of release month until we can make better estimate" [Source](https://raw.githubusercontent.com/freeciv/freeciv/main/fc_version). Forward planning in this category is expressed through branch state, milestone titles, and release cadence — which is why the ruleset `format_version` and engine-version gates examined earlier are the most reliable signal of what is coming next (§9.1, §9.2).
