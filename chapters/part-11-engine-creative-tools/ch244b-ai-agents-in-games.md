...
**NVIDIA's robotics-training stack** — Isaac Sim, Isaac Lab, and the GR00T foundation-model family — is the industrial-scale version of the same vectorized-simulation idea, with a proprietary renderer and a USD-based scene model. It is covered in Chapter 211b, with Chapter 69 covering the underlying Omniverse/OpenUSD renderer and Chapter 240 covering the synthetic-data-generation pipeline built on top of it.
...
### 11.1 The Commercial Precedent, Still Active: Unity ML-Agents

Unity ML-Agents (`Unity-Technologies/ml-agents`, **19,612 stars**, Apache-2.0, actively pushed) does not belong in the "no longer maintained" framing above — it predates every other project in this chapter and is the commercially-backed precedent for the synchronous engine-bridge shape that Godot RL Agents (Section 5) occupies in the open-source world, deserving separate treatment rather than a place at the end of a historical table.

Its architecture is the same synchronous-lockstep shape as Sections 5 and 6, described in Unity's own documentation as an **External Communicator** that "connects the Learning Environment with the Python Low-Level API" and "lives within the Learning Environment" — agents in the scene send observations through it to a Python trainer process launched via `mlagents-learn`, and receive actions back the same way [[Source](https://github.com/Unity-Technologies/ml-agents/blob/main/docs/ML-Agents-Overview.md)]. Once training is done, the same documentation describes swapping the Python round-trip for the **Unity Inference Engine**: "the medics still continue to generate their observations, but instead of being sent to the Python API, they will be fed into their (internal, embedded) model to generate the optimal action" — the same escape-Python pattern as Godot RL Agents' ONNX inference mode (Section 5.6), and Unity's own precedent for it [[Source](https://github.com/Unity-Technologies/ml-agents/blob/main/docs/ML-Agents-Overview.md)].

Two capabilities here go beyond what Sections 5 and 6 cover for their own frameworks. ML-Agents documents built-in support for **self-play**, "both symmetric and asymmetric adversarial games," and for **curriculum learning** via "modifying custom environment parameters during the training process" [[Source](https://github.com/Unity-Technologies/ml-agents/blob/main/docs/ML-Agents-Overview.md)] — both are DIY territory (a `call`-message level parameter change, in Godot RL Agents' terms) in every other synchronous-lockstep framework this chapter covers. *Note: needs verification* — the exact wire protocol between the Python Low-Level API and the External Communicator (gRPC versus raw sockets) is not confirmed in this pass; it is functionally the same problem Section 5.2 documents for Godot RL Agents, whichever transport it turns out to be.

### 11.2 Maintained but CPU-Only: OpenSpiel and Melting Pot
...
The chapter's argument, stated compactly. Section 7 established that Unreal Engine Learning Agents is a fourth shape — local, synchronous *inference* paired with external, episodic *training* — distinct from full lockstep, full vectorization, and full decoupling, so it gets its own column here rather than being folded into whichever of the other three it superficially resembles.

| | **Synchronous lockstep** | **Local inference, external training** | **Vectorized headless** | **Decoupled async** |
|---|---|---|---|---|
| Examples | Godot RL Agents, `bevy_rl`, Unity ML-Agents | Unreal Engine Learning Agents | Craftax (and MJX, Ch211a) | AI Town |
| Coupling | Separate processes, blocking IPC every step | In-engine inference; IPC only during training | One compiled program, no IPC | Network + transactional DB |
| Who owns the socket | Python (Godot RL); game (`bevy_rl`) | External trainer, only while `ULearningAgentsTrainer` is active | No socket | HTTP + DB |
| Rendering during training | Off by default (`--headless`, `render_to_buffer: false`) | Engine runs normally; inference is in-process (NNE) | No graphics API at all | Always on, client-side |
| Pixel observation path | GPU→CPU readback, then IPC encode | N/A — observations are structured, not pixels | Stays in device memory | N/A (human viewer, not agent) |
| Parallelism | One process per env | One engine instance; training-time parallelism is the external process's problem | `vmap` over thousands per device | One world per simulation |
| Characteristic latency | 16 ms tick, blocked on gradient step | 16 ms tick, uninterrupted; training happens out-of-band | ~30 s compile, then microseconds | 16 ms tick / 1 s agent / 120 s LLM |
| Existing engine content | Reusable | Reusable, and shippable at runtime once trained | Must be rewritten | Reusable |
| Dominant cost | IPC and readback | Communicator bandwidth during training only | Compilation, batch-uniform work | LLM inference |

Three cross-cutting observations, plus one about the shape §7 adds.
...
**"Local inference, external training" trades per-step IPC for shippability.** Every synchronous-lockstep framework pays a socket round-trip on every single decision, forever — training and deployment are the same architecture. Learning Agents instead pays the IPC cost only while `ULearningAgentsTrainer` is active, because the trained policy runs through NNE inside the engine process once training finishes. That is the one shape in this table where the shipped game does not depend on Python, a socket, or an external process at runtime at all — the price is that Learning Agents' training loop is episodic rather than the full per-step lockstep of Section 5 or 6, so it does not fit either of those sections' descriptions.

### 12.1 Choosing a Shape

The choice is largely forced by what you already have and what you need. An existing game with existing content that needs to keep training and inference coupled admits **synchronous lockstep**, the shape that reuses the engine, the assets, and the gameplay code at the cost of IPC on every step — mitigated by preferring structured observations over pixels and by Action Repeat. The same existing-content case, but where the trained policy must run standalone in a shipped build with no Python dependency, points instead to **local inference, external training** (Section 7): pay the IPC cost only during the episodic training loop, then ship an NNE-evaluated network. A benchmark that must absorb billions of steps justifies **vectorized headless**, whose price is a full rewrite in a functional array framework with every data-dependent branch reformulated as masked uniform work. Agents that reason or converse in language have no option but **decoupled async**: a 120-second cognition step cannot be made synchronous with a 16 ms frame, so the three clocks and the transactional store between them are structural requirements rather than design choices.
...
**Chapter 240 — NVIDIA Cosmos, OSMO, and Omniverse Farm** covers the case where the *renderer's output is the product* rather than an observation to be consumed by a policy in the same process — orchestrating Isaac Sim (Chapter 211b) at fleet scale to generate synthetic training data. That inverts this chapter's economics: Sections 5.4 and 6.3 treat GPU-to-CPU readback as pure overhead to be minimized or deleted, whereas a synthetic-data pipeline exists precisely to write rendered frames out at scale, making high-fidelity rasterization and domain randomization the goal rather than a tax.
...
- [Unity ML-Agents](https://github.com/Unity-Technologies/ml-agents) — commercially-backed precedent for the synchronous engine-bridge architecture; 19,612 stars, Apache-2.0, active (§11.1)
...
- [PettingZoo 1.26.0 release notes](https://github.com/Farama-Foundation/PettingZoo/releases/tag/1.26.0) — 2026-04-26; Python 3.13/3.14 support, MPE environments split out into the separate MPE2 package, Waterworld removed (§4.4, Roadmap)
- [PettingZoo issue #1392 — deprecation notice for per-module `env()` constructors](https://github.com/Farama-Foundation/PettingZoo/issues/1392) — 2026-07-10; migration to a Gymnasium-style `make("aec", "butterfly/pistonball-v6")` with removal of `env()` planned after a year (§4.4, Roadmap)
- [PettingZoo issue #1366 — "Migrate applicable classic environments to openspiel via shimmy"](https://github.com/Farama-Foundation/PettingZoo/issues/1366) — rlcard "no longer getting new updates or Python versions"; OpenSpiel backends judged better-tested and faster (§4.4, §11.2, Roadmap)
- [Godot RL Agents issue #193 — "Features for 0.9 release"](https://github.com/edbeeching/godot_rl_agents/issues/193) — open since 2024-07-02; pretrained behaviour library, dedicated Hugging Face org, and a comment proposing a `terminated`/`truncated` API split for 1.0 (§5, §5.1, Roadmap)
- [Godot RL Agents releases](https://github.com/edbeeching/godot_rl_agents/releases) — newest tag v0.8.2 dated 2025-02-25 despite continuing commit activity (§5, Roadmap)
- [bevy_rl on crates.io](https://crates.io/crates/bevy_rl) — version history showing 0.17.0 as the last stable release, with 0.18.0-rc1 and 0.19.0-rc1 published as release candidates only (§6, §6.4, Roadmap)
- [Craftax v1.6.0 release notes](https://github.com/MichaelTMatthews/Craftax/releases/tag/v1.6.0) — 2026-06-16; reward bug fixes and "Removed gymnax dependency as is somewhat unmaintained" (§8, §8.1, Roadmap)
- [Craftax commit `4fc1f58` — "remove gymnax dependency"](https://github.com/MichaelTMatthews/Craftax/commit/4fc1f58602e28e8d6f814b8d158d1592ea56eb13) — vendors gymnax's `Environment` base class and `spaces` module into `craftax/environment_base/` (§8.1, Roadmap)
- [Neural Network Engine (NNE)](https://dev.epicgames.com/documentation/en-us/unreal-engine/neural-network-engine-in-unreal-engine) — Epic's inference runtime underlying Learning Agents; documented as **Beta**, with shipped applications in denoising, post-processing, ML deformers and cloth (§7.1, Roadmap)
- [AI Town commit history](https://github.com/a16z-infra/ai-town/commits/main) — 5 commits in the 12 months to 2026-08-13; repository not archived (§10, Roadmap)
...
- **PettingZoo replaces per-module `env()` constructors with a Gymnasium-style `make()`.** A formal deprecation notice posted 2026-07-10 moves `from pettingzoo.butterfly import pistonball_v6; pistonball_v6.env()` to `from pettingzoo import make; make("aec", "butterfly/pistonball-v6")`, with the old constructors to be removed "after a year." Code written against the AEC and Parallel examples in this chapter keeps working through that window but should be migrated [Source](https://github.com/Farama-Foundation/PettingZoo/issues/1392) (§4.4).
- **Gymnasium ports its Box2D environments to pymunk.** The tracking issue is open and tagged `help wanted`: box2d's Python bindings have been unmaintained for roughly six years and carry no Python 3.14 support, which the maintainers call "especially urgent." The staged plan ships a parallel pymunk environment class with warnings before yanking the box2d one, and starts with Bipedal Walker because PettingZoo, MO-Gymnasium and MOMAland all depend on it — so environment version numbers will bump across the Farama suite [Source](https://github.com/Farama-Foundation/Gymnasium/issues/1597) (§4).
- **Gymnasium's `RepeatAction` wrapper standardises frame-skipping.** Shipped in v1.3.0 (2026-04-22), it supplies as a first-class wrapper the action-repeat behaviour that engine bridges have historically implemented ad hoc inside their environment loops [Source](https://github.com/Farama-Foundation/Gymnasium/releases/tag/v1.3.0) (§5.5).
- **Godot RL Agents is developed actively but released rarely.** The repository took 41 commits in the 12 months to 2026-08-13, yet v0.8.2 (2025-02-25) remains the newest tag — roughly eighteen months without a release, so `pip`-installed users trail the examples in the repository. Recent merged work is example-focused (an SB3-contrib `RecurrentPPO`/PPO-LSTM example landed 2026-06-13) rather than protocol change [Source](https://github.com/edbeeching/godot_rl_agents/releases) (§5).
- **bevy_rl has not cut a stable release across two Bevy cycles.** crates.io shows 0.17.0 (2025-10-08) as the last stable version, followed only by 0.18.0-rc1 and 0.19.0-rc1; all five commits in the 12 months to 2026-08-13 are Bevy version bumps, with the last substantive change dated 2025-04-27. The plugin tracks the engine but has no feature roadmap, published or implied [Source](https://crates.io/crates/bevy_rl) (§6, §6.4).
- **AI Town is in minimal maintenance, not archived.** Five commits landed in the 12 months to 2026-08-13, the most recent removing an unused dependency and bumping Convex; 72 issues and five pull requests are open, the oldest PR dating to 2025-08-08. The Convex/PixiJS architecture described here still runs, but a deployment is effectively self-maintained [Source](https://github.com/a16z-infra/ai-town/commits/main) (§10).

### Medium-term (1–3 years)

- **Gymnasium may become array-framework agnostic.** An open proposal would use the maturing Python Array API to make `torch`, `jax` and `cupy` first-class alongside numpy, concentrating the change in `spaces` and `vector` rather than in environments, and explicitly without a backwards-incompatible break — the target being a `Box` whose bounds are CUDA tensors. If it lands, the API-level gap this chapter draws between the socket-bridged CPU world and the on-device JAX world narrows considerably [Source](https://github.com/Farama-Foundation/Gymnasium/issues/1388) (§4, §8, §9).
- **PettingZoo intends to move its classic environments onto OpenSpiel via Shimmy.** The maintainers' stated reason is that the classic suite currently rests on self-maintained logic or on rlcard, which is "no longer getting new updates or Python versions," while OpenSpiel's implementations are "much better tested for bugs and much faster." Leduc and Texas hold'em are the high-priority targets, migrated one environment per PR, following the pattern already used for Hanabi [Source](https://github.com/Farama-Foundation/PettingZoo/issues/1366) (§4.4, §11.2).
- **Godot RL Agents' 0.9/1.0 backlog is published but largely unstarted.** The open feature issue still lists a pretrained behaviour library — point-to-point navigation, patrolling, exploration, enemy avoidance — that would let a game add NPC behaviours without installing the Python half of the plugin, plus a dedicated Hugging Face organisation for shipping those models. A comment on the same issue proposes splitting the bridge's single `done` flag into Gymnasium's `terminated`/`truncated` for 1.0, which is exactly the `# TODO update API to term, trunc` this chapter quotes from `godot_env.py`; a 2026-05-06 request for turn-based/async stepping remains open with a maintainer note that such environments "may be more challenging without further changes" [Source](https://github.com/edbeeching/godot_rl_agents/issues/193) (§5.1, §5.6).
- **Unreal's Learning Agents stays Experimental while its inference substrate reaches Beta.** Epic's plugin index still marks Learning Agents **Experimental** at UE 5.8, whereas NNE — the runtime the plugin relies on for in-process policy inference — is documented as **Beta**, with shipped production uses in denoising, post-processing, ML deformers and cloth rather than agent policies. Epic publishes no graduation timeline or version-support commitment for either, so the inference half of this architecture is the more settled half and the training half should still be treated as subject to breaking change [Source](https://dev.epicgames.com/documentation/en-us/unreal-engine/neural-network-engine-in-unreal-engine) (§7, §7.5).
- **Craftax has already cut gymnax loose, and the vendoring is the signal.** Release v1.6.0 (2026-06-16) removed the gymnax dependency with the note "as is somewhat unmaintained," and commit `4fc1f58` copied gymnax's `Environment` base class and `spaces` module into `craftax/environment_base/` rather than dropping the interface. The upstream package took 5 commits in the 12 months to 2026-08-13 and has had no release since v0.0.8 (2024-04-21), with 38 issues open — so readers should treat the gymnax *interface* as the ecosystem's shared contract while treating the gymnax *package* as one that downstream projects now copy rather than depend on. This supersedes the characterisation of gymnax as actively maintained earlier in this chapter [Source](https://github.com/MichaelTMatthews/Craftax/releases/tag/v1.6.0) (§8.1).

### Long-term

- **The API standard and the GPU-native ecosystem converge on the interface, not the runtime.** The Farama standard is reaching toward device-resident arrays from one side while the JAX suite reimplements Gymnasium-shaped contracts on the other; neither side is converging on a shared *implementation*, and no proposal on record merges the two runtimes. The durable artefact of this chapter's first half is therefore the `reset`/`step` contract and the `terminated`/`truncated` distinction, which have now been reimplemented independently in Python, GDScript, Rust, C++ and JAX [Source](https://github.com/Farama-Foundation/Gymnasium/issues/1388) (§4, §8, §12).
- **Unmaintained transitive dependencies are this ecosystem's dominant maintenance risk.** Four separately active projects covered here are each pinned to a dependency their own maintainers describe as unmaintained: bevy_rl to gotham, Gymnasium and PettingZoo to the box2d Python bindings, PettingZoo's classic suite to rlcard, and the JAX environment suite to gymnax. Every response so far has been to vendor, port or replace rather than to revive the upstream, which suggests the pattern is structural rather than incidental — an integration written against any of these stacks should budget for a dependency migration, not just an engine upgrade [Source](https://github.com/Farama-Foundation/PettingZoo/issues/1366) (§4.4, §6.4, §8.1).
