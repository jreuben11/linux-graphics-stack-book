...
OCCT is not the only open-source geometric kernel on Linux, and it is rarely consumed directly by end users — most people who touch B-rep CAD do so through a scripting layer, a browser tab, or an application (like SolveSpace) that solves a problem OCCT itself does not address. This section surveys four adjacent parts of the ecosystem: the code-first paradigm that CadQuery, build123d, and OpenSCAD all belong to; kernels written to compete with or wrap OCCT from Rust; a standalone constraint-based CAD tool that uses no OCCT code at all; and the scripting/web layers that make OCCT itself easier to drive — plus, closing the section, the commercial B-rep kernels OCCT competes with directly, and one prominent open geometry toolkit (Rhino's openNURBS) that looks like a competitor but isn't one.
...
One more name surfaces often enough in this space to need placing, precisely because it isn't a straightforward "competing B-rep kernel" the way Parasolid, ACIS, and C3D are:

- **CGM** (Common Geometry Module, Sandia National Laboratories / Argonne, LGPL) takes a different approach entirely: rather than competing with OCCT, it wraps it. CGM is an abstraction layer providing one topological API over multiple backend solid-modeling engines — historically ACIS, with an OCCT-based backend developed as an open-source-compatible alternative for exactly the license reason discussed above (ACIS-linked CGM code cannot itself be redistributed under an open-source license). CGM supplies the geometry layer for Sandia's CUBIT mesh-generation toolkit, meaning OCCT indirectly underlies part of the meshing pipeline for a substantial share of U.S. national-lab finite-element work, one abstraction layer removed from any CAD GUI. [Source](https://sigma.mcs.anl.gov/cgm/what-is-cgm/)
