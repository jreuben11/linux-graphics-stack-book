...
Two more names surface often enough in this space to need placing, precisely because neither is a straightforward "competing B-rep kernel" the way Parasolid, ACIS, and C3D are:

- **openNURBS** (McNeel, permissive — no copyleft, redistribution and modification both explicitly encouraged) is the geometry library behind Rhino's `.3dm` format, and it is genuinely open in a way none of the three commercial kernels above are. But it is not a competitor to OCCT's modeling role: its own documentation scopes it as a toolkit for *reading and writing* B-rep, NURBS-curve/surface, mesh, and SubD geometry for interchange, not a modeling kernel with Boolean operations, filleting, or a solver — the algorithms that make OCCT (or Parasolid/ACIS) useful for actually constructing a shape live in Rhino itself, which is closed-source, not in openNURBS. It is best understood as occupying the same "geometry data + I/O" layer as OCCT's `ModelingData` module (§2) without the `ModelingAlgorithms` layer beside it. [Source](https://developer.rhino3d.com/guides/opennurbs/what-is-opennurbs/)
