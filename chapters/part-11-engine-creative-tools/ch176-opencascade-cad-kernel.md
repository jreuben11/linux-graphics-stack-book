...
11. [OCCT Alternatives and Higher-Level Abstractions](#11-occt-alternatives-and-higher-level-abstractions)
    ...
    - 11.8 [openNURBS and rhino3dm: An Open Data Model Without a Modeling Kernel](#118-opennurbs-and-rhino3dm-an-open-data-model-without-a-modeling-kernel)
    - 11.9 [CGM: Wrapping OCCT/ACIS, Plus a Native Facet Kernel and a Reversible Virtual-Geometry Layer](#119-cgm-wrapping-occtacis-plus-a-native-facet-kernel-and-a-reversible-virtual-geometry-layer)
    - 11.10 [C3D Toolkit's Native Modules: A Bundled Solver, Mesh-to-B-Rep, and Native Format Exchange](#1110-c3d-toolkits-native-modules-a-bundled-solver-mesh-to-b-rep-and-native-format-exchange)
...
OCCT is not the only open-source geometric kernel on Linux, and it is rarely consumed directly by end users — most people who touch B-rep CAD do so through a scripting layer, a browser tab, or an application (like SolveSpace) that solves a problem OCCT itself does not address. This section surveys four adjacent parts of the ecosystem: the code-first paradigm that CadQuery, build123d, and OpenSCAD all belong to; kernels written to compete with or wrap OCCT from Rust; a standalone constraint-based CAD tool that uses no OCCT code at all; and the scripting/web layers that make OCCT itself easier to drive — plus, closing the section, the commercial B-rep kernels OCCT competes with directly, one prominent open geometry toolkit (Rhino's openNURBS) that looks like a competitor but isn't one, an abstraction layer (CGM) that wraps OCCT itself while also shipping capabilities OCCT doesn't have, and a look at what the youngest of the three commercial kernels (C3D Toolkit) bundles natively that OCCT ships only as separate community or commercial add-ons, if at all.
...
### 11.8 openNURBS and rhino3dm: An Open Data Model Without a Modeling Kernel

One more project sits adjacent to everything surveyed above, and belongs in neither the open-source-kernel list (§11.2) nor the commercial-kernel list (§11.7) because it is not, in OCCT's sense, a modeling kernel at all: **openNURBS** (McNeel, maintained at [github.com/mcneel/opennurbs](https://github.com/mcneel/opennurbs)) is the C++ toolkit behind Rhino's native `.3dm` file format, built to let any application read and write Rhino geometry — NURBS curves and surfaces, B-rep, polygon meshes, SubD, point clouds, and annotation objects — without needing Rhino itself. Its own developer documentation reports it in use by "more than 400 software development teams and applications" for exactly that interchange role. [Source](https://github.com/mcneel/opennurbs) Unlike OCCT's LGPL-2.1-with-exception, openNURBS carries a permissive, essentially zlib-style license: McNeel's own framing is direct — "commercial use is encouraged... there are no restrictions. Neither copyright nor copyleft restrictions apply" — with no requirement to open-source, attribute in a UI, or contribute code back. [Source](https://developer.rhino3d.com/guides/opennurbs/what-is-opennurbs/)

That openness is real, but it stops at data, not algorithms. McNeel's own documentation is explicit and specific about the boundary, listing by name what openNURBS does *not* provide: "closest point calculations, intersection calculations, surface tessellation (meshing), SubD to Brep conversion, interpolation, booleans, area and mass property calculations, [and] other miscellaneous geometry calculations" — all of it reserved for the closed-source Rhino SDKs. [Source](https://developer.rhino3d.com/guides/opennurbs/what-is-opennurbs/) Mapped onto OCCT's own module split (§2), openNURBS covers the equivalent of `ModelingData` — the geometry and topology types themselves — with nothing standing in for `ModelingAlgorithms`: no `BRepAlgoAPI_Fuse`, no `BRepFilletAPI_MakeFillet`, no `BRepMesh_IncrementalMesh` equivalent. A program linking only openNURBS can load a `.3dm` file, walk its B-rep structure, and write it back out, but cannot fillet an edge or Boolean two solids the way §4 of this chapter shows OCCT doing directly.

**rhino3dm** ([Source](https://github.com/mcneel/rhino3dm), MIT) is the scripting-layer counterpart to openNURBS that mirrors the pattern §11.4 and §11.5 describe for OCCT: a "RhinoCommon-style" API wrapping the native C++ library for Python (`rhino3dm.py`, CPython 3.8–3.14), JavaScript/WebAssembly (`rhino3dm.js`, browser and Node), and .NET — the same three-language spread `pythonocc-core`/OCP, `opencascade.js`, and OCCT's C++ core cover for OCCT, built by the same technique of exposing one core geometry library through multiple language bindings rather than maintaining parallel implementations. Because it inherits openNURBS's own scope limit, rhino3dm's documentation describes it as usable "as a client to make calls into the Rhino Compute cloud server" when a script needs an actual modeling operation — Booleans, meshing, offsetting — that the local library cannot perform. [Source](https://github.com/mcneel/rhino3dm) That is a structurally different answer than OCCT's to the same problem: OCCT ships its modeling algorithms in the open-source kernel itself (§4), so `pythonocc-core` or OCP calls stay local; a Boolean operation through rhino3dm, by contrast, is a network call to a Rhino-licensed server unless the calling application also links Rhino or the (closed-source) Rhino SDK directly.

**Where openNURBS and OCCT actually meet.** Despite both covering NURBS-based B-rep geometry, there is no code relationship between the two projects, and OCCT ships no native `.3dm` reader — a Rhino model reaching an OCCT-based pipeline (FreeCAD, CadQuery, Salome) crosses through STEP or IGES (§6) as the shared interchange format, with openNURBS/rhino3dm handling the `.3dm` side of that export and `STEPControl_Reader` (§6.1) handling the OCCT side, rather than any direct `.3dm` ↔ `TopoDS_Shape` conversion existing in either toolkit. openNURBS is, in short, not a competitor to OCCT the way Parasolid or ACIS are — it is a peer at the geometry-*data* layer only, with the modeling-*algorithm* layer that would make it comparable to OCCT deliberately withheld and sold separately as Rhino.

### 11.9 CGM: Wrapping OCCT/ACIS, Plus a Native Facet Kernel and a Reversible Virtual-Geometry Layer

**CGM** (Common Geometry Module, Sandia National Laboratories / Argonne, LGPL) takes yet another approach relative to everything else in this section: rather than competing with OCCT, it wraps it. CGM is an abstraction layer providing one topological query/modification API over multiple interchangeable backend solid-modeling engines — historically ACIS, with an OCCT-based backend developed as an open-source-compatible alternative for exactly the license reason discussed above (ACIS-linked CGM code cannot itself be redistributed under an open-source license). CGM supplies the geometry layer for Sandia's CUBIT mesh-generation toolkit, meaning OCCT indirectly underlies part of the meshing pipeline for a substantial share of U.S. national-lab finite-element work, one abstraction layer removed from any CAD GUI. [Source](https://sigma.mcs.anl.gov/cgm/what-is-cgm/)

But CGM is not purely a pass-through to ACIS or OCCT — it ships a third, native geometry engine for the case those two can't handle at all: a model with no analytic or NURBS representation in the first place, only a discrete mesh. **Mesh-Based Geometry (MBG)** is, in Cubit's own documentation, "not a third party library and has been developed specifically for use with CUBIT" — where ACIS and OCCT both represent surfaces as NURBS, MBG uses "a facetted representation" instead, built directly from imported triangle data (Cubit's own `facet` format, AVS, or STL, ASCII or binary). [Source](https://coreform.com/cubit_help/geometry/model_definitions/mesh_based_geometry.htm) The motivating case is one this chapter's OCCT-centric coverage doesn't otherwise address: data that arrives as a triangulated surface with no CAD source behind it at all — medical imaging segmentation, geotechnical survey data, laser-scanned or otherwise reverse-engineered geometry — where, as the documentation puts it, "only the finite element model is available," and CGM's job is to "build a complete boundary representation solid model from the mesh" rather than from a NURBS-authored one. [Source](https://coreform.com/cubit_help/geometry/model_definitions/mesh_based_geometry.htm) The trade-off is stated candidly rather than glossed over: Booleans, geometry decomposition, and geometry clean-up are explicitly listed as "not yet available with mesh-based geometry" — MBG shares CGM's topology API and supports transformations, merging, and virtual-geometry operations, but not the full modeling-algorithm surface ACIS/OCCT-backed CGM geometry gets. [Source](https://coreform.com/cubit_help/geometry/model_definitions/mesh_based_geometry.htm)

The second CGM capability with no direct OCCT equivalent is **virtual geometry**, and it sits above all three backends rather than being tied to any one of them. Cubit's documentation describes it as a way "to modify the topology of the model without affecting the underlying [ACIS/OCCT/facet] geometry representation and without making changes to the actual solid model" — a topology-only layer, held separately from the base geometry data structure, that can be discarded to restore the original model exactly. [Source](https://cubit.sandia.gov/files/cubit/15.8/help_manual/WebHelp/geometry/virtual_geometry/virtual_geometry.htm) It offers two operations: *compositing*, which merges a set of connected curves or surfaces into a single virtual entity and suppresses the child topology between them, and *partitioning*, which does the reverse — splitting an existing curve or surface to introduce additional topology that wasn't in the original model. Both exist for the same downstream reason: "adjusting the geometry to allow mapping, sub-mapping or sweeping mesh generation schemes to be applied," where a mesh generator needs a specific topological decomposition that the CAD-authored geometry doesn't happen to have, and the alternative — permanently editing the real B-rep to add that decomposition — would be destructive and backend-specific. [Source](https://cubit.sandia.gov/files/cubit/15.8/help_manual/WebHelp/geometry/virtual_geometry/virtual_geometry.htm) Nothing in OCCT itself corresponds to this: `TopoDS_Shape` (§3.2) is the one persistent topological representation OCCT has, edited destructively by whichever `BRepBuilderAPI`/`BRepAlgoAPI` call touches it, with no separate, reversible, meshing-only topology layer sitting above it — a capability gap in the same family as (though distinct from) the direct-modeling and feature-recognition gaps §11.7 already documents against ACIS.

### 11.10 C3D Toolkit's Native Modules: A Bundled Solver, Mesh-to-B-Rep, and Native Format Exchange

§11.7 already places **C3D Toolkit** (C3D Labs, an ASCON subsidiary) among the three commercial B-rep kernels OCCT competes with, and that framing stands — C3D is a genuine, licensed-per-seat competitor to Parasolid and ACIS, not an adjacent tool like openNURBS or a wrapper like CGM. What's worth adding is what C3D ships as standard, first-party modules rather than as bolt-ons: C3D Labs's own product page breaks the toolkit into five named components — **C3D Modeler**, the "high-performance geometric kernel" comparable to OCCT's `ModelingData`/`ModelingAlgorithms`; **C3D Solver**, a "parametric solver that allows you to add dimensional controls and constraints to create relationships between 2D and 3D entities" and "detects existing degrees of freedom and maintains constraints as geometry is modified"; **C3D Converter**, which "imports and exports geometric models and their attributes" in "both neutral and proprietary formats"; **C3D Vision**, a visualization library for "highest quality SW/HW-enabled 3D rendering"; and **C3D B-Shaper**, which simply "converts polygonal meshes to b-rep solids." [Source](https://c3dlabs.com/en/products/c3d-toolkit/) The toolkit's own documentation elaborates the Solver piece further, describing a "Geometric Constraint Solver in Three-Dimensional Space" alongside a companion "Geometric Constraint Solver in Two-Dimensional Space" for planar sketch problems. [Source](https://c3dlabs.com/en/doc/modules.html)

C3D Solver is the module worth dwelling on, because it fills exactly the gap §11.7's Roadmap note and §11.2 both already flag: OCCT itself "has never shipped a first-party parametric constraint solver," which is why FreeCAD bundles its own PlaneGCS-family solver at the application layer, and why §11.3's SolveSpace exists as a standalone constraint-solver-first application (`libslvs`) built with no OCCT code at all. C3D's answer is architecturally different from both: rather than an application vendor bolting a solver on top of a bare kernel, or an independent project built around a solver instead of a kernel, C3D Labs ships the constraint solver as a first-party module of the same commercial toolkit as the B-rep modeler — a licensee gets sketch-level dimensional constraints and a full NURBS B-rep kernel from one vendor, under one license, rather than having to integrate a separate open-source solver (as FreeCAD does) or forgo exact B-rep entirely (as SolveSpace does). This is precisely the packaging gap that keeps OCCT-based applications reaching for third-party solvers in the first place.

C3D B-Shaper is worth contrasting directly against §11.9's CGM Mesh-Based Geometry, because the two take opposite approaches to the same starting problem — a model that arrives as a triangulated mesh with no NURBS surfaces behind it. CGM's MBG keeps that data in a permanent, parallel facet representation, explicitly documented as lacking Booleans and geometry clean-up relative to its NURBS-backed geometry. C3D B-Shaper instead promotes the mesh *into* a true B-rep solid outright — "converts polygonal meshes to b-rep solids" [Source](https://c3dlabs.com/en/products/c3d-toolkit/) — so that once converted, the result is an ordinary C3D Modeler solid with full access to Booleans, fillets, and every other kernel operation, at the cost of whatever surface-fitting approximation the conversion itself introduces. Neither approach is strictly better: CGM's route preserves the mesh faithfully but limits what can be done to it, while C3D's route unlocks the full modeling-algorithm surface but only after an approximating conversion step OCCT itself has no native equivalent to either side of.

C3D Converter is the third capability with no OCCT or openNURBS equivalent: alongside the neutral formats every kernel in this section already reads and writes (STEP, IGES, JT, STL, VRML, plus GRDECL, 3MF, and OBJ), it reads and writes **Parasolid and ACIS's own native, proprietary formats** directly. [Source](https://c3dlabs.com/en/doc/modules.html) OCCT has no first-party Parasolid or ACIS reader at all — interchange with those ecosystems has to go through STEP, with whatever feature-tree and parametric-history loss that entails — and openNURBS (§11.8) is confined to its own `.3dm` format plus the neutral formats it separately supports. A C3D-based application can, in principle, open a native `.x_t` Parasolid file or an ACIS `.sat`/`.sab` file without a STEP round-trip at all, a direct-format-exchange capability none of the open-source kernels surveyed in this section can match.

C3D's own history explains why it, alone among the three commercial kernels, has this kind of cross-cutting module lineup rather than being a bare geometry engine: "In 2012 the earliest version of the C3D Modeller kernel was extracted from KOMPAS-3D CAD. It was later adapted to a range of different platforms and advertised as a separate product," [Source](https://en.wikipedia.org/wiki/C3D_Toolkit) and C3D Labs began licensing it to third parties the following year — meaning the Solver, Converter, and Vision modules grew up alongside a real end-user CAD application (ASCON's KOMPAS-3D) rather than as an SDK built for licensees from the outset, the reverse order from Parasolid's and ACIS's histories. That KOMPAS-3D lineage shows up directly in the toolkit's adopter list: besides KOMPAS-3D itself, C3D Modeler, Solver, and Converter underlie nanoCAD and nanoCAD Mechanica, Altium Designer's mechanical/PCB-enclosure tooling, the Quickfield finite-element package, and Renga Architecture/Structure's BIM authoring. C3D Toolkit's own platform matrix also stands out among the three commercial kernels for explicit Linux support: Windows, macOS, Android, iOS, **Linux**, and FreeBSD, with a C/C++ core plus C# and TypeScript APIs. [Source](https://c3dlabs.com/en/products/c3d-toolkit/)
