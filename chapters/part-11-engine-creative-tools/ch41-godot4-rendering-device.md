...
- [1. Godot 4 Rendering Architecture Overview](#1-godot-4-rendering-architecture-overview)
  - [1.1 What is RenderingDevice?](#11-what-is-renderingdevice)
  - [1.2 What is RenderingServer?](#12-what-is-renderingserver)
  - [1.3 What are the Forward+ and Mobile Renderers?](#13-what-are-the-forward-and-mobile-renderers)
...
### 1.1 What is RenderingDevice?

`RenderingDevice` (abbreviated `RD` throughout Godot's source) is Godot's internal GPU command abstraction layer, sitting immediately above the graphics driver backend and below the scene rendering logic. It provides a unified API for creating and destroying GPU resources, recording draw commands, dispatching compute work, and managing synchronization barriers. Every GPU resource in the RD path — buffers, textures, shaders, render pipelines, compute pipelines, framebuffers, and uniform sets — is referenced through an opaque `RID` handle. The `RID` type is an integer backed by a free-list allocator inside `RenderingDevice`; the concrete Vulkan objects reside in `RenderingDeviceDriverVulkan`, keyed by `RID` in internal hash maps.

`RenderingDevice` is not a cross-engine stable plugin API in the way that wgpu, bgfx, or Diligent Engine are. Its primary purpose is to give Godot's renderer code a Vulkan-structured but driver-portable interface: the same `RD` call sequence works whether the backend is `RenderingDeviceDriverVulkan`, the experimental `RenderingDeviceDriverD3D12`, or the `RenderingDeviceDriverMetal` backend used on Apple platforms. The abstract `RenderingDeviceDriver` interface isolates the rendering passes and resource management logic from any one graphics API. As of Godot 4.4, `RenderingDevice` is also accessible from GDExtension — Godot's C ABI plugin system — enabling custom render passes written in C++ or Rust without forking the engine. The Compatibility renderer (OpenGL ES 3.0) does not use `RenderingDevice` at all; it calls OpenGL directly through an EGL context managed by a parallel code path. [Source](https://docs.godotengine.org/en/4.4/classes/class_renderingdevice.html)

### 1.2 What is RenderingServer?

`RenderingServer` (formerly `VisualServer` in Godot 3) is the top-level, thread-safe API through which game code manages every rendered entity: meshes, lights, viewports, materials, particles, skeletons, and environment parameters. It defines an abstract interface; the concrete implementation, `RenderingServerDefault`, owns a rendering thread and dispatches commands to the compositor below it, or runs inline on the calling thread depending on the threading mode selected at startup.

`RenderingServer` is the boundary between game-world logic and the GPU rendering subsystem. Above it, `SceneTree` and GDScript code manipulate scene nodes and resource handles without any awareness of Vulkan draw calls or OpenGL state. Below it, `RendererCompositorRD` selects the active renderer path — Forward+, Mobile, or Compatibility — based on the project's rendering configuration and the device's reported capabilities at startup. The thread-safety guarantee means game code can queue commands to `RenderingServer` from any thread; all actual GPU work is serialized onto the render thread, which processes commands, calls into the compositor, and triggers frame submission. `RenderingServerDefault::_init()` registers this command loop, isolating game logic from GPU synchronization concerns that are handled entirely within and below `RenderingServer`. [Source](https://github.com/godotengine/godot/blob/4.4/servers/rendering/rendering_server_default.cpp)

### 1.3 What are the Forward+ and Mobile Renderers?

The Forward+ and Mobile renderers are Godot 4's two `RenderingDevice`-based scene renderer implementations, each targeting a different hardware capability tier. Both are subclasses of `RendererSceneRenderRD`, which provides shared infrastructure for scene management, environment, sky, and global illumination bookkeeping, and both record their GPU work exclusively through the `RenderingDevice` API.

The Forward+ renderer (`RenderForwardClustered`) targets discrete desktop GPUs with generous texture binding counts. It uses a clustered forward lighting model: a compute shader divides the view frustum into a three-dimensional grid of cells and assigns lights and decals to cells based on bounding-sphere tests. Fragment shaders look up only the lights relevant to their screen-space cell, avoiding per-fragment iteration over all lights. The pass sequence includes a depth pre-pass, optional GBuffer passes for screen-space ambient occlusion (SSAO), screen-space indirect lighting (SSIL), and screen-space reflections (SSR), shadow map rendering, and post-processing. Forward+ also drives SDFGI through a cascade of compute dispatches for voxelization, jump-flood SDF generation, and probe updates.

The Mobile renderer (`RenderForwardMobile`) uses a single-pass forward shading model without a clustered grid. It targets tile-based deferred rendering (TBDR) GPU architectures common on mobile and lower-end integrated GPUs, using `R10G10B10A2_UNORM` render targets instead of `RGBA16F` to reduce bandwidth and structuring Vulkan render passes to maximize use of on-chip tile memory. The compositor selects `RenderForwardMobile` when the rendering method is set to `"mobile"` or when the device reports fewer than 48 textures per shader stage via `RD::LIMIT_MAX_TEXTURES_PER_SHADER_STAGE`. [Source](https://github.com/godotengine/godot/blob/4.4/servers/rendering/renderer_rd/renderer_compositor_rd.cpp)
