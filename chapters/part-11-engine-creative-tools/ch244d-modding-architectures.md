# Chapter 244d: Modding Architectures — Scripting, Sandboxing, and Hot-Reload on Linux
...
- [8. LSL and OpenSimulator: A Server-Side Scripting Architecture](#8-lsl-and-opensimulator-a-server-side-scripting-architecture)
  - [8.1 Two Different Servers Behind One Protocol](#81-two-different-servers-behind-one-protocol)
  - [8.2 From LSO Bytecode to an Embedded Mono VM](#82-from-lso-bytecode-to-an-embedded-mono-vm)
  - [8.3 The Event/State Model as Host-API Surface](#83-the-eventstate-model-as-host-api-surface)
  - [8.4 OSSL: A Widened, Operator-Gated Surface](#84-ossl-a-widened-operator-gated-surface)
  - [8.5 Multi-Tenancy: What This Case Study Adds to the Trust Axis](#85-multi-tenancy-what-this-case-study-adds-to-the-trust-axis)
  - [8.6 Overte: A Smaller, Less-Verified Data Point](#86-overte-a-smaller-less-verified-data-point)
- [9. Comparison](#9-comparison)
- [10. Why There Is No Standard](#10-why-there-is-no-standard)
...
Each architecture in this chapter picks a different point in that cube, and the comparison table in §9 is organised along these axes rather than by language.
...
The system is explicitly pre-stable. The project warns that "the plugin API and the process for plugin development is under active development and we currently make no stability guarantees about APIs, tooling, etc." [Source](https://book.veloren.net/contributors/modders/writing-a-plugin.html). The relevant ABI observation for §9 is that shared linear memory plus typed event handlers is a much easier interface to keep stable than a Rust vtable layout, because the boundary is bytes and integers rather than compiler-dependent struct layout — but "easier" is not "stable," and Veloren has not committed yet.
...
A native `dlopen`ed mod shares the address space, so engine internals are readable and writable regardless of visibility. It shares the file-descriptor table, inheriting every open device node, socket, and pipe — including DRM device fds, which is the concrete GPU-reachability answer for its row in §9. It can issue syscalls directly, so a host function table is a convention rather than a boundary. It can execute code at load time through ELF constructors, before the engine's entry point runs. It cannot be resource-limited or preempted cleanly. And it is coupled to the engine's compiled layout, so it breaks across versions.
...
## 8. LSL and OpenSimulator: A Server-Side Scripting Architecture

§1.1 frames every mod architecture in this chapter as three independent decisions — trust, ABI stability, hot-reload — and §§2–7 answer them for engines where a mod loads into a single running process: a player's own game client, a mod author's DLL patched into one process, a library one developer links into their own engine build. Second Life's Linden Scripting Language (LSL) and its open-source-server reimplementation, OpenSimulator, are worth a case study because they answer the same three questions for a structurally different kind of "mod": a script attached to one in-world object, executing **server-side**, with a single simulator process routinely hosting thousands of such scripts at once, authored independently by residents who trust neither the region operator nor each other. The trust, ABI, and hot-reload questions do not change shape under that load — a script still either has ambient host authority or does not, is still text recompiled rather than a versioned binary, still updates by whatever mechanism the runtime provides — but answering "trust" now has to hold under many mutually distrusting guests sharing one process's CPU budget at once, which is a constraint none of this chapter's other six case studies face and which §8.5 returns to directly.

### 8.1 Two Different Servers Behind One Protocol

"Second Life" and "OpenSimulator" (OpenSim) are two different server implementations of a shared virtual-world protocol, and they have very different degrees of inspectability. The Second Life grid's simulator is operated by Linden Lab and is **not open source**; only the viewer client is — the reference Second Life Viewer is published under LGPL-2.1 [Source](https://github.com/secondlife/viewer), and the community-maintained Firestorm viewer is protocol-compatible with both grids. OpenSimulator, by contrast, is a from-scratch, BSD-3-Clause-licensed C# server that implements the same viewer-facing protocol so that Second Life viewers can connect to it, but every line of its script engine is public and buildable [Source](https://github.com/opensim/opensim/blob/master/LICENSE.txt). That split matters for what follows: claims about Second Life's own simulator internals rest on Linden Lab's published wiki documentation, not on inspectable code, while claims about OpenSimulator's engine internals are sourced directly from the project's own configuration and design documentation for its two script engines, XEngine and YEngine.

### 8.2 From LSO Bytecode to an Embedded Mono VM

Second Life originally compiled LSL to a custom bytecode format, LSO, executed by a per-simulator bytecode interpreter; a later compilation path targets an embedded **Mono** runtime instead, compiling LSL to CIL (Common Intermediate Language) bytecode [Source](https://wiki.secondlife.com/wiki/Mono). Linden Lab's own comparison states Mono is "up to 220 times faster than LSL2" for math-intensive scripts, while "for ordinary scripts, the performance gains are much more humble" — and documents concrete behavioral differences between the two targets: LSO scripts get a fixed 16 KB memory allocation regardless of actual use, while Mono scripts allocate dynamically; the two differ in supported Unicode range (RFC 2279 vs. RFC 3629); and integer division by zero throws an error under LSO but produces `Infinity` under Mono [Source](https://wiki.secondlife.com/wiki/Mono). Compilation happens entirely server-side, "distributed... on the sim hosts" — a script's source and bytecode never execute on the viewer, which only ever receives the effects of a running script through the normal object-update protocol [Source](https://wiki.secondlife.com/wiki/Mono). Second Life's own VM internals stop there for this chapter's purposes: with the simulator closed-source, "how the embedded Mono host isolates one script's CIL from another's" beyond that page's description is not independently verifiable and is not claimed here.

OpenSimulator's compilation path is the inspectable version of the same idea, and it is genuinely different in mechanism, not just in license. The default engine, **XEngine**, compiles LSL source to C#, hands that to the .NET compiler to produce an assembly, and — by default (`AppDomainLoading = true`) — loads each compiled script into its own **AppDomain**, the CLR's in-process isolation boundary, so that one script's static state and a crashing unhandled exception cannot corrupt another script's [Source](https://opensimulator.org/wiki/XEngine). XEngine's `OpenSim.ini` block documents an `AllowedCompilers` setting (`lsl,vb,js,cs` in the historical multi-language configuration) — LSL is not architecturally the only language XEngine's compiler pipeline can target, though LSL is what OpenSimulator regions run in practice [Source](https://opensimulator.org/wiki/XEngine). The newer **YEngine** skips the C# intermediate stage entirely and compiles LSL directly to CIL, which the wiki documents as both faster to compile and structurally simpler, at the cost of dropping multi-language support — YEngine "only supports the LSL language," a deliberate narrowing rather than an oversight [Source](https://opensimulator.org/wiki/YEngine). On the ABI-stability axis (§1.1) this compilation path lands where §2's embedded-Lua architectures do rather than where §6's IL patching or §7's native extension do: a script is source text recompiled by the server on every load, not a binary an author versions against a fixed engine ABI, so there is no cross-version compatibility question of the kind that breaks a GDExtension library across a Godot major version (§7.1) or a Harmony patch across a game update (§6).

### 8.3 The Event/State Model as Host-API Surface

A mod's host-API surface, in this chapter's terms, is whatever vocabulary the runtime lets a guest act through — Luanti's registration functions and callbacks (§3.2, §3.5), Extism's host-defined imports (§4.1). For LSL that vocabulary is a fixed set of named events. A script declares one or more `state`s, each holding handlers for a fixed vocabulary of events — `state_entry`, `state_exit`, `touch_start`/`touch`/`touch_end`, `collision_start`/`collision`/`collision_end`, `on_rez`, `attach`, `changed`, `listen`, `timer`, `http_response`, `money`, and others — and the runtime queues incoming events and dispatches them into whichever handler the script's current state defines [Source](https://wiki.secondlife.com/wiki/Category:LSL_Events). A script never calls into the simulator directly outside of its declared LSL/OSSL function vocabulary (§8.4); everything it does happens inside a handler the runtime chose to invoke, the same "host controls the entire surface by construction" property §1.1 attributes to a host function table. Entering a new `state` clears that script's pending event queue and drops any open `listen()` handles automatically, which is itself a scheduling primitive: a script can shed queued work simply by changing state [Source](https://wiki.secondlife.com/wiki/Category:LSL_Events).

That same dispatch queue is where OpenSimulator's per-guest quota enforcement lives — the mechanism §8.5 needs once many scripts share it. XEngine bounds each script instance's pending-event queue at `MaxScriptEventQueue` (300 by default) and silently drops events beyond that depth rather than growing unbounded [Source](https://opensimulator.org/wiki/XEngine). Queued events are not each given a dedicated OS thread; they become work items on a shared `SmartThreadPool` sized by `MaxThreads` (100 by default) — meaning script concurrency is bounded independent of how many scripts a region hosts, which is the actual answer to "what stops a few thousand scripts from each demanding a thread" [Source](https://opensimulator.org/wiki/XEngine). Per-event CPU time is bounded by `EventLimit` (30 seconds by default), checked by a periodic `MaintenanceInterval` sweep (10 seconds by default) that can intervene on a script that has overrun its budget; whether that intervention terminates the script or merely flags it is controlled by `KillTimedOutScripts`, and how a stop is actually enforced is controlled by `ScriptStopStrategy`, which the wiki documents as taking two values: `abort` — "the default as of OpenSimulator 0.8" — which uses `Thread.Abort()` and can leave the process in an unstable state under repeated use, and `co-op`, which relies on yield-check instructions the compiler inserts directly into the generated script code, a cooperative mechanism the script itself cannot opt out of [Source](https://opensimulator.org/wiki/XEngine). YEngine takes a different approach to the same fairness problem: rather than AppDomain isolation plus a CPU-time ceiling checked after the fact, it runs scripts under **preemptive** time-slicing with explicit millisecond budgets — roughly 60 ms for a freshly dispatched event and 30 ms on each subsequent resumption of that event — which the documentation frames as favoring many short, fast event handlers over allowing any single one to dominate a scheduling turn [Source](https://opensimulator.org/wiki/YEngine). YEngine also reclaims per-script memory without needing a full AppDomain per script, trading away some of XEngine's isolation boundary — cross-AppDomain calls in XEngine must serialize and deserialize every parameter and return value, which is measurably expensive — for lower per-script overhead [Source](https://opensimulator.org/wiki/YEngine).

### 8.4 OSSL: A Widened, Operator-Gated Surface

Beyond stock LSL, OpenSimulator regions can expose **OSSL** — a set of `os`-prefixed functions (`osTeleportAgent`, `osSetDynamicTextureURL`, `osParseJSON`, `osConsoleCommand`, and many more) implemented in an `OSSL_Api` class. Every OSSL function's implementation calls `CheckThreatLevel(ThreatLevel.X, "functionName")` before doing anything else, gating that specific function against a configured threat level [Source](https://opensimulator.org/wiki/OSSL). The `ThreatLevel` enum runs `None → VeryLow → Low → Moderate → High → VeryHigh → Severe`, with each level's documented meaning ranging from "no threat, can be used by anyone" up to functions capable of console or OS-level command execution [Source](https://opensimulator.org/wiki/OSSL). A region operator sets a global ceiling with `OSFunctionThreatLevel` in `OpenSim.ini` (default `VeryLow`) and, independent of that ceiling, can allow or deny individual functions outright, or restrict a specific function to a comma-separated allow-list of avatar UUIDs, via `Allow_osFunctionName = true|false|<uuid,uuid,...>` [Source](https://opensimulator.org/wiki/OSSL). OSSL access is opt-in at the region level — `AllowOSFunctions` defaults to `false` — so a region operator who never touches that setting gets stock LSL only, with none of the widened surface reachable regardless of per-function settings [Source](https://opensimulator.org/wiki/XEngine). This is a two-axis capability gate — a global ceiling plus a per-function, per-user allow-list — structurally similar to, but more granular than, Luanti's `secure.trusted_mods`/`secure.http_mods` gate from §3.6, which grants a mod's entire code an all-or-nothing widened API rather than clearing individual functions per caller.

### 8.5 Multi-Tenancy: What This Case Study Adds to the Trust Axis

§1.1 defines trust as binary: a mod either has the process's ambient authority or only the authority the host explicitly grants, and that answer does not change once granted. By that definition LSL/OSSL's trust story is unremarkable and sits with this chapter's other sandboxed architectures — managed IL execution inside an AppDomain (§8.2) plus OSSL's per-function gate (§8.4) mean a script has no ambient route to a file descriptor, a syscall, or a GPU handle, the same category of guarantee Luanti's Lua sandbox (§3) or a WASM guest (§4) provides. What OpenSimulator's deployment shape adds is a second trust question the chapter's other six case studies never have to answer, because none of them host thousands of mutually distrusting guests inside one process at once: a Luanti mod is untrusted relative to its operator but shares a world only with mods that operator also chose to install; a Harmony patch runs inside a game process one player owns; a GDExtension library is loaded by the one developer who built it into their own project. In a shared region, a script that cannot escape its sandbox can still be made effectively untrustworthy by simply monopolizing shared CPU — a guest can be memory-safe and still deny service to every other tenant's script and every connected avatar's frame rate, a failure mode that "can this guest escape" does not capture and that none of this chapter's other trust answers had to guard against.

This is why OpenSimulator's engines pair sandboxing with **scheduling and quota primitives** that this chapter's other architectures do not need: a bounded per-script event queue that drops rather than grows, a shared thread pool sized independent of script count, a wall-clock ceiling per event with a maintenance sweep to enforce it, and a stop strategy the script cannot itself defeat (§8.3). The closest analogue elsewhere in this chapter is WASM's fuel/epoch metering (§5.4), which also bounds a guest's CPU consumption per call — but fuel and epochs are designed against one runaway guest inside one host call, not against scheduling thousands of concurrently resident, independently-timed event queues fairly across a shared thread pool while a real-time simulation loop keeps running underneath them. OSSL's per-function, per-user threat-level gate (§8.4) is the same trust axis applied at finer grain than this chapter's other architectures need: which script, specifically, is allowed to reach outside its own object and touch shared region or world state at all, resolved at the individual-function, individual-caller level rather than the whole-mod granularity §3.6's Luanti gate or §4's WASI capability grants operate at.

### 8.6 Overte: A Smaller, Less-Verified Data Point

Overte is a separate, Apache-2.0-licensed open-source social-VR client-and-server platform, forked from Vircadia, itself a fork of the codebase High Fidelity open-sourced [Source](https://github.com/overte-org/overte). Its scripting model runs on an embedded **V8** engine, using JavaScript as the scripting language — WebAssembly is noted as usable underneath it, but the documentation is explicit that reaching Overte's own API surface requires JavaScript, and that the scripting environment is not a browser sandbox: it has no cookies, no `localStorage`, and no DOM framework [Source](https://docs.overte.org/en/latest/script/get-started-with-scripting.html). Overte documents five script types by where and when they run: an Interface Script (client-only, alive only while the application runs), an Avatar Script (per-avatar effects), a Client Entity Script (per-user, local, triggered by proximity or interaction), and two persistent, server-side types — an **Assignment Client Script**, which keeps running in the domain server independent of whether any user is connected, and a **Server Entity Script**, whose effects are visible to everyone in the domain — the closest structural analogue in Overte to LSL/OSSL's always-resident, server-side, multi-tenant scripts [Source](https://docs.overte.org/en/latest/script/get-started-with-scripting.html). The permission model documented for this is coarser than OpenSimulator's: a domain owner grants or denies script create-and-edit permission at the domain level, and an unauthorized attempt to edit an entity's script is rejected while the entity's existing script keeps running unaffected [Source](https://docs.overte.org/en/latest/script/get-started-with-scripting.html).

**Needs verification:** no per-script CPU-time or memory quota, no scheduling primitive analogous to XEngine's `EventLimit`/`MaxScriptEventQueue` or YEngine's time-sliced preemption, and no V8-isolate-level resource-isolation mechanism for Overte's persistent Assignment Client or Server Entity scripts were found in the documentation consulted for this section. Given that these are exactly the primitives that make LSL/OSSL's adversarial-multi-tenancy story legible, their absence from what is publicly documented is treated here as an open question rather than either a confirmed gap or a confirmed capability, and no rendering-layer claims about Overte's V8 embedding are made — that is outside this section's scope regardless.
...
| **LSL / OpenSimulator (XEngine, YEngine)** | Managed-code isolation, server-side, adversarial-multi-tenant: XEngine loads each script into its own AppDomain by default; OSSL adds a per-function, per-caller-UUID threat-level gate on top. No memory-unsafe escape route from LSL/OSSL itself. | Fixed LSL event/function vocabulary plus opt-in OSSL extension functions, each individually gated by `ThreatLevel` and operator allow/deny lists. No ambient OS or filesystem access. | None. LSL/OSSL affect in-world object state and avatar interaction; no path to the simulator's own renderer or a client GPU. Not live per-event-handler patching — a script instance is recompiled as a whole unit and its per-instance runtime state (AppDomain, event queue) is rebuilt with it, not hot-swapped mid-execution. |
...
**Chapter 41 — Godot 4: RenderingDevice and the Explicit Vulkan Path** documents the renderer that GDExtension exposes without restriction. Because GDExtension registers classes into ClassDB such that they are "indistinguishable from core classes," a native extension reaches `RenderingDevice` and Ch41's explicit Vulkan path directly — making it the concrete instance of this chapter's highest-GPU-reachability, zero-sandbox row (§7.1, §9), and the sharpest available contrast with a WASM guest that cannot name a GPU handle at all.

**Chapter 244a — Programmable Games and Competitive-Code Sandboxes** covers Screeps, whose V8-isolate sandbox solves this chapter's exact problem — executing untrusted third-party code inside a running game — a generation before WASM was available. The V8 isolate is the same architectural idea as §5.1's memory instance: a separate heap with a host-controlled global object, where the guest reaches only what was injected. Comparing the two shows which of WASM's properties are genuinely novel (language-agnostic compilation targets, capability-gated WASI filesystem access, fuel-based preemption) and which were already achieved by a scripting-engine isolate.

**Chapter 244c — Open-Source 2D Simulation-Game Engines** surveys engines embedding Squirrel, JavaScript/TypeScript, and Lua for scripting, which are the alternative points in §2's design space evaluated under a much narrower rendering model. Because a 2D sprite-blitting engine's visual surface is fundamentally declarative — sprites, atlases, batching parameters — the GPU-reachability question that dominates §9 largely dissolves there, which is a useful control case for how much of a mod system's security posture comes from the sandbox versus from the renderer simply having no dangerous API to expose.

**Chapter 244e — Simulation-Game Frameworks, §5, Thousand Parsec** describes the one other architecture in this book where the ruleset/mod itself is server-authoritative and the network protocol, not a local ABI, is the actual integration surface — Thousand Parsec's `tpserver-cpp` `dlopen`s a single compiled ruleset module and speaks to every client, admin tool, and AI player only over the wire. LSL/OpenSim (§8) push the same server-authoritative idea further along two axes Thousand Parsec never had to face: many independently-authored guest programs (scripts, not one ruleset module) run concurrently inside one server process, owned by mutually distrusting parties rather than one deployment's operator, which is why §8.3's per-script scheduling quotas and §8.4's per-function threat-level gate have no counterpart in Thousand Parsec's single-ruleset, single-operator model.
...
- [GitHub — secondlife/viewer](https://github.com/secondlife/viewer) — Reference Second Life Viewer, LGPL-2.1 (§8.1)
- [GitHub — opensim/opensim, LICENSE.txt](https://github.com/opensim/opensim/blob/master/LICENSE.txt) — OpenSimulator server, BSD-3-Clause (§8.1)
- [Second Life Wiki — Mono](https://wiki.secondlife.com/wiki/Mono) — LSO vs. Mono compilation targets, "up to 220 times faster" claim, fixed vs. dynamic memory allocation, Unicode range and divide-by-zero differences, server-side compilation (§8.2)
- [OpenSimulator Wiki — XEngine](https://opensimulator.org/wiki/XEngine) — Default script engine: LSL→C#→IL compilation, per-script AppDomain isolation (`AppDomainLoading`), `AllowedCompilers`, `SmartThreadPool`/`MaxThreads`, `MaxScriptEventQueue`, `EventLimit`, `MaintenanceInterval`, `KillTimedOutScripts`, `ScriptStopStrategy` (`abort`/`co-op`), `AllowOSFunctions`/`OSFunctionThreatLevel` defaults (§8.2, §8.3, §8.4)
- [OpenSimulator Wiki — YEngine](https://opensimulator.org/wiki/YEngine) — Direct LSL→IL compilation, LSL-only language support, preemptive time-sliced scheduling (~60 ms initial / ~30 ms subsequent event budgets), per-script memory reclamation without AppDomains (§8.2, §8.3)
- [OpenSimulator Wiki — OSSL](https://opensimulator.org/wiki/OSSL) — `OSSL_Api`/`CheckThreatLevel` pattern, `ThreatLevel` enum (`None`–`Severe`) definitions, `Allow_osFunctionName` per-function/per-UUID gating (§8.4)
- [Second Life Wiki — Category:LSL Events](https://wiki.secondlife.com/wiki/Category:LSL_Events) — LSL event/state vocabulary, state-change queue-clearing and `listen()`-drop behavior (§8.3)
- [GitHub — overte-org/overte](https://github.com/overte-org/overte) — Apache-2.0 social-VR platform, forked from Vircadia/High Fidelity (§8.6)
- [Overte Docs — Get Started With Scripting](https://docs.overte.org/en/latest/script/get-started-with-scripting.html) — V8/JavaScript scripting engine, non-browser environment, five script types including Assignment Client Script and Server Entity Script, domain-level create/edit permission model (§8.6)
- [lib.rs — bevy_dexterous_developer](https://lib.rs/crates/bevy_dexterous_developer) — Minimal-maintenance status (§7.2, §7.3)
- [Steamworks docs — Steam Workshop](https://partner.steamgames.com/doc/features/workshop) — ISteamUGC upload tooling, Workshop-handles-payment/hosting vs. developer-defined file formats (§10)
- [mod.io Documentation — Welcome](https://docs.mod.io/) — Cross-engine UGC hosting/identity framing (§10)
- [mod.io Documentation — Unreal Engine Plugin](https://docs.mod.io/unreal) — Plugin as a wrapper around the mod.io C++ SDK; auth, browsing, subscription management layered over Unreal's own cooking/`.pak` pipeline (§10)
- [fopdoc — Fallout 3/New Vegas plugin format documentation](https://tes5edit.github.io/fopdoc/) — Community-authored spec filling the gap left by no official Bethesda documentation (§10)
- [xEdit GitHub Page](https://tes5edit.github.io/) — Module editor and conflict detector spanning Oblivion through Fallout 4 (§10)
- [GitHub — UE4SS-RE/RE-UE4SS](https://github.com/UE4SS-RE/RE-UE4SS) — Injectable Lua/C++ modding system for UE4/5, MIT; "works for most games" rather than a vendor-sanctioned API (§10)
