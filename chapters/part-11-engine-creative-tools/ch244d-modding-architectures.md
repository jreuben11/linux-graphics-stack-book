...
- [8. LSL and OpenSimulator: Scripting at Persistent-World, Multi-Tenant Scale](#8-lsl-and-opensimulator-scripting-at-persistent-world-multi-tenant-scale)
  ...
  - [8.3 The Event/State Model and Per-Script Metering](#83-the-eventstate-model-and-per-script-metering)
  - [8.4 OSSL: A Widened, Operator-Gated Surface](#84-ossl-a-widened-operator-gated-surface)
  - [8.5 What Adversarial Multi-Tenancy Adds to the Trust Model](#85-what-adversarial-multi-tenancy-adds-to-the-trust-model)
...
## 8. LSL and OpenSimulator: Scripting at Persistent-World, Multi-Tenant Scale

Every scripting architecture in §§2–7 shares one property regardless of sandboxing technique: the guest code runs for a single tenant at a time — a player's own game client, a single mod author's DLL patched into one running process, a library one developer links into their own engine build. Second Life's Linden Scripting Language (LSL) and its open-source-server reimplementation, OpenSimulator, break that assumption. LSL scripts execute **server-side**, one script per in-world object, with potentially thousands of scripts owned by mutually distrusting residents running concurrently inside a single simulator process that also has to keep a real-time frame budget for every connected avatar. The sandboxing question stops being only "can this guest escape to the host OS" and becomes "can this guest, running next to a few thousand others it has never met, be starved, or starve everyone else."
...
### 8.5 What Adversarial Multi-Tenancy Adds to the Trust Model

Every other case study in this chapter answers "can the guest escape its sandbox" against a single-tenant threat model: a Luanti mod is untrusted relative to its operator but shares a world with mods that operator also chose to install; a Harmony patch runs inside a game process one player owns; a GDExtension library is loaded by the one developer who built it into their own project. OpenSimulator's script engines answer a structurally different question, because the tenants are mutually distrusting and the failure mode that matters most is not memory-safety escape but **fairness under shared scheduling** — whether script A, misbehaving or merely poorly written, can degrade the simulator for every other resident's script and every connected avatar's frame rate in the same region. That reframing is visible directly in where OpenSimulator's mitigations concentrate: not primarily on memory-safety isolation (though AppDomain separation provides some of that as a side effect of XEngine's design) but on **scheduling and quota primitives** — a bounded per-script event queue that drops rather than grows, a shared thread pool sized independent of script count, a wall-clock ceiling per event with a maintenance sweep to enforce it, and a stop strategy the script cannot itself defeat. The closest analogue elsewhere in this chapter is WASM's fuel/epoch metering (§5.4), which also bounds a guest's CPU consumption per call — but fuel and epochs are designed against one runaway guest inside one host call, not against the combinatorics of scheduling thousands of concurrently resident, independently-timed event queues fairly across a shared thread pool while a real-time simulation loop keeps running underneath them. OSSL's per-function, per-user threat-level gate answers the second half of the same problem: which script, specifically, is allowed to reach outside its own object and touch shared region or world state at all — a capability question this chapter's other four architectures mostly resolve at the mod or plugin granularity rather than the individual-function, individual-caller granularity OSSL supports.
