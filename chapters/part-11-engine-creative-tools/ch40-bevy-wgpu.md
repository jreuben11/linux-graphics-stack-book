...
Section 1 covers **Bevy**'s rendering architecture: the **Entity-Component-System** (**ECS**) pattern, in which entities are numeric identifiers, components are plain data, and systems are query-and-transform functions operating on a **World**. **Bevy** separates game logic from GPU work using a dual-world model — a Main World that holds simulation state and a Render World dedicated to GPU command recording — connected by the **ExtractSchedule** extract phase. As of **Bevy** 0.15, the render world uses a retained entity model tracked via **MainEntity** and **RenderEntity** components, eliminating archetype-rebuild overhead. The **Render** schedule advances through ordered system sets — **PrepareAssets**, **Queue**, **PhaseSort**, **Prepare**, **Render**, and **Cleanup** — and GPU work is organised by a **RenderGraph**, a directed acyclic graph whose nodes (**MainOpaquePass3dNode**, **DepthPrepassNode**, **ShadowPassNode**, **TonemappingNode**, **UpscalingNode**, and others) communicate via **SlotInfo** texture handles. The **RenderQueue** submits all recorded commands through a single **vkQueueSubmit** call per frame.

Section 2 examines **wgpu**: the cross-platform **WebGPU**-subset GPU API that provides **Bevy** with a safe Rust interface over **Vulkan**, **Metal**, **D3D12**, and **OpenGL ES** (via **ANGLE**). The **wgpu** monorepo's crates (**wgpu**, **wgpu-core**, **wgpu-hal**, **wgpu-types**, and **naga**) are described, along with the **Vulkan** backend internals in **wgpu-hal/src/vulkan/**: adapter enumeration via **vkGetPhysicalDeviceProperties2**, device creation requiring **VK_KHR_swapchain**, **VK_KHR_dynamic_rendering**, and **VK_KHR_timeline_semaphore**, and the **ash** Rust bindings generated from the Khronos **vk.xml** specification. The **wgpu** resource model — **wgpu::Buffer**, **wgpu::Texture**, **wgpu::BindGroup**, **wgpu::RenderPipeline**, **wgpu::CommandEncoder** — and queue submission via **vkQueueSubmit** with timeline semaphore signaling are also covered.

Section 3 traces the shader pipeline from **WGSL** (WebGPU Shading Language) through **naga**'s IR to **SPIR-V** and into **Mesa NIR** via **vk_spirv_to_nir()**. **naga::front::wgsl::parse_str()** parses **WGSL** source into a **naga::Module**; **naga::valid::Validator** checks **WebGPU** compliance; and **naga::back::spv::Writer** emits a minimal **SPIR-V** binary. The **SPIR-V** then enters **Mesa**'s standard compilation pipeline: **RADV** compiles via **ACO**, **ANV** via the Intel backend compiler, **NVK** via the **NVK** shader backend, and **Turnip** via **ir3** for Qualcomm **Adreno** hardware. The section compares **naga** with **Tint** and explains how both are opaque **SPIR-V** producers from **Mesa**'s perspective.

Section 4 covers **Linux** window integration: **winit** delegates all surface creation on Linux to either **Wayland** (via the **wayland-client** library, **wl_compositor**, **wl_surface**, **xdg_wm_base**, and **VK_KHR_wayland_surface** through **vkCreateWaylandSurfaceKHR**) or **X11**. The **EGL** fallback path via **EGL_KHR_platform_wayland** and **wl_egl_window** is also described. Explicit synchronisation via **wp_linux_drm_syncobj_v1** and **VK_EXT_external_semaphore_fd** is tracked in **wgpu** issue #8996, with **vulkan::Queue::add_signal_semaphore** landed in wgpu v25 and **add_wait_semaphore** queued for v30. Swapchain management via **VK_KHR_swapchain** and the **wgpu::SurfaceConfiguration** API, including **Wayland**'s **currentExtent** behaviour and present mode mappings, is also discussed.

Section 5 covers buffer and memory management: the **wgpu::Buffer** / **VkBuffer** mapping, staging buffer upload patterns using **wgpu::BufferUsages**, asynchronous CPU mapping via **buffer.map_async()**, and the **gpu-allocator** crate — a pure-Rust sub-allocator that manages large **VkDeviceMemory** blocks using **MemoryLocation** flags (**GpuOnly**, **CpuToGpu**, **GpuToCpu**) with attention to **bufferImageGranularity** alignment. **DMA-BUF** interop via **VK_KHR_external_memory_fd** and **vulkan::Device::texture_from_dmabuf_fd()** is also addressed, enabling import of buffers from sources such as **VA-API** video decoders. The **bevy_asset** staging pipeline and GPU mipmap generation via a **WGSL** compute shader are covered as well.

Section 6 covers compute in **Bevy**: authoring **WGSL** compute shaders with `@compute` and `@workgroup_size`, creating **wgpu** compute pipelines via **wgpu::ComputePipelineDescriptor** and **vkCreateComputePipelines**, dispatching via **dispatch_workgroups()** / **vkCmdDispatch**, and integrating compute work into the **RenderGraph** through a **ComputeNode** that implements the **Node** trait. The **PipelineCache** deferred compilation model prevents frame hitches; use cases include GPU particle simulation, procedural mesh generation, and physics solvers. The **WGSL** compute compilation path follows the same **naga** → **SPIR-V** → **NIR** → **ACO**/**LLVM** route as graphics shaders.

Section 7 addresses **Unreal Engine 5** and **Unity** as closed-source counterpoints. **UE5** uses a **Rendering Hardware Interface** (**RHI**) in **C++**, compiles **HLSL** shaders to **SPIR-V** via **DXC** (DirectX Shader Compiler), and supports **Nanite** virtualised geometry (using **VK_EXT_mesh_shader** for hardware mesh shading) and **Lumen** global illumination on Linux. **Unity** compiles **ShaderLab** / **HLSL** to **SPIR-V** via **HLSLcc** or **DXC**, supports **URP** and **HDRP** on Linux, and includes the **Burst** compiler (an **LLVM**-based ahead-of-time compiler for **DOTS** C# jobs) as a CPU compute tool separate from the **Vulkan** stack. Both engines enter **Mesa** as **SPIR-V** via **vk_spirv_to_nir()**, making them identical to **Bevy** from the driver's perspective.
