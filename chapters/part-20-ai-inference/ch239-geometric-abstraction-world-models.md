...
The rest of this chapter builds an explicit geometric stack on top of the empirical case just made. This section is a deliberate tangent from that stack: it surveys a parallel, mid-2026 trend on the *latent* side of the §1/§2 dichotomy, where a family of Joint-Embedding Predictive Architecture (JEPA) world models has started borrowing geometric structuring principles — not by reconstructing explicit 3D geometry, but by constraining the shape, manifold, or transition structure of the latent space itself. None of the systems below abandon the "pure latent" approach; each adds a narrow geometric regularizer to it for a specific purpose. This section exists because the Roadmap's long-term convergence question (§10) is already being answered, in a small way, from the latent-only side.
