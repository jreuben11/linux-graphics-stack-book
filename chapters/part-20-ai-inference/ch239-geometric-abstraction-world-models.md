...
4. [Structuring the Latent Space: Geometric Regularizers for JEPA World Models](#4-structuring-the-latent-space-geometric-regularizers-for-jepa-world-models)
   - [4.1 The Collapse Problem in Joint-Embedding Prediction](#41-the-collapse-problem-in-joint-embedding-prediction)
   - [4.2 V-JEPA 2 → 2.1: From Positional Geometry to Dense Supervision](#42-v-jepa-2--21-from-positional-geometry-to-dense-supervision)
   - [4.3 GeoWorld: Hyperbolic Latent Geometry for Long-Horizon Planning](#43-geoworld-hyperbolic-latent-geometry-for-long-horizon-planning)
   - [4.4 Delta-JEPA: Constraining Transition Geometry via Latent Displacement](#44-delta-jepa-constraining-transition-geometry-via-latent-displacement)
   - [4.5 Weak but Real: Probing for Emergent Geometric Structure](#45-weak-but-real-probing-for-emergent-geometric-structure)
   - [4.6 What This Synthesis Actually Claims](#46-what-this-synthesis-actually-claims)
5. [The Stack: A Layered Architecture for Geometric World Models](#5-the-stack-a-layered-architecture-for-geometric-world-models)
   - [5.1 Layer 0 — Substrate: 3D and 4D Gaussian Splatting](#51-layer-0--substrate-3d-and-4d-gaussian-splatting)
   - [5.2 Layer 1 — Part and Articulation Decomposition](#52-layer-1--part-and-articulation-decomposition)
   - [5.3 Layer 2 — Persistence Through Occlusion](#53-layer-2--persistence-through-occlusion)
   - [5.4 Layer 3 — Kinematic and Contact Scene Graphs](#54-layer-3--kinematic-and-contact-scene-graphs)
   - [5.5 Layer 4 — Per-Part Material and Physical Parameter Inference](#55-layer-4--per-part-material-and-physical-parameter-inference)
   - [5.6 Layer 5 — Executable and Latent Dynamics World Models](#56-layer-5--executable-and-latent-dynamics-world-models)
6. [The Gradient Problem](#6-the-gradient-problem)
   - [6.1 Representational Misalignment](#61-representational-misalignment)
   - [6.2 Contact Non-Smoothness](#62-contact-non-smoothness)
   - [6.3 Where the Gradient Enters](#63-where-the-gradient-enters)
   - [6.4 Parameter Identifiability](#64-parameter-identifiability)
7. [Storage, Loading, and Interchange](#7-storage-loading-and-interchange)
   - [7.1 KHR_gaussian_splatting and SPZ Compression](#71-khr_gaussian_splatting-and-spz-compression)
   - [7.2 UsdPhysics, SimReady, and the AOUSD Robot-Description Mapping](#72-usdphysics-simready-and-the-aousd-robot-description-mapping)
   - [7.3 MCAP and LeRobot: Trajectory and Episode Data](#73-mcap-and-lerobot-trajectory-and-episode-data)
8. [Implementation Toolchain](#8-implementation-toolchain)
   - [8.1 Differentiable Simulation: Warp, Newton, Genesis, and MJX](#81-differentiable-simulation-warp-newton-genesis-and-mjx)
   - [8.2 Geometric Foundation Models: Pointmap Regression](#82-geometric-foundation-models-pointmap-regression)
   - [8.3 3D Data Structures and Differentiable Rendering](#83-3d-data-structures-and-differentiable-rendering)
   - [8.4 Segmentation and Semantic Priors](#84-segmentation-and-semantic-priors)
   - [8.5 Collision Geometry and Serialization](#85-collision-geometry-and-serialization)
...
A 2026 position paper proposes resolving this by scoring world models against an explicit ladder of decision-making utility (an "L0–L7" framework) rather than against visual fidelity metrics like PSNR (Peak Signal-to-Noise Ratio, a pixel-wise reconstruction-error score in dB) or FID (Fréchet Inception Distance, a measure of how closely the distribution of generated images matches real images in a pretrained feature space), arguing that a world model's representation should be judged by how well it serves downstream policy optimization and planning, with a minimal reporting set defined for real-robot evaluation [Source](https://arxiv.org/abs/2606.15032). This is a *value-equivalence* framing: a representation is "good" not because it looks realistic, but because a planner or policy that consumes it makes decisions as good as if it had ground-truth state — the same principle that underlies value-equivalent model-based RL more broadly. Under this framing, "geometry vs. latent" isn't a single question; it's a family of questions, one per downstream task, and the answer can differ for grasp planning, long-horizon manipulation, and navigation even on the same robot.

A companion piece of evidence is architectural rather than philosophical: a controlled study of object-centric world models trained unsupervised from pixels (the DLPWM architecture — the paper does not spell out the acronym, but it names Deep Latent Particles (DLP) [Daniel & Tamar, ICML 2022] as the object-level representation the world model is built on, so DLPWM reads as "Deep Latent Particle World Model"; Note: needs verification against the authors' own naming) found strong reconstruction and out-of-distribution robustness, but policies trained on top of that representation underperformed a strong latent baseline (DreamerV3) on actual control — attributed to "representation shift during multi-object interactions" destabilizing policy learning even when the representation itself looked qualitatively good [Source](https://arxiv.org/abs/2511.06136). The lesson generalizes past this one architecture: a world model's *representation quality*, measured by reconstruction or visual metrics, does not automatically transfer to *policy quality* downstream. Any claim that explicit geometry (or its absence) is "better" needs to specify better *for what task*, evaluated *through what downstream consumer* — which is precisely what the L0–L7 framing above tries to force explicit.
...
## 4. Structuring the Latent Space: Geometric Regularizers for JEPA World Models

The rest of this chapter builds an explicit geometric stack on top of the empirical case just made. This section is a deliberate tangent from that stack: it surveys a parallel, mid-2026 trend on the *latent* side of the §1/§2 dichotomy, where a family of Joint-Embedding Predictive Architecture (JEPA) world models has started borrowing geometric structuring principles — not by reconstructing explicit 3D geometry, but by constraining the shape, manifold, or transition structure of the latent space itself. *Transition structure* here means the geometry of the mapping the predictor learns from one timestep's embedding to the next (z_t → z_{t+1}) — for example GeoWorld's hyperbolic geodesics in §4.3 — as distinct from *shape* (the static geometry of the embedding space at a single instant) or *manifold* (the surface those embeddings live on). This is a latent-space analogue of, but architecturally unrelated to, classical pixel-space optical flow (the per-pixel 2D motion field between consecutive frames used by front-end visual-odometry trackers like the Lucas-Kanade implementation in Chapter 209 §12) — none of the systems below compute or supervise against flow directly. None of the systems below abandon the "pure latent" approach; each adds a narrow geometric regularizer to it for a specific purpose. This section exists because the Roadmap's long-term convergence question (§10) is already being answered, in a small way, from the latent-only side.

### 4.1 The Collapse Problem in Joint-Embedding Prediction

A JEPA model predicts the *embedding* of a target (a masked patch, a future frame) from the embedding of visible context, rather than predicting raw pixels. This sidesteps the enormous capacity a pixel-level decoder would otherwise waste modeling irrelevant texture and noise, but it opens a well-known failure mode: a constant, input-independent embedding trivially minimizes an embedding-prediction loss, so nothing forces the model to preserve any information about the input at all. The standard defense against this collapse, used across the V-JEPA line, is architectural rather than loss-based: an asymmetric split between a predictor network and a target encoder, a stop-gradient on the target encoder's output so gradients from the prediction loss cannot flow back and degrade it, and an exponential-moving-average (EMA) update rule that lets the target encoder track the context encoder slowly rather than copying it exactly every step [Source](https://arxiv.org/abs/2506.09985). Every method surveyed below either relies on this same baseline defense or, in one case (§4.4), deliberately does without part of it and substitutes a different constraint instead — which is precisely why it is worth calling out as a shared frame of reference.

### 4.2 V-JEPA 2 → 2.1: From Positional Geometry to Dense Supervision

V-JEPA 2 already bakes a geometric prior directly into its attention mechanism rather than into the world state it predicts: a 3D rotary position embedding (3D-RoPE) splits the rotary encoding into temporal, height, and width segments and applies it inside attention to encode where each spatiotemporal "tubelet" token sits in the video volume [Source](https://arxiv.org/abs/2506.09985). This is worth distinguishing carefully from the geometric stack in the rest of this chapter: it is a positional prior over *tokens*, not a reconstructed 3D scene the model reasons over explicitly.

V-JEPA 2.1 refines the same latent-only architecture along three axes without adding new geometric priors of this kind: a Dense Predictive Loss that supervises all output tokens — visible and masked alike — rather than only the masked ones, which the authors argue prevents visible tokens from degenerating into pure aggregators that pass information through without encoding it themselves; Deep Self-Supervision, a hierarchical training objective applied at intermediate encoder layers rather than only at the final output; and Multi-Modal Tokenizers that let a single tokenizer handle both images and video in unified pretraining [Source](https://arxiv.org/abs/2603.14482). Reported gains over V-JEPA 2 include 7.71 mAP on Ego4D and 40.8 Recall@5 on EPIC-KITCHENS for action anticipation, a roughly 20-point improvement over V-JEPA-2 AC on robotic grasping success, 5.687 ATE on TartanDrive, 0.307 RMSE on NYUv2 depth, and 77.7 on Something-Something-V2 [Source](https://arxiv.org/abs/2603.14482). *Note: needs verification* — these are the paper's own reported benchmark figures; independent reproduction was not confirmed for this chapter.

### 4.3 GeoWorld: Hyperbolic Latent Geometry for Long-Horizon Planning

GeoWorld, accepted to CVPR 2026, is an energy-based predictive world model that goes further than positional encoding: it introduces a Hyperbolic JEPA component that maps otherwise-Euclidean latent embeddings onto a hyperbolic manifold, where geodesic distance — rather than straight-line Euclidean distance — is used to encode hierarchical relationships between states, such as a long-horizon task and its constituent sub-steps [Source](https://arxiv.org/abs/2602.23058). Hyperbolic space can embed tree-like hierarchical structure with far less distortion than Euclidean space at the same dimensionality, which is the specific property GeoWorld is exploiting here rather than a general claim that hyperbolic geometry is superior for all latent representations. The model adds a triangle-inequality regularization term along these geodesics, and uses a Geometric Reinforcement Learning formulation to optimize the resulting energy-based objective. On long-horizon planning benchmarks (CrossTask and COIN), GeoWorld reports roughly a 3-percentage-point success-rate improvement at 3-step planning horizons and roughly 2 points at 4-step horizons over V-JEPA 2 [Source](https://arxiv.org/abs/2602.23058). *Note on naming*: "H-JEPA" in some secondary summaries of this work refers to GeoWorld's internal Hyperbolic JEPA component, not a separately published, standalone system.

### 4.4 Delta-JEPA: Constraining Transition Geometry via Latent Displacement

Delta-JEPA takes a different tack: instead of reshaping the latent space's manifold (§4.3) or adding a positional prior to attention (§4.2), it constrains the geometry of *transitions* between latent states. Its Latent Difference Action Decoder (LDAD) reconstructs the action that was actually executed between two consecutive observations from the displacement vector between their latent embeddings — the vector difference, not the concatenation of the two endpoint embeddings [Source](https://arxiv.org/abs/2606.31232). This is a narrower architectural change than it might sound: the authors report that Delta-JEPA needs only two loss terms, a latent-prediction loss and an action-reconstruction loss computed from that displacement, and notably drops the frozen-encoder, stop-gradient, and explicit distribution-matching regularizers that §4.1 described as the standard collapse defense — the claim is that decoding the executed action from the displacement itself already constrains the latent trajectory's geometry enough to prevent collapse, without those additional mechanisms [Source](https://arxiv.org/abs/2606.31232). Delta-JEPA is evaluated on visual continuous-control tasks including Push-T, Reacher, and Cube manipulation, and ablations reported in the paper show the displacement-based decoding outperforms a baseline that instead concatenates the two endpoint embeddings before decoding the action [Source](https://arxiv.org/abs/2606.31232). *Note: needs verification* — this is a comparatively new preprint; whether dropping the standard collapse defenses generalizes beyond the specific continuous-control tasks evaluated has not been independently confirmed.

### 4.5 Weak but Real: Probing for Emergent Geometric Structure

A separate, more exploratory line of work asks whether geometric or symbolic structure emerges *unprompted* inside an already-trained JEPA latent space, rather than being explicitly regularized in as in §4.2–§4.4. One such study freezes a pretrained V-JEPA 2 encoder and attaches a lightweight vector-quantization (VQ) probe with an 8-entry codebook — described by the author as an "AI Mother Tongue" framework — to test whether discrete symbol-like structure emerges across categories such as grasp angle, object geometry, and motion timing [Source](https://arxiv.org/html/2603.20327v1). The reported results are statistically significant (chi-squared p < 10⁻⁴) but the effect sizes are weak: normalized mutual information between probe codes and category labels of only 1.2–3.9%, Jensen-Shannon divergence up to 0.343 between category distributions, and codebook utilization of 62.5% [Source](https://arxiv.org/html/2603.20327v1). *Note: needs verification* — this is a single-author, exploratory preprint; statistical significance with a weak effect size is consistent with a real but minor structuring effect, but is equally consistent with a probe picking up a small, incidental correlation rather than meaningful emergent geometry. Readers should treat this as a preliminary signal worth re-checking against follow-up or independent replication, not as established evidence that JEPA latents spontaneously organize into interpretable geometric or symbolic structure.

### 4.6 What This Synthesis Actually Claims

It is tempting to read §4.2 through §4.5 together as evidence that "geometry plus latent beats either alone" — but that overstates what four largely independent results actually show. What they demonstrate is narrower and more consistent with the value-equivalence lens already laid out in §2: positional geometric priors (3D-RoPE) were already present in the pure-latent V-JEPA line before this section's timeframe and are being refined incrementally, not newly discovered as a fix (§4.2); a specific manifold choice (hyperbolic space) measurably helps a specific, narrow task shape — long-horizon, hierarchically structured planning — without any claim of general superiority (§4.3); constraining the geometry of latent *transitions*, rather than the latent space itself, is a plausible alternative to the standard collapse defenses, evaluated so far only on a handful of continuous-control tasks (§4.4); and the strongest-sounding claim — that geometric or symbolic structure emerges from latent training without being asked for — currently has the weakest evidentiary support of the four (§4.5). Each of these is a targeted regularizer solving a specific problem in a specific setting, the same pattern the explicit-geometry stack in §5 through §8 exhibits at the layer level — not a demonstration that any one representation is optimal in the sense §2 already argued is not well-posed.

## 5. The Stack: A Layered Architecture for Geometric World Models
...
### 5.1 Layer 0 — Substrate: 3D and 4D Gaussian Splatting
...
### 5.3 Layer 2 — Persistence Through Occlusion
...
### 5.6 Layer 5 — Executable and Latent Dynamics World Models
...
### 6.4 Parameter Identifiability

A quieter but persistent issue across every method in §5.5 is identifiability: from video alone, is the inferred material parameter actually the physical quantity it claims to be, or merely *a* parameter setting that happens to reproduce the observed motion under the assumed simulator? Two different (density, stiffness) pairs can produce visually indistinguishable trajectories under a given force; a feed-forward regressor trained on synthetic data (PIXIE, UniPixie) inherits whatever identifiability structure — or lack of it — exists in its training distribution, and a self-supervised single-video method (ReconPhys) has strictly less signal to disambiguate with than a multi-view, multi-frame method (PhysTwin). None of the sources surveyed for this chapter claim to have solved general parameter identifiability from monocular video; UniPixie's shift to a *distribution* over material properties rather than a point estimate (§5.5) is best read as a partial, probabilistic acknowledgment of this limit rather than a resolution of it — it reports the shape of its uncertainty, not a guarantee of correctness. *Note: needs verification* — whether any of these methods report calibration or coverage statistics for their predicted distributions (i.e., whether the true parameter actually falls within the predicted uncertainty band at the claimed rate) was not confirmed from the abstracts alone and would require reading each paper's evaluation section in full before relying on it.

## 7. Storage, Loading, and Interchange
...
### 7.1 KHR_gaussian_splatting and SPZ Compression
...
### 7.2 UsdPhysics, SimReady, and the AOUSD Robot-Description Mapping
...
### 7.3 MCAP and LeRobot: Trajectory and Episode Data
...
**Genesis** is a separate physics-simulation project with a notably different performance profile and, as of 2026, a different compiler backend than its original Taichi/CUDA design: it now compiles via Quadrants, a compiler forked from Taichi in mid-2025, which lowers Python simulation kernels to CUDA, AMD ROCm, Apple Metal, Vulkan, x86, and ARM64 — i.e., it is no longer CUDA-exclusive. Its README reports roughly 10–80× speedup over other GPU robotic simulators, and up to 43M FPS for a simulated Franka arm on a single RTX 4090 [Source](https://github.com/Genesis-Embodied-AI/genesis-world). *Note: needs verification* — this is a self-reported benchmark from the project's own README rather than an independently reproduced figure; treat the specific multiplier with appropriate skepticism until checked against a third-party benchmark. On the MuJoCo side, **MJX** (MuJoCo's JAX-based reimplementation) and `mujoco_warp` are the two GPU-parallel backends feeding both Newton (via `mujoco_warp`) and the DiffMJX contact-gradient work discussed in §6.2. **Dojo**, **Brax**, and **PyBullet** round out the broader differentiable/parallel physics landscape referenced across the papers in §5 and §6, each with different tradeoffs between contact accuracy, differentiability, and raw throughput that are outside this chapter's scope to compare directly.

### 8.2 Geometric Foundation Models: Pointmap Regression

The pointmap regression models surveyed empirically in §3 — DUSt3R, MASt3R, VGGT, π³, and MoGe-2 — are the practical entry point for turning ordinary video into the Gaussian-splat or point-cloud substrate described in §5.1, replacing the classical Structure-from-Motion pipeline (COLMAP, and its GPU-parallel successor **GLOMAP**) for many world-model pipelines where per-frame calibration overhead is a bottleneck. Chapter 115 §8 and §17–18 document COLMAP integration and the DUSt3R/MASt3R family in the specific context of NeRFStudio's `ns-process-data` pipeline; the same models are increasingly used as a pose-and-geometry front end feeding directly into the part-decomposition and material-inference layers of this chapter, bypassing an explicit SfM stage entirely.

### 8.3 3D Data Structures and Differentiable Rendering
...
### 8.4 Segmentation and Semantic Priors

Part and object decomposition (§5.2) frequently needs a semantic or instance prior to bootstrap from, rather than discovering part boundaries purely from motion. Meta's **SAM 3** ("Segment Anything with Concepts") unifies detection, segmentation, and tracking behind a "Promptable Concept Segmentation" interface — prompting with short noun phrases or image exemplars rather than only points or boxes — trained on a data engine covering 4M unique concept labels, and reports roughly doubling the accuracy of prior systems on both image and video concept segmentation [Source](https://arxiv.org/abs/2511.16719). Paired with **DINOv2/v3** self-supervised visual features (used directly by several of the material-inference methods in §5.5 for zero-shot generalization), SAM 3 is a common front end for turning a raw Gaussian splat or point cloud into the segmented input the part-decomposition layer expects.

### 8.5 Collision Geometry and Serialization

Two smaller but load-bearing utilities close the loop between a reconstructed scene and a runnable simulation. **CoACD** (Approximate Convex Decomposition for 3D Meshes with Collision-Aware Concavity and Tree Search, SIGGRAPH 2022) generates the convex-decomposed collision meshes that rigid-body simulators require but that raw reconstructed geometry — Gaussian splats or arbitrary meshes — does not natively have [Source](https://github.com/SarahWeiii/CoACD); **V-HACD** is the older, still widely used alternative for the same job. For serializing trained model weights and inferred parameter fields (material fields, network checkpoints) safely across the Python/Rust/C++ boundary common in this toolchain, **safetensors** has become the default over pickle-based formats, for the same reason it has in the broader ML ecosystem: no arbitrary code execution on load. The **Foxglove SDK** provides client libraries for writing and visualizing MCAP logs (§7.3) directly from a robotics or simulation process, closing the loop from "physics simulation ran" to "the trajectory is inspectable in a standard log viewer."

## 9. Open Problems

Pulling the threads of §6 through §8 together, three problems remain genuinely open as of mid-2026, rather than merely "not yet productized":

- **No unified benchmark spans the whole stack.** E3D-Bench (§3) evaluates geometric foundation models; MPMWorlds (§5.6) evaluates dynamics inference and extrapolation; OBSBench (§3) evaluates observation-space choice for policy learning. No benchmark surveyed for this chapter evaluates the full pipeline — reconstruction, decomposition, material inference, and downstream control — end to end, which makes it hard to know whether an error introduced at, say, the part-decomposition layer (§5.2) is amplified or absorbed by the time it reaches a policy trained on top of the resulting world model.
- **Gradient correctness through contact remains partially unresolved.** §6.2's foundational skepticism about whether differentiable contact simulators produce correct gradients at all [Source](https://arxiv.org/abs/2207.05060) predates most of the specific methods surveyed in §5, and none of those later methods directly re-litigate that finding for their specific contact formulation. DiffMJX's straight-through estimator (§6.2) is a mitigation for one class of gradient-degradation mechanism, not a general proof of correctness for arbitrary contact-rich scenes.
- **Identifiability from monocular or sparse-view video is unresolved in general**, as discussed in §6.4. The field's current response is architectural (feed-forward regression to amortize the cost of a poorly-conditioned inverse problem, or probabilistic output to represent the resulting uncertainty) rather than a claim that the underlying ambiguity has been removed.

A world-model builder choosing among the layers in §5 should treat each layer's reported numbers as measured under that paper's specific evaluation protocol, not as a composable error budget for the full stack — the tooling to actually measure the composed system does not yet exist in a standardized form.

## 10. Integrations

**Chapter 115 — NeRFStudio, Neural Radiance Fields, and 3D Gaussian Splatting:** The substrate layer of this chapter (§5.1) is the same 3D Gaussian Splatting representation covered there in depth — the `gsplat` CUDA rasterizer (§6–7), COLMAP/DUSt3R/MASt3R integration (§8, §17–18), and the PLY/interchange conventions (§7.8) that `KHR_gaussian_splatting` (§6.1 of this chapter) is now standardizing around.

**Chapter 212 — Python 3D ML Libraries (Open3D, PyTorch3D, Kaolin, and Warp):** The data-structure layer beneath every method in §5 and §8 — PyTorch3D `Meshes`/`Pointclouds`, Kaolin's `GaussianSplatModel` and USD I/O, Open3D TSDF and registration, and NVIDIA Warp's `wp.Tape` differentiable-kernel autodiff (§8.1 of this chapter reuses that exact API) — is documented there.

**Chapter 69 — Omniverse and USD:** `UsdPhysics` (§7.2) and NVIDIA's SimReady specification build directly on the OpenUSD foundations covered there; Kaolin's USD I/O (Chapter 212) and Isaac Lab's simulation environments (§8.1) both round-trip through the same USD stage format.

**Chapter 64 — glTF:** `KHR_gaussian_splatting` (§7.1) is a Khronos extension to the core glTF 2.0 format documented there; readers implementing a loader for the extension need the base glTF mesh-primitive and accessor model from that chapter first.

**Chapter 209 — OpenSLAM and Chapter 210 — SLAM Theory and SOTA:** Pointmap regression models (§8.2 — DUSt3R, MASt3R, VGGT) and Gaussian-splat scene representations increasingly substitute for or complement classical visual SLAM front ends; the pose-graph and loop-closure theory in those chapters underlies why feed-forward pointmap regression, which sidesteps explicit bundle adjustment, is such a significant simplification for the reconstruction stage of this chapter's stack (§3).

**Chapter 211 — ROS 2 Sensor and Perception Pipelines:** MCAP (§7.3) is the same log format documented there as the ROS 2 rosbag2 default storage backend; the kinematic/contact scene graphs of §5.4 and the LeRobot trajectory data of §7.3 are natural companions to the sensor pipelines described in that chapter.

**Chapter 25 — GPU Compute and Chapter 66 — CUDA Runtime:** Every differentiable-simulation and pointmap-regression tool in §8 (Warp, `mujoco_warp`, gsplat, nvdiffrast) is a CUDA-first workload; the memory-management and kernel-launch fundamentals in those chapters apply directly to tuning the toolchain in §8.1–8.3.

**Chapter 48 — ROCm and Machine Learning on Linux GPUs:** Genesis's Quadrants-based multi-backend compilation and gsplat's AMD-maintained HIP/ROCm fork (§8.3) are the two ROCm on-ramps into this chapter's toolchain for readers on AMD hardware; Chapter 48's coverage of ROCm's maturity and gaps applies directly to both.

**Chapter 117 — Slang and the Differentiable Shading Language:** Chapter 115 §7.5 documents a Slang reimplementation of the gsplat tile rasterizer using `[Differentiable]` autodiff for SPIR-V/Vulkan targets; the same differentiable-shading approach is a plausible future path for the render-side half of the gradient-injection question discussed in §6.3 of this chapter, as an alternative to hand-written CUDA backward kernels.
...
- **Consolidation of feed-forward material-inference models.** PIXIE, UniPixie, and ReconPhys (§5.5) all published within months of each other across late 2025 and early-to-mid 2026, each with a different dataset and solver-output convention; a shared benchmark and output schema (analogous to what E3D-Bench did for geometric foundation models) would make these directly comparable.
- **Native ROCm support landing in upstream `gsplat`.** The pending pull request noted in §8.3 would move AMD GPU support for the Gaussian-splat substrate layer from a maintained fork to a first-class upstream target.

### Medium-term (1–3 years)
- **A composed, end-to-end benchmark spanning §5's full stack**, from raw video through reconstruction, part decomposition, material inference, and downstream policy performance — the gap identified in §9 — is a plausible next step once the individual-layer benchmarks (E3D-Bench, MPMWorlds, OBSBench) mature further.
- **Differentiable-shading-language backward passes** (Chapter 115 §7.5's Slang reimplementation) replacing hand-written CUDA backward kernels across more of the toolchain in §8, reducing the maintenance burden of keeping forward and backward passes manually synchronized as gradient-injection strategies (§6.3) continue to diversify.
- **UsdPhysics maturing beyond rigid bodies.** The AOUSD Physics Working Group's initial charter scope is rigid-body dynamics (§7.2); soft-body, cloth, and fluid schema coverage — needed to represent the MPM-based digital twins of §5.6 natively in USD rather than through engine-specific extensions — is a plausible medium-term expansion.

### Long-term
- **Resolution, or at least a widely adopted mitigation, for the contact-gradient correctness problem (§6.2, §9)** — either through provably-correct smoothed contact formulations, or through a shift toward learned dynamics models (§5.6) that sidestep differentiating through a hand-written contact solver entirely.
- **Convergence between the "explicit geometry" and "latent" world-model families framed as opposed in §1–§2** — plausibly not a winner-take-all outcome, but a hybrid where geometric layers (§5.1–§5.4) ground a latent dynamics model's early layers, combining V-JEPA-2-style broad video pretraining with the sample efficiency and physical plausibility that explicit geometry provides for the specific tasks the empirical evidence in §3 favors it for (§4 surveys early evidence of exactly this synthesis on the JEPA/latent side).
