...
### 4.5 ZLUDA: Binary-Level CUDA Compatibility

`hipify` (§4.4) translates CUDA *source* to HIP source, requiring a recompile against the ROCm toolchain. **ZLUDA** takes the opposite approach: it is a drop-in replacement for the CUDA driver and runtime libraries (`libcuda.so`, `libcudart.so`, `libnvrtc.so`, and CUDA library shims for cuBLAS/cuDNN/cuFFT/cuSPARSE) that lets an *unmodified* CUDA binary run against AMD hardware, translating PTX to AMDGPU ISA and CUDA API calls to HIP/HSA calls at load time via API interception (`zluda_inject`/`zluda_redirect`). No access to the application's source is required. [Source: ZLUDA repository](https://github.com/vosen/ZLUDA)

ZLUDA's history is unusual for a project in this stack. It was created by Andrzej Janik, originally as a CUDA implementation targeting Intel GPUs over the Level Zero API (the name derives from "Z" for Level Zero plus a nod to CUDA) — Intel funded that phase under an NDA and did not permit its release. AMD subsequently funded Janik quietly from 2022 to early 2024 to retarget ZLUDA at ROCm/HIP; when AMD's funding ended, the code was released as open source in February 2024 with AMD's email approval. Roughly six months later, AMD's legal department requested its removal, arguing the approval email was not binding. Janik rebuilt the project from a pre-AMD codebase and has continued it independently since, now dual-licensed under Apache-2.0/MIT and written in Rust. [Source: Phoronix, "Open-Source AMD GPU Implementation Of CUDA 'ZLUDA' Has Been Taken Down," https://www.phoronix.com/news/AMD-ZLUDA-CUDA-Taken-Down]

As of the project's Q4 2025 and Q1/Q2 2026 updates, ZLUDA runs against both ROCm 6 and ROCm 7, has full `llama.cpp` support reported at performance "nearly identical to that of the native ROCm backend," and bundles its own patched LLVM rather than depending on AMD's `comgr` library, working around AMDGPU-backend compiler bugs and compilation-time regressions. PyTorch support — the project's current focus, aimed specifically at `vLLM` — remains incomplete; where a workload depends on a CUDA library call with no optimized MIOpen equivalent (e.g., certain convolution shapes), performance can fall far short of the native ROCm path rather than merely mirroring hipify-translated code. AMD's public position, per the project's own account, is that there is "no business case" for running CUDA applications on AMD GPUs — the project now describes itself as unfunded, community-driven work rather than a supported migration path. [Source: ZLUDA blog, "ZLUDA update Q4 2025," https://vosen.github.io/ZLUDA/blog/zluda-update-q4-2025/; ZLUDA blog, "ZLUDA update Q1&Q2 2026," https://vosen.github.io/ZLUDA/blog/zluda-update-q1q2-2026/]

For a developer choosing between the two CUDA-portability paths this chapter covers: hipify (§4.4) produces HIP source that compiles natively against the ROCm toolchain and is the path AMD itself documents and supports, at the cost of requiring source access and a recompile; ZLUDA requires no source changes but runs unofficially, outside AMD's support surface, and its correctness and performance depend on library-by-library coverage that changes release to release. Neither path is a substitute for a codebase written against HIP from the outset.

### 4.6 Managed Memory and Unified Memory
...
**Ch245 — The JAX Ecosystem** and **Ch246 — JAX and PyTorch Internals**: PyTorch's ROCm build and `jax[rocm]` both ultimately execute through the HIP/hipBLAS/RCCL stack and `amdkfd` submission path this chapter documents. Ch246's dispatcher and codegen coverage (ATen kernel dispatch, TorchInductor's generated Triton kernels) targets the same `hipcc`/AMDGPU LLVM backend pipeline described in §7 and §11 above.
