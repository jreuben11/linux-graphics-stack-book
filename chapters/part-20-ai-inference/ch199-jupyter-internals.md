...
- **ZMQ transport encryption.** A Jupyter Enhancement Proposal for encrypting the ZMQ traffic between `jupyter_server`/desktop frontends and kernel processes is in progress, with CurveZMQ support for IPC transport already merged into `jupyter_client` and companion changes landing in `ipykernel` and `jupyter_server`. This closes a long-standing gap in which the five kernel sockets described in §2 carry unencrypted traffic even when HMAC message signing is enabled. [Source: JEP proposal](https://github.com/jupyter/enhancement-proposals/pull/145) · [jupyter_client PR](https://github.com/jupyter/jupyter_client/pull/1126)
- **WebSocket subprotocol authentication.** A companion JEP proposes moving the `kernels/<id>/channels` WebSocket's token authentication (§2) from a query-string token to a WebSocket subprotocol, avoiding token leakage through proxy logs, browser history, and `Referer` headers. [Source](https://github.com/jupyter/enhancement-proposals/pull/121)
- **Jupyter AI's agentic architecture.** `jupyter_ai` 3.x replaced the LangChain-backed `%%ai` magic architecture described in §13 with Agent Client Protocol (ACP) support for external coding agents and a Jupyter MCP server that lets an agent read, edit, and execute notebook cells directly rather than only receiving a cell's text as a prompt. Point releases have continued to expand this agent-integration surface. [Source: jupyter_ai v3.0.0 release notes](https://github.com/jupyterlab/jupyter-ai/releases/tag/v3.0.0) · [jupyter-server-mcp](https://github.com/jupyter-ai-contrib/jupyter-server-mcp)

### Medium-term (1-3 years)
- **Kubernetes Dynamic Resource Allocation for GPU scheduling.** Kubernetes' Dynamic Resource Allocation (DRA) replaces the device-plugin model's flat resource counts (the `nvidia.com/gpu: 1` style requests used by KubeSpawner in §12) with structured `ResourceClaim`/`DeviceClass` objects supporting CEL-based device selection and sharing between pods. An NVIDIA DRA driver already exists; as it and DRA-aware tooling mature, JupyterHub's Kubernetes spawner has a path toward MIG-aware, shareable GPU allocation without the current static per-pod GPU counts. [Source: Kubernetes DRA docs](https://kubernetes.io/docs/concepts/scheduling-eviction/dynamic-resource-allocation/) · [NVIDIA DRA driver](https://github.com/kubernetes-sigs/dra-driver-nvidia-gpu)
- **WebAssembly stack switching in Pyodide.** Recent Pyodide releases have reworked how Python thread state, `contextvars`, and `threading.local()` interact with Wasm stack-switching primitives underlying `pyodide.ffi.run_sync()`. This is foundational work toward giving JupyterLite's browser-side kernel (§11) more faithful async and blocking-call semantics than today's `SharedArrayBuffer`/`Atomics` message-passing model allows. [Source: Pyodide changelog](https://github.com/pyodide/pyodide/blob/main/docs/project/changelog.md)
- **Notebook 7 / JupyterLab lockstep convergence.** Jupyter Notebook 7.x continues to be built directly on the matching JupyterLab minor release (e.g., Notebook 7.6 on JupyterLab 4.6), so most future JupyterLab rendering-pipeline (§6) and real-time-collaboration (§10) investment flows into the Notebook interface automatically rather than requiring a parallel implementation. [Source](https://jupyter-notebook.readthedocs.io/en/latest/changelog.html)

### Long-term
- **WebGPU-native compute in JupyterLite.** If browser WebGPU support and Wasm-compiled `wgpu` continue to mature alongside Pyodide's stack-switching work, JupyterLite (§11) could plausibly host GPU-accelerated notebooks — matrix multiplication, shader-based image processing — with no server process at all, extending the "fully static, CDN-deployable notebook" model beyond CPU-only Pyodide packages. No Jupyter or browser-vendor roadmap currently commits to a date for this; it depends on WebGPU stability work outside the Jupyter project's control (Chapter 98).
- **Agents as first-class real-time collaborators.** Jupyter AI's ACP/MCP agent integration (above) and JupyterLab's Yjs-based real-time collaboration (§10) are currently separate subsystems: an agent acts on a notebook through kernel `execute_request` and file-write operations, not through the same CRDT awareness/editing channel a human collaborator uses. Extending the `@jupyter/ydoc` model so agents appear as tracked collaborators with their own awareness state is a plausible next step given both pieces already exist, but no JEP currently formalizes agent-as-collaborator semantics in the Yjs document model.
