...
**RDMA transport.** RDMA support landed as an optional transport, negotiated automatically at connection handshake and falling back to plain TCP unless both peers support it: RoCEv2 over `libibverbs` on Linux (Mellanox ConnectX-class NICs, enabled by default at compile time when the library is detected), and — more unusually — RDMA over Thunderbolt 5 on Apple Silicon (macOS 26.2+, enabled via `rdma_ctl enable` from macOS Recovery, using `librdma`). `GGML_RPC_NO_RDMA=1` forces TCP-only on either peer. [Source](https://raw.githubusercontent.com/ggml-org/llama.cpp/master/tools/rpc/README.md) It is not, however, a zero-copy datapath: inspecting `ggml/src/ggml-rpc/transport.cpp` shows the RDMA path uses two-sided `IBV_WR_SEND`/recv verbs in fixed 256 KiB chunks (`RDMA_CHUNK`), `memcpy`-ing application data into and out of pre-registered `tx_buf`/`rx_buf` staging buffers rather than issuing one-sided `RDMA_WRITE`/`READ` directly into destination tensor memory — no GPUDirect RDMA involved. [Source](https://raw.githubusercontent.com/ggml-org/llama.cpp/master/ggml/src/ggml-rpc/transport.cpp) That staged-copy design matches the merge author's own two-node benchmark: a modest +4.1% prompt-processing and +6.5% token-generation improvement over TCP, not the multi-fold gain typical of true zero-copy RDMA. [Source](https://github.com/ggml-org/llama.cpp/pull/20590) The RDMA path also still has open hardening gaps — a fuzzer-found receive-buffer overflow fix and an Intel E810 (non-Mellanox NIC) compatibility fix both remain unmerged as of this writing, and a multi-node RDMA deadlock reported against a 4-GPU cluster was closed `not_planned` by a stale-bot rather than fixed upstream, with the root cause (a fabric PFC/DSCP configuration gap) surfacing only in a community comment. [Source](https://github.com/ggml-org/llama.cpp/issues/24813) No NVIDIA involvement is evident in this RDMA work — no PR author is NVIDIA-affiliated, and NVIDIA's own answer to the same distributed-transfer problem shape, NIXL (§14), targets KV-cache handoff between prefill/decode workers rather than llama.cpp RPC's whole-node weight/compute sharding, and the two are not integrated.
...
### Toward Consolidation: Where This Overlap Is Likely Headed

The table above is a snapshot of genuine overlap, not a stable end state, and the overlap resolves differently at each of three layers rather than through one CRD displacing the rest.

**Routing and scheduling consolidates fastest**, because it is the one layer with actual upstream, multi-vendor Kubernetes governance rather than a single company or community behind it: GAIE's `InferencePool` is a Kubernetes SIG project, and projects are already deferring to it rather than reimplementing it — Kaito routes through it for KV-cache-aware load balancing, and, most tellingly, CNCF-Incubating KServe explicitly composes with Sandbox-stage llm-d for P/D disaggregation and scheduling instead of building its own (§27's CRD examples above). That KServe, further along in the CNCF maturity ladder, defers to llm-d rather than the reverse is a stronger consolidation signal than governance status alone would suggest.

**The KV-transfer/disaggregation fabric beneath that consolidates more slowly.** NIXL (NVIDIA), Mooncake, and llm-d's own transfer mechanism (§14) are still competing implementations without a SIG-level neutral standard-setter — even GAIE's own history shows this boundary is unsettled one layer up: the EPP/routing reference implementation moved *out* of the SIG-governed GAIE project and *into* llm-d at v1.6.0, meaning "neutral spec" and "reference implementation" are still being renegotiated. The more plausible path here is a de facto standard emerging around whichever transfer library gets the widest pluggable adoption (NIXL is already a connector for both vLLM and llm-d), rather than a formal merger.

**The model-lifecycle CRD layer — llm-d, KServe's `LLMInferenceService`, AIBrix, llmaz, KubeAI, Kaito — consolidates slowest, and not by merger at all.** Each project's CRD is tied to its own existing deployments and, for the vendor-led ones, to a corporate incentive that doesn't go away: Kaito (Microsoft/AKS) and AIBrix (ByteDance) each anchor a differentiated in-house inference offering for their parent's Kubernetes platform, giving neither a reason to fold into a shared CRD. llm-d's unusually broad multi-vendor backing (Red Hat, Google Cloud, IBM, CoreWeave, and NVIDIA all contributing) makes it the best-positioned candidate to become the neutral option others build on, mirroring the KServe pattern above. Projects without a comparable corporate sponsor — llmaz, KubeAI — are more likely to either lose mindshare over time or persist as focused tools valued for a specific capability (llmaz's LeaderWorkerSet-based multi-host-from-day-one design and broadest engine support; KubeAI's zero-external-dependency scale-to-zero) than to win a general-purpose consolidation outright. Individual **capabilities** are likely to cross-pollinate faster than CRDs merge: llmaz already implements its own Karpenter-based node auto-provisioning rather than adopting Kaito's `Workspace` CRD to get it, the more probable pattern for how Kaito's distinguishing feature spreads.

**Ray Serve/KubeRay sits outside this consolidation entirely** — it is chosen for Ray's general-purpose ecosystem (data pipelines, RL, batch compute), not as a head-to-head competitor for a Kubernetes-native LLM-serving CRD specifically, so it is unlikely to converge with the others regardless of how the rest of this table shakes out.
...
- [ggml-rpc transport.cpp source](https://raw.githubusercontent.com/ggml-org/llama.cpp/master/ggml/src/ggml-rpc/transport.cpp)
- [PR #20590: RoCEv2 RDMA transport for RPC backend](https://github.com/ggml-org/llama.cpp/pull/20590)
- [Issue #24813: RPC-RDMA multi-node deadlock](https://github.com/ggml-org/llama.cpp/issues/24813)
