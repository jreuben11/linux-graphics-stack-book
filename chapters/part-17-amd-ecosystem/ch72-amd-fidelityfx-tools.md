...
1. [AMD Developer Ecosystem Overview](#1-amd-developer-ecosystem-overview)
   - [1.1 What is the FidelityFX SDK?](#11-what-is-the-fidelityfx-sdk)
   - [1.2 What is FidelityFX Super Resolution (FSR)?](#12-what-is-fidelityfx-super-resolution-fsr)
   - [1.3 What is the Advanced Media Framework (AMF)?](#13-what-is-the-advanced-media-framework-amf)
   - [1.4 What are Radeon Developer Tools?](#14-what-are-radeon-developer-tools)
...
### 1.1 What is the FidelityFX SDK?

The FidelityFX SDK is AMD's open-source collection of image-quality and rendering-technique effects, published under the GPUOpen initiative at `github.com/GPUOpen-LibrariesAndSDKs/FidelityFX-SDK` under the MIT licence. The SDK bundles over twenty post-processing and rendering algorithms — upscaling, sharpening, global illumination, screen-space reflections, and more — into a unified host-side C API backed by a graphics-API-agnostic abstraction layer called `FfxInterface`. Effects are implemented as abstract GPU job graphs; the Vulkan or DirectX 12 backend translates those jobs into concrete command buffer operations. Shader permutations are compiled offline by the FidelityFX Shader Compiler (FFX-SC) into SPIR-V or DXIL blobs, so no runtime shader compilation occurs when an effect is initialised. On Linux the SDK runs entirely through the standard Vulkan driver stack — its SPIR-V blobs are processed by RADV's ACO backend or any other conformant Vulkan driver. SDK 2.0 introduced a separate `ffx-api/` layer, the "Upgradable API", which permits driver-level upgrades to neural-model weights for FSR 4 and future ML-based techniques without requiring an application rebuild. This chapter examines the SDK's internal architecture before focusing on its most visible component, FSR, and situates the whole suite within the Mesa/RADV driver stack described in Ch15 and Ch18.

### 1.2 What is FidelityFX Super Resolution (FSR)?

FidelityFX Super Resolution (FSR) is AMD's upscaling technology that reconstructs a high-resolution output image from a lower-resolution rendered frame, improving rendering performance at the cost of a bounded quality loss. FSR has evolved through four major generations. FSR 1 applied a spatial algorithm across two compute dispatches: Edge Adaptive Spatial Upsampling (EASU) for edge-aware reconstruction followed by Robust Contrast Adaptive Sharpening (RCAS). FSR 2 and FSR 3 replaced the purely spatial approach with temporal accumulation, accepting per-pixel motion vectors and a reactive mask to handle transparent and particle surfaces; FSR 3 additionally introduced frame interpolation that uses optical-flow-estimated motion to produce an interpolated frame between rendered frames, effectively doubling the presented frame rate. FSR 4, targeting RDNA 4 hardware, replaces the hand-crafted temporal algorithm with a neural network executed on RDNA 4's third-generation Matrix Cores via WMMA instructions, improving reconstruction quality for fine detail and thin geometry. All FSR generations integrate with a Vulkan application through a context/dispatch API pair — `ffxCreateContextDescUpscale` and `ffxDispatchDescUpscale` — and require a jitter sequence provided by the Halton(2,3) low-discrepancy generator queried via `ffxQueryDescUpscaleGetJitterOffset`. This chapter covers the architectural differences between generations and the input requirements and pipeline placement specific to FSR 4.

### 1.3 What is the Advanced Media Framework (AMF)?

The Advanced Media Framework (AMF) is AMD's hardware video encode and decode API, exposing the Video Core Next (VCN) fixed-function media engine present on RDNA and earlier AMD GPUs. AMF uses a COM-like factory and component architecture: an application obtains an `AMFFactory` from the shared runtime (`libamfrt64.so`), creates an `AMFContext` that represents a GPU device, and instantiates encode or decode `AMFComponent` objects such as `AMFVideoEncoderHW_HEVC` or `AMFVideoDecoderUVD`. Video frames are exchanged as `AMFSurface` objects backed by either host memory or Vulkan image handles through the `AMF_MEMORY_VULKAN` memory type, enabling zero-copy pipelines from a rendering workload directly into the encoder. On Windows, AMF communicates with the AMD kernel driver directly. On Linux the historical path relied on a proprietary `amf-amdgpu-pro` component outside the open Mesa stack, creating a dependency on the closed AMD driver package. Starting with driver 25.20, AMD recommends that Linux applications use the platform-standard VA-API through Mesa's `radeonsi_drv_video.so` backend for most encode and decode workloads, with AMF on Linux now acting as a delegating layer over that VA-API path rather than bypassing it. This chapter explains the AMF object model, the Linux transition strategy, and contrasts AMF with VA-API/Mesa and NVENC.

### 1.4 What are Radeon Developer Tools?

The Radeon Developer Tools are a suite of GPU profiling, memory analysis, and debugging utilities provided by AMD under the GPUOpen initiative for diagnosing Vulkan and compute workloads on RDNA hardware. The suite has three principal components. The Radeon GPU Profiler (RGP) captures Shader Queue Thread Trace (SQTT) token streams through the `VK_AMD_gpa_interface` Vulkan extension and a background Radeon Developer Service (RDS) daemon, recording fine-grained GPU timeline data — VALU and VMEM stall tokens, wavefront occupancy, barrier events, and pipeline state transitions — into `.rgp` binary files for offline analysis in the RGP desktop GUI. The Radeon Memory Visualizer (RMV) intercepts allocation and eviction events at the libdrm/UMD boundary, capturing `amdgpu_bo_alloc()` and `DRM_AMDGPU_GEM_CREATE` ioctl calls into `.rmv` trace files; these expose a Virtual Memory Heap Timeline, VRAM fragmentation patterns, and Smart Access Memory (SAM / Resizable BAR) placement across RADV, radeonsi, and ROCm/HIP workloads. The Radeon Developer Panel (RDP) coordinates the RDS daemon and provides a unified capture launch UI. On Linux these tools require Ubuntu 24.04 or later and operate exclusively over Vulkan. This chapter details the SQTT capture architecture, the `.rgp` and `.rmv` file formats, and the Linux-specific configuration steps required for each tool.
