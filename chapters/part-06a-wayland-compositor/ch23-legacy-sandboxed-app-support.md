...
The solution to the first problem is **XWayland**: a full X11 server that runs as a Wayland client, presenting X11 window content to the compositor as standard **wl_surface** objects while accepting connections from X11 applications on a Unix domain socket. From the perspective of an X11 application, **XWayland** is indistinguishable from a native X11 server. From the perspective of the Wayland compositor, **XWayland** is a privileged Wayland client that manages a set of surfaces. The chapter covers **XWayland**'s internal architecture in depth: the startup sequence (involving **xwayland_shell_v1**, **xdg_wm_base**, and the **_XWAYLAND_COMPOSITOR_CALLBACK** atom handshake), rootless versus full-screen mode (the latter used by **gamescope** for **Proton**/Steam games), the **xwl_window** structure and **XMapWindow()**/**RealizeWindow** hook that map X11 **Window** IDs to **wl_surface** objects, and input focus bridging via **wl_keyboard** enter/leave events translated through `xserver/hw/xwayland/xwayland-input.c`.

The solution to the second problem is **xdg-desktop-portal**: a **D-Bus** service that runs outside the sandbox and provides mediated access to compositor services — screen capture, file selection, camera access, and GPU context creation (the last of which remains under development as of 2025). Sandboxed applications call portal methods over **D-Bus**, which the portal backend translates into compositor-specific operations without giving the application direct access to kernel devices. The portal's two-process architecture separates a desktop-agnostic frontend (exposing **org.freedesktop.portal.*** well-known names, enforcing per-app permissions via `permissions.db`, and verifying app identity) from compositor-specific backends: **xdg-desktop-portal-gnome** for **Mutter**, **xdg-desktop-portal-kde** for **KWin**, and **xdg-desktop-portal-wlr** for wlroots-based compositors such as **Sway**. Backend selection is driven by the **XDG_CURRENT_DESKTOP** environment variable, and portal operations follow an asynchronous Request/Response pattern over **D-Bus**. **Flatpak** 1.16 introduced **wp_security_context_v1**, which propagates sandbox identity (app-ID, sandbox engine) at the **Wayland** protocol level so the compositor can further restrict which **Wayland** globals are visible to sandboxed clients.

These two mechanisms interact: a **Flatpak**-sandboxed application may itself be an X11 application running through **XWayland**. In that case, the X11 Unix socket is proxied into the sandbox via the Flatpak **--socket=x11** permission, and the **XWayland** server itself runs outside the sandbox. The portal and **XWayland** layers stack orthogonally. A sandboxed X11 application that relies on basic 2D drawing goes through the proxy socket to **XWayland**, where **Glamor** accelerates rendering entirely within the **XWayland** process (which has full GPU access). **Glamor** is the **OpenGL ES 2.0**/**OpenGL 3.1** DDX implementation inside the X server that translates X11 **Pixmap** operations — **XFillRectangle()**, **XRender** **COMPOSITE** — into **GLES2** draw calls (**glDrawArrays()**) against **GEM**-backed textures (**EGLImage** via **EGL_KHR_image_base**, allocated with **gbm_bo_create()**). **Glamor** initialises a surfaceless **EGL** context (**EGL_KHR_surfaceless_context**) in `xserver/glamor/glamor_egl.c`. For presenting **X11** content to the compositor, the **GEM** buffer's **DMA-BUF** file descriptor (obtained via **gbm_bo_get_fd()**) is wrapped in a **wl_buffer** using **zwp_linux_dmabuf_v1**. When X11 clients use hardware-accelerated **OpenGL** via **GLX** (**glXSwapBuffers()**) or **EGL** (**eglSwapBuffers()**), rendering bypasses **Glamor** entirely and follows the **DRI3**/X11 Present extension path — Mesa allocates **GPU** buffers via **DRI3** (**DRI3Open**, **DRI3PixmapFromBuffers**), renders directly into them, and the zero-copy result is submitted to the compositor as a **wl_buffer**. The **XWAYLAND_NO_GLAMOR** environment variable can disable **Glamor** and fall back to software rendering via **XShm**.

A sandboxed X11 application that uses **OpenGL** or **Vulkan** additionally requires the **--device=dri** permission so that **Mesa**'s **DRI3** client code inside the sandbox can open the **DRM** render node directly. The **--device=dri** **Flatpak** permission grants access to all **/dev/dri/renderD*** and **/dev/dri/card*** device nodes simultaneously — a coarse-grained grant with significant security implications. The chapter examines GPU compute side-channels (shared GPU caches leaking information between co-resident processes), **VRAM** residual data risks, the **DRM ioctl** attack surface, and **GPU VM** isolation (per-process **GPUVM** address spaces in the **DRM** subsystem) as a partial mitigation. The long-term goal of portal-mediated GPU context creation via **xdg-desktop-portal** — avoiding direct **/dev/dri** access entirely — remains unsolved as of 2025.

The chapter also covers the **xdg-desktop-portal** screen-cast and remote-desktop portals in detail: **org.freedesktop.portal.ScreenCast** (with **CreateSession**, **SelectSources**, **Start**, and **OpenPipeWireRemote** calls), the **PipeWire** **DMA-BUF** zero-copy path (**SPA_DATA_DmaBuf** buffer type, **eglCreateImageKHR** with **EGL_LINUX_DMA_BUF_EXT**, **glEGLImageTargetTexture2DOES**), output-level versus window-level capture limitations (**zwlr_screencopy_manager_v1** supports only full-output capture on wlroots compositors), and **org.freedesktop.portal.RemoteDesktop** synthetic input injection (**zwlr_virtual_pointer_v1**, **zwp_virtual_keyboard_v1**). **Chromium** and **Firefox** implement their own **PipeWire**/portal **ScreenCast** integration independently of **libportal**.

**XWayland** clipboard and drag-and-drop bridging is covered in depth: the X11 selection model (**CLIPBOARD**, **PRIMARY**, **SECONDARY** atoms, **XSetSelectionOwner()**, **XConvertSelection()**, **INCR** protocol) is bridged bidirectionally to the **Wayland** **wl_data_device_manager** / **wl_data_source** / **wl_data_offer** model via `xserver/hw/xwayland/xwayland-clipboard.c`. **PRIMARY** selection bridging uses **zwp_primary_selection_device_manager_v1**. Drag-and-drop bridging translates the **XDND** protocol (**XdndStatus**, **XdndDrop**, **XdndFinished** ClientMessages) to the **Wayland** **wl_data_device** drag protocol, with known limitations for cross-boundary **MIME** type conversion.

**DPI** and **HiDPI** scaling under **XWayland** is addressed through several mechanisms: the **Xwayland -dpi** flag, toolkit environment variables (**GDK_SCALE**, **QT_SCALE_FACTOR**), the **-scale** flag for integer **HiDPI**, and **wp_fractional_scale_v1** for fractional scaling communicated from the compositor via **wl_output.scale**. Per-output scaling state is reflected through **XRandR** in `xserver/hw/xwayland/xwayland-output.c`, and the **Xft.dpi** X resource (set via `~/.Xresources` and **xrdb**) is the universally honoured mechanism for informing X11 toolkits of display **DPI**.

Explicit synchronisation for **XWayland** is covered end-to-end: the implicit sync problem with **NVIDIA**'s proprietary driver (buffers submitted before GPU work completes, mitigated poorly by **glFinish()** serialisation), the partial **xwayland_explicit_synchronization_v1** protocol, and the complete solution via **wp_linux_drm_syncobj_v1** (merged in **wayland-protocols** 1.34, March 2024) based on **DRM** syncobj timelines (acquire and release timeline points per **wl_surface** commit). Full support requires **xserver 24.1**, **KWin 6.1** or **Mutter 46**, **Sway 1.9**, **NVIDIA driver 555**, and **Mesa 24.x**.

Practical guidance covers toolkit backend selection (**GDK_BACKEND**, **QT_QPA_PLATFORM**), common **XWayland** failure modes (wrong **DPI**, missing global hotkeys via **XGrabKey**, clipboard truncation, drag-and-drop failures, **xdotool** coordinate mismatches), detecting **XWayland** from within an application via the **XWAYLAND** X11 extension, and debugging tools (**xlsclients**, **xev**, **xrandr**, **XWAYLAND_DEBUG**, **XWAYLAND_NO_GLAMOR**). For **Flatpak** packagers, the chapter provides an audit workflow for **--device=dri** usage, guidance on **LIBGL_ALWAYS_SOFTWARE** and **MESA_LOADER_DRIVER_OVERRIDE** for CI, and comparison with **Snap**'s **AppArmor**-profile-based `hardware-observe`/`opengl` interface model.
