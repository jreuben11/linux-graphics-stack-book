...
1. [Design Philosophy and Historical Context](#1-design-philosophy-and-historical-context)
   ...
   - [1.3 What is the Wayland Wire Protocol?](#13-what-is-the-wayland-wire-protocol)
...
Wayland is an inter-process communication protocol and library that defines how graphical application clients communicate with a display compositor on Linux. Unlike the X Window System, which relied on a centralised server to both manage windows and execute drawing commands on behalf of clients, Wayland assigns rendering responsibility entirely to the client: each application renders its scene into a GPU-allocated buffer and hands that buffer to the compositor, which blends all client buffers into a final scene and drives the display hardware. The protocol is defined in XML files maintained in the `wayland-protocols` repository and compiled by the `wayland-scanner` code-generation tool into typed C headers and implementation stubs. The core protocol, versioned and frozen since Wayland 1.0, covers surface creation, buffer attachment, input event delivery, and output advertisement. Capabilities beyond the core — window management, HDR colour management, GPU buffer sharing, and explicit GPU synchronisation — are provided through separately versioned protocol extensions. The reference implementation of the Wayland protocol transport lives in `libwayland-client` and `libwayland-server`, which handle serialisation and deserialisation over a Unix domain socket without any knowledge of rendering APIs or display hardware. Because the protocol is transport-agnostic with respect to GPU operations, the same Wayland session can mix clients using OpenGL via EGL, Vulkan via `VK_KHR_wayland_surface`, or software rendering via `wl_shm`, as long as they all speak the same wire protocol over the same Unix socket. [Source](https://wayland.freedesktop.org/docs/html/)

### 1.2 What is a Wayland Compositor?

A Wayland compositor is the privileged process that owns the display hardware and acts as the sole authority over what appears on screen. It simultaneously fulfils three roles: display server (accepting connections from client applications over a Unix domain socket), compositing engine (blending all client surface buffers into a final scene), and KMS driver client (submitting the composited scene to the kernel's Direct Rendering Manager subsystem via `DRM_IOCTL_MODE_ATOMIC` atomic commits on `/dev/dri/cardN`). In contrast to the X Window System, where a separate compositing manager could be added or removed at runtime, in Wayland the compositor is not optional — it is the only process with the authority to drive the KMS scanout plane, so compositing is always active. The compositor controls input routing: it reads events from `evdev` input device nodes through libinput and delivers them to the client whose surface currently holds keyboard or pointer focus. Access to Wayland is controlled by filesystem permissions: only processes belonging to the session's user can open the compositor's socket, which appears under `$XDG_RUNTIME_DIR/wayland-0` by convention. The compositor also enforces the security model described in Section 10 — clients cannot observe each other's surfaces or intercept each other's input without explicit protocol-mediated mechanisms such as `zwlr_screencopy_manager_v1`. Production compositors on Linux include GNOME Shell (backed by Mutter), KDE Plasma (backed by KWin), sway (using `wlroots`), and `weston`, the reference implementation maintained by the Wayland project. [Source](https://gitlab.freedesktop.org/wlroots/wlroots)

### 1.3 What is the Wayland Wire Protocol?

The Wayland wire protocol is the binary message format carried over a Unix domain socket between client and compositor. It defines how method calls (called "requests" when sent from client to compositor) and notifications (called "events" when sent from compositor to client) are encoded as streams of 32-bit words. Every message begins with an 8-byte header encoding the target object's integer ID and a packed word containing the message size and opcode. Arguments follow the header, padded to 32-bit alignment according to type codes defined in the XML protocol schema. The most distinctive feature of the wire protocol is its handling of file descriptors: GPU buffer handles, DMA-BUF file descriptors, and synchronisation object handles are passed out-of-band using the Unix socket's `SCM_RIGHTS` ancillary data channel, enabling zero-copy buffer sharing — the client never copies GPU memory across address spaces. The protocol is stateful and ordered: the compositor processes requests from a given client in the order they arrive, and events reach the client in the order the compositor emits them. The `wayland-scanner` tool translates XML protocol definitions into typed C functions that call `wl_proxy_marshal_flags` (client side) or `wl_resource_post_event` (server side), making protocol version mismatches detectable at compile time rather than at runtime. Setting `WAYLAND_DEBUG=1` in the process environment enables libwayland's built-in protocol trace, printing every request and event with timestamps to stderr. Full wire-protocol details are covered in Section 3. [Source](https://wayland.freedesktop.org/docs/html/ch04.html)
