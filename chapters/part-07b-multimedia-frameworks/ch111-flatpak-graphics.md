...
- **CDI adoption in Flatpak**: The Container Device Interface (§10.2) is being evaluated as a replacement for the ad-hoc `--device=dri` + GL extension mechanism for GPU compute workloads (ROCm, CUDA), with initial prototypes targeting `/dev/kfd` and `/dev/nvidia-uvm` device injection via standard CDI JSON specs rather than per-format custom grants.
- **xdg-desktop-portal adoption in Snap**: Snap's `snapd-desktop-integration` daemon is incrementally implementing xdg-desktop-portal D-Bus interfaces (FileChooser, ScreenCast, Camera) alongside its existing `snapd`-native equivalents, reducing the Snap/Flatpak portal API surface divergence.
- **`nix-flatpak` stabilisation**: The NixOS `nix-flatpak` module (§10.3) is maturing toward inclusion in `nixpkgs` proper, with declarative remote management, per-app override support, and `systemd` activation ordering for reliable boot-time Flatpak state convergence.
...
- **`flatpak-builder` OCI image output**: Alexander Larsson's work toward `flatpak-builder` emitting standard OCI images in addition to OSTree commits (§10.2) would allow Flatpak applications to be stored, inspected, and distributed via container registries (`ghcr.io`, `quay.io`) while retaining full portal integration — collapsing the distinction between a Flatpak app and an OCI container for distribution purposes.
- **`systemd-sysext` for vendor GPU drivers**: GPU hardware vendors (Mesa packagers, NVIDIA, AMD) are evaluating `systemd-sysext` (§10.6) as a distribution channel for signed GPU driver overlay images that can be applied to any immutable OSTree-based system — eliminating the need for distribution-specific DKMS packaging or Flatpak GL extension rebuilds on driver updates.
- **Universal Blue / image-based Linux mainstream**: The Fedora Atomic + Universal Blue pattern (§10.4) of shipping GPU drivers in the base OSTree image is expected to spread across more distributions as `bootc` (image-based container OS tooling) matures, making the immutable OS + Flatpak app split the default Linux desktop deployment model.

### Long-term

- **Full GPU virtualisation without host driver bleed**: The logical endpoint of the Venus/virglrenderer approach (§10.5) is a Flatpak sandbox that never loads host userspace GPU driver code at all — the Mesa inside the runtime is the sole userspace driver, and it communicates to the host GPU exclusively through a stable, versioned virtio-gpu protocol. This would make the GL extension mechanism obsolete and remove the NVIDIA strict-ABI problem entirely. [Source](https://blog.sebastianwick.net/posts/flatpak-graphics-drivers/)
...
- **CDI as the universal GPU injection standard**: The long-term endpoint of the OCI/CDI convergence (§10.2) is a single `DeviceSpec` JSON file per GPU vendor that any container runtime — Flatpak, Snap, podman, `systemd-nspawn`, or a bare `bwrap` invocation — consults to inject the correct device nodes and userspace libraries. Format-specific GL extension, graphics-core content Snap, and nixGL workarounds all become CDI consumers rather than independent implementations.
- **Nix as the reproducibility substrate for all formats**: As `nix-flatpak`, `nixpkgs-wayland`, and NixOS GPU packaging mature, Nix's purely functional dependency model may become the preferred way to *build and pin* application bundles even when the *runtime delivery* uses Flatpak or OCI — separating the concerns of reproducible builds (Nix's strength) from sandboxed runtime execution (Flatpak/OCI's strength). Note: needs verification for ecosystem uptake timeline.
