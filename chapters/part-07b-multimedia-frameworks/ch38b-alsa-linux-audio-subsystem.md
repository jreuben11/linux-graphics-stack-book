...
- [11a. Real-Time Scheduling and rtkit](#11a-real-time-scheduling-and-rtkit)
...
## 11a. Real-Time Scheduling and rtkit

Period underruns (§11.6) are frequently a scheduling problem rather than a buffering one: the audio thread is ready to fill the ring buffer but the kernel scheduler does not run it before the next period interrupt. ALSA itself has no opinion on thread priority — that responsibility sits entirely in userspace, split across the kernel's preemption model, an unprivileged-RT-grant broker, and (on sandboxed desktops) a portal proxy in front of that broker.

**PREEMPT_RT and why xrun-free low-latency audio depends on it.** On a stock (non-RT) Linux kernel, spinlock-held sections and hardirq handlers run with preemption disabled, so even a `SCHED_FIFO` audio thread can be blocked for the duration of an unrelated critical section. PREEMPT_RT closes that gap by replacing most spinlocks with a preemptible, priority-inheriting `rtmutex` implementation and by forcing interrupt handlers to run threaded: the hardirq's top half only acknowledges the interrupt and wakes a kernel thread, and the actual handler body runs in process context where the scheduler can preempt it in favor of a higher-priority audio thread. The kernel remains fully preemptible except for a small set of paths — entry code, the scheduler itself, and the lowest-level interrupt plumbing. PREEMPT_RT was merged into mainline as a selectable `CONFIG_PREEMPT_RT` build (not a standing default) for x86, x86-64, ARM64, and RISC-V in the 6.12 development cycle, closing out roughly two decades of the realtime patch set living out-of-tree. [Source — Linux kernel real-time preemption theory of operation](https://docs.kernel.org/core-api/real-time/theory.html) [Source — How realtime kernels differ](https://docs.kernel.org/core-api/real-time/differences.html)

Threaded IRQ handling matters directly to audio-interface drivers: a USB audio interface's completion-URB handler and a PCIe/HDA controller's period-interrupt handler both become preemptible kernel threads under PREEMPT_RT, and their thread priority (set via `chrt` on the `irq/N-*` kernel thread, or by driver-supplied defaults) determines whether an interrupt handler can starve the very audio thread it's meant to wake. Most desktop distributions do not ship a `PREEMPT_RT` kernel by default; `CONFIG_PREEMPT_VOLUNTARY` or `CONFIG_PREEMPT` (a lower-latency but non-deterministic preemption model) is the common case, which is sufficient for the sub-20ms latencies PipeWire targets for consumer use but is not the bound a broadcast or pro-audio DAW workload needs.

**rtkit as the unprivileged RT-grant broker.** Historically, obtaining `SCHED_FIFO`/`SCHED_RR` required `CAP_SYS_NICE` or an elevated `RLIMIT_RTPRIO` — both effectively root-equivalent for scheduling purposes, which is unworkable for a desktop session's audio graph. `rtkit-daemon` is a D-Bus system service that grants bounded realtime privilege to unprivileged threads on request, over the `org.freedesktop.RealtimeKit1` interface (`/org/freedesktop/RealtimeKit1`), primarily via `MakeThreadRealtime(u64 thread_id, u32 priority)` (thread IDs are kernel TIDs from `gettid()`, not `pthread_t` handles) and `MakeThreadHighPriority` for nice-level requests. Before granting, the daemon enforces its own ceilings — a maximum realtime priority, a minimum nice level, a per-thread `RLIMIT_RTTIME` cap, and rate limits on the number of realtime grants per user, per process, and per time window — so that a runaway or malicious `SCHED_FIFO` thread cannot lock up a CPU indefinitely; the `RLIMIT_RTTIME` cap in particular exists because a starved realtime thread that never blocks would otherwise never be preempted at all. Threads are granted with `SCHED_RESET_ON_FORK` set so the privilege does not leak to children. [Source — rtkit-daemon.c](https://github.com/heftig/rtkit/blob/master/rtkit-daemon.c) [Source — Grokipedia rtkit summary, cross-checked against rtkit-daemon.c constants](https://grokipedia.com/page/rtkit)

**The portal Realtime interface** (`org.freedesktop.portal.Realtime`, part of `xdg-desktop-portal`) sits in front of rtkit for sandboxed applications. It exposes an analogous pair of methods, `MakeThreadRealtimeWithPID(t process, t thread, u priority)` and `MakeThreadHighPriorityWithPID(t process, t thread, i priority)`, plus read-only `MaxRealtimePriority`, `MinNiceLevel`, and `RTTimeUSecMax` properties mirroring rtkit's own ceilings. The portal proxies the request to rtkit rather than replacing it, remapping PIDs across the sandbox's PID namespace — necessary because a Flatpak-confined app's view of its own TID does not match the TID the host-side scheduler needs — but it enforces no looser a policy than rtkit itself does. [Source — xdg-desktop-portal Realtime interface docs](https://flatpak.github.io/xdg-desktop-portal/docs/doc-org.freedesktop.portal.Realtime.html)

**JACK and PipeWire** both depend on one of these two paths for their audio-graph processing thread: PulseAudio and early PipeWire versions call `rtkit-daemon` directly, while PipeWire 1.0+ prefers the portal path when a portal session is available, falling back to direct rtkit access otherwise (see §16.7 for the version-specific rollout and the MIDI 2.0 UMP interaction). JACK has called into rtkit since `jackd` grew `--realtime` support without requiring a setuid binary or `/etc/security/limits.d` RT entries.

**Diagnosing priority-inversion and RT-grant failures.** When a client cannot obtain realtime priority, `rtkit-daemon` logs the refusal (and its reason — rate limit, per-user thread cap, or priority above the configured ceiling) to the system journal (`journalctl -u rtkit-daemon` or `--user` depending on distro packaging); a JACK/PipeWire client that silently falls back to `SCHED_OTHER` is the most common visible symptom, and shows up as the crackling/xrun pattern in §11.6. `chrt -p <tid>` confirms the actual policy and priority a thread ended up with after negotiation, independent of what the application requested. `ulimit -r` (or `prlimit --rttime`) reports the process's own `RLIMIT_RTTIME` ceiling, which is distinct from — and typically tighter than — the daemon-side `RTTimeUSecMax` a grant is capped at; a thread that runs at `SCHED_FIFO` without ever blocking for longer than that window will be forcibly demoted by the kernel rather than by rtkit. Threaded-IRQ priority inversion (an interrupt thread scheduled below the audio thread it should be waking) is diagnosable with `chrt -p` against the relevant `irq/N-*` kernel thread alongside the audio thread's own priority.
