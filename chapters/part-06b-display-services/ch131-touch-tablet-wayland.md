...
This chapter covers each layer of the chain in depth:

- **Section 2** — the Linux kernel HID and evdev models
- **Section 3** — libinput's tablet and touch event API
- **Section 4** — libwacom for device capability querying
- **Section 5** — the `wl_touch` core protocol for touch surfaces
- **Section 6** — the `zwp_tablet_manager_v2` extension for professional stylus input
- **Section 7** — wlroots compositor implementation
- **Section 8** — how creative applications (GIMP, Krita, Inkscape) consume tablet events on Wayland
...
Wayland separates touch and stylus input into two distinct protocol namespaces. Touch surfaces use `wl_touch`, the core Wayland protocol object obtained from a `wl_seat`; it delivers per-contact down, up, motion, and cancel events identified by integer slot IDs, with optional shape and orientation extensions added in Wayland core version 6. Professional drawing tablets are handled by the `zwp_tablet_manager_v2` extension, defined in `wayland-protocols` under `unstable/tablet/tablet-unstable-v2.xml`. This extension exposes tablets as three kinds of Wayland objects:

- **`zwp_tablet_v2`** — the tablet device itself
- **`zwp_tablet_tool_v2`** — individual tools (pen, eraser, airbrush, mouse, lens cursor), with normalised axes for pressure, tilt, rotation, distance, and slider
- **`zwp_tablet_pad_v2`** — the physical pad, with rings, strips, and button-mode groups

Compositors implement these protocols by forwarding libinput tablet and touch events to the Wayland objects bound by each client. Applications reach this data through toolkit abstractions — GTK4 `GdkDeviceTool`, Qt `QTabletEvent` — rather than calling the Wayland protocol objects directly.
