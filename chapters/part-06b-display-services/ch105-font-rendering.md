...
FreeType2 is the open-source C library that converts a font file's vector outline data for a given glyph into a pixel bitmap. It sits at the rasterization layer of the Linux text pipeline: given a font file path, a glyph ID, a pixel size, and a rendering mode, FreeType2 reads the relevant font table, optionally applies hinting transformations (either TrueType bytecode or the auto-hinter), and returns an 8-bit grayscale or three-channel subpixel bitmap. FreeType2 does not perform text layout, codepoint-to-glyph mapping for complex scripts, or font discovery — those responsibilities belong to HarfBuzz and fontconfig respectively. Its scope is deliberately narrow: take a glyph ID and produce pixels. On Linux, FreeType2 underpins virtually every text-rendering path outside the browser's own engine. GTK uses it through Pango and Cairo; KDE uses it through Qt's internal shaping stack; terminal emulators use it directly or through libraries such as libvte. It supports TrueType quadratic outlines, CFF cubic outlines, bitmap formats (PCF, BDF), and the color emoji extensions (CBDT/COLR). The library is configured at build time and at runtime through render mode flags that control hinting strength, gamma correction, and subpixel order. [Source: FreeType2 Design](https://freetype.org/freetype2/docs/design/design-1.html)

### 1.2 What is HarfBuzz?

HarfBuzz is the open-source text shaping engine used throughout the Linux graphics stack. Text shaping is the process of converting a sequence of Unicode codepoints, a font face, and a set of OpenType feature tags into a sequence of glyph IDs with precise x/y advance and offset values for each glyph. For Latin scripts the mapping is nearly one-to-one, but for Arabic, Devanagari, Hebrew, Tamil, and many other scripts, shaping is mandatory: the correct visual glyph depends on neighboring characters, script directionality, and font-specific substitution tables. HarfBuzz reads the font's GSUB (glyph substitution) and GPOS (glyph positioning) tables directly from the font binary and applies their rules to produce the final glyph sequence. It also handles bidirectional text reordering in cooperation with Unicode's BiDi algorithm, though it relies on the caller (Pango or equivalent) to first segment text by script and direction. HarfBuzz is used directly by Pango, Qt, LibreOffice, Cairo, and several terminal emulators. In Chromium, HarfBuzz replaced the Pango-based shaping stack around 2013. The library exposes a stable C API centered on `hb_buffer_t` (the input/output glyph sequence) and `hb_font_t` (the font object). [Source: HarfBuzz Documentation](https://harfbuzz.github.io/)

### 1.3 What is fontconfig?

fontconfig is the library and configuration system that resolves an abstract font description — a family name, weight, slant, and size — into a concrete file path and face index on disk. Without fontconfig, every application would need to hardcode font paths or implement its own font-discovery logic, and there would be no mechanism for users to express systemwide preferences or aliases. fontconfig scans directories such as `/usr/share/fonts/` and `~/.local/share/fonts/`, reads metadata from each font file, and stores the result in a cache under `/var/cache/fontconfig/`. When an application requests a font by description, fontconfig applies substitution rules from its configuration files (in `/etc/fonts/conf.d/`) and then scores all cached fonts by a priority-weighted distance function to find the best match. It also implements generic family aliases (`sans-serif`, `serif`, `monospace`) that map to locale-appropriate fallback lists. The configuration syntax is XML; users can add rules in `~/.config/fontconfig/fonts.conf` to override system defaults, pin specific fonts, or define fallback chains for scripts that the primary font does not cover. Both GTK/Pango and Qt use fontconfig for font enumeration on Linux, and it is the mechanism by which applications respond correctly to user-level font substitution preferences without restarting. [Source: fontconfig Developer Reference](https://fontconfig.pages.freedesktop.org/fontconfig/fontconfig-devel/)

### 1.4 What is Pango?

Pango is the paragraph-level text layout library used by GTK and the broader GNOME ecosystem. While HarfBuzz shapes individual runs of text in a single script and direction, Pango operates at a higher level: it takes a string of Unicode text, breaks it into runs by script, language, and font, invokes HarfBuzz to shape each run, applies the Unicode Bidirectional Algorithm (via the Fribidi library), and wraps the result into lines at a specified width. Pango exposes this functionality through `PangoLayout`, a stateful object that accepts text, a `PangoFontDescription`, and layout parameters (width, wrap mode, ellipsization) and produces a fully laid-out sequence of glyphs ready for compositing. Applications typically reach Pango through GTK's text widgets and label system, though direct use via `pango_layout_new()` is straightforward. Pango interfaces with fontconfig for font selection, HarfBuzz for shaping, and Cairo or Skia for compositing the result. The `pango_cairo_show_layout()` function bridges Pango and Cairo, rendering a fully laid-out `PangoLayout` onto a Cairo surface in a single call. Pango also supports OpenType font features and variable fonts through the `PangoAttrFontFeatures` attribute and related `PangoFontDescription` variation API. [Source: Pango Documentation](https://docs.gtk.org/Pango/)
