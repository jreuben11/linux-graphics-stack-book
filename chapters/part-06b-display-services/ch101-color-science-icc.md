...
Color science solves this by defining device-*independent* reference spaces through which colors pass on their way between devices. This chapter works through each layer in that pipeline — the science, the data formats, the APIs, and their Linux-specific plumbing:

- **CIE XYZ** — the device-independent reference space the International Commission on Illumination (CIE) defined from human psychophysical experiments in 1931
- **The ICC profile format** — codified by the International Color Consortium (ICC) to characterize each device relative to CIE XYZ
- **LittleCMS (lcms2)** — a software color management engine that applies ICC profiles to transform pixels between device spaces
- **The Linux graphics stack** — from KMS color pipeline properties through colord to compositor protocols, carrying this metadata from hardware measurement to final display
...
From CIE XYZ, the CIE derived additional spaces suited to different tasks:

- **xy chromaticity diagram** — projects XYZ to a 2D plane, enabling visualization of color gamuts as triangles whose vertices are the xy coordinates of device primaries
- **CIELAB (L\*a\*b\*)** — applies a non-linear transform to XYZ to produce a more perceptually uniform space in which equal numerical distances correspond more closely to equal perceived color differences, a property raw XYZ lacks; this uniformity makes Lab the practical working space for color difference measurement (the ΔE metric) and gamut-mapping algorithms
- **ICC Profile Connection Space (PCS)** — defined in CIE XYZ, or equivalently in CIELAB, relative to the D50 reference illuminant, making CIE spaces the language that every ICC-compliant color management engine speaks

### 1.3 What is an ICC Profile?

An ICC (International Color Consortium) profile is a binary file that characterizes a color device or color space by describing the mathematical relationship between that device's native values and the CIE XYZ reference space, defined by the ICC specification (currently ICC.1:2022, [icc.color.org](https://www.color.org/specification/ICC.1-2022-05.pdf)). A profile begins with a 128-byte header identifying the profile class (display, printer, scanner, or abstract), the device's native color space, and the rendering intents it supports. The body consists of a tag table followed by tag data elements carrying the actual colorimetric data:

- **Primary chromaticity** — `rXYZ`, `gXYZ`, `bXYZ` tags
- **Tone response curves** — `rTRC`, `gTRC`, `bTRC` tags
- **Multidimensional lookup tables** — `A2B` and `B2A` tags, for complex device behaviors that cannot be expressed as a simple matrix
...
- **Matrix-shaper profiles** — for simple display characterizations
- **CLUT-based A2B/B2A transforms** — for print devices with non-linear gamut behavior
- **Device-link profiles** — chain multiple transforms into a single optimized operation

In the Linux graphics stack, lcms2 sits behind several major applications and daemons:

- **GIMP, Inkscape, darktable, LibreOffice** — image and publishing applications that use lcms2 to transform pixels between working and output color spaces
- **Ghostscript** — uses lcms2 in its print pipeline
- **colord** — uses lcms2 itself for verifying profile consistency

The lcms2 library exposes a C API whose central types — `cmsHPROFILE`, `cmsHTRANSFORM`, and `cmsColorSpaceSignature` — map directly to ICC profile concepts. A compositor that needs to apply a monitor profile to composited output either uses lcms2 to build a software transform, or extracts the profile's VCGT data and loads it into the KMS display controller's hardware gamma LUT for zero-overhead per-scanout correction. Section 5 of this chapter covers the lcms2 API in detail.
