...
- [1. The Fragmentation Problem](#1-the-fragmentation-problem)
  ...
  - [1.4 What is the Wayland Protocol Extension System?](#14-what-is-the-wayland-protocol-extension-system)
  - [1.5 What is Explicit GPU Synchronisation?](#15-what-is-explicit-gpu-synchronisation)
  - [1.6 What is Mesa NIR?](#16-what-is-mesa-nir)
...
DMA-BUF (Direct Memory Access Buffer) is a Linux kernel mechanism for sharing GPU and hardware buffer objects across process and driver boundaries using standard Unix file descriptors. Introduced in Linux 3.3, it replaced the prior per-driver approach in which every cross-process buffer transfer required a CPU copy. The kernel allocates a contiguous or scatter-gather memory region — backed by a DRM GEM buffer object — and wraps it in a file descriptor. That file descriptor can be passed over Unix sockets (and therefore over the Wayland protocol socket) to any process that needs to read from or write to the same physical memory. Each consumer opens the fd through its own driver interface (`drmPrimeFDToHandle()`, `eglCreateImageKHR(EGL_LINUX_DMA_BUF_EXT)`, or `vaImportSurfaceHandle()`) without copying data; the kernel reference-counts the allocation and frees it when the last fd holder closes. DMA-BUF is the primitive that makes zero-copy paths in the Linux graphics stack possible: an application renders into a GBM buffer object, exports it as a DMA-BUF fd, and delivers it to the compositor via `wp_linux_dmabuf_v1`. The compositor imports it into its own GL or Vulkan texture unit without a copy, and the kernel DRM display engine can scan it out directly from GPU-resident memory. Alongside the fd, a 64-bit **DRM format modifier** token describes the buffer's tiling and compression layout so that the importing driver knows how to read the memory correctly. [Source: https://www.kernel.org/doc/html/latest/driver-api/dma-buf.html]

### 1.4 What is the Wayland Protocol Extension System?

The Wayland display protocol is intentionally minimal: the core protocol (`wayland.xml`) defines surfaces, input events, and output configuration at the most primitive level and deliberately omits buffer format negotiation, GPU synchronisation, color management, and frame timing. These are left to an open extension mechanism. A Wayland protocol extension is an XML file that defines new interfaces, requests, and events layered on top of the core protocol wire format. The `wayland-protocols` repository hosts the canonical set of extensions ratified by the Wayland community, organised into three maturity tiers: `stable` (frozen ABI, safe for production use), `staging` (implemented in at least two compositors and two clients, candidate for stabilisation), and `unstable` (early drafts subject to change). In the cross-stack integration context, this extension mechanism is the primary vehicle through which layer-boundary contracts are standardised. When GPU driver developers, Mesa engineers, compositor authors, and toolkit maintainers need a new agreed protocol — for zero-copy buffers (`wp_linux_dmabuf_v1`), explicit GPU fences (`wp_linux_drm_syncobj_v1`), HDR metadata (`wp_color_management_v1`), or frame pacing (`wp_fifo_v1`) — they draft, review, and ratify a protocol extension. The resulting XML document becomes the specification; each implementation (Mesa WSI, KWin, Mutter, GTK, Qt) is developed independently but to the same wire format. This decoupled approach is what allows independent communities within the Linux graphics ecosystem to reach interoperability without requiring code-level coupling between components. [Source: https://gitlab.freedesktop.org/wayland/wayland-protocols]

### 1.5 What is Explicit GPU Synchronisation?

Explicit GPU synchronisation is a programming model in which the producer of a GPU buffer explicitly exports a synchronisation primitive — a fence or timeline semaphore — that signals when the GPU has finished writing, and passes that primitive directly to the consumer. The consumer then waits on the primitive at the GPU level, enabling the CPU to return immediately rather than blocking. The alternative — implicit synchronisation — relies on the kernel's internal fence propagation between drivers, which works reliably only when both producer and consumer share the same GPU and the driver exposes implicit fence semantics to compositors. On multi-GPU configurations and on hardware whose driver did not propagate implicit fences through the Wayland path, compositors had to insert explicit CPU stalls (`glFinish()`, `eglClientWaitSync()`) at every frame boundary, serialising what should be pipelined work. The Linux kernel primitive for explicit synchronisation is the **DRM timeline syncobj**: a monotonically-incrementing counter whose GPU-signalled points can be exported as file descriptors via `DRM_IOCTL_SYNCOBJ_HANDLE_TO_FD`. The Wayland protocol extension `wp_linux_drm_syncobj_v1` carries acquire and release timeline points alongside each `wl_surface_commit`, allowing the compositor to schedule its own GPU operations to start precisely after the producer's release point without any CPU involvement. This enables a fully pipelined frame delivery path in which CPU and GPU work overlap across producer and consumer boundaries. [Source: https://www.kernel.org/doc/html/latest/gpu/drm-mm.html#drm-sync-objects]

### 1.6 What is Mesa NIR?

Mesa NIR (New Intermediate Representation) is a typed, SSA-form shader intermediate representation used inside Mesa to unify GPU driver compilation across the many hardware backends Mesa supports. Before NIR, each Mesa GPU driver maintained its own compiler IR — TGSI (for Gallium-based drivers) and various private forms for other hardware — leading to duplicated optimisation passes, inconsistent compilation behaviour, and high per-driver maintenance cost. NIR defines a common IR that shader compilation frontends (GLSL, SPIR-V, OpenCL C, and others) lower into, and that hardware backends consume. All driver-independent optimisation passes — algebraic simplification, dead code elimination, constant folding, loop unrolling, divergence analysis, and more — are implemented once in `src/compiler/nir/` in the Mesa source tree and shared by every driver. In the cross-stack integration context, NIR addresses a parallel fragmentation problem at the shader compilation layer that mirrors the fragmentation at the display protocol layer: just as the Wayland protocol extension system provides a common coordination layer for the display and buffer path, NIR provides a common compilation layer for the GPU path. A new Mesa GPU driver does not need to implement a full optimising compiler; it writes a backend that lowers NIR to hardware machine code and inherits the shared front-end and middle-end optimisation infrastructure. Cross-driver GPU features — shader caching, ray tracing support, mesh shaders — can be implemented once in NIR and reach all compliant backends. [Source: https://gitlab.freedesktop.org/mesa/mesa/-/tree/main/src/compiler/nir]
