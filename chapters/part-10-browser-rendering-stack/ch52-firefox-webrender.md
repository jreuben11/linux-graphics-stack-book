...
> **What is servoshell?** `ports/servoshell/` is Servo's own demo browser — the application the Servo team uses to test and showcase the engine. It implements `WindowMethods` using winit and surfman, presents a minimal browser UI (address bar, back/forward), and is the primary target for Servo's automated WPT test runs. It is **not** an embeddable library: there is no stable public API for external crates to depend on it. The analogy to Chromium is `//chrome` (the browser binary) vs `//content` (the embeddable content API): servoshell is `//chrome`, `servo::Servo<Window>` is `//content`. **Verso** (see below) is what fills the embedding-library role that servoshell does not.
...
- **Unified wgpu render path for WebRender primitives**: An architectural direction discussed informally in the Mozilla graphics team is whether WebRender's own primitive batching (currently OpenGL draw calls) could be re-expressed as wgpu render passes, unifying the rendering stack under a single GPU abstraction and eliminating the dual OpenGL/wgpu code paths. (Note: needs verification — speculative direction, no public design document confirmed.) The Servo project tracks two approaches to this in [servo#37149](https://github.com/servo/servo/issues/37149): (a) a wgpu port of WebRender's `Device` struct, and (b) a renderer trait abstraction allowing pluggable backends such as **Vello**.

  > **What is Vello?** [Vello](https://github.com/linebender/vello) is a 2D vector graphics renderer written in Rust by the [Linebender project](https://linebender.org/) (initiated by former Google Fonts engineers). Its defining property is that it performs **all 2D rendering — curves, fills, strokes, gradients, text — entirely in GPU compute shaders**, compiled through wgpu. On Linux, wgpu defaults to the Vulkan backend, so Vello renders via Vulkan with no OpenGL dependency. The compute-shader approach (a "sort-middle" architecture using prefix-sum algorithms for coverage) contrasts with WebRender's draw-call batching: rather than issuing many `glDraw*` calls each covering a rectangular region, Vello dispatches a small number of large compute passes that process the entire scene. Vello is already in production use as the renderer for [Xilem](https://github.com/linebender/xilem) (Linebender's Rust GUI framework) and [Bevy's 2D rendering path](https://bevyengine.org/). If Servo adopted Vello as a WebRender replacement, the result would be a fully Rust, fully Vulkan 2D rendering stack with the same wgpu abstraction that Servo already uses for WebGPU — eliminating the OpenGL dependency in both the 2D compositor and the WebGPU path simultaneously.
