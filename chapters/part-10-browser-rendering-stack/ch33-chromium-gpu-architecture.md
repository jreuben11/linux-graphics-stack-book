...
GPU feature detection—querying `GL_RENDERER`, `GL_VENDOR`, **`VkPhysicalDeviceProperties`**, and PCI device IDs at process startup—drives selection between the **Vulkan** rendering backend, the **ANGLE**-backed **GL** backend (`--use-gl=angle`), the native **Mesa** **EGL** backend (`--use-gl=egl`), and the **SwANGLE** software fallback backed by **SwiftShader**. The `gpu_driver_bug_list.json` database maps hardware-and-driver conditions to workaround flags that influence **ANGLE**'s behaviour, the raster decoder's path, and whether hardware-accelerated video decode is permitted; `chrome://gpu` exposes the full resolved state.

The display compositor component — **Viz** (**`viz::DisplayCompositor`**) — runs inside the **GPU process** under the **OOP-D** architecture (enabled by default since Chrome 71). **Viz** receives **`viz::CompositorFrame`** submissions from all renderer processes and the browser UI, aggregates them via **`viz::FrameSinkManager`**, and presents the final frame through the **Ozone** output surface. The **`BeginFrameSource`** driven by **`wp_presentation`** feedback events ties the frame cadence to the Wayland compositor's vsync signal. **OOP-R** (Out-of-Process Raster), enabled since approximately Chrome 79, complements **OOP-D** by performing tile rasterisation in the **GPU process** using **Skia**'s **Ganesh** backend rather than in renderer processes.

The GPU process lifecycle begins at the **`GpuMain`** entry point in `gpu/ipc/service/gpu_main.cc`. **`GpuInit::InitializeAndStartSandbox()`** performs feature detection and installs the **`seccomp-BPF`** policy; then **`GpuServiceImpl`** and **`GpuChannelManager`** are created, and the **`VizCompositorThreadRunner`** starts the **Viz** compositor thread. A **`GpuWatchdogThread`** monitors the main GPU thread and sends **`SIGABRT`** on timeout to convert driver deadlocks into recoverable crashes. GPU process death is detected synchronously via **Mojo** pipe closure, triggering a browser-side relaunch and `webglcontextlost` DOM events for affected **WebGL** contexts.
