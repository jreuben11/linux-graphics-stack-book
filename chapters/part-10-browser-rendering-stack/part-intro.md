...
**Chapter 203 — WebXR: Browser-Based Immersive Experiences on Linux** explains how the **WebXR Device API** (W3C Living Standard) maps JavaScript VR/AR sessions onto the native **OpenXR** runtime — **Monado** or **SteamVR** — through the same multi-process Mojo IPC architecture that the rest of Chromium uses. The chapter covers all three session modes (`inline`, `immersive-vr`, `immersive-ar`), the `XRSession.requestAnimationFrame` frame loop and its relationship to `XRViewerPose`/`XRView`/`projectionMatrix`, reference spaces and their `XrReferenceSpaceType` equivalents, `XRInputSource` controller mapping via **`GetOpenXrInputProfilesMap`**, and `XRHand` 26-joint skeleton via `fillJointRadii`/`fillPoses`. The rendering path is examined at two levels: the legacy `XRWebGLLayer` (an **ANGLE** framebuffer sharing pixels with the OpenXR swapchain via `gpu::SharedImage`) and the more efficient **WebXR Layers API** (`XRProjectionLayer`, `getViewSubImage`) that gives the application direct swapchain image access. The chapter traces Chromium's process model — renderer → browser → `isolated_xr_device` service — through the key Mojo interfaces (`VRService`, `XRFrameDataProvider`, `XRPresentationProvider`) and the `OpenXrDevice` → `OpenXrRenderLoop` → `OpenXrApiWrapper` class hierarchy. Crucially, it documents the Linux gap: Chromium's official codebase only includes D3D11 and OpenGL ES graphics bindings, leaving `XR_KHR_vulkan_enable2` unimplemented; the community `webxr-linux` patch adds the Vulkan binding and is the current path to `immersive-vr` on Linux. Firefox's implementation via `wgpu`'s Vulkan backend — which creates `XrGraphicsBindingVulkanKHR` naturally — provides `immersive-vr` on Linux without patching. A side-by-side tradeoff table compares WebXR against native OpenXR across latency, API surface, GPU path, and scene-understanding depth.
...
Chapter 203 (WebXR) presupposes Chapter 33 for the Mojo IPC and isolated-process model, Chapter 34 for ANGLE's role as the `XRWebGLLayer` backend, and Chapter 35 for the forthcoming `XRWebGPUBinding` via Dawn. It also draws on Chapter 52 (Firefox's `wgpu` Vulkan device as the Linux `XrGraphicsBindingVulkanKHR`) and connects backward to Chapter 27 (OpenXR/Monado), making it a bridge between Part X and Part VII-A.
...
graph LR
    ...
    Ch203["Ch203: WebXR\n(XRSession · XRWebGLLayer\nOpenXR backend · Linux gap\nFirefox wgpu path)"]
    ...
    Ch33 --> Ch203
    Ch34 -.->|"XRWebGLLayer\nANGLE framebuffer"| Ch203
    Ch35 -.->|"forthcoming\nXRWebGPUBinding"| Ch203
    Ch52 -.->|"Firefox wgpu\nVulkan binding"| Ch203
```
