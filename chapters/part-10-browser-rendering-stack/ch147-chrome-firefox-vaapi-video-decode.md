...
The scale argument is straightforward: a single 4K H.264 stream decoded in software consumes approximately 1.5–2 CPU cores on a modern x86-64 processor, while the same stream decoded in fixed-function GPU silicon — available on every Intel, AMD, and Apple Silicon chip shipped since around 2017 — draws roughly 200 mW of dedicated video engine power and frees those cores entirely. Multiply across the web as a distribution medium and the savings in carbon, battery life, and thermal headroom are significant. For a laptop user streaming 4K YouTube, software video decode is the difference between two hours of battery life and five.
...
Thermal throttling is a less visible but practically important reason organisations care about this at scale. A server-side transcoding or viewing cluster that soft-decodes high-resolution streams will throttle CPU frequency under sustained load, causing decode frame-rate drops that manifest as buffering stalls even at adequate network bandwidth. On client laptops, sustained CPU decode at 4K creates enough heat to trigger fan activity, reducing user experience. The industry's shift from H.264 to VP9 (YouTube 2015) and then to AV1 (YouTube/Netflix 2018–2020) has progressively increased decode complexity, making the software-decode cliff steeper with each codec generation. AV1 at 4K requires roughly 3× the CPU cycles of H.264 at the same resolution, which makes hardware acceleration not a luxury but a prerequisite for smooth AV1 4K playback on battery-powered devices.
