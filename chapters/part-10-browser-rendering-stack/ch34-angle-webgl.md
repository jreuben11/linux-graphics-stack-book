...
1. [ANGLE's Role in the Browser Stack](#1-angles-role-in-the-browser-stack)
   - [1.1 What is WebGL?](#11-what-is-webgl)
   - [1.2 What is ANGLE?](#12-what-is-angle)
   - [1.3 What is OpenGL ES?](#13-what-is-opengl-es)
...
WebGL is a JavaScript API, standardised by the Khronos Group, that exposes GPU-accelerated rendering to web pages through the HTML `<canvas>` element. It is a browser-hosted binding over OpenGL ES: WebGL 1.0 maps onto the OpenGL ES 2.0 feature set, and WebGL 2.0 maps onto OpenGL ES 3.0. Web authors access the API through `canvas.getContext('webgl')` or `canvas.getContext('webgl2')`, which returns a context object whose methods correspond one-for-one with the OpenGL ES specification, modulo a set of deliberate restrictions. Those restrictions remove features that are difficult to implement safely in a multi-origin, shared-GPU environment: synchronous queries without a timeout, client-side vertex arrays, and rendering context sharing without explicit opt-in are all excluded. Internally, Blink's `WebGLRenderingContext` and `WebGL2RenderingContext` classes validate each WebGL call and forward it over an inter-process command channel to the GPU process, which is described in Chapter 33. The GPU process calls into ANGLE's `libGLESv2` entry points. WebGL does not call Mesa or any system OpenGL library directly on Linux: the entire OpenGL ES layer is provided by ANGLE. Correctness across browsers and operating systems is measured against the Khronos WebGL Conformance Test Suite, which Chrome runs against every ANGLE commit. [Source: Khronos WebGL specification](https://www.khronos.org/webgl/)

### 1.2 What is ANGLE?

ANGLE — Almost Native Graphics Layer Engine — is an open-source C++ library that implements the OpenGL ES 2.0, 3.0, and 3.1 specifications together with EGL 1.5, and translates those API calls into the native GPU API available on the host platform. On Linux the target is Vulkan; on macOS it is Metal; on Windows it is Direct3D 11 or 12; and a software path through SwiftShader serves environments where no GPU is available. ANGLE's source lives at `https://chromium.googlesource.com/angle/angle` and is developed as part of the Chromium project with open participation. Chrome bundles and controls its own ANGLE binary rather than calling Mesa's OpenGL implementation directly, because this arrangement insulates the browser from the wide variation in Mesa version, vendor extension support, and driver correctness found across Linux distributions. Chrome's ANGLE is tested against the Khronos deqp-GLES suite and the WebGL Conformance Test Suite on every commit, providing a correctness guarantee that a system Mesa installation cannot offer. The library is used beyond Chrome: Firefox uses ANGLE on Windows for its Direct3D 11 backend and is evaluating the Vulkan backend on Linux; Flutter uses ANGLE on Android; Qt WebEngine and WebKit on non-Apple platforms also embed it. On Linux, ANGLE's Vulkan backend is the production path. Each OpenGL ES draw call passes through `ContextVk`, which records Vulkan commands against whichever Mesa Vulkan driver is present — RADV for AMD hardware, ANV for Intel, or NVK for NVIDIA. This chapter traces that path from API entry point to `vkQueueSubmit`.

### 1.3 What is OpenGL ES?

OpenGL ES (Open Graphics Library for Embedded Systems) is a subset of the OpenGL API defined by the Khronos Group, originally targeting mobile and embedded GPU environments. It removes deprecated fixed-function features from desktop OpenGL — display lists, immediate-mode rendering with `glBegin`/`glEnd`, wide lines, polygon stippling — and enforces a fully programmable shader model throughout. OpenGL ES 2.0 introduced mandatory vertex and fragment shaders written in GLSL ES, replacing the fixed-function pipeline entirely. OpenGL ES 3.0 added transform feedback, uniform buffer objects, multiple render targets, occlusion queries, instanced rendering, and a wider set of texture formats including ETC2 and EAC compressed formats. OpenGL ES 3.1 introduced compute shaders, shader storage buffer objects, atomic counter buffers, image load/store, and indirect draw commands. OpenGL ES 3.2 merged in geometry shaders, tessellation, ASTC texture compression, and several Android Extension Pack features. OpenGL ES is the API surface that ANGLE implements in its `src/libGLESv2/` entry points, and it is the API surface that WebGL is layered on top of: WebGL 1.0 corresponds to OpenGL ES 2.0 and WebGL 2.0 corresponds to OpenGL ES 3.0. Understanding which OpenGL ES features are available at each stage of ANGLE's Vulkan translation is therefore directly relevant to diagnosing WebGL rendering correctness bugs and performance issues on Linux. Conformance is validated against the Khronos deqp-GLES3 test suite. [Source: Khronos OpenGL ES registry](https://registry.khronos.org/OpenGL/index_es.php)
