...
1. [WebGPU and Dawn: The Big Picture](#1-webgpu-and-dawn-the-big-picture)
   - [1.1 What is WebGPU?](#11-what-is-webgpu)
   - [1.2 What is Dawn?](#12-what-is-dawn)
   - [1.3 What is WGSL?](#13-what-is-wgsl)
...
The WebGPU ecosystem on Linux is not a single implementation but a set of competing runtimes that converge on the same W3C specification and the same `webgpu.h` C header. The major split is between **Dawn** (Google/Chrome, C++) and **wgpu** (gfx-rs/Firefox, Rust), with ANGLE's experimental WebGPU translation path providing a fallback where Vulkan is unavailable. A fourth entry point, **wgpu-native**, exposes the wgpu Rust core to native (non-browser) C and Python applications via a stable C FFI. All four implementations share the WebGPU Conformance Test Suite (CTS) as a common quality bar.

| **Implementation** | **Language** | **Backends (Linux)** | **WGSL compiler** | **Primary consumer** | **Open source** | **Linux status (2026)** |
|---|---|---|---|---|---|---|
| Dawn (Google) | C++ | Vulkan (primary), OpenGL ES (fallback via ANGLE) | Tint (C++, AST-based, multi-target) | Chrome/Chromium; also usable standalone | Yes (BSD, Chromium project) | Shipped in Chrome; Vulkan primary path |
| wgpu (Rust) | Rust | Vulkan (wgpu-hal-vulkan), OpenGL (wgpu-hal-gles), DX12 | naga (Rust, IR-based; WGSL/GLSL/SPIR-V input) | Firefox (wgpu-core in Gecko); Bevy; standalone Rust apps | Yes (MIT/Apache 2.0) | Shipped in Firefox 131+; Bevy default |
| ANGLE WebGPU | C++ | OpenGL ES 3.1 → WebGPU translation path (experimental) | Glslang + ANGLE shader translator | Chrome on platforms where Vulkan unavailable | Yes (BSD) | Experimental fallback path |
| wgpu-native (C API) | Rust (wgpu with C headers) | Same as wgpu | naga | Native (non-browser) applications via C FFI | Yes | Stable; used by Python/C wgpu bindings |

The rest of this chapter examines **Dawn**'s internals in depth:

- **Section 2 (Dawn's Layered Architecture)** — the public **`webgpu.h`** API layer with its code-generated **`dawn.json`** scaffolding, the **`dawn::native`** implementation with its reference-counted object hierarchy (**`InstanceBase`**, **`AdapterBase`**, **`DeviceBase`**), the **`dawn::wire`** layer with **`WireClient`** and **`WireServer`**, and the **`GPUDevice.lost`** error and device-loss model.
- **Section 3 (The Vulkan Backend)** — **`DeviceVk`** initialisation (extension enumeration including **`VK_KHR_external_memory_fd`**, **`VK_KHR_external_semaphore_fd`**, **`VK_KHR_timeline_semaphore`**, and **`VK_EXT_descriptor_indexing`**; queue family selection; **`VkDevice`** creation via **`vkCreateDevice`**; **Vulkan Memory Allocator** (**VMA**) setup), **`RenderPipelineVk`** translation of **`GPURenderPipelineDescriptor`** to **`VkGraphicsPipelineCreateInfo`** with optional **`VK_KHR_dynamic_rendering`**, **`BindGroupVk`** and descriptor management via **`VkDescriptorSetLayout`** and **`VkDescriptorSet`**, **`CommandBufferVk`** render pass recording, and **`QueueVk`** submission with timeline semaphore synchronisation via **`vkQueueSubmit2`**.
- **Section 4 (Tint: The WGSL Compiler)** — the recursive-descent parser in **`src/tint/lang/wgsl/reader/`**, the two-generation architecture transitioning from an AST-based pipeline to the high-level **SSA** Tint **IR** in **`src/tint/lang/core/ir/`**, transform passes including **Robustness** (bounds-checking for security), **ZeroInitWorkgroupMemory**, **BindingRemapper**, and **VertexPulling**, **SPIR-V** emission via **`src/tint/lang/spirv/writer/`** feeding **`vkCreateShaderModule`** with optional **SPIRV-Tools** validation and optimisation, **`VkPipelineCache`**-backed shader caching, and **WGSL** as a security boundary preventing malicious shader source from reaching GPU drivers.
- **Section 5 (DawnWire IPC Protocol)** — why **Chrome**'s sandboxed renderer process cannot call **DRM** ioctls directly, the **`WireClient`**/**`WireServer`** command serialisation over a **Mojo** shared-memory **`DataPipe`**, the client-side integer object handle model with its two-layer validation, asynchronous callbacks for **`buffer.mapAsync()`**, **`createRenderPipelineAsync()`**, and **`createComputePipelineAsync()`**, and the **`wgpuQueueSubmit`** flush point.
- **Section 6 (Memory Management and Buffer Mapping)** — **`BufferVk`** wrapping **`VkBuffer`** and **`VmaAllocation`** with **`GPUBufferUsage`**-to-**`VkBufferUsageFlags`** mapping, **`GPUBuffer.mapAsync`** staging through **`HOST_VISIBLE`** memory and **`vmaMapMemory`**, the **Chrome** shared-memory path using **`mojo::SharedMemoryRegion`** and **`VkImportMemoryFdInfoKHR`** for zero-copy **`buffer.getMappedRange()`**, the **`UploadRingBuffer`** used by **`writeBuffer`** and **`writeTexture`**, and per-subresource barrier tracking in **`TextureVk`** via **`SubresourceStorage<T>`**.
- **Section 7 (WebGPU Canvas Presentation)** — **`GPUCanvasContext`** configuration, **Viz** **`SharedImage`** backing via **`VkImage`** with **`VK_EXTERNAL_MEMORY_HANDLE_TYPE_DMA_BUF_BIT_EXT`**, the **`getCurrentTexture()`** lend/return ownership cycle, sync **FD** export via **`vkGetSemaphoreFdKHR`** with **`VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_SYNC_FD_BIT`** delivering a **`gfx::GpuFence`** to the **Viz** compositor, zero-copy scanout via **Wayland** **`linux-dmabuf`**, and colour space conversion handling for **`display-p3`** canvases.
- **Section 8 (Adapter Enumeration and Feature Detection on Linux)** — **`navigator.gpu.requestAdapter()`** triggering **`vkEnumeratePhysicalDevices`** via **`InstanceBase::EnumerateAdapters`**, **`powerPreference`**-guided physical device selection between **ANV** (Intel integrated) and **RADV**/**NVK** (discrete) adapters, **`gpu_driver_bug_list.json`** blocklist gating, **`GPUAdapter.requestDevice()`** feature negotiation mapping **`GPUFeatureName`** strings to **Vulkan** device extensions and features, **`GPUSupportedLimits`** clamping to prevent GPU fingerprinting, **`GPUAdapterInfo`** coarsening, and resource sharing constraints on multi-GPU systems via **`VK_KHR_external_memory_fd`**.

### 1.1 What is WebGPU?

WebGPU is a W3C web standard that exposes a modern, explicit GPU programming model to JavaScript and WebAssembly applications running inside a browser sandbox. It replaces the stateful, implicit-synchronisation model of WebGL — which was derived from OpenGL ES 2.0 — with an API that deliberately mirrors the design of Vulkan, Metal, and Direct3D 12. In WebGPU, resources such as buffers and textures are explicitly allocated; pipeline state is compiled into immutable pipeline objects rather than accumulated through global state mutations; shader interfaces are declared through typed bind group layouts rather than implicit sampler-unit assignments; and GPU memory mapping is an asynchronous operation rather than a blocking stall. This eliminates whole categories of driver-side synchronisation guesswork that degraded WebGL performance on tiling GPU architectures.

The specification is maintained by the W3C GPU for the Web Working Group and is published at `https://www.w3.org/TR/webgpu/`. WebGPU is not a thin wrapper around any single native API; it defines its own object model — devices, queues, command encoders, render passes, compute passes, bind groups, and pipelines — that can be translated faithfully to Vulkan, Metal, or Direct3D 12 without driver-specific hacks. On Linux, the translation target is always Vulkan, mediated through Dawn (Chrome) or wgpu (Firefox). Security is a first-class concern: all shader source submitted as WGSL is validated and recompiled by the browser before it reaches the GPU driver, preventing malformed shader code from triggering driver vulnerabilities. This chapter examines how the WebGPU object model is translated to Vulkan commands through Dawn, and how Chrome's sandbox boundary is enforced across that translation.

### 1.2 What is Dawn?

Dawn is the C++ implementation of the WebGPU API used by Chromium and Chrome across all platforms. It is open source under the BSD license and lives at `https://dawn.googlesource.com/dawn` (mirrored at `https://github.com/google/dawn`). Dawn implements the `webgpu.h` C header — the stable, specification-derived API surface that corresponds one-to-one with the JavaScript WebGPU IDL. Applications can also use Dawn as a standalone library for native GPU programming without committing to Vulkan-specific idioms, making it a portable abstraction layer over Vulkan, Metal, and Direct3D 12.

On Linux, Dawn's sole production backend is Vulkan. The library is structured in layers: the public `webgpu.h` API dispatches through a proc table (`dawn_proc`) to either `dawn::native` (direct in-process execution) or `dawn::wire` (serialised IPC), depending on the runtime context. In Chrome, the sandboxed renderer process holds a `dawn::wire` client that serialises WebGPU calls into a command stream; the GPU process holds a `dawn::wire` server that deserialises and dispatches into `dawn::native`, which in turn calls Vulkan through Mesa. This separation enforces Chrome's GPU sandbox boundary — the renderer process never issues DRM ioctls or Vulkan calls directly. The same Dawn library also powers the WebGPU Conformance Test Suite (CTS) infrastructure and supports standalone native use by applications that want portable GPU compute or rendering without the browser layer.

### 1.3 What is WGSL?

WGSL (WebGPU Shading Language) is the shader language defined by the W3C as part of the WebGPU specification. Its specification lives at `https://www.w3.org/TR/WGSL/`. WGSL is a statically-typed, memory-safe language with C-like syntax, designed to be compiled and validated entirely inside the browser before any shader source reaches GPU driver territory. Unlike GLSL — which browsers must accept as a string and pass through translation layers with imperfect coverage of all driver behaviours — WGSL is fully specified and restricts the operation set to a known-safe subset: there is no pointer arithmetic, out-of-bounds array accesses are bounds-checked by the compiler, and driver-specific extensions are excluded from the language surface.

Within Dawn, WGSL is compiled by Tint, a C++ compiler integrated directly into the Dawn source tree at `src/tint/`. Tint parses WGSL via a recursive-descent parser, constructs a typed AST or SSA IR depending on the compilation path, runs security and correctness transform passes (including robustness bounds-checking and workgroup memory zeroing), and emits SPIR-V for the Vulkan backend. The SPIR-V module is then passed to the Vulkan driver via `vkCreateShaderModule`. WGSL is the only shader language accepted by the WebGPU API; there is no path to submit raw SPIR-V or GLSL through the WebGPU surface. This is a deliberate security decision that keeps all shader validation inside the browser's trust boundary and decouples the web-facing API from any particular driver's native shader input format.
