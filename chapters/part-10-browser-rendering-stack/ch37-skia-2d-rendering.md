...
**What Wayland provides and what it lacks.** Wayland's core protocol has two relevant mechanisms. First, `wl_output.subpixel` — present since the earliest Wayland releases — broadcasts the physical display's subpixel geometry as an enum (`horizontal_rgb`, `horizontal_bgr`, `vertical_rgb`, `vertical_bgr`, `none`, `unknown`) in the `wl_output` global. Clients including browsers receive this event and know the correct sub-pixel order for the connected monitor. Second, `wl_surface.set_opaque_region` allows a client to declare a set of pixels as fully opaque. When the *entire* window is marked opaque and the compositor places it without alpha blending — for example, on a hardware scanout plane via KMS overlay promotion — some compositors treat this as a signal that LCD rendering in the surface is safe. **KWin** (the KDE compositor) does attempt this since KWin 6 on Wayland: when a surface's opaque region covers the whole window and the display's `wl_output.subpixel` value is not `none` or `unknown`, KWin enables LCD-aware blending for that surface. GNOME Shell / Mutter does not implement this path and keeps grayscale as the universal default.

What Wayland lacks is a mechanism for *subpixel-aware compositing*. The core problem is that subpixel fringe colours are baked into the buffer at render time, computed assuming the text sits on a specific background colour. If the compositor subsequently alpha-blends that buffer against a different background (for window shadows, rounded-corner clipping, or backdrop-blur effects), the fringe colours become incorrect and produce chromatic artefacts worse than grayscale. The closest active proposal — a **surface background-colour hint** (sometimes called a "matte colour" extension) — would let a client annotate its surface with an assumed background colour, enabling the compositor to mathematically undo and redo the fringe for the actual background. As of mid-2026 this exists only as mailing-list discussion on `wayland-devel`; no staging or stable protocol has been proposed.

**The HiDPI resolution.** The implicit consensus in the Wayland compositor community is that HiDPI proliferation resolves this problem without a protocol. At 192 dpi (2× scaling), physical pixels are small enough that the gamut of grayscale antialiasing is visually indistinguishable from RGB subpixel rendering — each physical pixel subtends less than 0.13 mm of arc at normal viewing distance, below the resolution limit of human foveal vision. The quality difference is only meaningful at 96–120 dpi, which is increasingly rare in developer hardware. Browser vendors have accepted this framing: Chrome and Firefox on Wayland are not actively pushing for a subpixel compositing protocol. The practical path to LCD text for native applications on Wayland is `wl_surface.set_opaque_region` combined with a KWin-based compositor, which already works for non-sandboxed GTK and Qt applications. For browsers, the GPU process sandbox complicates the opaque-region path because the sandbox's Wayland proxy mediates `wl_surface` requests and does not expose the full opaque-region API to the renderer.

Developers who prefer LCD antialiasing can investigate `--enable-font-antialiasing` (an experimental flag) or disable Wayland mode to fall back to XWayland, which restores X11 font configuration. The design document `src/core/SkRasterPipeline.cpp` and the commentary in Skia's "raster tragedy" design document discuss the full history of these trade-offs. [9] The full protocol mechanics of `wl_output.scale`, `wp_fractional_scale_v1`, and cursor/subpixel rendering under fractional scaling are covered in **Chapter 138 §15** (Wayland Fractional Scaling and HiDPI — Cursor, Icons, and Subpixel Rendering).
...
The Wayland protocol provides two pieces of the LCD puzzle. `wl_output.subpixel` delivers the display's physical subpixel layout to every connected client — Chrome reads this via the Ozone/Wayland backend's `WaylandOutput::OnOutputGeometry()` handler but currently ignores it for font rendering purposes, because knowing the sub-pixel order is necessary but not sufficient. `wl_surface.set_opaque_region` is the second piece: by declaring the window's entire client area opaque, a client signals to the compositor that it may place the surface on a hardware scanout plane without alpha blending, which would make subpixel rendering safe. KWin 6 on Wayland honours this combination — reading `wl_output.subpixel` and honouring `set_opaque_region` to enable LCD-aware surface handling — and non-sandboxed GTK and Qt applications benefit from it today. Chrome's GPU process sandbox intercepts Wayland protocol messages through a security broker (`WaylandConnection` inside the sandbox), and the broker's current implementation does not pass `set_opaque_region` through to the compositor, leaving Chrome unable to assert opacity even when the window has no transparency. A mailing-list proposal for a **surface background-colour hint** protocol extension — which would allow a client to declare an assumed matte colour so the compositor could redo subpixel fringe compositing correctly regardless of the actual background — has not advanced to a staging protocol as of mid-2026. In practice, the community has accepted that HiDPI proliferation (192+ dpi displays where grayscale and LCD antialiasing are visually equivalent) makes this problem progressively less relevant.
