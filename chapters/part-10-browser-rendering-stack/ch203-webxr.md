...
1. [What WebXR Is and Why It Exists](#1-what-webxr-is-and-why-it-exists)
   - [1.1 What is OpenXR?](#11-what-is-openxr)
   - [1.2 What is Monado?](#12-what-is-monado)
   - [1.3 What is Mojo IPC?](#13-what-is-mojo-ipc)
...
OpenXR is a royalty-free, open standard from the Khronos Group that defines a unified API for XR hardware, covering VR headsets, AR devices, and mixed-reality systems. Before OpenXR, each XR platform required its own SDK, forcing application developers to maintain separate code paths for each headset manufacturer. OpenXR collapses this into a single device-agnostic API surface: applications create an `XrInstance`, enumerate available systems via `xrGetSystem`, negotiate extensions, create an `XrSession`, and drive a render loop using `xrWaitFrame`/`xrBeginFrame`/`xrEndFrame`. The runtime — a platform-supplied shared library — implements the API against the actual hardware. On Linux, runtimes include Monado (the open-source reference runtime), SteamVR, and the Varjo runtime for professional headsets. WebXR in Chromium and Firefox does not implement OpenXR extensions directly in JavaScript; instead, the browser process acts as a privileged OpenXR client and exposes a sanitized, process-isolated subset of XR capabilities to web content through the WebXR Device API. Understanding OpenXR's extension model — and which extensions each Linux runtime exposes — is essential for understanding why certain WebXR features (hand tracking, depth sensing, passthrough) are or are not available on a given Linux configuration. [Source: OpenXR 1.0 Specification](https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html)

### 1.2 What is Monado?

Monado is the primary open-source OpenXR runtime for Linux and the reference platform for WebXR development on that operating system. It implements the full OpenXR 1.0 core specification and a substantial extension set, translating OpenXR calls from applications and browsers into hardware-specific backends: libusb for USB devices, Bluetooth for controllers, V4L2 for camera input, and Vulkan or OpenGL for compositing. Monado's compositor submits frames to the Wayland compositor or uses the DRM/KMS direct-mode path — analogous to the direct-mode path described in Chapter 27 — for low-latency display to an HMD. From WebXR's perspective, Monado appears as a standard OpenXR runtime located by Chromium through the `active_runtime.json` manifest, typically installed at `/usr/share/openxr/1/openxr_monado.json` or pointed to by the `XR_RUNTIME_JSON` environment variable. Monado also provides `libopenxr_monado.so`, the shared library that Chromium's process loads and binds against via the standard OpenXR loader. Monado's modular driver architecture (`xrt_device`, `xrt_compositor`) allows new hardware to be supported without changing the runtime core, making it the upstream most likely to gain complete WebXR support on Linux first. [Source: Monado GitLab](https://gitlab.freedesktop.org/monado/monado)

### 1.3 What is Mojo IPC?

Mojo is Chromium's inter-process communication framework, used throughout the browser to enforce process isolation between the renderer (where JavaScript runs), the browser process (which holds privileged system state), and the GPU process (which performs rendering). For WebXR, Mojo is the conduit through which JavaScript's calls to the WebXR Device API cross process boundaries: the renderer process calls a Mojo interface — defined in `device/vr/public/mojom/` — that is implemented in the browser process or a dedicated VR device service process. Chromium's WebXR architecture defines several Mojo interfaces (`XRDevice`, `XRSession`, `XRFrameData`) that carry tracking data, projection matrices, and input state from the OpenXR backend back to the renderer. The `XRFrameData` struct is populated each frame by the browser process calling OpenXR's `xrWaitFrame`, `xrBeginFrame`, and `xrLocateViews`, then serialised over Mojo and deserialised in the renderer to populate the `XRFrame` object visible to JavaScript. Understanding Mojo is important for diagnosing WebXR latency issues on Linux: each frame crosses the renderer-to-browser IPC boundary, and failures in this path — such as the GPU process not supporting a required buffer-sharing mechanism — are the primary reason immersive WebXR sessions fail silently on Linux without a compatible runtime and graphics stack. [Source: Chromium source — device/vr/public/mojom/](https://source.chromium.org/chromium/chromium/src/+/main:device/vr/public/mojom/)
