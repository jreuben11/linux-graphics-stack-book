...
The first half of this chapter establishes the wlroots architecture: the backend abstraction model and how the DRM backend maps to the KMS atomic commit path covered in Chapter 2; the renderer abstraction and how GLES2 and Vulkan renderers sit behind it; and the scene graph API that tracks damage and drives incremental compositing. The second half focuses on a practical walkthrough: building a minimal but functional Wayland compositor using wlroots' API, handling output configuration, seat and input routing, XDG shell surface management, and the screencopy protocol for PipeWire integration.
