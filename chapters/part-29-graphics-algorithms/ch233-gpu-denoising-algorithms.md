...
- **Tighter Vulkan synchronisation for OIDN.** OIDN has added an API for importing external semaphores from graphics APIs so that Vulkan and Direct3D 12 timeline semaphores can drive filter execution directly instead of relying on coarse host-side waits. CUDA and HIP devices gained this first; SYCL device support (the Intel Xe inference path used in §6.2) is planned for a subsequent release, which will close the remaining gap in the external-memory interop flow described in §6.4. [Source](https://github.com/OpenImageDenoise/oidn/releases/tag/v2.5.0)
- **NRD refinements aimed at upscaler pairing.** Recent NRD point releases have focused on improving history-confidence handling and the signal-domain resolve/re-jittering step that keeps ReBLUR and ReLAX output stable when composed downstream with temporal upscalers (DLSS, FSR, XeSS, TAAU), plus low-cost anti-firefly filtering now enabled by default in ReBLUR — refinements directly relevant to the frame-graph placement discussed in §1.3 and §10. [Source](https://github.com/NVIDIA-RTX/NRD/releases/tag/v4.17.0)
- **Broader GPU coverage for OIDN's compute backends.** OIDN continues to extend device support across newer Intel Xe, AMD RDNA, and NVIDIA GPU generations alongside compiler/toolkit updates (CUDA 13, ROCm 7 support), widening which Linux GPUs can run the neural RT filter from §6 without falling back to the CPU device. [Source](https://github.com/OpenImageDenoise/oidn/releases/tag/v2.4.0)

### Medium-term (1-3 years)

- **Neural radiance caching as a complement to spatial denoising.** NVIDIA's Neural Radiance Cache (NRC), distributed as part of the RTXGI SDK, trains a small network live during rendering to predict outgoing radiance and terminate path-tracing rays early — reducing the variance a downstream spatio-temporal denoiser has to remove rather than only filtering it after the fact. It targets both Vulkan and Direct3D 12 and is explicitly documented as experimental and under active development, so its Linux/Vulkan integration path is likely to keep changing. [Source](https://github.com/NVIDIA-RTX/RTXGI/blob/main/Docs/NrcGuide.md)
- **Inline neural-network evaluation moving into the shader.** The Vulkan `VK_NV_cooperative_vector` extension adds shading-language support for matrix-vector multiplies sized for small per-invocation MLPs — the same primitive underlying U-Net-style denoisers and radiance caches. It points toward denoising and inference work migrating from separate SDK-driven dispatch models (NRD's `GetComputeDispatches`, OIDN's external-filter execution) into ordinary Vulkan compute or ray-tracing shaders, which would change the descriptor-set and barrier patterns described in §10.1-§10.2. [Source](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/proposals/VK_NV_cooperative_vector.adoc)
- **AI upscalers absorbing the denoising step.** NVIDIA has moved DLSS Ray Reconstruction to transformer-based models, explicitly marketed as replacing hand-tuned denoisers — the SVGF/à-trous and ReBLUR/ReLAX filter families covered in this chapter — with a single network trained to reconstruct a clean, high-resolution frame directly from noisy ray-traced input. Where available, this folds the denoise and upsample stages that §1.3's frame graph treats as separate into one pass, at least on hardware running the proprietary implementation. [Source](https://www.nvidia.com/en-us/geforce/technologies/dlss/)

### Long-term

- **Open, cross-vendor pipelines stay filter-based.** No public vendor roadmap currently commits to an open, Vulkan-portable equivalent of DLSS Ray Reconstruction or NRC: NRD is Vulkan-generic but tuned primarily for NVIDIA RTX hardware, and OIDN's neural path remains a standalone offline/semi-interactive filter rather than an integrated temporal upscaler. Classic SVGF/ASVGF/à-trous pipelines (§2-§4) are therefore likely to remain the practical default for open-source and cross-vendor Linux path tracers even as proprietary stacks continue folding denoising into AI-driven upscaling.
- **Denoiser design work shifting upstream toward sample quality.** As reservoir-resampling techniques and live-trained radiance caches reduce the variance of the input signal itself, the marginal benefit of a stronger spatial filter narrows. The profiling methodology in §11 — sweeping spp against a perceptual metric such as FLIP — is a reasonable way to keep deciding, on a per-scene basis, whether further engineering effort belongs in the denoiser or upstream in the sampler; that question is likely to become more central as radiance-caching techniques mature from experimental to production status.
