...
- **GUDHI adds native Euler-characteristic queries and begins landing multi-parameter filtrations.** The GUDHI simplex tree gained a built-in `euler_characteristic` method, giving library users a direct combinatorial-invariant query instead of the manual boundary-matrix-rank workaround described in §2; in parallel, support for multiple filtration values in the C++ simplex tree is landing as preliminary functionality through a staged sequence of in-review pull requests rather than as a single feature drop. [Source](https://github.com/GUDHI/gudhi-devel/releases/tag/tags/gudhi-release-3.13.0) [Source](https://github.com/GUDHI/gudhi-devel/pull/1191)
- **TTK extends distributed-memory persistence computation.** A hybrid MPI-plus-thread implementation of the Discrete Morse Sandwich persistence-diagram algorithm — the same algorithm family surveyed in §5 — has been merged into TTK's development branch, targeting datasets that exceed single-node memory by distributing work across cluster nodes rather than offloading to a GPU. [Source](https://github.com/topology-tool-kit/ttk/pull/1112) [Source](https://arxiv.org/abs/2505.21266)

### Medium-term (1-3 years)
- **A GPU cubical-persistence prototype targets GUDHI integration.** cuCube, a GPU implementation of cubical persistent homology released as supplementary material for a research paper, is documented by its authors as intended for possible future integration into GUDHI, which would close the GPU gap for image- and grid-structured data noted in §4.5. [Source](https://github.com/mglisse/cucube)
- **Learned representations of topological summaries remain part of TTK's scope.** TTK's PyTorch-backed functionality for encoding merge trees and persistence diagrams with Wasserstein auto-encoders, built on published research, continues to be maintained as part of the toolkit, indicating sustained interest in coupling topological summaries with the machine-learning pipelines discussed in §7.3 and §10. [Source](https://github.com/topology-tool-kit/ttk/pull/1006) [Source](https://arxiv.org/abs/2307.02509)

### Long-term
- **The Ripser/Ripser++ CPU-GPU split looks likely to persist.** Ripser++ has seen no substantive feature development since its CUDA 12 build-compatibility fix, and no actively maintained successor project has emerged to extend GPU-accelerated Vietoris-Rips persistence; absent such a project, the pattern described in §4.4 — GPU apparent-pair extraction paired with a CPU library (Ripser) for the general case — is likely to remain the practical default rather than being subsumed by a single GPU-native tool. [Source](https://github.com/simonzhang00/ripser-plusplus)
- **Scaling continues to favor distributed CPU over GPU for the harder topology problems.** The boundary-matrix reduction, discrete-Morse gradient pairing, and contour-tree algorithms surveyed in §4.6, §5.3, and §6.3 remain research-stage or CPU-cluster-oriented rather than GPU-native; the direction visible in current TTK development — investing in MPI-plus-thread distributed-memory scaling for exactly these algorithm families — suggests that large-scale computational topology is more likely to keep scaling out across CPU clusters than to be re-architected around GPU parallelism in the foreseeable future. [Source](https://github.com/topology-tool-kit/ttk/pull/1112)
