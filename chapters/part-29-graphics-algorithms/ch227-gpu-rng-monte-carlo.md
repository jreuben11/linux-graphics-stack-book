...
- **ROCm's RNG libraries are consolidating into a single monorepo.** Both the `ROCm/rocRAND` and `ROCm/hipRAND` repositories referenced in §10.1 have been retired as standalone repos and folded into `ROCm/rocm-libraries` (`projects/rocrand`, `projects/hiprand`), continuing AMD's broader effort to unify its GPU math-library development and release process rather than shipping each library as a separately versioned repo. Existing package names, headers, and the host/device APIs shown in §10.1 are unaffected, but source and issue-tracking activity has moved. [Source: ROCm/rocm-libraries repository](https://github.com/ROCm/rocm-libraries) [Source: ROCm/rocRAND deprecation notice](https://github.com/ROCm/rocRAND)
- **rocRAND has added an experimental SPIR-V code path.** ROCm 7.2.0 introduced a `-DAMDGPU_TARGETS=amdgcnspirv` build mode alongside changes to the generator `launch` path so the same host and device APIs described in §10.1 can target SPIR-V rather than only native AMDGPU ISA — a step toward using rocRAND-generated kernels through non-HIP compute front ends. It remains marked experimental as of this writing. [Source: rocRAND ROCm 7.2.0 release notes](https://github.com/ROCm/rocRAND/releases/tag/rocm-7.2.0)
- **`VK_EXT_ray_tracing_invocation_reorder` has reached ratified, cross-vendor status.** This extension generalises NVIDIA's earlier `VK_NV_ray_tracing_invocation_reorder` (shader execution reordering) into an `EXT`-prefixed, ratified Vulkan extension that lets a ray generation shader explicitly reorder invocations by hit-object locality before shading. It directly addresses the divergence cost noted for rejection sampling (§5.3) and BSDF/light-sampled scatter rays (§9.4): Monte Carlo path tracers scatter rays in directions chosen by per-thread random samples, which destroys the spatial coherence the ray-tracing pipeline relies on for SIMT efficiency. [Source: Khronos VK_EXT_ray_tracing_invocation_reorder proposal](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/proposals/VK_EXT_ray_tracing_invocation_reorder.adoc)

### Medium-term (1-3 years)
- **NVIDIA's reservoir-resampling reference runtime has extended from direct lighting to full path tracing.** RTXDI 3.0 added ReSTIR PT resampling functions and reservoir-visualisation tooling to the reference implementation, applying the spatiotemporal reservoir merge pattern in §7.1–7.2 to indirect bounces rather than only the direct-lighting case worked through in this chapter. The API is described as still evolving (the release notes flag a breaking change to `RTXDI_PathTracerContext` in the same version), so treat the ReSTIR DI reservoir struct and merge logic in §7 as the stable subset and ReSTIR PT as the actively developing frontier. [Source: RTXDI v3.0.0 release notes](https://github.com/NVIDIAGameWorks/RTXDI/releases/tag/v3.0.0)
- **Intel's oneMKL RNG interface has moved to open, cross-vendor governance as oneMath.** The project referenced in §10.2 (`oneapi-src/oneMKL`) was transferred to the UXL Foundation and renamed oneMath; its RNG domain dispatches not only to Intel's oneMKL backend but also to AMD rocRAND and NVIDIA cuRAND depending on the selected device, making it a genuine cross-vendor abstraction over the same three generator families (Philox, MRG, Sobol) discussed throughout §2–§4 rather than an Intel-only interface. [Source: oneMath repository and backend table](https://github.com/uxlfoundation/oneMath)
- **Path guiding is maturing as a learned alternative to static MIS heuristics.** Intel's Open Path Guiding Library continues active development of online-learned directional sampling distributions (typically SD-trees or vMF mixtures fit incrementally from path contributions) that adapt spatially and can outperform the fixed balance/power-heuristic combination of BSDF and light sampling covered in §6.1 for scenes with difficult indirect lighting. OpenPGL is integrated as an optional component in production renderers (Blender Cycles, several ray-tracing frameworks) rather than replacing MIS outright — the two techniques compose, with path guiding supplying a better proposal distribution that MIS then weights. [Source: Intel Open Path Guiding Library](https://github.com/RenderKit/openpgl)

### Long-term
- **Counter-based RNGs remain the point of cross-vendor convergence.** Every consolidation move surfaced above — rocRAND's SPIR-V experiment, oneMath's multi-backend dispatch, RTXDI's reservoir framework — is built on Philox- or cuRAND/rocRAND-equivalent counter-based generators (§3) rather than stateful per-thread designs, because CBRNGs are the only family that maps identically onto a stateless `(seed, counter) → value` interface regardless of which vendor's library sits underneath. No vendor roadmap signals a move away from this pattern, so it is likely to remain the portable default for GPU Monte Carlo work for the foreseeable future.
- **PBRT-v4 shows no signs of a successor edition.** Commit activity on the reference implementation cited throughout §4 and §10.3 remains confined to correctness fixes and small tooling changes rather than new sampler architectures, and no public roadmap or repository points to a fifth edition. Absent that, the `ZSobolSampler`/Owen-scrambling design in §4.4 and §10.3 is likely to remain the field's reference implementation of scrambled QMC sampling rather than being superseded by a new canonical sampler. [Source: pbrt-v4 repository](https://github.com/mmp/pbrt-v4)
- **Ray-reordering hardware support is likely to broaden unevenly across vendors.** `VK_EXT_ray_tracing_invocation_reorder` is ratified at the API level, but its performance benefit depends on scheduling hardware that not all GPU architectures implement; expect the extension to expose itself as a no-op or minimal-benefit path on hardware without dedicated reordering logic for some time, meaning renderers cannot yet assume it is a free win and must still design ray-generation shaders (§9.4) to minimise divergence on their own where the feature is absent or ineffective.
