...
- **Microscaling quantization formats (MXFP4, NVFP4) maturing as first-class GGUF quantization types.** Beyond the per-group INT4/INT8 K-quant schemes described in §4.1 and §4.5, llama.cpp is completing native dense-tensor and mixture-of-experts variants of the OCP microscaling MXFP4 format and NVIDIA's NVFP4 format, including CUDA MMA kernels targeting Blackwell (SM120) tensor cores — extending the dequantize-then-GEMM pipeline of §4.5 toward hardware paths that consume the block-scaled 4-bit format more directly. [Source: llama.cpp pull request, "quantization: complete MXFP4 and NVFP4"](https://github.com/ggml-org/llama.cpp/pull/26869)
- **EAGLE-family speculative decoding broadening beyond the Medusa-style heads covered in §5.2.** llama.cpp merged native EAGLE3 draft-model support, which predicts multiple future tokens from intermediate transformer features rather than fine-tuned prediction heads, and has continued extending EAGLE3 draft-model conversion to additional base-model families; the two approaches now coexist as alternative decode-acceleration strategies within the same runtime. [Source: llama.cpp pull request, "feat: add EAGLE3 speculative decoding support"](https://github.com/ggml-org/llama.cpp/pull/18039)
- **Compiler-level fixes for the decode-time skinny-GEMM bottleneck on AMD matrix cores.** The batch-1 GEMV degeneration described in §1.5 wastes AMD MFMA matrix-core cycles on padding when the batch dimension is smaller than the instruction's native tile height; IREE's `linalg`-to-`rocdl` lowering (§7.4) now synthesizes "virtual dense MFMA" sequences that pair GPU lanes over AMD's structured-sparsity MFMA instructions to cover skinny matrices without padding, showing single-digit-percent gains on CDNA3 in early testing. [Source: IREE blog, "Virtual Dense MFMAs for Skinny GEMM"](https://iree.dev/community/blog/2026-05-28-virtual-dense-mfmas-for-skinny-gemm/)

### Medium-term (1-3 years)
- **NVFP4 consolidating as the default low-precision inference format on Blackwell-class GPUs.** NVFP4 pairs 4-bit floating-point elements with a two-level micro-block and tensor-level scale, and NVIDIA reports it recovers accuracy closer to FP8 than the block-scaled MXFP4 format used elsewhere in §4.5 while retaining a similar memory footprint; it is already supported end-to-end in both TensorRT-LLM and vLLM, alongside the llama.cpp integration described above, making it a candidate to displace GPTQ/AWQ INT4 (§4.2–§4.3) as the default weight format on newer hardware. [Source: NVIDIA Developer Blog, "Introducing NVFP4 for Efficient and Accurate Low-Precision Inference"](https://developer.nvidia.com/blog/introducing-nvfp4-for-efficient-and-accurate-low-precision-inference/)
- **Disaggregated prefill/decode serving (§5.3) extending from CUDA-first deployments to AMD hardware.** vLLM's PagedAttention-based continuous batching already separates scheduling from generation; an open RFC proposes dedicated CI coverage for running disaggregated prefill/decode alongside wide expert-parallelism on multi-node ROCm clusters over RDMA-capable NICs, treating that deployment topology as a first-class, continuously validated configuration rather than a CUDA-only capability. [Source: vLLM RFC, "Disaggregated P/D + Wide-EP CI Enablement on ROCm"](https://github.com/vllm-project/vllm/issues/48117)

### Long-term
- **Sparse mixture-of-experts (MoE) architectures pushing GEMM and quantization kernels toward irregular, per-expert batching.** This chapter's GEMM (§2), attention (§3), and quantization (§4) algorithms are described largely in terms of dense per-token compute; frontier open-weight models increasingly route each token to a small subset of many experts instead, as popularized by large-scale MoE designs such as DeepSeek-V3. No GPU vendor currently ships dedicated hardware for expert routing or dispatch, so the irregular batching this creates is being absorbed in software — fused MoE kernels, expert-parallel scheduling, and the wide-EP serving work above — rather than in the ISA, meaning the dense-GEMM mental model this chapter builds on will increasingly need an MoE-aware companion. [Source: DeepSeek-V3 Technical Report, https://arxiv.org/abs/2412.19437](https://arxiv.org/abs/2412.19437)
