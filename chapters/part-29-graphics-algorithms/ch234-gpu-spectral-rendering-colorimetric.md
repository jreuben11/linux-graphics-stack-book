...
- **A generic KMS plane colour pipeline (`drm_colorop`) has landed upstream and is under active refinement.** The kernel now exposes a `DRM_CLIENT_CAP_PLANE_COLOR_PIPELINE` capability and a chain of `drm_colorop` objects (1D/3D LUT, matrix, and curve stages) that a compositor can program per-plane, giving hardware-side gamut mapping and colour-space conversion an alternative to the shader-based approaches in §9–§10. Recent upstream work has focused on state-lifecycle correctness — making LUT interpolation properties mutable and fixing blob property reference tracking across atomic commits. [Source: DRM KMS documentation, https://www.kernel.org/doc/html/latest/gpu/drm-kms.html] [Source: kernel commit fixing colorop LUT interpolation mutability, https://github.com/torvalds/linux/commit/94ff735296d371045fce163451a3d65e44ac4729] [Source: kernel commit fixing colorop blob property lifecycle, https://github.com/torvalds/linux/commit/235b333e2878d791cee09e1e72f44611a9400114]
- **`wp_color_management_v1` remains a staging-tier Wayland protocol.** The protocol (currently at interface version 2, adding 64-bit image description identifiers and additional transfer-function options) is explicitly described by its own specification text as still in a testing phase; near-term work is compositor-side consumption of image descriptions rather than further protocol churn. This confirms the verification note already carried in §12.3 of this chapter. [Source: Wayland protocol reference, https://wayland.app/protocols/color-management-v1]
- **OpenColorIO 2.5 shipped built-in ACES 2.0 configs and reworked GPU shader code paths.** The tetrahedral 3D-LUT GPU renderer referenced in §10.6 now ships ACES 2.0 as a built-in configuration rather than requiring an externally distributed config file, and a following point release reworked Vulkan texture-binding indices used by OCIO's GPU shader backend. [Source: OpenColorIO v2.5.0 release notes, https://github.com/AcademySoftwareFoundation/OpenColorIO/releases/tag/v2.5.0] [Source: OpenColorIO v2.5.1 release notes, https://github.com/AcademySoftwareFoundation/OpenColorIO/releases/tag/v2.5.1]

### Medium-term (1-3 years)

- **KMS colorop bridge chaining and connector/panel interaction is still explicitly flagged as in-flux.** The kernel documentation for the plane colour pipeline API notes this gap directly; closing it is a precondition for exposing panel-side (rather than only plane-side) colour transform hardware — such as per-connector 3D LUTs on some AMD and Intel display controllers — to the same pipeline used for the spectral-renderer HDR output path described in §12. [Source: DRM KMS documentation, https://www.kernel.org/doc/html/latest/gpu/drm-kms.html]
- **Dr.Jit's backend surface is broadening (Metal added in Mitsuba 3.9) without a corresponding Vulkan or HIP/ROCm target appearing.** Mitsuba 3's JIT-compiled spectral variants (§11.2) gained a `metal_ad_*` backend for Apple Silicon on top of the existing `cuda_ad_spectral` and `llvm_ad_spectral` variants; on Linux this means AMD GPUs still have no native Dr.Jit compute backend, and CUDA (NVIDIA) or LLVM-vectorised CPU execution remain the only GPU-accelerated spectral path-tracing options for AMD hardware. [Source: Mitsuba 3.9.0 release notes, https://github.com/mitsuba-renderer/mitsuba3/releases/tag/v3.9.0]
- **Static HDR metadata (SMPTE ST 2086 / CTA 861.3, §12.2) remains the only metadata type in wide use on the open Linux stack.** Neither the kernel's `drm_hdr_output_metadata` type nor `wp_color_management_v1` currently define a dynamic (per-scene or per-frame) metadata path comparable to proprietary dynamic-HDR schemes; the static MaxCLL/MaxFALL pipeline described in §12.2 is likely to remain the baseline a spectral renderer targets for the medium term. [Source: DRM KMS documentation, https://www.kernel.org/doc/html/latest/gpu/drm-kms.html]

### Long-term

- **Spectral rendering is likely to stay confined to offline and research GPU compute rather than migrating into real-time rasterisation.** No mainline open rasterisation pipeline — Mesa's OpenGL/Vulkan drivers, Blender's EEVEE, or browser compositors — currently carries a public roadmap item to add wavelength-resolved light transport; the per-bounce cost multiplier discussed in §1.5 makes this an unlikely near-real-time target, so PBRT-v4, Mitsuba 3, and LuxCoreRender are likely to remain the primary Linux GPU spectral renderers. [Source: Blender Cycles rendering documentation (no spectral mode listed), https://docs.blender.org/manual/en/latest/render/cycles/]
- **As KMS colorops and `wp_color_management_v1` stabilise out of staging, more of the colour-space conversion and gamut-mapping work in §9–§10 is likely to migrate from compositor/application shader code into a kernel-exposed, hardware-accelerated pipeline.** This would not eliminate the OKLab, ICtCp, and tetrahedral-LUT shader techniques covered in this chapter — they remain necessary wherever no fixed-function colour hardware exists, or where a renderer computes its own perceptual colour space ahead of hand-off to the display pipeline — but it would shift where the final gamut-mapping stage executes on newer hardware. [Source: DRM KMS documentation, https://www.kernel.org/doc/html/latest/gpu/drm-kms.html]
