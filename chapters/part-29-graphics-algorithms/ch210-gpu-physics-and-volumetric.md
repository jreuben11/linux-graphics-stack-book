...
- **[VI. Physics and Simulation](#vi-physics-and-simulation)**
  - [1.1 What is GPU Physics Simulation?](#11-what-is-gpu-physics-simulation)
  - [1.2 What is Fluid Simulation?](#12-what-is-fluid-simulation)
  - [1.3 What is a Signed Distance Field (SDF)?](#13-what-is-a-signed-distance-field-sdf)
  - [1.4 What is Volumetric Ray Marching?](#14-what-is-volumetric-ray-marching)
...
### 1.1 What is GPU Physics Simulation?

GPU physics simulation moves the numerical computation of physical laws — particle dynamics, constraint resolution, field evolution — from CPU threads into Vulkan compute shaders that execute on the GPU's parallel processing units. A physics frame typically consists of a sequence of compute dispatches: integrate velocities, detect collisions, resolve constraints, and update positions. Each dispatch maps a physical entity — a particle, a grid cell, a finite element, or a collision pair — to one GPU invocation. On Linux, the Vulkan compute queue is the primary vehicle: `vkCmdDispatch` schedules work on the GPU, and `VkMemoryBarrier` with `VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT` synchronizes passes within a frame. Simulation results land in `VkBuffer` objects (as vertex or SSBO data) or `VkImage` 3D textures that the subsequent rendering pass consumes without a CPU readback. The GPU's memory bandwidth — typically 400–900 GB/s on discrete GPUs versus 50–80 GB/s on CPU — determines throughput for memory-bound kernels such as SPH neighbor summation and Eulerian grid advection. This chapter maps two broad physical domains to GPU compute: Category VI covers particle and grid-based physical simulation (fluid, rigid body, cloth, fracture), and Category VII covers signed-distance-field and volumetric methods that represent geometry implicitly as scalar fields computed and evolved in GPU memory.

### 1.2 What is Fluid Simulation?

Fluid simulation computes the time evolution of a fluid's velocity and pressure fields by numerically integrating the Navier-Stokes equations or a derived kinetic model. Two mathematical frameworks dominate: Eulerian methods maintain a fixed voxel grid and update field values at each cell, making them well suited for large-scale incompressible flow with a global pressure solve; Lagrangian methods track individual fluid particles moving through the domain, avoiding numerical diffusion at the cost of sparse, irregular memory access patterns. Hybrid schemes such as FLIP and APIC combine a particle representation for low-dissipation advection with an Eulerian grid for the divergence-free pressure projection, offering a practical balance for visual effects. GPU implementations exploit data parallelism naturally: Eulerian pressure solves dispatch one thread per grid cell, particle updates dispatch one thread per particle, and the critical conjugate-gradient pressure iterations run entirely in GPU memory without CPU intervention. An alternative kinetic model, the Lattice Boltzmann Method (LBM), replaces the Navier-Stokes equations with local collision and streaming steps on a uniform lattice, achieving near-perfect parallelism with a uniform per-cell compute pattern. This chapter covers SPH (smoothed particle hydrodynamics), PBF (position-based fluids), FLIP, and LBM as a spectrum from purely Lagrangian to Eulerian formulations, each implemented as a Vulkan compute pipeline dispatching over particles or grid cells.

### 1.3 What is a Signed Distance Field (SDF)?

A signed distance field is a scalar-valued function defined over a 3D domain such that the value at each point equals the algebraic distance to the nearest surface: positive outside, negative inside, and zero on the surface itself. SDFs provide a unified, resolution-agnostic representation for geometry that naturally supports smooth boolean operations (union, intersection, subtraction via min/max), gradient-based surface normals derived from the field gradient, and efficient proximity queries without explicit triangle-triangle intersection tests. On the GPU, an SDF is stored as a `VK_IMAGE_TYPE_3D` floating-point texture; the surface is the zero isosurface of that texture, and trilinear sampling provides continuous interpolation between voxels. Level set methods evolve SDFs through time using partial differential equations to simulate dynamic surfaces such as fluid interfaces, flame boundaries, or fracture fronts. TSDF (Truncated Signed Distance Field) is a variant restricted to a narrow band around the surface, used in real-time 3D reconstruction from depth camera frames. SDF-based collision detection samples the SDF texture at query points to retrieve penetration depth and contact normal in a single texture fetch, avoiding costly mesh-mesh tests. Isosurface extraction algorithms such as Marching Cubes and Dual Contouring convert the zero isosurface back to a renderable triangle mesh. All eight subsections in Category VII of this chapter build on SDF as their core data structure.

### 1.4 What is Volumetric Ray Marching?

Volumetric ray marching is a rendering technique in which rays cast from the camera accumulate optical contributions — emission, absorption, and in-scattering — as they step through a participating medium represented by a 3D density or radiance field. Unlike rasterization, which renders opaque surfaces at discrete depth samples, ray marching integrates the volume rendering equation along each ray at a series of step positions, sampling a 3D texture each time. The classical emission-absorption model accumulates color and transmittance as the ray advances: each step multiplies the existing transmittance by an extinction coefficient and adds emitted radiance weighted by the current transmittance. GPU implementations dispatch one thread per screen pixel in a Vulkan compute or fullscreen fragment pass; each thread iterates over step positions and samples a bound `VK_IMAGE_TYPE_3D` texture via `textureLod`, accumulating color and alpha until transmittance falls below a threshold or the ray exits the volume. Step sizes can be uniform, jittered to reduce banding, or adaptive where density approaches zero. The same iteration pattern drives SDF sphere tracing, where the step size at each position equals the SDF value — guaranteeing no surface overshoot while minimizing total iteration count. Sparse volume formats require indirection structures to skip empty space and can be accessed via buffer-based tile indirection in Vulkan. Within this chapter, volumetric ray marching appears in ambient occlusion volumes, participating media, and level-set surface visualization.
...
- **Open, GPU-native multi-solver physics frameworks consolidating rigid body, MPM, cloth, soft-body, and cable dynamics into one shared runtime.** Newton, hosted under Linux Foundation governance and built on NVIDIA Warp with MuJoCo Warp as its primary backend, targets the same MPM (§49.3), rigid body (§50), cable (§53), and cloth (§54) solver families this chapter treats as separate compute-shader implementations, combining them inside one differentiable, actively developed engine. [Source](https://github.com/newton-physics/newton)
- **Sparse GPU volume libraries extending beyond static rendering use toward on-device dynamic topology changes.** OpenVDB's NanoVDB component, the format underlying the narrow-band and VDB-traversal techniques in §67–§68, has been growing GPU-resident support for level-set tracking, morphological operations, and grid merging directly on-device rather than requiring a CPU round-trip for topology edits. [Source](https://github.com/AcademySoftwareFoundation/openvdb/releases/tag/v13.0.0)
- **Ray-traced indirect illumination increasingly amortized across learned radiance caches rather than scaled by raw per-frame ray budget.** RTXGI's neural and spatial-hash radiance-cache components cache indirect lighting across frames instead of resampling it every frame, the same per-frame ray-budget tradeoff that §66.4's ray-traced AO and bent-normals approach makes for occlusion rather than full indirect light. [Source](https://github.com/NVIDIAGameWorks/RTXGI)

### Medium-term (1-3 years)

- **GPU-driven generation of whole command sequences, not just variable dispatch counts.** `vkCmdDispatchIndirect` already reads its group count from a GPU buffer, so the atomic-counter compaction patterns used for contact-pair lists (§50, §56) and active-voxel lists (§68) can already size a single dispatch from GPU-computed data without a CPU readback. What they cannot do is vary *which* commands run — state and pipeline changes still have to be authored on the CPU as separate, fixed passes. The ratified `VK_EXT_device_generated_commands` extension lets the GPU itself generate whole sequences of draws and dispatches, including pipeline switches, which is the next constraint these compaction patterns run into as workloads grow more heterogeneous. [Source](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/appendices/VK_EXT_device_generated_commands.adoc)
- **Unified multi-physics platforms replacing bespoke per-technique compute shaders as the default authoring model.** Genesis integrates rigid body, FEM, MPM, particle-based (PBD/SPH), and Incremental-Potential-Contact solvers behind one scene and one shared state, plus an explicit multi-solver coupler — the same coupling problem this chapter's fluid-solid coupling (§55) and CFD-DEM coupling (§56.3) sections handle by hand for their specific solver pairs. [Source](https://github.com/Genesis-Embodied-AI/genesis-world)
- **GPU TSDF/ESDF fusion converging with SDF-based motion planning into a single pipeline rather than separate algorithm families.** nvblox fuses depth data into both a TSDF (§63) and a Euclidean signed-distance field on GPU, feeding the same field directly into collision and motion-planning queries (§60, §64) instead of treating reconstruction and planning as independent stages. [Source](https://github.com/nvidia-isaac/nvblox)

### Long-term

- **No ratified, engine-agnostic physics-scene interchange format exists, and none is close.** Unlike glTF's core mesh and material model, the community `OMI_physics_body`/`OMI_physics_shape` extensions are not a ratified `KHR_` extension and have a single confirmed engine implementation. Absent a standards-body commitment, physics-scene portability across PhysX-, Bullet-, Jolt-, and MuJoCo-based pipelines is likely to stay fragmented, with each engine's native format remaining the practical interchange point. [Source](https://github.com/omigroup/gltf-extensions/tree/main/extensions/2.0/OMI_physics_body)
- **Barrier-based contact formulations are a plausible long-term replacement for the discrete-solve-then-CCD-patch pattern this chapter documents in §50.3 and §59.1.** Incremental Potential Contact guarantees intersection- and inversion-free frictional-contact trajectories regardless of time-step size or deformation severity, removing the need to tune substeps and patch missed collisions after the fact. GPU-native IPC frameworks now run this coupling for rigid bodies, soft bodies, cloth, and threads together, and are already appearing as an integrated solver option inside unified platforms like Genesis rather than only as standalone research code — a sign the approach is moving toward production use rather than staying academic. [Source: ipc-sim/IPC](https://github.com/ipc-sim/IPC) · [Source: libuipc](https://github.com/spiriMirror/libuipc)
