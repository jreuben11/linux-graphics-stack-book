...
- **FlashAttention 4 maturing toward general release on newer NVIDIA architectures.** The project's beta release channel is actively iterating on SM90/SM100/SM110 (Hopper through Blackwell-class) kernel paths, and vLLM has already begun integrating it for FP8 KV cache and larger attention head dimensions — the natural continuation of the FlashAttention-3/Hopper-specific optimizations described in §5, extended toward the next hardware generation as it ships. [Source: flash-attention releases](https://github.com/Dao-AILab/flash-attention/releases) [Source: vLLM v0.27.0 release notes](https://github.com/vllm-project/vllm/releases/tag/v0.27.0)
- **llama.cpp's Vulkan backend adopting vendor cooperative-matrix decode extensions for faster quantized matmul.** A merged change uses NVIDIA's `GL_NV_cooperative_matrix_decode_vector` extension to decode four quantized elements per load instead of two when feeding cooperative-matrix fragments, measurably improving prompt-processing throughput on the K-quant and IQ formats covered in §4 and the coopmat path in §11; the extension currently ships only in NVIDIA's developer driver channel, with general driver availability still pending. [Source: llama.cpp pull request](https://github.com/ggml-org/llama.cpp/pull/23541)
- **ROCm's GEMM and kernel libraries consolidating into a single monorepo.** rocBLAS, hipBLASLt, MIOpen, and composable_kernel — the four ROCm library-tier components §10 walks through individually — now live as subprojects of a unified `rocm-libraries` repository, with the standalone `composable_kernel` repository marked deprecated in favor of the merged tree. The practical effect for inference deployments is a single build and version cadence across the HIP BLAS tier rather than four independently versioned projects. [Source: ROCm/rocm-libraries](https://github.com/ROCm/rocm-libraries) [Source: ROCm/composable_kernel (deprecated)](https://github.com/ROCm/composable_kernel)

### Medium-term (1-3 years)
- **Microscaled FP4 formats (MXFP4/NVFP4) maturing as a quantization tier beyond block-wise INT4.** llama.cpp's GGUF pipeline and vLLM's quantization layer are both actively completing support for these block-scaled floating-point formats, extending the weight-only quantization landscape in §4 — GPTQ, AWQ, and the K-quant GGUF types — toward formats with native hardware GEMM support on newer GPU generations. [Source: llama.cpp pull request, "quantization: complete MXFP4 and NVFP4"](https://github.com/ggml-org/llama.cpp/pull/26869) [Source: vLLM v0.27.0 release notes](https://github.com/vllm-project/vllm/releases/tag/v0.27.0)
- **Inference stacks pre-adapting to GPU generations that are not yet broadly deployed.** vLLM has landed early enablement for AMD's `gfx1250` architecture target and NVIDIA's `sm_107` (Rubin-class) compute capability ahead of general hardware availability — the same gfx-target pattern §10 describes for RDNA3/CDNA (`gfx1100`, `gfx942`) recurring as vendor architectures advance. [Source: vLLM v0.27.0 release notes](https://github.com/vllm-project/vllm/releases/tag/v0.27.0)
- **AITER continuing to absorb ROCm-native attention and GEMM kernel work at a rapid release cadence.** Since becoming vLLM-ROCm's preferred attention backend alongside composable_kernel (§5, §10), AITER has kept shipping frequent point releases expanding FP8 attention and MoE kernel coverage, reflecting AMD's ongoing consolidation of LLM-serving kernels into fewer, more tightly coupled libraries. [Source: ROCm/aiter releases](https://github.com/ROCm/aiter/releases)

### Long-term
- **Structured-conditioning world models and raw-pixel world models remain a live, unresolved fork rather than a progression.** NVIDIA's Cosmos platform continues to expand its ControlNet-based structured-buffer conditioning line (Cosmos-Transfer) alongside a growing reasoning and reinforcement-learning stack (Cosmos-Reason, Cosmos-RL) built on the world-model foundation described in §8, while GameNGen/DIAMOND/Genie-style raw-pixel world models have not adopted structured-buffer conditioning. No public roadmap from either line currently commits to unifying the two approaches, so the two-track split documented in §8 is likely to persist. [Source: nvidia-cosmos organization](https://github.com/nvidia-cosmos)
- **Decode-phase throughput will keep depending on shrinking bytes moved per token, not on added FLOPS.** No GPU vendor roadmap currently commits to closing the gap between HBM bandwidth growth and compute-throughput growth; because single-request decode is a GEMV bound by weight-read bandwidth (§2, §13), further throughput gains are likely to keep coming primarily from the KV-cache and weight-quantization techniques in §3 and §4 — narrower weight and KV-cache formats plus continuous batching to amortize reads — rather than from raw compute-throughput increases that mostly benefit the already compute-bound prefill phase.
