...
- **Zero-shot foundation models displacing task-specific stereo networks.** FoundationStereo established that a stereo matcher trained on large-scale synthetic data with a monocular-prior-adapted backbone can generalise zero-shot across domains without the per-dataset fine-tuning that RAFT-Stereo, CREStereo, and HITNet (§3) require; a follow-up, Fast-FoundationStereo, distils and prunes the same architecture to real-time frame rates via knowledge distillation and blockwise neural architecture search, closing the gap with the real-time task-specific networks it aims to replace. [Source](https://github.com/NVlabs/FoundationStereo) [Source](https://github.com/NVlabs/Fast-FoundationStereo)
- **GLOMAP-style global SfM absorbed into COLMAP as a first-class mapper.** COLMAP has integrated the GLOMAP global structure-from-motion pipeline directly as the `global_mapper` command, offered as an alternative to the incremental bundle-adjustment loop described in §5.4 rather than a separate tool, giving pipelines a much faster reconstruction path for the sparse SfM stage feeding MVS (§6). [Source](https://github.com/colmap/colmap/blob/main/CHANGELOG.rst) [Source](https://github.com/colmap/glomap)
- **Competing Gaussian-splat compression formats under Khronos review.** Multiple draft glTF extensions for compressing 3D Gaussian Splatting scenes — including a GPU-texture-native codec (EGSC) that stores spherical-harmonics coefficients in ASTC/UASTC textures for direct GPU sampling, alongside separate MPEG- and vendor-proposed formats — are open for review simultaneously, indicating the 3DGS asset format used by pipelines like §7.3/§12.2 has not yet converged on a single interchange standard. [Source](https://github.com/KhronosGroup/glTF/pull/2605)

### Medium-term (1-3 years)
- **Unified geometry foundation models blurring monocular depth and SfM.** Depth Anything 3 predicts spatially consistent depth and camera pose from arbitrary image collections — with or without known poses — from a single plain transformer backbone, reporting improvements over both its own monocular-only predecessor (§2.3) and multi-view pose/depth baselines; if this direction continues, the boundary between the monocular depth models of §2 and the SfM camera-pose pipeline of §5 becomes increasingly blurred for applications that do not require metrology-grade accuracy. [Source](https://github.com/ByteDance-Seed/Depth-Anything-3)
- **Environment-depth passthrough extension registered while open-source runtime support stays a design problem.** `XR_META_environment_depth` is a registered OpenXR extension in the Khronos registry, going beyond the experimental status described for it in §12.4, but Monado's own architecture discussion for wiring vendor camera/passthrough frames into the compositor is still at the design-proposal stage, so open-source AR passthrough on Linux is likely to keep depending on the custom depth-injection drivers described in §12.4 for some time yet. [Source](https://registry.khronos.org/OpenXR/specs/1.1/html/xrspec.html) [Source](https://gitlab.freedesktop.org/monado/monado/-/issues/490)
- **Mesh-extraction techniques for 3DGS converging toward production tooling.** The SuGaR/GOF/2DGS lineage covered in §8.2 continues to be refined for watertight, artifact-free surface extraction directly from splat-based scene representations, tracking the same compression and interchange standardization pressure noted above as 3DGS moves from a research renderer toward an asset format with a mesh-export path.

### Long-term
- **CUDA-only fused kernels remain a structural ROCm gap for hash-grid NeRF.** `tiny-cuda-nn`'s fused hash-encoding-plus-MLP kernel (§7.1, §7.4) has no official HIP/ROCm port, and no public roadmap commits to one; because 3D Gaussian Splatting's tile-rasterisation approach ports far more directly to non-CUDA backends than Instant-NGP-style fused hash-grid kernels do, the AMD/ROCm NeRF ecosystem is likely to keep consolidating around gsplat-style splatting rather than closing the Instant-NGP training-speed gap. [Source](https://github.com/NVlabs/tiny-cuda-nn)
- **Foundation-model geometry unlikely to fully displace bundle-adjustment SfM.** No public roadmap for COLMAP, GLOMAP, or the foundation-geometry model lineage (Depth Anything 3, VGGT) proposes retiring classical bundle adjustment; the accuracy and calibration guarantees that surveying, photogrammetry, and metrology applications depend on mean COLMAP-style optimisation (§5.4) is likely to remain the reference pipeline that faster, learned alternatives are validated against rather than a component they replace outright.
