...
### 1.1 What is Virtual Texturing?

Virtual texturing (also called sparse virtual textures or megatextures) is a technique for managing texture datasets far larger than GPU VRAM by streaming only the texture pages currently visible to the camera. The core idea is that a scene may reference a logically contiguous address space of texture data — potentially gigabytes in size — but at any given frame only a small fraction of that data is needed at the resolution the camera can resolve. The GPU maps this large virtual address space through a page table (an indirection texture) to a physical backing store: a fixed-size pool of resident texture tiles, typically 128×128 or 256×256 texels. A feedback pass reads which tiles were accessed, and an async-compute or CPU job streams missing tiles from disk or a compressed cache. On modern Vulkan hardware, `VK_EXT_sparse_texture` and sparse image memory (introduced in Vulkan 1.0 through `vkBindImageMemory2` with `VkSparseImageMemoryBind`) expose these page-table operations directly, allowing fine-grained control over which pages are committed to physical memory. The related concept of bindless texturing removes the per-draw descriptor set limit through `VK_EXT_descriptor_indexing`, enabling a single descriptor set to reference thousands of textures indexed at runtime. This chapter covers the shader-side components of both: the feedback pass, the indirection lookup, mipmap bias calculation for correct tile resolution, and the bindless array access pattern.

### 1.2 What is Order-Independent Transparency?

Order-independent transparency (OIT) refers to a family of techniques that resolve the blending order of semi-transparent geometry without requiring the scene to be sorted by depth before rendering. Classical alpha-blending requires back-to-front draw order because each fragment reads and modifies the framebuffer in place; interleaved opaque and transparent geometry, dynamic objects, or large particle systems make an exact depth sort impossible to guarantee at draw-call granularity. OIT algorithms replace the per-pixel blend-in-place model with a per-pixel data structure that accumulates multiple fragment contributions in arbitrary order and resolves them into a correct blended result in a separate pass. The two dominant families are A-Buffer-style per-pixel linked lists — each pixel stores an unbounded list of depth, colour, and alpha records, sorted and composited at resolve time — and moment-based or weighted approximations that trade exact order correctness for bounded memory and simpler arithmetic. On Vulkan, per-pixel linked lists rely on `VK_EXT_fragment_shader_interlock` or image atomics against a UAV buffer to serialize concurrent fragment writes. Weighted blended OIT (WBOIT) requires only standard additive and multiplicative blending modes, making it compatible with fixed-function hardware. This chapter documents the GLSL shader code for linked-list OIT, WBOIT, and moment shadow maps, including the resolve pass that converts accumulated records into a final composited pixel colour.

### 1.3 What is Temporal Anti-Aliasing and ML Upscaling?

Temporal anti-aliasing (TAA) is a post-processing technique that accumulates colour samples across multiple frames by reprojecting previous-frame pixels into the current frame using a per-pixel motion vector buffer. By jittering the camera projection matrix sub-pixel between frames using a Halton or similar low-discrepancy sequence, each frame samples a different point within each pixel footprint; the TAA accumulation filter blends the new sample with a weighted exponential moving average of prior frames, effectively computing a wide reconstruction filter across both space and time at minimal per-frame cost compared with MSAA. Machine-learning upscaling — NVIDIA DLSS, AMD FSR 3, and Intel XeSS — extends this concept by rendering at a lower internal resolution and using a trained neural network or high-quality spatial upscaler to reconstruct a higher-resolution output, amortising the GPU cost of rendering at native resolution. DLSS uses a convolutional neural network evaluated on Tensor Cores; FSR 3 uses a spatial Lanczos-derivative spatial filter with temporal accumulation; XeSS uses a hardware neural model on Intel XMX matrix execution units with a generic DPAS fallback. On Linux, all three integrate through Vulkan external memory and timeline semaphore mechanisms, and vendor SDK layers handle the platform-specific memory export. This chapter covers motion vector generation, disocclusion detection, ghosting suppression via neighbourhood clamping, and the blend-weight computation that constitutes the core TAA algorithm, plus the integration contracts that vendor upscalers impose on the application's rendering pipeline.

### 1.4 What is GPU Compute and Wave Intrinsics?

GPU compute, exposed through Vulkan's compute pipeline and GLSL or HLSL compute shaders, moves general data-parallel algorithms onto the GPU without routing work through the rasterization pipeline. A compute shader dispatches a three-dimensional grid of workgroups; each workgroup consists of a fixed number of invocations that execute in lockstep on a SIMD execution unit called a warp (NVIDIA), a wavefront (AMD), or a subgroup in Vulkan terminology (exposed via `VK_KHR_shader_subgroup_operations`, promoted to Vulkan 1.1 core). Wave intrinsics are cooperative operations across all active invocations within a subgroup: reductions such as `subgroupAdd` and `subgroupMax`, inclusive and exclusive prefix scans via `subgroupInclusiveAdd`, ballot queries via `subgroupBallot`, and lane-to-lane shuffle operations via `subgroupShuffle`. These primitives communicate data between invocations without shared memory or explicit synchronization barriers, and they underlie the most efficient GPU algorithms: radix sort, parallel prefix sums, histogram construction for auto-exposure, stream compaction for culling, and cooperative matrix multiplication via `VK_KHR_cooperative_matrix`. Subgroup size varies by hardware: 32 lanes on NVIDIA, 64 on AMD, 8–32 on Intel; algorithms must either query `VkPhysicalDeviceSubgroupProperties` or be written to handle variable subgroup sizes. This chapter documents GLSL compute shader implementations of each algorithm, discusses occupancy, bank-conflict avoidance, and shared-memory layout, and maps each dispatch structure to the surrounding Vulkan command buffer.

## Table of Contents

- [1.1 What is Virtual Texturing?](#11-what-is-virtual-texturing)
- [1.2 What is Order-Independent Transparency?](#12-what-is-order-independent-transparency)
- [1.3 What is Temporal Anti-Aliasing and ML Upscaling?](#13-what-is-temporal-anti-aliasing-and-ml-upscaling)
- [1.4 What is GPU Compute and Wave Intrinsics?](#14-what-is-gpu-compute-and-wave-intrinsics)
...
- **Bindless descriptors move to buffer-resident descriptor heaps.** `VK_EXT_descriptor_heap` extends the descriptor-buffer model from `VK_EXT_descriptor_buffer` so that bindless texture arrays (as used throughout this chapter's texturing and material sections) can live in ordinary buffer memory managed alongside other GPU allocations, rather than through the more rigid `descriptorIndexing`/`UPDATE_AFTER_BIND` model; the extension's task list shows drivers already shipping on NVIDIA and AMD, with Mesa RADV and ANV merge requests in progress. [Source: VK_EXT_descriptor_heap release tracking](https://github.com/KhronosGroup/Vulkan-Docs/issues/2662)
- **ML upscalers converge on transformer and multi-frame-generation models across vendors.** NVIDIA DLSS 4 replaced its convolutional super-resolution network with a transformer model and added Multi Frame Generation, with DLSS 4.5 introducing a second-generation transformer and Dynamic MFG. [Source: NVIDIA DLSS overview](https://developer.nvidia.com/rtx/dlss) — AMD's FSR SDK 2.3.0 release extended ML-based FSR Upscaling 4.1 and FSR Frame Generation 4.0 (both previously RDNA4-only) to RDNA 3 discrete GPUs. [Source: FidelityFX SDK v2.3.0 release notes](https://github.com/GPUOpen-LibrariesAndSDKs/FidelityFX-SDK/releases/tag/v2.3.0) — Intel's XeSS SDK 3.0 added 3x/4x multi-frame generation alongside improved frame-generation models. [Source: Intel XeSS SDK v3.0.0 release notes](https://github.com/intel/xess/releases/tag/v3.0.0)
- **3D Gaussian Splatting gains a standardized interchange extension.** Khronos merged `KHR_gaussian_splatting` into the glTF extension registry, giving the splatting techniques covered in this chapter's Transparency, Reflections, and Compositing category a vendor-neutral asset format; multiple compression sub-extensions (EGSC, an updated spz variant, and an MPEG-aligned Gaussian Splats extension) remain under active review. [Source: KHR_gaussian_splatting extension](https://github.com/KhronosGroup/glTF/tree/main/extensions/2.0/Khronos/KHR_gaussian_splatting)

### Medium-term (1-3 years)

- **Neural rendering primitives standardize at the shader level.** NVIDIA's Vulkan `VK_NV_cooperative_vector` extension exposes cooperative-vector types in SPIR-V for evaluating small neural networks (MLP-based BRDFs, neural texture compression) without requiring full subgroup occupancy, and NVIDIA's open-source RTX Neural Shading SDK builds on it (and the equivalent DirectX Cooperative Vector path) using the Slang shading language as the cross-platform front end. [Source: VK_NV_cooperative_vector specification](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/appendices/VK_NV_cooperative_vector.adoc); [Source: RTX Neural Shading SDK](https://github.com/NVIDIA-RTX/RTXNS)
- **Cooperative matrix hardware moves beyond simple GEMM.** `VK_NV_cooperative_matrix2` extends the Vulkan 1.3-core `VK_KHR_cooperative_matrix` model with type/use conversions, reductions, per-element operations, tensor addressing, and workgroup-scope matrices with compiler-managed shared-memory staging — capabilities aimed squarely at fusing this chapter's GPU compute primitives (sorting, spatial hashing, sampling) with inline ML inference in the same compute dispatch. [Source: VK_NV_cooperative_matrix2 specification](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/appendices/VK_NV_cooperative_matrix2.adoc)
- **Gaussian-splat compression consolidates around the glTF extension family.** Several independently proposed compression schemes for `KHR_gaussian_splatting` payloads (EGSC, self-organizing/spz-style quantization, layered LGSC) are being reviewed in parallel in the glTF working group, the same pattern Draco and Basis Universal followed before becoming the de facto glTF mesh/texture compression defaults. [Source: glTF KHR_gaussian_splatting compression PRs](https://github.com/KhronosGroup/glTF/pulls?q=is%3Apr+gaussian_splatting_compression)

### Long-term

- **Cooperative-vector/matrix ML primitives are likely to stay vendor-fragmented before any cross-vendor ratification.** `VK_NV_cooperative_vector` and `VK_NV_cooperative_matrix2` are NVIDIA-specific extensions with no Khronos-ratified `KHR`-level equivalent yet; no public Vulkan roadmap milestone currently commits to one, so Slang's role as a vendor-neutral shading-language front end (already used by RTXNS) is likely to remain the practical portability layer for neural-shading code for some time. [Source: RTX Neural Shading SDK requirements](https://github.com/NVIDIA-RTX/RTXNS)
- **ML frame generation shifts from an optional toggle to a load-bearing part of the pipeline.** As DLSS Multi Frame Generation, FSR Frame Generation, and XeSS multi-frame generation all move toward generating multiple interpolated frames per rendered frame, the motion-vector generation, disocclusion detection, and ghosting-suppression techniques this chapter documents for TAA become correctness-critical inputs to frame generation rather than only an anti-aliasing concern. [Source: NVIDIA DLSS overview](https://developer.nvidia.com/rtx/dlss)
- **Order-independent transparency is likely to remain a software technique.** No current Vulkan or GPU vendor roadmap proposes fixed-function OIT resolve hardware; as long as that remains true, WBOIT, per-pixel linked lists, and moment-based OIT will continue to coexist as software trade-offs between memory cost and accuracy rather than being superseded by a hardware primitive.
