...
- **SAM 3 extends the SAM/SAM2 line to open-vocabulary, text-prompted segmentation.** SAM 3 unifies a DETR-style detector and a SAM2-derived tracker behind a shared vision encoder, adding the ability to exhaustively segment every instance of a concept specified by a short text phrase or exemplar image, rather than requiring point/box prompts per object as in §4. A shared-memory multi-object tracking mode further reduces per-object tracking overhead for video segmentation workloads. [Source: SAM 3 repository, https://github.com/facebookresearch/sam3]
- **GPU-resident, zero-copy ROS 2 message transport continues active development.** NVIDIA's Isaac ROS NITROS package, which underlies the GPU tensor topic pattern described in §11.3, continues to receive regular releases extending type-adapted, zero-copy message passing between GPU-accelerated perception nodes, reducing the CPU-serialisation overhead this chapter flags as a current limitation. [Source: Isaac ROS NITROS repository and releases, https://github.com/NVIDIA-ISAAC-ROS/isaac_ros_nitros]

### Medium-term (1-3 years)
- **Vulkan is gaining a native tensor resource type for ML compute.** The `VK_ARM_tensors` extension (with a corresponding `SPV_ARM_tensors` SPIR-V capability) adds first-class tensor resources, tensor views, and tensor-aware descriptor bindings to Vulkan, rather than requiring inference frameworks to hand-lower GEMM and convolution onto buffer/image resources as described for the Vulkan compute fallback path in §1.3 and §10.3. Ratification and multi-vendor driver support are still pending, but the specification work signals Khronos consolidating ML-inference primitives into the core Vulkan compute model. [Source: Vulkan-Docs `VK_ARM_tensors.adoc`, https://github.com/KhronosGroup/Vulkan-Docs/blob/main/appendices/VK_ARM_tensors.adoc]
- **The sparse-convolution library landscape is consolidating around actively maintained projects.** Of the three sparse-convolution backends surveyed in §5.3, `spconv` and TorchSparse continue to see regular maintenance releases, while MinkowskiEngine's development pace has slowed markedly; point-cloud and voxel semantic-segmentation pipelines built on sparse convolution are likely to converge on the actively maintained backends over this window. [Source: spconv repository, https://github.com/traveller59/spconv; TorchSparse repository, https://github.com/mit-han-lab/torchsparse; MinkowskiEngine repository, https://github.com/NVIDIA/MinkowskiEngine]

### Long-term
- **Cross-vendor GPU inference for scene understanding remains fragmented, with Vulkan compute as the slow-converging common path.** No vendor currently commits to a single cross-platform runtime spanning NVIDIA (TensorRT/CUDA), AMD (MIGraphX/ROCm), and Intel (OpenVINO/Level Zero) at parity — the multi-runtime table in §1.3 and §10 is likely to persist. Native ML primitives entering the Vulkan core (per the `VK_ARM_tensors` work above) are the most plausible long-run convergence point for a vendor-neutral scene-understanding inference path, since Vulkan compute is already the fallback used by cross-vendor projects such as `llama.cpp`'s vision-encoder shaders, but full driver-level ML tensor support across all major vendors is not yet committed on any published roadmap.
- **Open-vocabulary 3D scene understanding is likely to keep tracking foundation-model concept segmentation rather than fixed class vocabularies.** The CLIP-feature-distillation techniques in §6 (Gaussian Grouping, LangSplat, LERF) predate SAM 3's exhaustive concept-segmentation capability; extending SAM 3-style text-conditioned, multi-instance concept detection into multi-view 3D fusion — in place of the single-mask, per-click prompting these 3D methods currently build on — is a natural continuation of the trend from closed-set semantic labels toward open-vocabulary, language-conditioned 3D scene representations, though no production pipeline combining SAM 3 with 3D Gaussian Splatting has been published as a stable reference implementation.
