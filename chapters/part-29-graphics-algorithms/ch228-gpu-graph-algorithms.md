...
- **AMD is filling the graph-analytics gap noted in §11.4 with a native HIP library.** `rocGRAPH` mirrors cuGraph's algorithm surface — traversal, centrality, community detection, components, link analysis, and GNN sampling — built directly on HIP and linking `hipcub`, `rocthrust`, and a ROCm port of RAPIDS' RMM allocator, rather than relying on the RAPIDS-on-ROCm compatibility path described in §11.4. As of this writing it is a source-only release with documentation still in progress, but the algorithm-category layout already closely tracks cuGraph's. [Source: AMD-Ecosystem/rocGRAPH README](https://github.com/AMD-Ecosystem/rocGRAPH)
- **The cuGraph GNN stack has consolidated onto PyTorch Geometric.** `cugraph-dgl`, referenced implicitly by this chapter's DGL coverage in §10.2, has been removed from the RAPIDS GNN monorepo; the project states it does not plan further work in the DGL ecosystem and directs users to `cugraph-pyg` for GPU-accelerated sampling, heterogeneous graph support, and a unified loader API. [Source: rapidsai/cugraph-gnn README](https://github.com/rapidsai/cugraph-gnn)
- **Gunrock is broadening from a CUDA-only research library toward a dual-backend, cross-framework tool.** Recent work adds a HIP code path alongside the existing CUDA path (enabling builds against AMD's ROCm libraries) and `pygunrock`, a nanobind-based Python binding layer that exposes Gunrock's algorithms through zero-copy PyTorch tensors — extending the C++ operator model in §11.2 to GPU-resident tensor workflows without a host round-trip. [Source: gunrock/gunrock repository](https://github.com/gunrock/gunrock)

### Medium-term (1-3 years)

- **The DGL vs. PyG gap described in §10.2–10.3 is likely to widen.** DGL's release and commit cadence has slowed markedly compared to PyTorch Geometric, which continues to ship regular minor releases; combined with RAPIDS' explicit move away from DGL support (above), production GPU-accelerated GNN pipelines are likely to continue consolidating around PyG plus `cugraph-pyg`/WholeGraph for large-graph sampling rather than splitting effort across both frameworks. [Source: dmlc/dgl releases](https://github.com/dmlc/dgl/releases) / [Source: pyg-team/pytorch_geometric releases](https://github.com/pyg-team/pytorch_geometric/releases)
- **GraphBLAS's CUDA backend continues incremental, non-production groundwork.** Recent CPU-side refactoring in SuiteSparse:GraphBLAS (memory-arena support, simplified static matrix headers) is explicitly framed as preparation for "future CUDA kernels," but the CUDA directory itself remains labelled a draft with an open, multi-year TODO list covering basic functionality such as per-GPU device selection and full monoid/semiring coverage — the GPU acceleration path for the algebraic BFS/SpMV formulation in §11.3 is unlikely to reach production maturity on this timeframe. [Source: DrTimothyAldenDavis/GraphBLAS README and CUDA/TODO.txt](https://github.com/DrTimothyAldenDavis/GraphBLAS)
- **AMD's graph library is likely to mature past its source-only stage.** rocGRAPH already implements the same algorithm categories cuGraph exposes (§11.1's table), so continued investment toward tagged releases and documented Python bindings — mirroring cuGraph's own Python-over-C++ layering — is a reasonable expectation given ROCm's broader pattern of building HIP counterparts to RAPIDS libraries. [Source: AMD-Ecosystem/rocGRAPH](https://github.com/AMD-Ecosystem/rocGRAPH)

### Long-term

- **A GPU-native GraphBLAS implementation is not a near-term certainty.** The CUDA backend's TODO list has carried largely the same open items (device/GPU selection, full complex-type and monoid support, JIT kernel handling) across multiple releases with no committed production date, so the CPU reference implementation in SuiteSparse:GraphBLAS is likely to remain the practical choice for the semiring-based formulation this chapter describes in §11.3 for the foreseeable future, with cuSPARSE-based SpMV/SpMM (§5.1, §10.1) continuing to serve as the GPU alternative for the algorithms that map onto it. [Source: DrTimothyAldenDavis/GraphBLAS CUDA/TODO.txt](https://github.com/DrTimothyAldenDavis/GraphBLAS)
- **No vendor currently commits to a single cross-platform GPU graph analytics API.** cuGraph (CUDA), rocGRAPH (HIP), and Gunrock's emerging dual-backend model each target their own execution model rather than converging on a shared portable interface such as GraphBLAS's semiring abstraction or a SYCL-based library; readers building portable graph pipelines should expect to keep maintaining vendor-specific code paths, similar to the SpMV/SpMM portability story covered in Ch226. [Source: cuGraph, rocGRAPH, and Gunrock repositories, cited above]
