...
- **ONNX Runtime's WebGPU execution provider offers a vendor-agnostic path for the neural inference workloads in §6.2 and §8.2.** Rather than requiring CUDA or TensorRT, the WebGPU EP dispatches through Dawn to a Vulkan backend on Linux (Direct3D 12 on Windows, Metal on macOS), letting RAFT optical-flow and ESRGAN/Real-ESRGAN super-resolution models run on AMD and Intel GPUs without a vendor SDK; it currently ships as a separate plugin EP with graph-capture support for static-shape models. [Source: ONNX Runtime WebGPU Execution Provider docs](https://onnxruntime.ai/docs/execution-providers/WebGPU-ExecutionProvider.html)
- **libcamera's software ISP continues gaining ground as the fallback path described in §1.6 and §10 for sensors without dedicated ISP hardware.** The `soft_simple` IPA module (`src/ipa/libipa` and `src/ipa/simple/soft_simple.cpp`) implements debayer, black-level, and statistics stages entirely in software; project announcements point to ongoing work on moving per-pixel stages of this pipeline onto the GPU rather than the CPU, which would let the GPU-side deployment pattern in §1.6 apply even on hardware that currently has no ISP block at all. [Source: libcamera project site](https://libcamera.org/) [Source: soft_simple.cpp, libcamera main branch](https://github.com/libcamera-org/libcamera/blob/main/src/ipa/simple/soft_simple.cpp)

### Medium-term (1-3 years)
- **AMD is consolidating its upscaling technologies into a single FidelityFX SDK spanning FSR 2, FSR 3, and the newer ML-based FSR 4 generation, rather than maintaining FSR 2 as a standalone Vulkan-native repository.** Development on the original `FidelityFX-FSR2` repository referenced in §8.3 has effectively stopped in favor of the unified SDK; notably, the unified SDK's own documentation currently lists Vulkan as unsupported, so Linux/Vulkan integrations that need the temporal-upscaling algorithm described in §8.3 still depend on the older standalone FSR2 API rather than the newest FSR4 path. [Source: FidelityFX SDK repository](https://github.com/GPUOpen-LibrariesAndSDKs/FidelityFX-SDK) [Source: FidelityFX-FSR2 repository](https://github.com/GPUOpen-Effects/FidelityFX-FSR2)
- **`VK_KHR_cooperative_matrix`, ratified across NVIDIA, AMD, and Arm as a cross-vendor Khronos extension, gives Vulkan compute shaders direct access to matrix-multiply hardware for the kind of small-network inference used in learned denoising and super-resolution.** This is a more portable alternative to routing neural ISP or super-resolution stages (§8.2, §10.3 denoise) through a separate framework like ONNX Runtime or LibTorch, since the matrix operations execute inside the same compute pipeline as the surrounding image-processing shader passes described throughout this chapter. [Source: VK_KHR_cooperative_matrix specification](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/appendices/VK_KHR_cooperative_matrix.adoc)

### Long-term
- **Vendor-specific extensions for per-invocation neural-network evaluation, such as `VK_NV_cooperative_vector`, point toward evaluating small MLPs (multi-layer perceptrons) directly inside a SIMT compute shader invocation rather than through a separate inference pass.** The proposal targets exactly the per-pixel network evaluation pattern used by neural super-resolution and denoising, letting a future GPU image-processing shader call a matrix-vector multiply against a loaded weight buffer in the same dispatch that performs conventional filtering. As of now this capability is NVIDIA-vendored rather than KHR-ratified the way `VK_KHR_cooperative_matrix` is, so whether it becomes a portable, cross-vendor primitive — as cooperative matrix did — remains an open question rather than a settled roadmap item. [Source: VK_NV_cooperative_vector proposal](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/proposals/VK_NV_cooperative_vector.adoc)
- **No Vulkan or SPIR-V core extension currently proposes a dedicated FFT primitive, so hand-rolled compute-shader FFTs (§5.1) and JIT-compiled libraries such as VkFFT (§5.2) are likely to remain the practical path for GPU frequency-domain image processing rather than a future hardware-accelerated FFT instruction.** VkFFT's own development in this period has concentrated on backend breadth and stability fixes across its Vulkan, CUDA, HIP, OpenCL, and Metal targets rather than new algorithm classes, consistent with FFT support settling into library-level maturity rather than moving into the graphics API itself.
