...
- **Native sphere and linear-swept-sphere ray tracing primitives.** Vulkan's `VK_NV_ray_tracing_linear_swept_spheres` proposal adds hardware-traced sphere and swept-sphere primitives to the ray tracing pipeline, aimed at replacing hand-written analytic intersection shaders like the one in §75.4 with a built-in geometry type — reducing both storage and traversal cost for point-cloud and particle-heavy scenes. [Source](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/proposals/VK_NV_ray_tracing_linear_swept_spheres.adoc)
- **Direct vertex-position fetch at a ray hit.** `VK_KHR_ray_tracing_position_fetch` lets a hit shader read the original triangle vertex positions straight from the acceleration structure instead of duplicating them in a side buffer, simplifying the BLAS-refit workflow described in §75.5 for skinned and procedurally deformed geometry. [Source](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/proposals/VK_KHR_ray_tracing_position_fetch.adoc)
- **NVK closing the open-source ray tracing and mesh shading gap.** Mesa's NVK driver has been adding ray tracing intrinsics and `VK_EXT_mesh_shader` support in recent Mesa release cycles, extending open-source Linux driver coverage of the Vulkan ray tracing pipeline (§75) and mesh/task-shader techniques (§69.4, §72.2, §86.2) beyond RADV to a second vendor. [Source](https://docs.mesa3d.org/relnotes/26.2.0.html)

### Medium-term (1-3 years)
- **Cluster acceleration structures for fast BLAS rebuilds.** `VK_NV_cluster_acceleration_structure` introduces a Cluster-Level Acceleration Structure (CLAS) and a Cluster BLAS built from pre-generated triangle clusters, targeting exactly the dynamic-geometry and LOD-switching build-time bottleneck that terrain streaming (§71) and animated BLAS refit (§75.5) both encounter as scene complexity grows. [Source](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/proposals/VK_NV_cluster_acceleration_structure.adoc)
- **Ray tracing and mesh shading remain outside the portable Vulkan baseline.** Neither the 2022 nor the 2024 Vulkan Roadmap milestone profile requires any ray tracing or mesh shading extension, so engines built on the §75 RT pipeline or the §69.4/§72.2/§86.2 mesh-shader techniques will likely need to keep branching on hardware support rather than assume it as a portable baseline for the medium term. [Source](https://github.com/KhronosGroup/Vulkan-Profiles/blob/main/profiles/Khronos/VP_KHR_roadmap.json)
- **3D Tiles widening to a general geospatial streaming format.** The in-progress 3D Tiles 2.0 draft adds voxel content, time-dynamic tiles, and additional bounding-volume shapes (cylinder and ellipsoid regions) on top of its existing quadtree/octree implicit tiling, extending the kind of GPU-consumable tile hierarchy that §74.3's CDLOD terrain streaming and §74.2's LIDAR decimation pipeline both target. [Source](https://github.com/CesiumGS/3d-tiles/tree/main/next/2.0)

### Long-term
- **Multi-GPU FFT for ocean and spectral terrain synthesis.** VkFFT's published roadmap lists multi-GPU job splitting as a future goal; if delivered, it would let the inverse-FFT ocean synthesis of §73.2 and other spectral geospatial compute scale past a single device's memory and compute budget without a bespoke multi-GPU FFT implementation. [Source](https://github.com/DTolm/VkFFT)
- **CPU navmesh tooling continuing alongside GPU pathfinding.** Recast/Detour — the reference algorithm that §70's GPU voxelisation and region-labelling stages parallelise — has resumed regular, semantically-versioned releases after a long period of dormancy, adding dynamic obstacle support and modern build tooling; no GPU navmesh project currently proposes replacing it outright, so it is likely to remain the correctness baseline that GPU pathfinding pipelines validate against. [Source](https://github.com/recastnavigation/recastnavigation/releases)
