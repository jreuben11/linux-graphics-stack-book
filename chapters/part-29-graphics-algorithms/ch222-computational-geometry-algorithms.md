...
- **FAISS deepens its GPU backend diversity.** Recent FAISS releases have added a `cuVS` IVF scalar-quantiser backend alongside the existing CAGRA/HNSW interop and expanded the Metal GPU backend for non-CUDA platforms, continuing the library's shift toward multiple interchangeable GPU execution backends rather than a single CUDA-only path. This directly extends the ANN infrastructure covered in §8. [Source](https://github.com/facebookresearch/faiss/releases/tag/v1.15.0)
- **CGAL adds a 2D Alpha Wrapping package.** CGAL 6.2 introduced a new 2D Alpha Wrapping component that takes a polygon/segment soup or point set and shrink-wraps a Delaunay triangulation into a guaranteed watertight, intersection-free enclosure with two tunable parameters (alpha, offset) trading fidelity for output complexity — a robustness-first complement to the exact CDT and Boolean operations discussed in §3 and §4.2. [Source](https://github.com/CGAL/cgal/releases/tag/v6.2)
- **Clipper2 gains a native Constrained Delaunay Triangulation module.** Clipper2 2.0.1 added a CDT module to the same library that performs the polygon Boolean operations covered in §3.2, narrowing the gap between Clipper2's fast integer-clipping pipeline and the CGAL CDT path described in §4.2 for applications that previously needed both libraries. [Source](https://github.com/AngusJohnson/Clipper2/releases/tag/Clipper2_2.0.1)

### Medium-term (1-3 years)

- **GPU-native graph-based ANN indices displace brute-force flat search for large datasets.** NVIDIA's cuVS library (part of RAPIDS) continues active development of CAGRA, a GPU-resident graph index with GPU-to-HNSW interop, and FAISS now builds directly against cuVS as an alternative backend to its own IVF/PQ implementations described in §8.1–8.2. This shifts large-scale ANN workloads from the brute-force SGEMM-based flat search this chapter documents toward graph-traversal kernels. [Source](https://github.com/rapidsai/cuvs/releases/tag/v26.08.00)
- **CGAL continues modularising its mesh-processing packages.** The former monolithic "Polygon Mesh Processing" package was split into dedicated Boolean Operations, Meshing/Remeshing, and Mesh Repair packages in CGAL 6.2 (header compatibility preserved), reflecting a broader trend of decomposing exact CPU geometry kernels into independently consumable components — the same boundary this chapter's GPU-upload pattern (§1, §3.2, §4.2) already exploits. [Source](https://github.com/CGAL/cgal/releases/tag/v6.2)
- **Research on GPU-friendlier exact predicates continues without displacing Shewchuk's adaptive arithmetic.** Libraries such as Indirect Predicates and Geogram remain under active development as CPU-side alternatives/extensions to Shewchuk's 1996 adaptive predicates (§12.2) for mesh-processing pipelines, but none has yet produced a GPU-native exact arithmetic kernel; the fixed-point and filtered-float approaches in §12.3 remain the practical GPU path. [Source](https://github.com/MarcoAttene/Indirect_Predicates)

### Long-term

- **No upstream project currently offers a GPU-native arbitrary-precision geometry kernel.** The CPU-exact/GPU-approximate split this chapter documents throughout — CGAL and Clipper2 for topology, compute shaders for data-parallel queries — has no committed replacement on any vendor or open-source roadmap; it is likely to persist until GPU-resident arbitrary-precision or interval arithmetic becomes practical at SIMT scale.
- **FLANN's role continues to shrink relative to FAISS and cuVS.** FLANN (§8.4) has seen little upstream activity in recent years while FAISS and cuVS have continued frequent GPU-focused releases; ANN workloads on Linux graphics and ML pipelines are likely to keep consolidating around the FAISS/cuVS combination rather than FLANN's CPU-only KD-tree and LSH indices.
- **Cluster-based acceleration structures hint at a broader hardware direction for GPU-resident spatial data.** Vulkan's `VK_NV_cluster_acceleration_structure` proposal introduces pre-built triangle clusters (CLAS) that dramatically reduce BVH build time for dynamic geometry. It targets ray tracing rather than general point-location or broadphase queries, but if GPU vendors extend similar incrementally-updatable, GPU-resident spatial primitives beyond ray tracing, the CPU-topology/GPU-query split used for BVH point location (§5.2) and broadphase (§6.2) could narrow — though no vendor roadmap currently commits to that generalisation. [Source](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/proposals/VK_NV_cluster_acceleration_structure.adoc)
