...
- **Opacity micromaps have not reached the open Linux drivers.** `VK_EXT_opacity_micromap`, a ratified cross-vendor Khronos extension that lets an acceleration structure encode per-triangle transparency directly rather than routing every alpha-tested hit through an any-hit shader, is specified with contributions from NVIDIA, Intel, and AMD but is not yet implemented in RADV. Landing it would let the any-hit alpha-masking pattern described in this chapter be replaced with build-time-baked opacity data on Mesa's open drivers. [Source](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/appendices/VK_EXT_opacity_micromap.adoc)
- **Driver-level ray tracing coverage across Mesa remains uneven.** RADV (targeting AMD RDNA2 and later) carries substantially more of the `VK_KHR_acceleration_structure` / `VK_KHR_ray_tracing_pipeline` / `VK_KHR_ray_query` extension set than NVK (targeting NVIDIA Turing and later), so which of the hardware ray tracing shader patterns in this chapter are usable end-to-end on Linux still depends on the driver, not just the GPU generation. [Source: Mesa driver extension matrix](https://mesamatrix.net/)
- **NRD continues iterating on cross-vendor denoising robustness.** NVIDIA's open-source Real-Time Denoisers library keeps refining REBLUR and RELAX for artifact reduction on non-NVIDIA hardware, upscaler compatibility, and performance, extending the same spatiotemporal-filtering lineage as the SVGF/A-SVGF denoisers this chapter documents rather than introducing a new denoising paradigm. [Source](https://github.com/NVIDIAGameWorks/RayTracingDenoiser)

### Medium-term (1-3 years)
- **Cluster acceleration structures target the exact BVH-rebuild bottleneck this chapter identifies.** `VK_NV_cluster_acceleration_structure` lets an application build bottom-level acceleration structures from pre-generated triangle clusters via indirect GPU-side commands, aimed at the acceleration-structure rebuild cost that this chapter names as "the primary limit on displacement frequency" for compute-displaced, ray-traced geometry. It remains an NVIDIA-only vendor extension with no committed cross-vendor ratification. [Source](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/appendices/VK_NV_cluster_acceleration_structure.adoc)
- **Displacement and hair primitives are moving into the acceleration-structure format itself.** `VK_NV_displacement_micromap` encodes fine surface detail as a compact displacement map baked into the BVH rather than a fully expanded vertex buffer, and `VK_NV_ray_tracing_linear_swept_spheres` adds native sphere and linear-swept-sphere primitives for hardware-traced hair and fur. Both target patterns this chapter documents as workarounds — compute-shader displacement with full BVH rebuilds, and geometry-shader fur shells noted as superseded by strand-based approaches — with hardware-native equivalents; both remain NVIDIA-only today. [Source: displacement micromap](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/appendices/VK_NV_displacement_micromap.adoc) [Source: linear swept spheres](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/appendices/VK_NV_ray_tracing_linear_swept_spheres.adoc)
- **Neural evaluation primitives are entering the shading pipeline.** `VK_NV_cooperative_vector` is a Khronos proposal (not yet a ratified extension) for per-invocation matrix-vector multiply acceleration aimed at evaluating small neural networks — neural materials, neural texture decompression — inline in fragment and ray-tracing shaders, alongside the classical procedural noise and SDF techniques this chapter catalogues. [Source](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/proposals/VK_NV_cooperative_vector.adoc)

### Long-term
- **Khronos's own cross-vendor baseline does not yet commit to ray tracing.** The Vulkan Roadmap 2026 milestone requires `VK_KHR_cooperative_matrix` as a baseline capability but includes no ray tracing extension requirement, indicating that hardware ray tracing acceleration is still treated as a vendor-differentiated capability rather than an assumed baseline across the Vulkan device ecosystem for the foreseeable future. [Source](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/appendices/roadmap/Roadmap-2026.adoc)
- **The newer NVIDIA-only geometry extensions may stay NVIDIA-only for some time.** Cluster acceleration structures, displacement micromaps, and linear swept spheres each solve real problems this chapter's procedural and ray-traced geometry pipelines run into, but no public Khronos or competing-vendor roadmap currently commits to a cross-vendor `EXT`/`KHR` successor for any of them, so applications targeting non-NVIDIA Linux GPUs are likely to keep relying on the compute-displacement-and-rebuild and geometry-shader-shell patterns this chapter documents as the portable fallback.
