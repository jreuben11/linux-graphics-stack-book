...
- **nvcomp continues expanding standard-format coverage.** NVIDIA's nvcomp GPU compression library has added a GZIP compression backend and a Float8 (e4m3) mode for its ANS entropy coder that roughly doubles ANS throughput with minimal ratio loss, alongside incremental LZ4 and Bitcomp throughput gains; the batched native Bitcomp API is marked for removal in a future major version, signalling a shift toward the newer non-batched interface. [Source: nvCOMP Release Notes](https://docs.nvidia.com/cuda/nvcomp/release_notes.html)
- **The ASTC reference encoder keeps hardening rather than adding new block formats.** Recent `astc-encoder` releases have focused on decompressor-only builds that shrink context memory footprint, shared read-only data tables across parallel image contexts to cut memory use in multi-image parallel encoding, and a RISC-V Vector extension SIMD backend, rather than new ASTC block modes — the ASTC bitstream itself is stable and no new footprints are in development. [Source: astc-encoder releases](https://github.com/ARM-software/astc-encoder/releases)
- **meshoptimizer's meshlet codec is maturing toward GPU-resident decode.** The `meshopt_encodeMeshlet`/`meshopt_decodeMeshlet` API family and the compute-shader meshlet decoder (`meshletdec.slang`) — reaching over 100 GB/s on recent discrete GPUs — moved from experimental to stable status, alongside a cluster-position shared-exponent quantisation helper aimed at the upcoming DXR2 `Compressed1` position-encoding format. [Source: meshoptimizer v1.2 release notes](https://github.com/zeux/meshoptimizer/releases/tag/v1.2)

### Medium-term (1-3 years)
- **KHR_meshopt_compression is positioned to succeed EXT_meshopt_compression as the default glTF geometry-compression path.** The Khronos-ratified extension improves attribute compression ratio over the older multi-vendor EXT extension while keeping meshoptimizer's fast branchless decode; `gltfpack` already supports emitting it, though loader/runtime support is still catching up to the long-established EXT extension. [Source: KHR_meshopt_compression proposal](https://github.com/KhronosGroup/glTF/pull/2517)
- **Fixed-rate compression is becoming an explicit, application-controllable Vulkan concept rather than a vendor-opaque display-pipe optimisation.** `VK_EXT_image_compression_control` lets an application request a fixed compression rate for an image and query the rate an implementation actually applied, formalising in the Vulkan API the same class of trade-off that AMD DCC, Intel FBC, and ARM's newer fixed-rate format already make at the hardware level; the extension remains EXT-only, so broader driver adoption or a future KHR promotion would extend explicit compression control beyond its current Arm/Broadcom/Imagination-driven scope. [Source: VK_EXT_image_compression_control specification](https://registry.khronos.org/vulkan/specs/latest/man/html/VK_EXT_image_compression_control.html)
- **ARM Fixed-Rate Compression (AFRC) broadens the AFBC-era Linux modifier vocabulary.** AFRC modifiers (`DRM_FORMAT_MOD_ARM_AFRC`) are already defined in the upstream DRM UAPI headers as a fixed-rate counterpart to AFBC's variable-rate scheme, intended for buffers such as textures where predictable footprint size matters more than maximum compression ratio; driver- and compositor-side enablement is expected to follow the same Panfrost/Panthor and KMS plane-modifier negotiation path that AFBC took. [Source: Linux kernel drm_fourcc.h](https://github.com/torvalds/linux/blob/master/include/uapi/drm/drm_fourcc.h)

### Long-term
- **Dedicated offline CPU/OpenCL encoder SDKs face pressure from encoding paths integrated directly into GPU compute shaders.** AMD's Compressonator SDK, one of the few tools spanning BC, ETC, and ASTC encoding in a single package, has seen markedly less release activity than the actively developed ASTC and meshoptimizer projects it sits alongside; if that gap persists, texture and mesh compression tooling is likely to consolidate further around in-engine compute-shader encoders (as sketched in §5.4) and format-specific reference encoders rather than general-purpose offline SDKs. [Source: Compressonator repository](https://github.com/GPUOpen-Tools/Compressonator)
- **No vendor roadmap currently commits to a BC/ASTC successor bitstream**, so the block-compression landscape — BC1–BC7 on desktop GPUs, ASTC across mobile and mixed deployments — is likely to remain the fixed-function baseline for the foreseeable future. Most near-term quality gains are coming from better encoders (RDO passes, exhaustive mode-search heuristics) rather than new bitstreams, since any new texture-compression format would need dedicated fixed-function decode hardware across the GPU install base before applications could rely on it.
