...
- **`KHR_meshopt_compression` loader adoption still catching up to the spec.** The Khronos-prefixed successor to the vendor `EXT_meshopt_compression` extension has been added to the glTF extension registry, but the meshoptimizer maintainers note that loader support for it is "not yet widely available," so `gltfpack` continues to default to emitting the older `EXT_` form until runtimes catch up — an adoption lag that mesh-compression pipelines using §16's `meshopt_encodeVertexBuffer`/`decodeVertexBuffer` path should track. [Source](https://github.com/zeux/meshoptimizer/releases/tag/v1.0), [Source](https://github.com/KhronosGroup/glTF/tree/main/extensions/2.0/Khronos/KHR_meshopt_compression)
- **Cluster-based LOD scaffolding maturing alongside stabilized simplification APIs.** meshoptimizer's core clusterization and simplification entry points have been promoted to a stable API surface, but the maintainers flag permissive (topology-changing) simplification as still experimental and likely to change in upcoming releases, while `clusterlod.h` continues to be developed in the same repository as reusable scaffolding for Nanite-style hierarchical cluster LOD — directly relevant to the mesh-shader LOD techniques in §10.4. [Source](https://github.com/zeux/meshoptimizer/releases/tag/v1.0)
- **Manifold's boolean/CSG engine moving from experimental integration to shipped default.** The library's point-release cadence has stayed rapid and its README documents production consumers (including Blender's boolean modifier and Babylon.js's CSG2 module) that moved from optional/experimental integration toward being the shipped default solver — evidence for §7's guidance to prefer a robust, actively developed winding-number engine over older Cork/CGAL-style boolean pipelines for new work. [Source](https://github.com/elalish/manifold/releases), [Source](https://github.com/elalish/manifold)

### Medium-term (1-3 years)
- **NanoVDB extending past static rendering toward dynamic-topology GPU operations.** OpenVDB/NanoVDB release notes describe GPU-side level-set tracking, grid building, morphological operations, and grid merging as capabilities being built out on top of NanoVDB's sparse index-grid layout, with this work forming the explicit foundation for the newer `fVDB` differentiable-voxel framework hosted alongside the core library — a trajectory that extends §3.6/§3.7's coverage of NanoVDB from a read-mostly rendering structure toward a general dynamic GPU volume-editing substrate. [Source](https://github.com/AcademySoftwareFoundation/openvdb/releases/tag/v13.0.0)
- **Half-Grid (16-bit float) storage still an active-optimization target.** The same release notes flag that "more work is planned on adapting algorithms" to run efficiently over OpenVDB's memory-saving Half Grid representation, meaning the precision/bandwidth trade-offs relevant to large-scale SDF and volumetric mesh-extraction pipelines (§4's marching cubes and §6's SDF CSG operate on comparable dense/sparse voxel data) are still being tuned rather than settled. [Source](https://github.com/AcademySoftwareFoundation/openvdb/releases/tag/v13.0.0)
- **OpenSubdiv's Vulkan-native evaluator gap persists as an open community request.** A long-standing request to add a Vulkan compute backend alongside OpenSubdiv's existing CUDA, OpenCL, and GLSL/D3D compute evaluators remains open with no `Osd::VkComputeEvaluator` merged, even as the library continues to cut new tagged releases past the 3.6.0 baseline covered in §1.4 — keeping the custom Vulkan stencil-evaluation approach in §1.5 the practical integration path rather than a stopgap. [Source](https://github.com/PixarAnimationStudios/OpenSubdiv/tags), [Source](https://github.com/PixarAnimationStudios/OpenSubdiv/issues/1153)

### Long-term
- **Mesh-compression codecs likely to keep consolidating around the meshopt family for real-time content.** Draco's tagged-release cadence has stalled to occasional dependency-maintenance commits with no new versioned release in some time, while the meshopt lineage has gained a Khronos-registered glTF extension of its own; absent a resumed Draco release cycle, real-time and game-asset pipelines are likely to keep favoring meshopt-derived codecs (§16.3) for new content, with Draco's stronger entropy-coded compression ratio (§16.2) retained mainly where existing WebGL2/Draco-decoder toolchains are already deployed. [Source](https://github.com/google/draco/releases), [Source](https://github.com/KhronosGroup/glTF/tree/main/extensions/2.0/Khronos/KHR_meshopt_compression)
- **Cluster/meshlet-granular acceleration structures likely to stay vendor-specific.** Vendor proposals for cluster-level ray-tracing acceleration structures exist only as single-IHV Vulkan extensions with no cross-vendor Khronos equivalent yet in the public proposals directory, so GPU-driven meshlet and LOD pipelines built on §10's mesh-shader and cluster-culling techniques are likely to keep requiring per-vendor code paths rather than a unified API for the foreseeable future. [Source](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/proposals/VK_NV_cluster_acceleration_structure.adoc)
