...
- **World-space radiance caching is consolidating around hash-based and neural probe replacements.** NVIDIA's RTXGI SDK line has moved from the original DDGI probe-grid algorithm (§Global Illumination, §World-Space Radiance Cache) to a second-generation architecture built on a Spatially Hashed Radiance Cache (SHaRC, no vendor-specific hardware required) and an experimental Neural Radiance Cache (NRC, requiring Tensor Core-class hardware), with the legacy DDGI code path retained only in an archived branch. [Source: RTXGI SDK](https://github.com/NVIDIAGameWorks/RTXGI)
- **glTF is standardizing thin-surface diffuse transmission.** `KHR_materials_diffuse_transmission`, the same phenomenon the Translucency section models with ad hoc wrap lighting and thickness maps, is progressing through the Khronos Release Candidate stage toward ratification as a first-class interchange extension. [Source: KHR_materials_diffuse_transmission](https://github.com/KhronosGroup/glTF/blob/main/extensions/2.0/Khronos/KHR_materials_diffuse_transmission/README.md)
- **Cooperative-vector shader primitives are landing in Vulkan.** The `VK_NV_cooperative_vector` extension proposal adds matrix-vector multiply types and instructions scoped to a single shader invocation (rather than a full subgroup, as cooperative matrices require), specifically to let ordinary fragment/compute shaders evaluate small per-invocation MLPs — the mechanism NVIDIA's RTX Neural Materials and Neural Radiance Cache rely on for inference inside the shading pipeline. [Source: VK_NV_cooperative_vector proposal](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/proposals/VK_NV_cooperative_vector.adoc)

### Medium-term (1-3 years)
- **Neural compression and inference are being folded into the material and GI pipeline stages this chapter catalogues.** NVIDIA's RTX Neural Shaders initiative packages neural texture compression, neural materials (compressing multi-layer BRDF stacks for faster evaluation), and the Neural Radiance Cache into an SDK trainable on end-user RTX hardware, with cooperative-vector-based inference support expanding from a single vendor extension toward broader API coverage. [Source: NVIDIA RTX Neural Rendering](https://developer.nvidia.com/blog/nvidia-rtx-neural-rendering-introduces-next-era-of-ai-powered-graphics-innovation/)
- **Radiance Cascades is moving from a 2D research result toward a general GI primitive.** The technique catalogued in the Global Illumination section began as an explicitly work-in-progress 2D formulation; follow-on work extending it toward volumetric/3D radiative transfer and community engine integrations (including a Unity URP implementation) indicate continued research investment, though production engine adoption beyond Godot-community experiments remains limited. [Source: Radiance Cascades](https://radiance-cascades.com/)
- **A vendor-neutral über-shader is emerging to replace ad hoc Principled-BSDF variants.** OpenPBR, developed jointly by Autodesk and Adobe and hosted under the Academy Software Foundation, packages base substrate, coat, and surface-detail layers (the same layering this chapter's Clear Coat, Sheen, and Iridescence sections implement piecemeal) into a single portable specification intended to reduce the need for per-engine reimplementations of the Disney Principled BRDF and Autodesk Standard Surface. [Source: OpenPBR Surface Specification](https://academysoftwarefoundation.github.io/OpenPBR/)

### Long-term
- **Analytic BRDF catalogs like this one are likely to persist as the fallback and training-data layer beneath neural material representations, not be replaced by them.** No vendor roadmap currently commits to eliminating hand-authored GGX/Disney-style BRDF evaluation; instead, neural materials and neural radiance caching are being positioned as compression and acceleration layers on top of conventional shading models, so the analytic models in this chapter should remain the reference implementation that neural approximations are trained against and validated for a long time.
- **World-space, hardware-agnostic radiance caching (SHaRC-style) is likely to become the default mid-tier GI solution**, sitting between cheap screen-space techniques and full hardware ray-traced path tracing, as its explicit design goal of running without vendor-specific ML hardware makes it more portable across the Linux driver ecosystem (open-source RADV/NVK included) than Tensor Core-dependent neural caching. [Source: RTXGI SDK](https://github.com/NVIDIAGameWorks/RTXGI)
