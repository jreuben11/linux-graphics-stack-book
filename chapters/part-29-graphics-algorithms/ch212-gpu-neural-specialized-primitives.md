...
- **glTF gains a Gaussian Splatting extension.** The Khronos glTF working group is actively developing an official representation for 3D Gaussian Splatting scenes (§91, §94) as a glTF extension, with parallel proposals covering the core splat data layout, a coordinate-convention clarification, and multiple competing compression schemes (a generic entropy-coded variant, an MPEG-backed variant, and an anti-aliasing filter proposal). None of these has yet been ratified as a stable Khronos-approved extension, so scene interchange between splat tools remains ad hoc in the meantime. [Source: "Gaussian Splatting in glTF" tracking issue](https://github.com/KhronosGroup/glTF/issues/2454) · [Coordinate convention proposal](https://github.com/KhronosGroup/glTF/issues/2613) · [Compression proposal (EGSC)](https://github.com/KhronosGroup/glTF/issues/2605) · [MPEG Gaussian Splats extension](https://github.com/KhronosGroup/glTF/pull/2573)
- **A vendor-neutral OpenXR reprojection extension.** `XR_EXT_frame_synthesis` generalises the vendor-specific timewarp/spacewarp techniques in §100.1–100.2 into a single cross-vendor mechanism: the application supplies motion-vector and depth images and the runtime performs frame extrapolation/reprojection, rather than each headset vendor (Meta ASW, Varjo, HTC) shipping its own incompatible extension. Contributors span Meta, Varjo, Samsung, Microsoft, Collabora, and ByteDance, signalling convergence toward a single reprojection contract that engines can target once instead of per-runtime. [Source: `XR_EXT_frame_synthesis` specification chapter](https://github.com/KhronosGroup/OpenXR-Docs/blob/main/specification/sources/chapters/extensions/ext/ext_frame_synthesis.adoc)
- **Portable GPU radix sort libraries keep pace with 3DGS demand.** Community Vulkan radix-sort implementations used for the tile/depth key sort in the 3DGS pipeline (§91.2, §94.2, §107) continue to see active maintenance independent of any single splat renderer, reflecting radix sort's role as shared infrastructure across splatting, BVH construction (§24), and GPU sorting workloads generally. [Source: vulkan_radix_sort repository](https://github.com/jaesung-cs/vulkan_radix_sort)

### Medium-term (1-3 years)

- **Ray-traced Gaussian particles as a hybrid alternative to tile rasterization.** NVIDIA's 3DGRUT project extends 3D Gaussian Splatting (§91, §94) with hardware ray tracing of Gaussian particles instead of (or alongside) the tile-based rasterizer described in §91.2/§94.2, enabling secondary rays — shadows, reflections, refraction — to interact with splat geometry the way they already do with triangle BLAS/TLAS content (§75). This is an active, regularly updated research codebase rather than a shipped renderer feature, but it demonstrates a concrete path for unifying splat and ray-traced triangle geometry in the same acceleration structure. [Source: nv-tlabs/3dgrut repository](https://github.com/nv-tlabs/3dgrut)
- **Shading-language-level automatic differentiation for inverse rendering.** The Slang shading language's autodiff support is under continued active development, aiming to let differentiable rendering pipelines (§93) generate backward-pass gradient shaders automatically from a forward-pass shader rather than requiring the hand-written gradient accumulation kernels shown in §93.1–93.3. This does not eliminate hand-rolled backward passes for performance-critical paths (3DGS training in particular remains a hand-tuned CUDA kernel in practice), but it lowers the barrier for smaller-scale inverse-rendering research and prototyping. [Source: shader-slang/slang autodiff issue tracker](https://github.com/shader-slang/slang/issues?q=is%3Aissue+autodiff)
- **Cooperative-vector shading for per-invocation MLP inference.** `VK_NV_cooperative_vector` adds shading-language matrix-vector multiply support so that each shader invocation can independently evaluate a small MLP — precisely the neural-SDF (§92.3), NeRF (§95.1), and Instant-NGP hash-grid MLP (§95.2) inference patterns in this chapter — using dedicated matrix-multiply hardware instead of the manual unrolled-loop GLSL shown in those sections. It complements rather than replaces `VK_KHR_cooperative_matrix` (Ch141), which targets subgroup-wide matrix operations instead of the per-invocation SIMT model these neural-geometry techniques use. [Source: `VK_NV_cooperative_vector` proposal](https://github.com/KhronosGroup/Vulkan-Docs/blob/main/proposals/VK_NV_cooperative_vector.adoc)
- **GPU compute vector rendering as a complement to SDF font atlases.** Actively developed GPU compute-centric 2D renderers that rasterize vector paths (including glyph outlines) directly via compute shaders offer an alternative to the precomputed multi-channel SDF atlas approach in §103: no atlas-resolution/quality tradeoff, at the cost of a more complex per-frame compute pipeline than a simple SDF texture sample. Where this settles relative to atlas-based MSDF rendering for text-heavy UI workloads is still an open engineering tradeoff rather than a resolved consensus. [Source: linebender/vello repository](https://github.com/linebender/vello)

### Long-term

- **Splat and mesh representations continuing to converge rather than one replacing the other.** Mesh-extraction techniques from 3DGS (§91.3, SuGaR-style Poisson reconstruction) and ray-traced Gaussian particle pipelines (3DGRUT) both point toward splats and triangle meshes coexisting in the same scene and the same acceleration structure rather than splats displacing meshes outright — no public renderer or engine roadmap currently commits to Gaussian splats as a wholesale mesh replacement, so hybrid pipelines (mesh for solid/animated geometry, splats for captured detail) are the likely durable pattern. [Source: nv-tlabs/3dgrut repository](https://github.com/nv-tlabs/3dgrut); [SuGaR project](https://github.com/Anttwo/SuGaR)
- **Neural network inference becoming a standard GPU shading primitive rather than a bespoke compute kernel.** The trajectory from hand-written MLP-evaluation GLSL (as in §92.3, §95.1–95.3 of this chapter) toward dedicated shading-language primitives (`VK_NV_cooperative_vector`, `VK_KHR_cooperative_matrix`) suggests neural geometry inference will increasingly be expressed with vendor-portable shading-language constructs instead of unrolled dot-product loops — narrowing, though not eliminating, the gap between "graphics compute" and "ML compute" shader code for techniques like neural SDFs and Instant-NGP. No cross-vendor extension currently guarantees performance parity with a hand-tuned CUDA path, so CUDA-first tools (Instant-NGP, gsplat) are likely to remain the reference implementations for some time even as portable equivalents mature.
- **Scene-interchange formats for neural/learned geometry remaining fragmented longer than mesh interchange did.** Given that a stable glTF Gaussian Splatting extension is still contested among several competing compression proposals (see Near-term), and that NeRF/neural-SDF weights have no interchange format at all beyond framework-specific checkpoints, this chapter's Library Landscape (§108) observation — that GPU geometry tooling has not consolidated the way linear-algebra tooling did around BLAS/LAPACK — is likely to remain true for neural and learned geometry specifically for longer than it does for classical mesh geometry.
