...
- **`VK_EXT_shader_split_barrier` reaching wide driver support.** This ratified extension adds `OpControlBarrierArriveEXT` / `OpControlBarrierWaitEXT`, splitting a workgroup barrier into separate arrive and wait phases so fast subgroups can start issuing the next iteration's global-memory loads while slower subgroups are still doing ALU work — directly targeting the synchronisation bubble described for the double-buffering pattern in §4.2 and the barrier cost model in §6.1, and is called out by Khronos as particularly beneficial for tiled cooperative-matrix loops (§8.3). As of mid-2026 the GLSL-extension and conformance-test line items are still open on its Khronos task list, so application-level availability depends on driver and toolchain rollout. [Source: Khronos Vulkan-Docs task-list tracker](https://github.com/KhronosGroup/Vulkan-Docs/issues/2746)
- **RGP continuing to track new hardware and divergence analysis.** Radeon GPU Profiler v2.7 (mid-2026) added support for RDNA 4-based hardware and thread-divergence metrics in the instruction-timing view for non-ray-tracing workloads — a direct extension of the occupancy and divergence analysis workflows in §2.4, §5, and §10.1. [Source: Radeon GPU Profiler v2.7 release notes](https://github.com/GPUOpen-Tools/radeon_gpu_profiler/releases/tag/v2.7)
- **`VK_QCOM_cooperative_matrix_conversion` moving toward full release.** This extension adds SPIR-V instructions to convert directly between per-invocation vectors and cooperative-matrix fragments — avoiding the shared-memory staging round trip — for convolution, LLM inference, and flash-attention-style workloads built on `VK_KHR_cooperative_matrix` (§8.3). Most of its task-list components (SPIR-V spec, headers, tooling, GLSL extension) are complete; validation-layer support remains outstanding at time of writing. [Source: Khronos Vulkan-Docs task-list tracker](https://github.com/KhronosGroup/Vulkan-Docs/issues/2726)

### Medium-term (1-3 years)
- **`VK_NV_cooperative_vector` broadening beyond NVIDIA.** This ratified (non-provisional) extension exposes per-invocation MLP evaluation — small matrix-vector products for neural network layers — as a first-class shader operation on Turing-and-later NVIDIA hardware, complementing the tile-based `VK_KHR_cooperative_matrix` GEMM path covered in §8.3 for the growing class of neural-rendering and shader-embedded-inference workloads. It currently has no cross-vendor KHR equivalent; whether AMD and Intel follow with their own implementations is an open question the roadmap does not yet answer. [Source: NVIDIA `vk_cooperative_vector_perf` sample](https://github.com/jeffbolznv/vk_cooperative_vector_perf)
- **GPU-side dynamic dispatch (Vulkan Work Graphs) remaining unresolved cross-vendor.** An open Khronos feature request asks Vulkan to expose DirectX 12-style Work Graphs; AMD ships a vendor-specific preview (`VK_AMDX_shader_enqueue`, still flagged `provisional` in the Vulkan registry) but no ratified, cross-vendor extension exists yet. Until one lands, the atomic work-queue persistent-kernel pattern in §7 remains the portable way to handle dynamically generated GPU work across AMD, Intel, and NVIDIA on Linux. [Source: Khronos Vulkan-Docs feature-request tracker](https://github.com/KhronosGroup/Vulkan-Docs/issues/2153)
- **Forward-progress guarantees for compute workgroups under active discussion.** An open Khronos roadmap-feedback issue documents that Vulkan currently gives no guarantee that a resident workgroup will ever be scheduled to make progress, which makes the cross-workgroup rendezvous that a fully general persistent-kernel or work-stealing scheme (§7) would want to rely on technically undefined behaviour today — current implementations that avoid deadlock (as in §7.3) do so by sizing dispatches conservatively rather than by a spec guarantee. [Source: Khronos Vulkan-Docs roadmap-feedback tracker](https://github.com/KhronosGroup/Vulkan-Docs/issues/2233)

### Long-term
- **The bandwidth wall continues to widen the ridge point.** Each hardware generation has kept adding FLOPS faster than DRAM bandwidth, and no vendor roadmap currently commits to closing that gap; the practical consequence for the roofline analysis in §1 is that a growing share of naively-written kernels will land in the memory-bound region even as peak TFLOPS figures keep climbing, making the memory-access and cache-locality techniques in §3 and §4 more, not less, load-bearing over time.
- **Split barriers, forward-progress guarantees, and cooperative-vector/matrix interop are converging on the same problem.** `VK_EXT_shader_split_barrier`, the open forward-progress-guarantee request, and the cooperative-matrix-conversion work all target finer-grained, lower-stall synchronisation for tiled and persistent-kernel-style compute. If Khronos ratifies a cross-vendor answer to GPU-side dynamic dispatch on top of these building blocks, it would formalise a pattern this chapter currently has to hand-roll with atomic work queues (§7) — but as of today that remains a roadmap discussion, not a shipped cross-vendor feature, so the hand-rolled pattern is likely to remain the production-portable choice for some time yet.
