...
The scale of Mesa reflects this breadth. At the close of 2025 the Mesa repository contained approximately **5.9 million lines of code** — comparable in order of magnitude to the Linux kernel itself — and was adding roughly one million new lines per year as new GPU generations, driver families, and compiler infrastructure land. The yearly churn (~1M lines added, ~840k removed in 2025) alone exceeds the total size of wlroots, the most widely deployed Wayland compositor library. [[Source: Phoronix Mesa 2025 git statistics]](https://www.phoronix.com/news/Mesa-Git-Stats-2025)
...
- **Vulkan 1.4 consolidation across the Mesa ICD fleet.** Mesa 26.1 (May 2026) delivered Vulkan 1.4 conformance for RADV, ANV, NVK, and Turnip through the shared `vk_device_dispatch_table` infrastructure; near-term effort continues filling gaps in PanVK, v3dv, and other smaller ICDs. The loader's ICD discovery path (`loader_get_driver_for_fd()`) is tracking new PCI ID entries for Intel Nova Lake P and PowerVR hardware.
- **Rust GPU compiler backends proliferating.** ARM's KRAID Rust NIR backend for Mali Valhall (v9+) merged into Mesa 26.2-devel (June 2026), joining NAK (NVK/NVIDIA) as the second Rust-written ISA backend in the tree. Intel's new Jay compiler (merged Mesa 26.1-devel) follows the same architectural philosophy in C; near-term work enables Jay by default on Xe2/Battlemage hardware. Together these signal a structural shift in how new ISA backends are authored.
- **ACO becomes the default AMD compiler across all API frontends.** Mesa 26.0 switched RadeonSI OpenGL from LLVM to ACO by default, completing ACO's takeover of the AMD Mesa stack. GFX12 (RDNA4) production hardening and ray-tracing register-allocation improvements (2× RT speedups in Unreal Engine 5 Lumen) are the active correctness work.
- **Zink and NVK as the production NVIDIA OpenGL path.** Mesa 25.1 retired the legacy Nouveau Gallium OpenGL driver in favour of Zink on NVK for Turing and later GPUs. A CodeWeavers merge request (April 2026) proposes building Zink as a Windows PE for Wine so all Wine OpenGL calls route through Zink to the host Vulkan driver, eliminating Wine's legacy WGL/GL layer.
- **NIR type system and intrinsic expansion.** Cooperative matrix arithmetic (`nir_op_cmat_reduce`), `nir_lower_vars_to_scratch_global()` for mobile driver register spilling, and soft-float32 denormal preservation at the NIR level all landed in Mesa 26.0; follow-on work targets SPIR-V 1.6 / Vulkan 1.4 intrinsic coverage and `SPV_KHR_maximal_reconvergence` lowering in `spirv_to_nir()`.
- **Gallium frontier expansion: NPU, media, and mesh shaders.** The Arm Ethos-U85 NPU via the TEFLON framework, Microsoft's 62,000-line MFT video-decode Gallium frontend (Mesa 25.2), and mesh/task shader plumbing for the `pipe_context` draw dispatch path all extend Gallium's compute footprint well beyond traditional graphics workloads.

### Medium-term (1–3 years)

- **Zink as the universal OpenGL consolidation point.** The converging medium-term trajectory across Chapters 12, 13, 17, and 119 is a Mesa tree where hardware vendors ship Vulkan-only drivers and rely on Zink for OpenGL backward compatibility. This entails: deprecation of the legacy DRI `__DRIscreen`/`__DRIcontext` ABI (superseded by Kopper WSI), retirement of the Clover OpenCL frontend in favour of rusticl, and Gallium Nine's already-completed removal (deprecated Mesa 25.1, deleted Mesa 25.2). The `loader_get_driver_for_fd()` path will need graceful fallback to `zink` when no native Gallium OpenGL driver is registered.
- **Explicit synchronisation throughout the Gallium and Vulkan WSI stacks.** Gallium's implicit-fence model conflicts with Vulkan's explicit barrier model and limits Zink batching. Proposed `pipe_barrier` objects mirroring `VkPipelineBarrier` are under discussion; on the display side, `VK_EXT_present_timing` plumbed through the KMS atomic commit path and the Wayland `linux-dmabuf-v1` feedback mechanism will make frame timing explicit end-to-end.
- **Vulkan Roadmap 2026 and descriptor heap rearchitecting.** The Khronos Roadmap 2026 milestone mandates variable-rate shading, host image copies (`VK_EXT_host_image_copy`), and `VK_EXT_descriptor_heap` — a wholesale overhaul of the descriptor system providing direct CPU access to descriptor memory. Because Mesa's `vk_descriptor_set_layout` is the shared abstraction across all Mesa Vulkan drivers, adopting `VK_EXT_descriptor_heap` requires significant reworking of the Vulkan common layer (`src/vulkan/runtime/`) before per-driver adoption is feasible.
- **Nova kernel driver as Nouveau successor.** The Rust-based `nova-drm` is converging on mainline merge (Hopper/Blackwell patches at 12th iteration, mid-2026); the medium-term plan is for Nova to supersede classic Nouveau for Turing and later GPUs, with NVK consuming the Nova userspace API directly. DRM Rust abstraction bindings (Linux 7.2: GPUVM immediate mode, HRT) support this trajectory.
- **Rust convergence across Mesa compiler infrastructure.** NAK, KRAID, and Jay all share an ACO-derived philosophy. Medium-term discussion targets extracting shared Rust utilities (register allocator, liveness analysis, instruction scheduling) into a common crate for NIR-consuming backends. Portions of the Vulkan common runtime (object lifetime tracking, hash table management) are also named as Rust rewrite candidates.
- **MLIR/IREE Triton Vulkan backend feature parity.** Triton's SPIR-V path currently lacks async-copy instructions and Hopper-class TMA; bridging these gaps via new SPIR-V extensions enables Triton kernels on AMD and Intel GPUs. `VK_KHR_cooperative_matrix` v2 with FP8/INT4 operands and IREE HAL NPU backend expansion extend the MLIR-to-Mesa pipeline toward heterogeneous inference workloads.

### Long-term

- **Rust-native Mesa internals and unified AMD compiler strategy.** As NAK, KRAID, and exploratory proposals for Rust-based Mesa loader components (`src/loader/`, `src/util/disk_cache.c`) mature, Mesa's architecture is moving toward a Rust-safe core for security-sensitive paths. In parallel, ACO's potential extension to ROCm HIP and rusticl/OpenCL could converge the AMD Mesa compiler surface to a single IR-and-backend strategy, eliminating the parallel LLVM and ACO maintenance burden.
- **Zink as the sole OpenGL implementation and Vulkan as the universal hardware interface.** With Nouveau GL already retired, Wine proposing Zink-by-default, and hardware vendors shipping Vulkan-only drivers, the long-term endpoint is a Mesa tree where Zink is the only maintained OpenGL implementation, all OpenGL calls route through a single Vulkan command stream per process, and the DRI ABI and classic Gallium OpenGL state tracker have been retired. The enabling condition is Zink reaching full performance parity with native Gallium backends on AMD and Intel hardware.
- **MLIR as a convergence IR for graphics and ML compute pipelines.** The long-term LLVM/MLIR architectural goal is to converge the GLSL→SPIR-V→Mesa NIR shader compilation path and the linalg→gpu→SPIR-V ML compute path on a shared MLIR foundation, potentially allowing Mesa to accept MLIR directly for offline-compiled compute pipelines. Hardware-native FP8/INT4 types in SPIR-V and Vulkan — driven by RDNA4, Blackwell, and Xe3+ hardware — would flow through this unified type system.
