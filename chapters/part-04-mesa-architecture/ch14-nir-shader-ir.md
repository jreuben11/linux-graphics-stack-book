...
This chapter explains **NIR** from first principles, covering:

- **TGSI limitations** — why **TGSI** (Tungsten Graphics Shader Infrastructure), the earlier **Gallium** shader **IR**, became inadequate as **GPU** hardware evolved
- **In-memory data structures** — the complete hierarchy from **`nir_shader`** down to **`nir_function`**, **`nir_block`**, **`nir_if`**, **`nir_loop`**, and the individual instruction types (**`nir_alu_instr`**, **`nir_intrinsic_instr`**, **`nir_tex_instr`**, **`nir_deref_instr`**, **`nir_phi_instr`**, **`nir_jump_instr`**), together with **`nir_variable`** and the **`nir_variable_mode`** storage class system
- **Front ends** — the two primary paths that produce **NIR**: **SPIR-V** via **`spirv_to_nir()`** and **GLSL** via **`glsl_to_nir()`**
- **Optimisation passes** — the library of passes (**`nir_opt_algebraic`**, **`nir_opt_copy_prop`**, **`nir_opt_dce`**, **`nir_opt_cse`**, **`nir_opt_constant_folding`**, **`nir_opt_if`**, **`nir_opt_loop_unroll`**) that transform **NIR** toward more efficient code
- **Lowering passes** — **`nir_lower_io()`**, **`nir_lower_system_values()`**, **`nir_lower_ubo_vec4()`**, **`nir_lower_int64()`**, **`nir_lower_doubles()`**, **`nir_lower_tex()`**, and **`nir_lower_indirect_derefs()`**, which encode hardware constraints from the **`nir_shader_compiler_options`** struct
- **Debugging and validation infrastructure** — **`nir_print_shader()`**, **`nir_validate_shader()`**, **`MESA_SHADER_DUMP_PATH`**, **`NIR_DEBUG`**, **`RADV_DEBUG`**, **`ANV_DEBUG`** — that makes **NIR** development tractable
- **Universal interchange layer** — **NIR**'s architectural position between all producers and consumers and the implications of its non-**ABI** nature for the **Mesa** disk cache
- **Tessellation and geometry shaders** — how **NIR** represents **TCS**, **TES**, and **GS** stages, including the **`nir_lower_gs_intrinsics()`** and **`nir_lower_tess_coord_z()`** passes
- **Mesh and task shaders** — the modern pipeline introduced by **`VK_EXT_mesh_shader`** and **`VK_NV_mesh_shader`**, including **RADV**'s **RDNA2** (**GFX10.3**) mesh shader lowering path via **`aco_compile_shader()`**
