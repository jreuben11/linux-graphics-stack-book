...
This chapter covers the complete ACO compilation pipeline: its design philosophy, its internal representation, its instruction selection pass, its register allocator, its scheduler, and the binary emission step that produces the GCN/RDNA machine code the GPU executes. Readers will also find coverage of RDNA-specific features — the NGG geometry pipeline, fragment shader prolog/epilog split compilation, wave32 versus wave64 selection — and a realistic account of the benchmark evidence comparing ACO against LLVM. After reading this chapter, the reader will understand how ACO turns NIR into working AMD GPU code, why the VGPR/SGPR register model makes AMD compilation harder than most other targets, and where ACO sits relative to LLVM across the AMD software stack.
