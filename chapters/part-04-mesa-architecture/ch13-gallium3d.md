...
- **OpenGL mesh and task shader support via Gallium**: Mesa 25.3 landed preparatory work for `GL_EXT_mesh_shader` targeting the radeonsi Gallium backend, with full integration into the `pipe_context` draw dispatch path expected imminently. The Gallium infrastructure — task/mesh shader stages as compute shader variants feeding back into the draw module — was proven first in Lavapipe/llvmpipe. [Source](https://www.phoronix.com/forums/forum/linux-graphics-x-org-drivers/opengl-vulkan-mesa-gallium3d/1571179-mesa-25-3-lands-more-changes-to-prepare-for-opengl-mesh-shaders)
- **Zink as the default OpenGL path on NVIDIA hardware**: Mesa 25.3 set Zink + NVK as the default OpenGL provider on Turing (RTX 20xx) and later NVIDIA GPUs, effectively retiring the legacy nouveau Gallium backend for those generations. Stabilising this path — improving Zink's render-pass inference, descriptor management, and synchronisation logic — is the primary near-term Zink focus. [Source](https://www.phoronix.com/linux/Mesa)
- **Microsoft MFT Gallium frontend stabilisation**: A 62,000-line Media Foundation Transforms (MFT) Gallium frontend landed in Mesa 25.2 to provide video-decode acceleration on Windows via the D3D12 Gallium backend. Near-term work includes conformance fixes, format coverage, and plumbing the frontend through the `pipe_video_codec` interface for additional codec types. [Source](https://www.phoronix.com/news/Microsoft-Lands-MFT-Gallium3D)
- **rusticl conformance expansion**: rusticl achieved official OpenCL 3.0 conformance on Intel Gen12 and passes the CTS on radeonsi. Near-term effort is broadening conformant hardware coverage — ARM Mali (panfrost), Qualcomm (freedreno), and additional Intel generations — and improving interoperability with ML frameworks that target OpenCL. [Source](https://www.phoronix.com/news/Rusticl-OpenCL-3.0-Conformance)
- **Ethos-U NPU Gallium driver (TEFLON framework)**: Mesa 26.1 enabled the Arm Ethos-U85 NPU via the EthosU Gallium driver within the TEFLON ML inference framework, extending the `pipe_compute` path to dedicated inference accelerators. Near-term work covers additional operator coverage and TFLite delegate integration. Note: needs verification of exact PIPE_CAP additions required.

### Medium-term (1–3 years)

- **Explicit synchronisation throughout the Gallium pipe interface**: Gallium's current implicit synchronisation model — where the pipe driver internally tracks all GPU hazards — conflicts with Vulkan's explicit barrier model and limits Zink's ability to batch work efficiently. A proposed extension to `pipe_context` would expose explicit `pipe_barrier` objects mirroring Vulkan's `VkPipelineBarrier`, enabling Zink and future backends to avoid redundant barriers. Note: needs verification of specific MR numbers in Mesa GitLab.
- **NIR-to-SPIR-V round-trip improvements for Zink**: As Zink matures as the primary OpenGL-on-Vulkan path, the `nir_to_spirv` pass in `src/compiler/spirv/` is under increasing pressure to generate SPIR-V that Vulkan validation layers accept and that downstream SPIR-V-to-ISA compilers optimise well. Medium-term work includes geometry shader, tessellation, and mesh shader SPIR-V output correctness and the elimination of unnecessary `OpExtInst` calls. [Source](https://docs.mesa3d.org/drivers/zink.html)
- **Clover retirement and full rusticl migration**: The legacy Clover OpenCL frontend (`src/gallium/frontends/clover/`) has been in maintenance mode since rusticl's introduction. Its removal from the Mesa tree is under discussion; the medium-term plan is to complete rusticl's coverage of remaining Clover-only hardware paths and formally deprecate Clover in a Mesa release. Note: needs verification of final deprecation timeline.
- **`pipe_screen` capability query rationalisation**: The `PIPE_CAP` enumeration has grown organically for 15+ years and contains overlapping, redundant, and driver-inconsistently-implemented caps. A medium-term mesa-dev discussion thread proposes auditing and consolidating caps — particularly around image formats, geometry shader limits, and compute memory models — to reduce the conformance surface that new backend drivers must implement. Note: needs verification of specific merge-request or RFC.
- **PowerVR Vulkan + Zink for embedded OpenGL**: Imagination's PowerVR Vulkan driver in Mesa is targeting Zink as its OpenGL provider for Mesa 26.1+, replacing the unmaintained PowerVR Gallium backend. Medium-term work covers Zink path correctness on tile-based deferred-rendering (TBDR) architectures, where Vulkan render pass structure matters critically for performance. [Source](https://www.phoronix.com/linux/Mesa)

### Long-term

- **Direct State Access (DSA) path through Gallium**: OpenGL 4.5's Direct State Access API eliminates the bind-to-modify pattern that has historically caused redundant state changes through the CSO cache. A long-term goal in the GL state tracker is to route DSA calls through a faster path that bypasses unnecessary bind/unbind cycles in the `cso_context`, reducing driver overhead for DSA-heavy workloads such as Vulkan wrappers and modern game engines.
- **Gallium compute as a general ML inference path**: With rusticl achieving OpenCL 3.0 conformance and the TEFLON framework extending `pipe_compute` to NPUs, there is a longer-term architectural question about whether Gallium can provide a unified kernel-to-userspace compute path for AI inference workloads across CPUs, GPUs, and fixed-function accelerators — analogous to what it provides for rasterisation.
- **Replacement or retirement of the GL state tracker**: As Zink matures and Vulkan-native applications become dominant, the long-term pressure on the OpenGL state tracker (`src/mesa/state_tracker/`) will increase. The architectural question — whether to maintain the GL state tracker indefinitely or eventually redirect all GL through Zink — is not settled, but depends critically on Zink achieving performance parity with native Gallium backends on AMD and Intel hardware.
- **`pipe_context` threading model evolution**: The current threading model — one `pipe_context` per thread, with a `threaded_context` wrapper in `src/gallium/auxiliary/` for asynchronous command dispatch — was designed for single-GPU OpenGL workloads. Multi-GPU compute, heterogeneous CPU+GPU workloads (as in TEFLON), and high-thread-count Vulkan translation all stress this model. Long-term evolution may involve first-class multi-queue and timeline semaphore concepts at the Gallium pipe level.
