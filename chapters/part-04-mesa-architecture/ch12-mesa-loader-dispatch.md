...
- [1. The OpenGL Dispatch Problem](#1-the-opengl-dispatch-problem)
  - [1.1 What is GLVND (GL Vendor-Neutral Dispatch)?](#11-what-is-glvnd-gl-vendor-neutral-dispatch)
  - [1.2 What is DRI (Direct Rendering Infrastructure)?](#12-what-is-dri-direct-rendering-infrastructure)
  - [1.3 What is the Mesa Loader?](#13-what-is-the-mesa-loader)
...
- **Mesa's role as a registered GLVND vendor**
- **The DRI interface ABI** — defined in `include/GL/internal/dri_interface.h`, covering **__DRIscreen**, **__DRIcontext**, and **__DRIdrawable**
- **The removal of all classic (non-Gallium) DRI drivers in Mesa 22** — including the Intel **i965** Classic, AMD **r100**/**r200**, and classic **nouveau**

**Mesa**'s entry-point and frontend architecture centres on the **Mesa** loader library in `src/loader/`, whose primary function **loader_get_driver_for_fd()** maps a **DRM** render node file descriptor to a driver name string by checking `MESA_LOADER_DRIVER_OVERRIDE`, consulting per-driver **PCI** ID tables such as **amd_pci_ids.h** and **intel_device_info.c**, and falling back to **drmGetVersion()**. Driver shared objects — named `<driver>_dri.so` (e.g., **radeonsi_dri.so**, **iris_dri.so**, **swrast_dri.so**) — are located via `LIBGL_DRIVERS_PATH` or the compiled-in `$DRI_DRIVER_SEARCH_DIR`. The chapter covers the full driver name resolution table for major hardware:

- **radeonsi** — AMD GCN+
- **iris** — Intel Gen8+
- **crocus** — Intel Gen4–Gen7
- **r600** — pre-GCN AMD
- **nouveau** — NVIDIA
- **virtio_gpu** — virtualised

The **Kopper** interface, merged into **Mesa** in 2022, is also explained: it is a **DRI** interface extension that allows **Zink** (**Mesa**'s **OpenGL**-over-**Vulkan** implementation) to create a **VkSurfaceKHR** and manage presentation through the **Vulkan WSI** stack — bypassing the **DRI2**/**DRI3** buffer exchange path — via `src/gallium/frontends/dri/kopper.c`.

On the **Vulkan** side, the **Khronos Vulkan** loader (**libvulkan.so**) is a separate project from **Mesa**. **Mesa** ships **Installable Client Drivers** (**ICD**s) — shared objects such as **libvulkan_radeon.so** (**RADV**) and **libvulkan_intel.so** (**ANV**) — accompanied by **JSON** discovery manifests in `/usr/share/vulkan/icd.d/`. The chapter explains:

- **ICD enumeration and `dlopen()` loading**
- **`vk_icdNegotiateLoaderICDInterfaceVersion`** — the version-negotiation handshake
- **Instance dispatch tables** — populated via **vkGetInstanceProcAddr**
- **Device dispatch tables** — populated via **vkGetDeviceProcAddr**
- **Layer injection** — via **VK_LAYER_KHRONOS_validation** and manifest directories `/usr/share/vulkan/implicit_layer.d/` and `/usr/share/vulkan/explicit_layer.d/`

Multi-**GPU** systems and the environment variables that control **ICD** enumeration are also detailed:
...
The chapter then covers **Mesa**'s disk shader cache, which amortises the expensive **SPIR-V**-to-machine-code compilation step — involving **NIR** optimisation passes and backend code generation (**ACO** for **RADV**, **brw** for **ANV**) — by storing compiled binaries in `$XDG_CACHE_HOME/mesa_shader_cache`. Cache key construction uses **SHA-1** over the following inputs:

- A driver identifier string
- The `.note.gnu.build-id` **Mesa** binary hash
- A `driver_keys_blob` encoding **GPU** family and compiler flags
- The shader source hash

Cache entries use an atomic `rename()` write strategy; size limits default to 1 GiB and are controlled by `MESA_SHADER_CACHE_MAX_SIZE` with **LRU** eviction. `MESA_SHADER_CACHE_DISABLE` and `MESA_SHADER_CACHE_SHOW_STATS` are the primary diagnostic environment variables. The Steam shader pre-caching system is also explained: Valve pre-compiles shader caches for known **GPU**/driver combinations (**AMD** **GCN**, **RDNA**, Intel **Xe**, etc.) and distributes them before first launch. The two-layer caching architecture — **DXVK** (`~/.cache/dxvk/`) and **VKD3D-Proton** (`VKD3D_SHADER_CACHE_PATH`) caching the **HLSL**-to-**SPIR-V** step above **Mesa**'s **ISA** cache — is diagrammed. **VK_EXT_graphics_pipeline_library** (**GPL**), supported by **RADV** and **ANV**, enables staged pipeline compilation and the "fast link then async replace" strategy that reduces first-draw hitching; `RADV_PERFTEST=gpl` enables it explicitly.

Finally, the **EGL** platform architecture is examined: the four production platforms all converge at **dri2_create_screen()** in `src/egl/drivers/dri2/egl_dri2.c`:

- **EGL_PLATFORM_WAYLAND_KHR** — `platform_wayland.c`, using **wl_display \*** and **zwp_linux_dmabuf_v1**
- **EGL_PLATFORM_GBM_KHR** — `platform_drm.c`, using **gbm_device \*** for direct **KMS**/**DRM** rendering
- **EGL_PLATFORM_DEVICE_EXT** — `platform_device.c`, using **EGLDeviceEXT** handles enumerated via **eglQueryDevicesEXT()** for headless rendering
- **EGL_PLATFORM_SURFACELESS_MESA** — `platform_surfaceless.c`, using **EGL_DEFAULT_DISPLAY** for surfaceless compute

Also covered:

- Automatic platform detection from `WAYLAND_DISPLAY` and `DISPLAY` environment variables
- Headless **GPU** selection via **EGL_DRM_DEVICE_FILE_EXT**
- **GBM**-backed surfaces for direct **KMS** scanout using **gbm_surface_create_with_modifiers2()** and **drmModeAddFB2WithModifiers()**
- **Wayland** modifier negotiation via **zwp_linux_dmabuf_v1_get_default_feedback** for direct scanout
- **EGL_EXT_buffer_age** semantics under double and triple buffering
...
### 1.1 What is GLVND (GL Vendor-Neutral Dispatch)?

GLVND is the library infrastructure that allows multiple OpenGL vendors to coexist on a single Linux system without overwriting each other's shared objects. Before GLVND, the OpenGL API library `libGL.so` was simultaneously the function entry-point table and the hardware driver implementation, meaning only one vendor could own `libGL.so` at any time. GLVND solves this by splitting the library into two layers: a thin, vendor-neutral `libGL.so` that provides the OpenGL entry points as dispatch stubs, and per-vendor implementation libraries such as `libGLX_mesa.so` and `libGLX_nvidia.so` that contain the actual driver code. The dispatch mechanism is per-context: when an application calls `glXMakeCurrent` or `eglMakeCurrent`, the GLVND layer records the owning vendor library in a thread-local variable, and all subsequent OpenGL calls on that thread are routed through that thread-local dispatch table. For GLX, vendor association is resolved at the X screen level via the `GLX_EXT_libglvnd` protocol extension; for EGL, it is resolved at the display level via JSON vendor configuration files in `/usr/share/glvnd/egl_vendor.d/`. The two threads of the same process can therefore use different OpenGL implementations simultaneously without interference. Mesa participates as a GLVND vendor by shipping `libGLX_mesa.so` and `libEGL_mesa.so` alongside `50_mesa.json`. The GLVND project is maintained at `https://github.com/NVIDIA/libglvnd` and has been the default dispatch model on most Linux distributions since 2017.

### 1.2 What is DRI (Direct Rendering Infrastructure)?

DRI is the subsystem that allows OpenGL and other graphics APIs to communicate directly with GPU hardware from userspace without routing rendering commands through the X server. The DRI architecture defines a stable binary interface between Mesa's API frontends (GLX, EGL) and its hardware driver backends (radeonsi, iris, nouveau, and others). This interface is specified in `include/GL/internal/dri_interface.h` and centres on three opaque object types: `__DRIscreen`, `__DRIcontext`, and `__DRIdrawable`. Each begins with a pointer to a C function dispatch table; the hardware driver allocates and populates these objects while the frontend holds opaque pointers and calls through the table. Capabilities beyond the base interface are negotiated at runtime through DRI extensions, which are structs pairing a name string and version integer with a set of function pointers. DRI depends on the Linux kernel's DRM (Direct Rendering Manager) subsystem for GPU memory management, command submission, and synchronisation, accessed through device nodes such as `/dev/dri/card0` for modesetting and `/dev/dri/renderD128` for render-only access. The protocol evolved from DRI1 through DRI2, which introduced GEM buffer object sharing with the X server, to DRI3, which replaced the shared-memory buffer model with dma-buf file-descriptor passing and DRM format modifier negotiation. Within this chapter, the DRI interface ABI is the boundary surface through which the Mesa loader connects an API-level call — whether from GLX, EGL, or an internal Gallium frontend — to a hardware-specific `pipe_screen` object.

### 1.3 What is the Mesa Loader?

The Mesa Loader is a small library in `src/loader/` that performs the foundational task of mapping a DRM device file descriptor to a Mesa driver name string and then loading the corresponding driver shared object. It is the single hardware-identification layer shared by Mesa's GLX frontend, EGL frontend, and Vulkan frontends, making every Mesa API surface route through the same device-to-driver resolution logic. The loader's primary function, `loader_get_driver_for_fd()`, accepts a file descriptor for a DRM render node — typically `/dev/dri/renderD128` — and applies a three-stage resolution strategy: first checking the `MESA_LOADER_DRIVER_OVERRIDE` environment variable for a forced driver name, then consulting per-vendor PCI ID tables (`amd_pci_ids.h` for AMD hardware, `intel_device_info.c` for Intel) to map a `(vendor_id, device_id)` pair to a driver name, and finally falling back to the kernel module name returned by `drmGetVersion()`. The resolved driver name — such as `radeonsi`, `iris`, `crocus`, `nouveau`, `r600`, or `virtio_gpu` — determines which DRI driver shared object is opened from the path specified by `LIBGL_DRIVERS_PATH` or the compiled-in `DRI_DRIVER_SEARCH_DIR`. Driver shared objects follow the naming convention `<driver>_dri.so`. On ARM and other non-PCI platforms, the loader consults a separate KMSRO table that maps display-only kernel drivers (such as `vc4`, `panfrost`, and `lima`) to dedicated render drivers, providing a portable abstraction across x86, ARM, and RISC-V GPU ecosystems. Understanding the loader's resolution order is essential for diagnosing unexpected driver selection on hybrid-GPU systems or inside virtualised environments.
...
- **NVK pipeline shader cache and implicit caching maturation**: Mesa's NVK Vulkan driver landed a working pipeline shader cache and implicit pipeline caching (analogous to RADV's long-standing support) enabling DXVK and other applications to benefit from cached ISA without explicit `VkPipelineCache` handles. Ongoing work targets parity with RADV/ANV cache hit rates and cache key stability across NVK driver updates. [Source](https://www.phoronix.com/news/NVK-Pipeline-Shader-Cache) [Source](https://www.phoronix.com/news/NVK-Implicit-Pipeline-Caching)
- **`VK_EXT_graphics_pipeline_library` enabled by default on ANV**: RADV has shipped GPL as the default since Mesa 23.1; ANV's implementation exists but enablement by default on Intel hardware is expected in the near term to align the two primary Mesa Vulkan ICDs on "fast link then async replace" pipeline compilation and eliminate first-draw stutter for Proton/DXVK workloads. Note: needs verification of exact Mesa release target.
- **Vulkan 1.4 conformance across all major Mesa ICDs**: Mesa 26.1 (May 2026) implements the Vulkan 1.4 API in RADV and ANV; NVK is a conformant Vulkan 1.4 implementation for all officially supported NVIDIA GPUs. Near-term work focuses on filling remaining Vulkan 1.4 extension gaps in smaller drivers (v3dv, tu, etc.). [Source](https://www.phoronix.com/news/Mesa-26.1-Released)
- **Mesa shader cache reduced footprint**: Ongoing work following the Mesa 24.2 new shader cache implementation targets reducing on-disk cache size without sacrificing hit rates, particularly for RADV ACO and ANV brw backends. [Source](https://www.gamingonlinux.com/2024/08/mesa-24-2-0-released-with-a-new-shader-cache-implementation/)
- **Intel Nova Lake P experimental support**: Mesa 26.1 includes experimental support for Intel Nova Lake P GPUs, requiring loader PCI ID table updates and new `iris`/`xe` driver identification entries in `loader_get_driver_for_fd()`. [Source](https://www.phoronix.com/news/Mesa-26.1-Released)

### Medium-term (1–3 years)

- **Gradual deprecation of the classic DRI interface ABI**: The DRI2/DRI3 `__DRIscreen` / `__DRIcontext` / `__DRIdrawable` interface defined in `dri_interface.h` is a compatibility layer whose removal has been discussed on mesa-dev. As Kopper (Zink WSI) matures and all OpenGL-on-Vulkan paths go through Kopper's `VkSurfaceKHR`-based presentation, the DRI buffer exchange protocol becomes redundant. The `src/gallium/frontends/dri/` layer is expected to shrink or be replaced by a thinner Gallium-direct path. Note: needs verification of a firm deprecation timeline.
- **Zink/Kopper as the primary OpenGL path for GPU-less or Vulkan-only hardware**: The long-term direction expressed by Christian Schaller (Red Hat) and upstream Mesa developers is for GPU vendors to ship Vulkan-only drivers and rely on Zink for backward-compatible OpenGL support. The `loader_get_driver_for_fd()` driver name resolution will need to gracefully fall back to `zink` when no native Gallium OpenGL driver is available. [Source](https://blogs.gnome.org/uraeus/2022/04/07/why-is-kopper-and-zink-important-aka-the-future-of-opengl/)
- **GLVND EGL vendor priority refinement**: The `50_mesa.json` numeric priority system is a blunt instrument on hybrid-GPU systems where NVIDIA and Mesa ICDs must coexist. Ongoing discussion in the GLVND issue tracker concerns per-display, per-GPU vendor selection at the GLVND level rather than requiring `__EGL_VENDOR_LIBRARY_FILENAMES` overrides. [Source](https://github.com/NVIDIA/libglvnd)
- **Vulkan Loader portability layer integration**: The Khronos Vulkan Loader continues to absorb portability layers (MoltenVK for macOS, lavapipe on CPU); on Linux this affects the `VK_DRIVER_FILES` and `VK_LAYER_PATH` search logic. Planned work includes tighter integration of driver and layer discovery via systemd's `sysext` overlay mechanism for immutable OS deployments (Steam Deck, Fedora Atomic). Note: needs verification.
- **`VK_EXT_layer_settings` for programmatic layer configuration**: Rather than environment variables like `VK_LAYER_KHRONOS_VALIDATION`, the `VK_EXT_layer_settings` extension allows applications to configure validation and other layer behaviours at `VkInstance` creation time, reducing reliance on loader-level env-var parsing in the ICD dispatch path. [Source](https://github.com/KhronosGroup/Vulkan-Loader)

### Long-term

- **Unified Gallium-direct EGL/GLX without DRI indirection**: The architectural goal articulated by Mesa maintainers is to eventually let `libEGL_mesa.so` call Gallium `pipe_screen` functions directly, without the `__DRIscreen` wrapper layer. This would simplify `dri2_create_screen()` to a thin platform-abstraction shim and eliminate one level of function-pointer indirection in the hot path. Note: speculative; no confirmed timeline.
- **Rust-based loader components**: Following the `Nova` Rust NVIDIA kernel driver (Chapter 10) and growing interest in memory-safe systems code in the Linux graphics stack, exploratory proposals exist for rewriting parts of the Mesa loader (`src/loader/`) and the disk shader cache (`src/util/disk_cache.c`) in Rust. This would align the Mesa loader with the broader kernel and driver push toward Rust for security-sensitive code paths. Note: speculative; no merged patchsets as of 2026.
- **Content-addressed shader cache with deduplication**: The current SHA-1-keyed LRU disk cache stores per-GPU-family binaries independently; a content-addressed store where identical NIR shader trees produce a single binary entry (deduplicated across driver versions or GPU generations) has been proposed as a long-term successor to the current `disk_cache.c` design. This would significantly reduce `~/.cache/mesa_shader_cache` growth in multi-GPU or multi-driver installations. Note: speculative research direction.
