...
On the OpenGL side, the chapter traces the historical evolution from a world where `libGL.so` was simultaneously the API definition and the implementation — making proprietary and open-source drivers mutually exclusive — to the GLVND vendor-neutral dispatch model that allows multiple OpenGL vendors to coexist on a single system. Mesa's role as a registered GLVND vendor, the DRI interface ABI, and the removal of all classic (non-Gallium) DRI drivers in Mesa 22 are covered in depth.

On the Vulkan side, the Khronos Vulkan loader is a separate project from Mesa. Mesa ships Installable Client Drivers (ICDs) — shared objects accompanied by JSON discovery manifests — that the loader finds, interrogates, and wraps in dispatch tables. The chapter explains ICD enumeration, the version negotiation handshake, instance and device dispatch tables, and the layer injection mechanism. It closes with Mesa's disk shader cache — a cross-driver optimisation that amortises the expensive SPIR-V-to-machine-code compilation step — and the Steam shader pre-compilation workflow that warms this cache before a player ever launches a game.
