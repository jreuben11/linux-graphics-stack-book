...
- **llvmpipe**: A **Gallium** pipe driver that implements **OpenGL** (and indirectly **Vulkan** via **Lavapipe**) entirely on the CPU, using **LLVM** to JIT-compile shaders to native **SIMD** machine code. Its core architecture is a tile-based rendering engine that bins primitives into 64×64-pixel tiles processed by worker threads. Fragment shaders compile through **lp_build_nir_soa()** in **lp_bld_nir.c**, generating **LLVM IR** that is lowered to **SSE4.1**, **AVX2**, or **AVX-512** native code via the **gallivm** infrastructure. The **lp_build_fs_llvm_iface** structure abstracts texture sampling, image access, and buffer reads for the **NIR**-to-**LLVM** translation. Performance is tuned via environment variables including **LP_NUM_THREADS**, **LP_NATIVE_VECTOR_WIDTH**, **LP_NO_RAST**, and **LIBGL_ALWAYS_SOFTWARE**.
- **Lavapipe**: A **Vulkan** **ICD** that uses **llvmpipe** as its rasterisation backend, exposed via **lvp_icd.x86_64.json**. **Lavapipe** presents a full **Vulkan** API to applications and translates **Vulkan** commands into **Gallium** pipe calls executed synchronously on the calling CPU thread via **lvp_execute_cmds()** in **lvp_execute.c**. The **rendering_state** structure tracks dirty flags (**blend_dirty**, **rs_dirty**, **dsa_dirty**, **vp_dirty**) flushed lazily by **emit_state()**. Its shader pipeline is **SPIR-V** → **NIR** (via **spirv_to_nir**) → **gallivm** JIT. All **VkDeviceMemory** allocations are backed by **aligned_malloc** with every memory type exposing **VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT** and **VK_MEMORY_PROPERTY_HOST_COHERENT_BIT**. **Lavapipe** is conformant to **Vulkan 1.3** and exposes extensions including **VK_KHR_acceleration_structure**, **VK_KHR_ray_query**, **VK_KHR_ray_tracing_pipeline** (implemented via a software **BVH** using **lvp_bvh_box_node** and **lvp_bvh_triangle_node**), **VK_EXT_mesh_shader**, and **VK_EXT_shader_object**. Key environment variables include **MESA_VK_ABORT_ON_DEVICE_LOST** and **MESA_SHADER_CAPTURE_PATH**.
- **Zink**: A **Gallium** pipe driver that translates **Gallium** calls into **Vulkan** API calls. **Zink** is not a rasteriser at all — it is a translation layer. It can run on top of any **Vulkan** driver, whether that is hardware (**RADV**, **ANV**, **NVK**) or software (**Lavapipe**). Its core structures are **zink_screen** (implementing **pipe_screen** → **VkInstance** / **VkPhysicalDevice**), **zink_context** (implementing **pipe_context** → **VkDevice** / **VkQueue**), and **zink_resource** (wrapping **VkBuffer** / **VkImage**). State translation relies on **VK_EXT_extended_dynamic_state** to handle mutable **Gallium** state without pipeline explosion, and **VK_EXT_graphics_pipeline_library** to amortise shader compilation. Descriptor management offers three strategies (**auto**, **lazy**, **db**), with the **db** mode using **VK_EXT_descriptor_buffer**. Synchronisation uses **VK_KHR_synchronization2**. **Zink** is used on platforms without native **OpenGL** drivers (e.g. via **MoltenVK** on macOS), for correctness testing, and as the full-software path **OpenGL** → **Gallium** → **Zink** → **Lavapipe** in CI. The **NIR**-to-**SPIR-V** translation (the inverse of **spirv_to_nir**) is handled by **nir_to_spirv()** in **nir_to_spirv/nir_to_spirv.c**, with all translation state held in **ntv_context** and **SPIR-V** emission performed via the **spirv_builder** API.
