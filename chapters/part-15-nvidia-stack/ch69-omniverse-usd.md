...
1. [Overview](#overview)
   - [1.1 What is OpenUSD?](#11-what-is-openusd)
...
12. [PhysX 5 and NVIDIA Warp: GPU Simulation for Rendering](#12-physx-5-and-nvidia-warp-gpu-simulation-for-rendering)
    - 12.1 [PhysX 5: GPU-Accelerated Simulation](#121-physx-5-gpu-accelerated-simulation)
    - 12.2 [NVIDIA Warp: Python GPU Computing](#122-nvidia-warp-python-gpu-computing)
...
13. [Audio2Face-3D: Neural Facial Animation](#13-audio2face-3d-neural-facial-animation)
    ...
    - [Python Inference API (NVIDIA ACE SDK)](#python-inference-api-nvidia-ace-sdk)
    - [Integration with Omniverse RTX](#integration-with-omniverse-rtx)
    - [Deployment: NVIDIA NIM Microservice](#deployment-nvidia-nim-microservice)
...
OpenUSD (Universal Scene Description) is a high-performance, extensible scene description and interchange format originally developed for feature animation production and now standardised under the Alliance for OpenUSD (AOUSD) as a Linux Foundation project. At its core, USD provides a typed, time-sampled property model layered on top of a compositional algebra that allows multiple files — called layers — to contribute opinions to the same scene element without destructive merging. A USD scene is composed from one or more `SdfLayer` files (`.usd`, `.usda`, `.usdc`, `.usdz`), with the `UsdStage` resolving opinion precedence via the LIVERPS strength ordering: Local sublayers, Inherits, Variants, Relocates, References, Payloads, and Specializes. This non-destructive composition model is USD's defining feature: it allows a lighting department to override a character department's asset without modifying it, and permits the same base asset to express multiple variants — damaged versus pristine geometry, for example — without duplicating data. The schema system extends the core type model through C++ and Python APIs, covering geometry (`UsdGeomMesh`), cameras (`UsdGeomCamera`), lights (`UsdLuxSphereLight`), materials (`UsdShadeMaterial`), physics (`UsdPhysicsRigidBodyAPI`), and domain-specific schemas such as `UsdVolParticleField3DGaussianSplat`. On Linux, OpenUSD is distributed as an open-source C++ library from `github.com/PixarAnimationStudios/OpenUSD`, with Python bindings in the `pxr` namespace and CMake build support targeting GCC and Clang.

### 1.2 What is NVIDIA Omniverse?

NVIDIA Omniverse is a real-time collaboration and simulation platform that uses OpenUSD as its native scene representation. Where USD defines the data model and composition rules, Omniverse adds the runtime infrastructure needed to operate on that data at production scale: a collaboration server (Nucleus) that propagates live USD layer edits across workstations in real time, a modular application framework (the Kit SDK) through which simulation, rendering, and domain tools are assembled from extensions, and an RTX rendering engine that exposes hardware ray tracing and path tracing via OptiX 8 and Vulkan 1.3. Omniverse applications on Linux are packaged as Kit-based executables that load `.kit` manifest files declaring which extensions are active; the Kit runtime manages dependency resolution, hot-reload, and the per-frame update loop that synchronises physics, animation, and rendering tasks. The platform targets both interactive workstation use and headless server-side workflows for synthetic data generation, robot simulation, and autonomous-vehicle sensor simulation. On Linux, Omniverse requires the proprietary NVIDIA driver at version 570.124.06 or later for full RTX path tracing, CUDA 12.8, and Vulkan 1.3 with `VK_KHR_ray_tracing_pipeline`. Container-based deployment via NVIDIA GPU Cloud (NGC) images using the NVIDIA Container Toolkit simplifies dependency management in cloud and data-centre environments.
...
**Why a fixed ordering rather than authoring-order or per-scene configurable priority?** With hundreds of layers, references, and variants potentially contributing opinions to the same attribute, resolution has to be decidable by structure alone — any tool must be able to determine the winning opinion by walking the composition graph, without evaluating semantics or depending on load order. Pinning strength to arc *category* rather than to authoring sequence is what makes non-destructive, multi-user editing safe: a Nucleus live session (§7) can let many artists write into the same stage concurrently because the merge outcome is unambiguous by construction, not by convention. The specific ordering also mirrors production editorial intent rather than being arbitrary: Local wins first because direct edits on the prim being worked on should always be able to override anything upstream; Inherits sit just below because they broadcast to a class of prims and should beat generic asset content but lose to a specific instance's direct override; Variants outrank the Reference/Payload content they select between because a chosen configuration should win over the raw asset; and Specializes is weakest by design, functioning as a fallback/base-class mechanism — comparable to a CSS cascade default or an OOP base class — that anything more specific is expected to override. Relocates required a fixed slot of its own once formalised in 2025 because its namespace-reparenting effects change how every arc below it resolves, so leaving it implicit within Local composition (as pre-2025 LIVRPS did) produced correctness bugs in edge cases.
...
**glTF delivery format (Ch64):** USD and glTF are complementary rather than competing formats — USD's layered, composed scene description (§2–§4 of this chapter) is built for production authoring, while glTF is a flat, non-composable format optimised for GPU upload at delivery time. Production pipelines built on this chapter's Kit/USD stage typically flatten and export a real-time-ready subset to glTF for game engines, WebGL viewers, or headsets; Asset Converter (an Omniverse Kit extension) and Google's `usd_from_gltf` are the tools bridging the two today, since no formal Khronos/AOUSD interchange specification standardises the conversion yet. Ch64 covers the glTF side of this bridge in depth, including its MaterialX convergence with USD's material model.
