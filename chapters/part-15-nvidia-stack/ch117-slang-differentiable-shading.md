...
**Why differentiate a shader.** Ordinary shading code computes a forward mapping — material and lighting parameters in, a colour out. Making that mapping differentiable turns it into an optimisable function: given a *loss* between the shader's output and some target, backpropagation through the shader itself yields a gradient with respect to its inputs, and any gradient-based optimiser (SGD, Adam) can then adjust those inputs to reduce the loss. This is useful whenever the parameters worth solving for are exactly the ones a classical, non-differentiable shader would take as fixed input:

- **Inverse rendering and appearance capture.** Given one or more photographs of a surface, differentiate a BRDF evaluation with respect to its material parameters (roughness, `F0`, normal) and optimise until the rendered appearance matches the captured images — the same `evalGGX` pattern introduced in §7.2 below, driven by a photometric loss instead of a fixed parameter set.
- **Differentiable asset compression.** Treat a lossy encoder as a differentiable function of its output bits and optimise those bits directly against the source data, rather than using a closed-form or heuristic encoder. §10.1 and the Falcor BC7 case study in §12.2 cover this in depth: gradient descent on block endpoints outperforms ISPC's heuristic BC7 encoder on both speed and PSNR.
- **Neural components inside a classical render loop.** A small MLP evaluated per-pixel or per-sample (a neural BRDF, a neural texture decompressor, a NeRF or 3D Gaussian splatting decoder) needs its weights trained with backpropagation, and — for real-time inference — needs to run inside the same shader as the surrounding rasterisation or ray tracing code. §9 (cooperative vectors), §10.2–10.3, and the world-model rendering case study in §15 all build on the mechanism introduced in this section.
- **Gradient-based procedural parameter tuning.** More generally, any shader parameter that is expensive or impossible to hand-tune (light placement, procedural material coefficients, denoiser weights) becomes a candidate for gradient descent once the shader that consumes it is differentiable, without requiring a separate offline differentiable-rendering framework outside the shading language itself.
...
**Experimental: Rust and Bevy.** Outside the Python/PyTorch path, Slang integration with Rust game engines is nascent rather than production-ready. `bevy_mod_slang` ([github.com/DGriffin91/bevy_mod_slang](https://github.com/DGriffin91/bevy_mod_slang)) is a community plugin that lets **Bevy** (Chapter 40) applications author shaders in Slang instead of **WGSL**: it shells out to the `slangc` command-line compiler — rather than binding against the Slang compilation API described in §4 — to produce SPIR-V for Bevy's Vulkan and D3D12 backends, with WebGPU listed and WebGL2 explicitly broken due to a reserved-identifier conflict in generated fragment shaders. It supports shader hot-reloading but is an early, single-maintainer project (single-digit commits and stars at the time of writing), with no stated Bevy-version compatibility guarantee. Should this integration mature, the natural next step would be binding directly to `libslang` through one of the existing Rust FFI crates — `shader-slang` ([github.com/FloatyMonkey/slang-rs](https://github.com/FloatyMonkey/slang-rs)) or the earlier `slang` crate ([github.com/tangmi/slang-rs](https://github.com/tangmi/slang-rs)) — to gain access to the reflection API (§5) and incremental module compilation (§2.3) rather than invoking `slangc` as a subprocess per build. Note: needs verification against upstream if used in production — this is flagged here as an experimental, not a recommended, integration path.
