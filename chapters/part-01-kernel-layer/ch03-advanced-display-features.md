...
This chapter is organised around the problems each feature solves rather than alphabetically by feature name. Variable refresh rate (**VRR**) eliminates the stutter that arises when **GPU** frame time does not align with a fixed display interval; it is exposed through the **`vrr_capable`** connector property and the **`VRR_ENABLED`** **CRTC** property, and interacts with the **`wp_tearing_control_v1`** and **`wp_presentation`** **Wayland** protocols for application frame pacing. **HDR** and colour management deliver images whose luminance, gamut, and transfer function match the creative intent: the **`hdr_output_metadata`** connector property carries **`struct hdr_output_metadata`** to the display, the **`colorspace`** property selects the output colour space, and compositors such as **gamescope** implement **SDR**-to-**HDR** tone expansion and **HDR**-to-**SDR** fallback in software. The **KMS** colour management pipeline — built from the **`DEGAMMA_LUT`**, **`CTM`**, and **`GAMMA_LUT`** **CRTC** properties — provides degamma linearisation, a **`struct drm_color_ctm`** gamut-mapping matrix, and re-encoding gamma in a three-stage hardware pipeline; companion to this, the **`wp_color_representation_v1`** **Wayland** protocol addresses **YCbCr**-to-**RGB** conversion matrix selection (**BT.601**, **BT.709**, **BT.2020**), chroma range (**full** vs. **limited**), and chroma siting for video surfaces. Explicit synchronisation resolves the fundamental race that occurs when the **GPU** and display engines on different vendors' hardware cannot share fence state through the common **DMA-BUF** implicit fence system; the kernel mechanism is **`struct drm_syncobj`** and timeline sync objects, the **KMS** interface is **`IN_FENCE_FD`** and **`OUT_FENCE_PTR`**, and the **Wayland** protocol is **`wp_linux_drm_syncobj_v1`**. **HDCP** restricts content presentation to authorised displays via the **`Content Protection`** connector property and the **`drm_hdcp_helper_funcs`** callback structure. **MST** multiplexes several independent video streams over a single **DP** physical link using **`drm_dp_mst_topology_mgr`**, **`drm_dp_mst_branch`**, and **`drm_dp_mst_port`** data structures, with slot budgeting via **`drm_dp_atomic_find_time_slots()`** and optional **DSC** compression to fit high-resolution configurations within the 63-slot link budget. In each case the chapter traces the feature from its hardware reality through the **KMS** abstraction and into the protocols that expose it to compositors and applications.
