...
This chapter is organised around the problems each feature solves rather than alphabetically by feature name:

- **Variable Refresh Rate (VRR)** — eliminates the stutter that arises when **GPU** frame time does not align with a fixed display interval; exposed through the **`vrr_capable`** connector property and the **`VRR_ENABLED`** **CRTC** property, and interacts with the **`wp_tearing_control_v1`** and **`wp_presentation`** **Wayland** protocols for application frame pacing.
- **HDR and colour management** — delivers images whose luminance, gamut, and transfer function match the creative intent: the **`hdr_output_metadata`** connector property carries **`struct hdr_output_metadata`** to the display, the **`colorspace`** property selects the output colour space, and compositors such as **gamescope** implement **SDR**-to-**HDR** tone expansion and **HDR**-to-**SDR** fallback in software.
- **KMS colour management pipeline** — built from the **`DEGAMMA_LUT`**, **`CTM`**, and **`GAMMA_LUT`** **CRTC** properties; provides degamma linearisation, a **`struct drm_color_ctm`** gamut-mapping matrix, and re-encoding gamma in a three-stage hardware pipeline; companion to this, the **`wp_color_representation_v1`** **Wayland** protocol addresses **YCbCr**-to-**RGB** conversion matrix selection (**BT.601**, **BT.709**, **BT.2020**), chroma range (**full** vs. **limited**), and chroma siting for video surfaces.
- **Explicit synchronisation** — resolves the fundamental race that occurs when the **GPU** and display engines on different vendors' hardware cannot share fence state through the common **DMA-BUF** implicit fence system; the kernel mechanism is **`struct drm_syncobj`** and timeline sync objects, the **KMS** interface is **`IN_FENCE_FD`** and **`OUT_FENCE_PTR`**, and the **Wayland** protocol is **`wp_linux_drm_syncobj_v1`**.
- **HDCP** — restricts content presentation to authorised displays via the **`Content Protection`** connector property and the **`drm_hdcp_helper_funcs`** callback structure.
- **MST (Multi-Stream Transport)** — multiplexes several independent video streams over a single **DP** physical link using **`drm_dp_mst_topology_mgr`**, **`drm_dp_mst_branch`**, and **`drm_dp_mst_port`** data structures, with slot budgeting via **`drm_dp_atomic_find_time_slots()`** and optional **DSC** compression to fit high-resolution configurations within the 63-slot link budget.

In each case the chapter traces the feature from its hardware reality through the **KMS** abstraction and into the protocols that expose it to compositors and applications.
...
- **HDMI 2.1 Fixed Rate Link (FRL) with VRR**: AMD submitted HDMI 2.1 FRL support targeting Linux 7.2 ([Phoronix](https://www.phoronix.com/news/HDMI-FRL-2.1-Submitted-DRM)), initially landing without VRR to avoid regressions; VRR-over-HDMI-2.1 is the immediate follow-on goal, unlocking 4K@120Hz and beyond on HDMI displays with adaptive sync. DSC-over-FRL patches enabling 4K@240Hz and 8K@120Hz were posted in May 2026 as a follow-on series.
- **DRM per-plane colour pipeline API — NVIDIA driver support**: NVIDIA announced an April 2026 preview driver implementing the DRM per-plane colour pipeline API ([GamingOnLinux](https://www.gamingonlinux.com/2026/04/nvidia-announce-a-preview-of-drm-per-plane-color-pipeline-api-support-on-linux-good-for-hdr/)), with stable driver support expected within a release cycle, bringing the per-plane HDR pipeline to all three major GPU vendors.
- **KDE Plasma and KWin adoption of per-plane colour API**: KDE Plasma 6.6 already implements per-plane colour adjustments for mixed-display setups ([WebProNews](https://www.webpronews.com/kde-plasma-6-6-debuts-advanced-color-management-and-hdr-support/)); the upstream stabilisation of the DRM API in Linux 6.19+ will allow KWin to drop its interim workarounds and use the kernel ABI directly for all colour stages including the 3D LUT.
- **`wp_color_management_v1` compositor rollout**: Protocol merged into wayland-protocols 1.41 (February 2025) and adopted by GNOME 48 Mutter; remaining compositors (Weston, Sway, river) are expected to follow within the 6–12-month window. Note: needs verification for exact Weston and sway timelines.
- **Adaptive Sync over PCON (Protocol Converter) for HDMI VRR**: A patchset enabling Adaptive Sync signalling through DisplayPort-to-HDMI protocol converters (used in many USB-C docks) was posted in January 2026 ([lkml](https://lkml.iu.edu/2601.1/13000.html)), which will bring VRR to HDMI monitors connected through DP Alt Mode docks without requiring native HDMI 2.1 FRL.

### Medium-term (1–3 years)

- **DRM colour pipeline API broadening across hardware**: The API that merged in Linux 6.19 exposes AMD DCN3+ 8-stage pipeline (1D EOTF curves, 3×4 CTM, 17³ 3D LUT) and Intel Xe2 equivalents ([Melissa Wen's blog](https://melissawen.github.io/blog/2025/05/19/drm-info-with-kms-color-api)); medium-term work will extend the same API to Qualcomm Adreno display controllers, Arm Mali-DP, and other SoC display engines that currently expose only legacy GAMMA_LUT properties.
- **HDCP 2.x over MST (DisplayPort Multi-Stream)**: Current kernel HDCP support is primarily per-connector; extending HDCP 2.3 authentication and key-exchange to MST branch devices requires per-port capability negotiation and multi-stream content key derivation. RK3588 DP controller patches note HDCP as a future stage (Note: needs verification for a specific upstream RFC targeting MST-HDCP integration).
- **HDR dynamic metadata (HDR10+, Dolby Vision over KMS)**: The existing `hdr_output_metadata` property carries only static HDR10 metadata (CTA-861.3 Type 1). HDR10+ and Dolby Vision require per-frame dynamic metadata blobs; a new KMS property or blob extension will be needed. Discussions on the dri-devel list have noted this gap but no merged RFC exists as of mid-2026. Note: needs verification for current RFC status.
- **`wp_fifo_v1` and frame-pacing protocol integration with VRR**: The `wp_fifo_v1` Wayland protocol proposal aims to give applications a way to signal minimum-latency vs. frame-paced rendering intent to the compositor, complementing `wp_tearing_control_v1` and improving VRR frame scheduling across games and video playback. Note: needs verification for current draft status.
- **DisplayPort 2.1 UHBR (Ultra High Bit Rate) mainline enablement**: DP 2.1 UHBR10/20 (up to 80 Gbps) is present in Intel Meteor Lake and AMD RDNA4 hardware; kernel enablement requires new DPCD register handling and new link training sequences. Initial Intel patches landed in 2024; AMD UHBR support is in progress and expected to stabilise within 1–2 years. Note: needs verification for AMD UHBR patch status.

### Long-term

- **Unified cross-vendor HDR compositor architecture**: Today each compositor (gamescope, KWin, Mutter) implements its own SDR-to-HDR tone mapping and colour management policy. A long-term goal discussed at XDC is a shared userspace library (analogous to libplacebo's role in media players) that compositors can call for standard-conformant tone mapping, so that HDR rendering quality does not diverge by compositor. Note: speculative direction without a committed upstream project.
- **Hardware-accelerated ICC profile application via 3D LUT**: The 17³ hardware 3D LUT exposed by AMD DCN3 and Intel Xe2 can in principle apply a full ICC gamut-mapping transform in hardware without a GPU shader pass. Realising this requires compositor tooling to compile ICC profiles to 3D LUT entries and use the `drm_color_lut_3d` ABI; research prototypes exist in gamescope and KWin experimental branches but a stable, broadly adopted pipeline is a 2–4-year horizon. Note: needs verification for current prototype status.
- **Content-adaptive VRR floor negotiation**: Current VRR implementations have a hard minimum refresh floor (typically 40–48 Hz) below which the display reverts to fixed rate. Future panel and driver work may allow negotiating a lower floor (down to 1 Hz for power-saving still-image scenarios), which is relevant for OLED panels where static frame power consumption depends on refresh rate. Note: speculative; no current kernel RFC.
- **Composited HDCP policy for multi-tenant desktops**: On a Wayland desktop where one application window is HDCP-protected content and others are not, current KMS HDCP enforcement is per-connector and forces the entire output into content-protection mode, restricting screenshot and screen-capture APIs. A finer-grained per-plane or per-surface HDCP policy is architecturally desirable but requires both KMS ABI changes and Wayland protocol additions. Note: speculative direction; no committed design proposal as of mid-2026.
- **AV1 display-side metadata and HDR integration**: AV1 supports Film Grain Synthesis and HDR10+ metadata natively in the bitstream; when display hardware accelerates AV1 decode with film grain, passing the grain parameters and HDR metadata to the display engine rather than the compositor could reduce a GPU blit. This requires new KMS property types for video-plane grain descriptors. Note: speculative, dependent on hardware vendor roadmaps.
