...
GPU memory management in Linux is a collaborative system built from several interlocking kernel subsystems. GEM (Graphics Execution Manager) handles per-driver object allocation and lifetime within a single process context. DMA-BUF provides a kernel-wide buffer-sharing mechanism that crosses driver and subsystem boundaries. TTM (Translation Table Manager) implements the generic buffer migration and eviction policy that backs GEM on complex, VRAM-equipped GPUs. GBM (Generic Buffer Manager) is the userspace library that ties these pieces together for EGL-compatible allocation. Layered on top of all of these are DRM format modifiers — a negotiation protocol for the hardware-specific tiling and compression layouts that modern GPUs and display controllers require.
