...
- **Integrations**: Ch176 (OpenCASCADE Technology — §9.6's OCCT-specific AI-CAD tooling as the concrete instance of this chapter's representation-level survey); Ch124 (Local LLM Inference on Linux — every LLM/VLM-based system here is inference-backend-agnostic); Ch205 (Blender AI/MCP — shared agent-wraps-scripting-API pattern between Blender's `bpy` bridge and CadQuery/build123d/FreeCAD MCP tooling); Ch40 (Bevy and wgpu) and Ch98 (WebAssembly and WebGPU — rendering targets for generated/reconstructed CAD geometry)
