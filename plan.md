...
- Command-sequence generative models: DeepCAD (six-command vocabulary — Line/Arc/Circle/SOL/Extrude/EOS — Transformer autoencoder for reconstruction and random generation); SkexGen (ICML 2022, disentangled topology/geometry/extrusion codebooks, autoregressive decoding)
...
- Reverse engineering: ComplexGen (B-rep as a chain complex — joint vertex/edge/face primitive detection plus global validity optimization); CAD-Recode (point cloud → executable Python via small LLM + point-cloud projector, reports large gains over prior point-cloud-to-CAD methods); CAD-Coder (image → CadQuery code, GenCAD-Code dataset of 163K image/code pairs, reports 100% valid-syntax rate and some generalization to real photographs); Img2CADSeq as a sequence-diffusion alternative
