...
- Audacity 4.0 as the non-destructive waveform-editor counterpart to Ardour's multitrack DAW model: the 4.0 rewrite's move to a real-time, non-destructive effects-preview architecture (effects apply live during playback rather than the pre-4.0 destructive apply-and-wait model), backed by a new project-graph engine distinct from the legacy `AudacityProject`/`WaveTrack` in-memory model; the modernized module/plugin system unifying LV2, VST3, and Audacity's own Nyquist scripting behind a common plugin-host interface, replacing several generations of ad hoc plugin loaders; audio backend on Linux via PortAudio, itself sitting on top of PipeWire/ALSA (the same backend layering as Ardour, but through PortAudio's abstraction rather than talking to PipeWire/JACK directly); the `.aup3` SQLite-backed project file format (carried over from 3.x) storing audio blocks and edit history in a single file, contrasted with Ardour's directory-of-files session model; real-time spectrogram and analysis view rendering — CPU-side FFT display rather than GPU-accelerated, making it a candidate workload for the general-purpose 2D/CPU rasterization and pixman coverage elsewhere in the book (Ch47) if a GPU-accelerated view is ever added
