...
*A paired reference catalog for the algorithms that run on top of the Linux graphics stack. Both chapters are structured as decision-oriented references: algorithm name, what it computes, when to use it, named variants, costs and limitations, and a primary source. They are intended for use at the whiteboard before implementation, and as vocabulary references when reading GPU codebases and papers.*

### Chapter 204: Shader Algorithm Catalog — Recipes, References, and Use Cases *(Part XXIX)*

- Scope: graphics application developers selecting a shader technique; systems and driver developers understanding what the GPU is computing; browser engineers mapping WebGPU workloads.
- 108 named shader algorithms across 13 categories: I. Rendering Architecture (GPU-driven, Clustered Forward+, Deferred, Visibility Buffer, VRS, Tiled Deferred, Forward+ Light Lists, Multi-Draw Indirect), II. Lighting and PBR (Cook-Torrance, Disney Principled, Oren-Nayar, Cloth, Hair BSDF, Spherical Harmonics, LTC area lights, Subsurface Scattering), III. Shadows (Shadow Maps, Cascaded Shadow Maps, PCSS, VSSM, Ray-Traced Shadows, SDF Shadows), IV. Texture and Sampling (Anisotropic filtering, Parallax Occlusion Mapping, Virtual Texturing/Megatextures, Texture Streaming, BC/ASTC compression), V. Post-Processing (TAA, DLSS/FSR/XeSS, SSAO, SSR, Bloom, Tone Mapping, Chromatic Aberration, Depth of Field), VI. Global Illumination (DDGI, Lumen, RTXGI, VXGI, Radiance Cascades, Irradiance Probes), VII. Ray Tracing and Path Tracing (Whitted RT, Path Tracing, ReSTIR DI/GI, BDPT, MLRT, Denoising — OIDN/NRD), VIII. Compute and Data-Parallel (Prefix Scan, Stream Compaction, Radix Sort, FFT on GPU, Reduction, Histogram Equalization), IX. GPU Simulation (Particle Systems, Fluid SPH/FLIP, Cloth, Rigid Body Narrowphase, Ocean FFT, Fire/Smoke VDB), X. Neural Rendering (NeRF, 3D Gaussian Splatting, Neural Texture Compression, Diffusion Upscaling, DLSS Neural Shaders), XI. Hardware-Specific and Specialised (Mesh Shaders Amplification, Cooperative Matrix GEMM, Subgroup operations, Sparse Textures, VRS tile classification), XII. Animation and Deformation (Linear Blend Skinning, Dual Quaternion Skinning, Blend Shapes, Vertex Animation Textures, Morph Targets), XIII. 2D and Vector Graphics (GPU Bezier Rendering, SDF Text, GPU-accelerated 2D compositing, Signed-area vector fill)
- Each entry: what it computes (one sentence), when to use it, named variants, costs and limitations, primary reference URL
- Integrations: Ch135 (ray tracing Vulkan API for category VII), Ch154 (GPU-driven rendering for category I), Ch127 (mesh shaders for category XI), Ch25 (Vulkan compute for category VIII), Ch152 (Rust GPU implementations)

### Chapter 208: GPU Geometry Algorithms — Subdivision, Implicit Surfaces, Skinning, and IK *(Part XXIX)*

- Scope: graphics application developers implementing character animation, procedural geometry, or physics-driven deformation pipelines on Linux using Vulkan or OpenGL compute.
- 107 geometry algorithms plus Algorithm Selection Guide across 13 categories: I. Surface Representation (Subdivision — OpenSubdiv/Loop/Catmull-Clark, NURBS/B-Spline, Metaballs, Spline Tessellation, 2D Vector on GPU, CAD curve rendering, CSG Boolean, SDF blending, Poisson reconstruction), II. Mesh Processing (Simplification — meshoptimizer/QEM, Remeshing, UV Seam/Atlas, Mesh Welding, Half-Edge, Decimation, Laplacian Smoothing, Dual Mesh, Topology repair, Curvature-adaptive tessellation, Manifold repair, Mesh Boolean, Conformal mapping, Mesh segmentation), III. Spatial Data Structures (BVH build/traverse, Octree, k-d tree, Sparse Voxel, Hash Grid, PVS), IV. Differential Geometry (Discrete curvature, Geodesics, Parameterisation, Medial axis, Differential coordinates), V. Animation (Skeletal/LBS/DQS, Blend shapes, Cloth, IK solvers, Crowd simulation, Procedural animation, Blend trees, Morph streaming), VI. Physics (Rigid body BVH, Softbody FEM, SPH fluid, FLIP fluid, Position-Based Dynamics, Joint constraints, Collision response, Fracture, Buoyancy, Rigid body contact), VII. SDF and Volumetric (SDF CSG, Sphere tracing, Volume rendering DVR, Opacity micro-maps, SDF GI, Marching Cubes, Dual Contouring), VIII. Terrain and Procedural (Heightmap, Voxel terrain/Minecraft-style, Fractal noise, L-systems, Wave Function Collapse, Procedural city), IX. Ray Tracing Geometry (TLAS/BLAS construction, BVH traversal, Ray-triangle intersection, Instance transform, Deformable BVH, ReSTIR reservoir, Ray differentials, Watertight intersection, Coherent batching, Opacity micro-map integration, Adaptive sampling, Motion blur BVH), X. Point Cloud (Gaussian Splatting, Point splatting, Surface reconstruction from points, Point cloud compression), XI. Neural Geometry (NeRF geometry, Mesh-conditioned diffusion, Learned LOD, Neural SDF), XII. Specialised (Hair rendering, Water surface simulation, Decal projection, Fur shell/fin), XIII. GPU Primitives (Prefix scan, Stream compaction, Bitonic sort, Histogram, Parallel reduction)
- Algorithm Selection Guide: decision tables by problem type and runtime constraint covering all 13 categories
- Integrations: Ch135 (Vulkan acceleration structure API for category IX), Ch127 (mesh shaders for geometry pipelines), Ch133 (async compute for geometry passes), Ch25 (Vulkan compute foundations), Ch154 (GPU-driven rendering — meshlet culling), Ch204 (shader algorithms that consume geometry output)
