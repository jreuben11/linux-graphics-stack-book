...
- **Part XI — Engines & Creative Tools**
  ...
  - [Chapter 205a: Programmable Games and Competitive-Code Sandboxes](#chapter-205a-programmable-games-and-competitive-code-sandboxes)
  - [Chapter 205b: AI Agents in Games — RL Environments and LLM-Driven NPCs](#chapter-205b-ai-agents-in-games--rl-environments-and-llm-driven-npcs)
  - [Chapter 205c: Open-Source 2D Simulation-Game Engines: Sprite Blitters, GPU Batching, and Reimplementation Modding](#chapter-205c-open-source-2d-simulation-game-engines-sprite-blitters-gpu-batching-and-reimplementation-modding)
  - [Chapter 205d: Modding Architectures — Scripting, Sandboxing, and Hot-Reload on Linux](#chapter-205d-modding-architectures--scripting-sandboxing-and-hot-reload-on-linux)
  - [Chapter 205e: FOSS Simulation-Game Frameworks — Data Models, Rules Engines, and Modding as SDKs](#chapter-205e-foss-simulation-game-frameworks--data-models-rules-engines-and-modding-as-sdks)
  - [Chapter 205f: Artificial Life on the GPU — Cellular Automata, Lenia, and Digital-Organism Simulators](#chapter-205f-artificial-life-on-the-gpu--cellular-automata-lenia-and-digital-organism-simulators)
...
- **Integrations**: Ch176 (OpenCASCADE Technology — §9.6's OCCT-specific AI-CAD tooling as the concrete instance of this chapter's representation-level survey, alongside this chapter's own broader CAD-scripting-language survey); Ch124 (Local LLM Inference on Linux — every LLM/VLM-based system here is inference-backend-agnostic); Ch205 (Blender AI/MCP — shared agent-wraps-scripting-API pattern between Blender's `bpy` bridge and CadQuery/build123d/FreeCAD MCP tooling); Ch40 (Bevy and wgpu) and Ch98 (WebAssembly and WebGPU — rendering targets for generated/reconstructed CAD geometry)
...
### Chapter 205a: Programmable Games and Competitive-Code Sandboxes *(Part XI — Engines & Creative Tools)*
...
- Bridge to WASM sandboxing: why a WASM-based competitive-code sandbox (linear-memory isolation, no ambient syscall/GPU access) is the natural successor to Screeps' custom V8-isolate approach — cross-reference Ch98's WASM compilation model and Ch205d's WASM mod-sandboxing coverage
- **Integrations**: Ch98 (WebAssembly and WebGPU as a Deployment Target — WASM sandboxing as the architectural successor to Screeps' V8-isolate model); Ch205d (Modding Architectures — Extism/wasmtime sandboxing patterns applicable to competitive-code execution); Ch210 (GPU physics — ODE's role in RoboCup 3D's simulation server); Ch40 (Bevy and wgpu — PixiJS/WebGL 2D renderer as a comparison point for a Rust-native equivalent)

### Chapter 205b: AI Agents in Games — RL Environments and LLM-Driven NPCs *(Part XI — Engines & Creative Tools)*
...
### Chapter 205c: Open-Source 2D Simulation-Game Engines: Sprite Blitters, GPU Batching, and Reimplementation Modding *(Part XI — Engines & Creative Tools)*
...
- **Integrations**: Ch17 (Software Renderers — llvmpipe/Lavapipe as the general-purpose analog of these games' CPU blitter stacks); Ch205d (Modding Architectures — Squirrel/JS-TS/Lua as three embedded-scripting choices contrasted against the WASM/Harmony/GDExtension approaches covered there); Ch84 (bgfx and cross-platform rendering abstraction — a contrasting modern approach to the instanced-sprite-batching problem OpenRCT2 solves by hand)

### Chapter 205d: Modding Architectures — Scripting, Sandboxing, and Hot-Reload on Linux *(Part XI — Engines & Creative Tools)*
...
- **Integrations**: Ch98 (WebAssembly and WebGPU as a Deployment Target — the same Emscripten/WASM compilation model applied to mod sandboxes instead of full applications); Ch40 (Bevy and wgpu — the dynamic-plugin ABI gap as a concrete open problem in Ch40's engine); Ch41 (Godot 4 RenderingDevice — GDExtension as the native-code extension path into the explicit Vulkan renderer); Ch205a (Programmable Games — Screeps' V8-isolate sandbox as a pre-WASM precedent for the same untrusted-code-execution problem); Ch205c (2D Simulation-Game Engines — Squirrel/JS-TS/Lua as alternative embedded-scripting choices in a narrower rendering context)

### Chapter 205e: FOSS Simulation-Game Frameworks — Data Models, Rules Engines, and Modding as SDKs *(Part XI — Engines & Creative Tools)*

*(Delivered chapter note: chapter text has been written as `ch205e-simulation-game-frameworks.md`. See the two "Delivered chapter note" bullets below for what the drafted chapter adds beyond this outline.)*
...
- **Integrations**: Ch205d (Modding Architectures — embedded-Lua and data-driven rulesets here as domain-specific counterparts to Ch205d's general-purpose scripting/sandboxing survey); Ch205c (2D Simulation-Game Engines — a companion "reimplementation" chapter, there scoped to rendering architecture, here scoped to rules/data-model architecture for a related project family); Ch205f (Artificial Life on the GPU — a sibling Part XI chapter on a different class of simulation software, there deliberately GPU-compute-focused where this chapter is deliberately non-graphics)

### Chapter 205f: Artificial Life on the GPU — Cellular Automata, Lenia, and Digital-Organism Simulators *(Part XI — Engines & Creative Tools)*

*(Note: outline only — chapter text not yet written. Verify all version numbers, dates, and API names against current upstream before drafting, per this book's accuracy standard.)*

- Scope: unlike Ch205e's deliberately non-graphics framing, this chapter leans into GPU-compute architecture — artificial-life (ALife) systems as compute-shader/GPGPU workloads, covering three distinct parallelization patterns and the projects that exemplify them
...
- **Integrations**: Ch64 (glTF 2.0 — the runtime format this chapter's standards feed into or wrap); Ch176 (OCCT — STEP import/export kernel); Ch42 (Blender GPU — Bonsai/IfcOpenShell IFC authoring); Ch205 (Blender AI and MCP — generative pipelines drawing on ML-scale shape corpora); Ch212 (Python 3D ML Libraries — Open3D/PyTorch3D/Kaolin consuming ShapeNet/Objaverse); Ch115 (NeRFStudio and 3D Gaussian Splatting — photogrammetry sources feeding Objaverse-XL); Ch241 (GIMP, Krita, and darktable — texture authoring for glTF/Voyager publication)
